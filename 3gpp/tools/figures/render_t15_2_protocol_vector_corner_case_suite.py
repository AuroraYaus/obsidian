#!/usr/bin/env python3
""" @file render_t15_2_protocol_vector_corner_case_suite.py
@brief 渲染T15.2协议向量与边界用例套件图 —— 将Rel-19协议条款和译码器边界用例转化为可复现的通过/失败证据
@date 2025 """
...
def centered(
    ...
    @return None
    @note 使用 anchor="mm" 实现真正的水平和垂直居中 """
    x0, y0, x1, y1 = rect
...
def main() -> None:
    """ @brief 渲染T15.2协议向量与边界用例套件图的主入口
    @note 生成的图片展示从 Rel-19 证据到 Suite Manifest 再到 LTE Turbo / NR LDPC / NR Polar 三条向量通道的完整流程。
    下半部分包含 Pass/Fail Policy、Expected Outcome、Failure Bundle 三种结果卡片，以及 Family 向量用例表和 Suite Tier 分级表。
    输出至 docs/L3/assets/T15.2_protocol_vector_corner_case_suite.png
    @see render_t15_1_decoder_testbench_architecture.py, render_t15_3_coverage_regression_strategy.py """
