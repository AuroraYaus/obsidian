...
tags:
  ...
  - protocol
  - l2
source_spec: "TS 38.300 Rel-19 §5.4; TS 38.213 Rel-19 §10"
---

# Carrier Aggregation 载波聚合

载波聚合（Carrier Aggregation, CA）把多个载波（CC，分量载波，Component Carrier）聚合起来服务同一 UE：一个主载波（PCell，主小区，Primary Cell）管连接与移动性，多个辅载波（SCell，辅小区，Secondary Cell）加带宽提吞吐。CA 是吞吐扩展的机制——单载波带宽受限（FR1（频率范围 1，Frequency Range 1）100 MHz），聚合多个 CC 可达数百 MHz。每载波有独立的 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）进程与调度，跨载波调度经 CIF（载波指示字段，Carrier Indicator Field）指示。
...
- CC（分量载波）：每个 CC 是独立带宽的载波（有独立 numerology（参数集）/带宽配置）；UE 聚合 2-N 个 CC（NR 最多 16 CC）。
...
- 双连接（DC，Dual Connectivity）是 CA 的演进：不同基站的小区组聚合（MCG（主小区组，Master Cell Group）/SCG（辅小区组，Secondary Cell Group））。
...
| 误解 | 正确理解 |
|:---|:---|
| CA 只在 FR2 | CA 在 FR1（频率范围 1，Frequency Range 1）/FR2（频率范围 2，Frequency Range 2）都用（FR1 聚合 100 MHz CC、FR2 聚合 400 MHz CC） |
...
- CA 架构：TS 38.300（Rel-19 j20）§5.4（5.4.1 Carrier aggregation），本地 `3GPP_Rel19/processed/TS_38.300_38300-j20`。
...
- - [[T16.3_CA_BWP|T16.3 载波聚合与 BWP 讲义]]
[[概念图谱入口]]
