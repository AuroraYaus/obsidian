...
| `docs/L2_协议算法/assets/T14.2_dci_field_layout.svg` | 手绘 SVG（2026-08-13 新增，无生成脚本） | DCI 格式 1_1 教学配置位宽清单（TS 38.212 §7.3.1.2.2，21 字段 59 位）。 | T14.2 图 1 | 2026-08-13 新增：audit_svg_layout.py R1-R11 全过（SVG_LAYOUT_AUDIT_PASS）+ cairosvg 渲染验证。 | 3 行 x 7 列字段网格 + 合计条 + 字段组图例；文字 11.5-13px，格间距 9px；字段位宽随 RRC 配置变化。 |
| `docs/L2_协议算法/assets/T14.2_dci_to_descriptor.svg` | 手绘 SVG（2026-08-13 新增，无生成脚本） | DCI 字段到 T9.0 descriptor 映射流程（左列字段、右列 descriptor、中间映射方式）。 | T14.2 图 2 | 2026-08-13 新增：audit_svg_layout.py R1-R11 全过（SVG_LAYOUT_AUDIT_PASS）+ cairosvg 渲染验证。 | 左右两列 7 行框 + 箭头 + 方式标注；文字 11.5-12.5px；映射字段名与 T9.0 一致。 |
| `docs/L2_协议算法/assets/T14.1_pdcch_blind_decoding.svg` | 手绘 SVG（2026-08-13 新增，无生成脚本） | PDCCH 盲检流程状态图（候选驱动试错：候选位置 × RNTI × DCI 尺寸）。 | T14.1 图 2 | 2026-08-13 新增：audit_svg_layout.py R1-R11 全过（SVG_LAYOUT_AUDIT_PASS）+ cairosvg 渲染验证。 | 流程节点 7 个 + 判定分支；940x760；文字 12-14px；盲检三步循环 + CRC/RNTI 终选。 |
| `docs/L2_协议算法/assets/T14.1_pdcch_cce_aggregation.svg` | 手绘 SVG（2026-08-13 新增，无生成脚本） | CORESET 资源格 / REG / CCE / 聚合等级示意（38.211 §7.3.2）。 | T14.1 图 1 | 2026-08-13 新增：audit_svg_layout.py R1-R11 全过（SVG_LAYOUT_AUDIT_PASS）+ cairosvg 渲染验证。 | 资源格 48x2 + REG 束 + CCE 聚合；940x620；文字 11.5-13px；非交织 L=6 / 交织 L∈{2,6}、{3,6}。 |
| `docs/concepts/assets/Preemption_Indication_bitmap.svg` | 手绘 SVG（2026-08-17 新增，无生成脚本） | PI 位图时频映射对照（set0 纯时域 / set1 时频二维；TS 38.213 §11.2）。 | Preemption_Indication_抢占指示 图 1（概念笔记首个图资产） | 2026-08-17 新增：audit_svg_layout.py R1-R11 全过（SVG_LAYOUT_AUDIT_PASS）+ cairosvg 渲染验证；会话图片目检不可用，已按验证缺口声明并做坐标核算四查。 | 两面板对照 + 14 位/位对标签 + 图例；1100x660；文字 11-19px；格间距 12-42px；无 rotate/polygon/path 盲区元素。 |
