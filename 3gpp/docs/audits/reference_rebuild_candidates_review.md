...
```bash
python3 tools/audit_reference_rebuilds.py docs/L1/T*.md docs/L2/T*.md > docs/audits/reference_rebuild_candidates_full.txt
```
...
| `docs/L2/T8.3_NR_LDPC_lifting_QC_matrix.md` | 40 | TS 38.212 Table 5.3.2-1/2/3 已图片化复现；候选多来自表格证据和 QC 构造公式。 |
...
| `docs/L2/T6.4_LTE_Turbo_internal_interleaver.md` | 34 | Table 5.1.3-3 输入 CSV/HTML、生成脚本、输出资产和使用位置已由正文、`image_asset_inventory.md`、`final_delivery_status.md` 共同闭合。 |
| `docs/L2/T8.4_LDPC_Tanner_graph_message_passing.md` | 33 | 多数为 Tanner 图、syndrome 和 LDPC 教学/协议边界候选；正文已说明协议表图承接关系。 |
| `docs/L2/T8.5_LDPC_sum_product_BP.md` | 25 | 多数为 BP/SPA 推导公式和 toy 计算公式；不等同为外部协议公式缺失。 |
| `docs/L2/T11.5_decoder_selection_by_channel_type.md` | 23 | 多数为协议映射表和 selector 证据行；正文已作为信道/信息类型到译码器家族映射总结。 |
...
| TS 36.212 Figure 5.1.3-2 Turbo 编码器结构图 | 已用 Python 图片化重建，解释两个 8 状态组成编码器、内部交织器、三路母码流和 trellis termination 虚线路径；T6 系列引用已指向该资产。 | `tools/figures/render_lte_turbo_encoder_structure.py`；`docs/L2/assets/T6.3_TS36.212_Figure_5.1.3-2_turbo_encoder_rebuild.png`；`docs/L2/T6.3_LTE_Turbo_encoder_trellis_termination.md`。 |
...
| Closed | T6.4 Table 5.1.3-3 资产证据链 | T3.3 已复现长表，T6.4 使用地址公式。 | `docs/L2/T6.4_LTE_Turbo_internal_interleaver.md` 已列 `table_0009.csv/html` 和输出图；`docs/audits/image_asset_inventory.md` 已列脚本 `render_lte_turbo_interleaver_table.py`、输入证据、输出图片和 T3.3/T6.4 使用位置。 |
