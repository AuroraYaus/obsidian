...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2/T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0.md"
---
# T9.3 NR LDPC HARQ 软缓存、RV k0 与 CBG 部分重传
## 本节学习目标

本节使用的单篇技术缩写先统一说明：低密度奇偶校验码是 NR 数据业务的信道编码；Turbo是 LTE 数据业务使用的译码对照对象；混合自动重传请求是在译码失败后保留软信息并支持重传合并的机制；对数似然比是 soft buffer 保存的软信息；传输块是调度和循环冗余校验交付的基本数据块；码块是 TB 分段后送入 LDPC 编码/译码的子块；CBG是 NR 为部分重传引入的 CB 分组；下行控制信息携带译码控制字段；调制与编码方案和传输块大小决定分段、码率和本次可用比特数；码块组传输信息（Code Block Group Transmission Information, CBGTI）指示本次哪些 CBG 被传输；码块组刷新信息（Code Block Group Flushing Out Information, CBGFI）在下行 CBG 重传中指示旧 CBG 实例是否可与本次合并；新数据指示（New Data Indicator, NDI）用于区分新 TB 和旧 TB 重传；冗余版本（Redundancy Version, RV）决定本次从 LDPC circular buffer 的哪个起点选择编码比特；上行共享信道和下行共享信道是 TS 38.212/38.214 中承载 LDPC TB 的主要数据信道；块错误率用于后续链路仿真评估；静态随机存取存储器是硬件 soft buffer 的常见存储形态；寄存器传输级/专用集成电路实现要把这些字段落成地址生成、软缓存、mask、饱和加法和释放控制。
...
- 说明 LTE T7.3 与 NR T9.3 的共同点和差异，尤其是 LTE 没有 NR-style CBG partial retransmission 粒度。
...
roadmap 的 T9.3 卡片聚焦 NR LDPC HARQ soft buffer、RV、`k0`、有限缓存和 toy LLR 放置例子。当前执行登记表把 T9.3 扩展为“NR HARQ Soft Buffer、RV、CBG”，并要求本节覆盖 CBG partial retransmission、状态字段、失败案例和最小 dump。后续 T9.4 仍会专门深化 CBG handling；本节先把 CBG 放在 HARQ/RV 软缓存主线里讲清楚。
...
| 登记表 T9.3 | 对比 LTE T7.3 | “与 LTE T7.3 的共同点和差异”。 |
...
- TS 38.212 规定信道编码和 rate matching。对 T9.3 来说，它回答“给定某个 CB、RV、`Ncb`、`E` 后，本次从 circular buffer 选择哪些编码位置”。
- TS 38.214 规定物理层过程中的调制阶数、码率、TBS、RV 字段、CBG 分组和 CBGTI 解释。对 T9.3 来说，它回答“接收端怎么知道本次是哪个 RV、哪些 CBG 出现、哪些 CB 被调度”。

| 协议位置 | 本地证据路径 |
| :--- | :--- |
| TS 38.212 缩写表 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:461-503` |
| TS 38.212 §5.4.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:1179-1309` |
| TS 38.212 §6.2.5 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:1397-1407` |
| TS 38.212 §7.2.5 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:3771-3775` |
| TS 38.214 §5.1.3 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:1157-1211` |
| TS 38.214 §5.1.7.1/§5.1.7.2 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:2407-2437` |
| TS 38.214 §6.1.4 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:6829-6841` |
| TS 38.214 §6.1.5.1/§6.1.5.2 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:7099-7125` |

边界说明：

- 协议规定的是发送侧 bit selection 和控制字段语义；接收端 soft buffer 的物理 SRAM bank、LLR 位宽、压缩方式、饱和阈值不是 3GPP 直接规定，但必须保持相同编码位置的软信息语义。
- TS 38.212 的 `TS_36.212_36212-j30_content.md` 对 Table 5.4.2.1-2 的公式符号抽取不完整，`tables/table_0016.csv` 和 `table_0017.csv` 也未能保留完整公式内容。因此本节复现的是人工核验后的协议公式形式，并把真实数值例子写成 toy 参数，不冒充 bit-exact 3GPP 向量。
...
CBG partial retransmission 是本节区别于 LTE T7.3 的重点。它不改变单个 CB 的 rate recovery 公式，而是在 rate recovery 之前先决定“哪些 CB 本次有 LLR 输入”。
...
LTE T7.3 和 NR T9.3 的共同点是：都要按 HARQ process 保存 LLR，RV 改变 circular buffer 选择区域，同一编码位置的重传证据要累加或饱和。
...
因此不能把 LTE T7.3 的软缓存逻辑直接复制成 NR T9.3。共同的“按地址累加”可以复用思想，但 NR 的 descriptor 必须增加 CBG 维度和 CBGTI mask 解释。
...
6. NR T9.3 相比 LTE T7.3 增加了哪个重要控制维度？
...
6. CBG partial retransmission。NR 可以按 CBG 指示部分 CB 传输/保持，LTE T7.3 没有这个 NR-style CBG 粒度。

## 协议证据表

| 结论 | 协议依据 | 本地证据 |
|:---|:---|:---|
| CBG/CBGTI 的协议缩写来源 | TS 38.212 缩写表 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:461-463` |
| HARQ 的协议缩写来源 | TS 38.212 缩写表 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:501` |
| TS 38.212 §5.4.2.1 对未被 CBGTI 调度的 coded block 设定相关分支 | TS 38.212 §5.4.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:1251-1289` |
| `rvid` 取值为 0、1、2、3，并按 Table 5.4.2.1-2 和 base graph 给出 `k0` | TS 38.212 §5.4.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:1289-1309`；`source.docx` 中 Table 5.4.2.1-2 原表 |
| PDSCH 读取 DCI 中 MCS 与 RV 字段，RV/TBS 进入接收过程 | TS 38.214 §5.1.3 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:1157-1171` |
| PDSCH CBG 分组由 higher-layer 配置最大 CBG 数和 CB 数确定 | TS 38.214 §5.1.7.1 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:2407-2423` |
| PDSCH CBGTI 指示本次哪些 CBG 出现，0 表示不传输、1 表示传输 | TS 38.214 §5.1.7.2 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:2423-2433` |
| PDSCH CBGFI 若存在，0 表示早前相同 CBG 实例可能损坏，1 表示可与早前实例合并 | TS 38.214 §5.1.7.2 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:2435` |
| PUSCH 读取或确定 RV 字段 | TS 38.214 §6.1.4 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:6829-6841` |
| PUSCH CBG 分组和 CBGTI 传输规则与 bit order | TS 38.214 §6.1.5.1/§6.1.5.2 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:7099-7125` |
## 执行与证据记录

| 项 | 记录 |
|:---|:---|
| 写作范围 | 新增 `docs/L2/T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0.md`。 |
| Prompt 对齐 | 已覆盖 roadmap T9.3 的 RV、`k0`、HARQ soft combining、limited-buffer、toy LLR 放置；同时覆盖登记表 T9.3 的 CBG、CBGTI、partial retransmission、状态字段、失败案例和最小 dump。 |
| 协议精读 | 已读取 TS 38.212 §5.4.2.1、§6.2.5、§7.2.5，以及 TS 38.214 §5.1.3、§5.1.7.1、§5.1.7.2、§6.1.4、§6.1.5.1、§6.1.5.2。 |
| 协议表复现 | 已在正文修正并复现 Table 5.4.2.1-2 的 `rvid/BG/k0` 公式形式，包含 $Z_c$ 对齐项；本地 `TS_36.212_36212-j30_content.md` 和 `table_0016/0017` 抽取不完整，因此使用人工复核公式并保留本地证据说明。 |
...
3. 本项目 T4.3：`docs/L1/T4.3_HARQ_soft_combining_basics.md`。
4. 本项目 T7.3：`docs/L2/T7.3_LTE_HARQ_soft_buffer_RV.md`。
5. 本项目 T9.1：`docs/L2/T9.1_NR_LDPC_rate_recovery_overview.md`。
6. 本项目 T9.2：`docs/L2/T9.2_NR_LDPC_circular_buffer_states.md`。
