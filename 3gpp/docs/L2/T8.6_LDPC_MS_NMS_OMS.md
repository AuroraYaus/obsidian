...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2/T8.6_LDPC_MS_NMS_OMS.md"
---
# T8.6 LDPC Min-Sum、归一化 Min-Sum 与偏移 Min-Sum
## 本节学习目标

本节讲低密度奇偶校验码译码中最常见的校验节点近似算法：Min-Sum、归一化 Min-Sum（Normalized Min-Sum, NMS）和偏移 Min-Sum（Offset Min-Sum, OMS）。T8.5 已经给出和积算法（Sum-Product Algorithm, SPA）的精确校验节点公式：
...
如果 T8.5 的 SPA 公式还不熟，先抓住一句话：Min-Sum 不是重新定义 LDPC 码，而是在同一个 Tanner 图上用更便宜的方式近似校验节点输出。
...
min1/min2 可以在一个 layer 或一个 row group 的并行向量上展开。对 QC-LDPC 来说，同一 base graph edge 对应 $Z_c$ 个 bit-level 位置，硬件常按 $Z_c$ 并行度或分片并行度处理一组消息。bank conflict、layered schedule 和 read-modify-write 生命周期会在 T8.7 继续展开。
...
| 结论 | 协议证据 | 本地核验 |
|:---|:---|:---|
| NR LDPC 的 Tanner 图结构来自 $H$ | TS 38.212 §5.3.2 | `content.md` 行 948-989；T8.3/T8.4 已解释 BG、$Z_c$ 和 Tanner 图。 |
...
## 图示
![图 1：LDPC MS NMS OMS compare](assets/T8.6_LDPC_MS_NMS_OMS_compare.png)

## 执行与证据记录

| 项目 | 记录 |
|:---|:---|
| 路线图卡片 | `2026-06-19-lte-nr-decoding-learning-roadmap.md` T8.6，Prompt/产出/验收/3GPP 证据已覆盖。 |
| 讲义文件 | `docs/L2/T8.6_LDPC_MS_NMS_OMS.md`。 |
| 协议来源 | TS 38.212 Rel-19 `38212-j30` §5.3.2；本节算法不作为规范译码算法。 |
| 图表资产 | `tools/figures/render_ldpc_min_sum_variants.py` 生成 `docs/L2/assets/T8.6_LDPC_MS_NMS_OMS_compare.png`。 |
| Python 校验 | 已执行正文 Min-Sum variants 片段，输出 `T8.6_MIN_SUM_VARIANTS_OK`。 |
| 资产文件检查 | `python3` 打开 `docs/L2/assets/T8.6_LDPC_MS_NMS_OMS_compare.png` 输出 `IMAGE_OK ... (1600, 1080) RGB`。 |
| 单篇审计 | `python3 tools/audit_lesson_terms.py docs/L2/T8.6_LDPC_MS_NMS_OMS.md` 输出 `LESSON_TERM_AUDIT_OK`；`python3 tools/audit_markdown_headings.py docs/L2/T8.6_LDPC_MS_NMS_OMS.md` 输出 `MARKDOWN_HEADING_AUDIT_OK`；`python3 tools/audit_lesson_depth.py --strict docs/L2/T8.6_LDPC_MS_NMS_OMS.md` 输出 `LESSON_DEPTH_AUDIT_OK`；`python3 tools/audit_latex_render.py docs/L2/T8.6_LDPC_MS_NMS_OMS.md` 输出 `LATEX_RENDER_AUDIT_OK formulas=109`。 |
