...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2/T8.5_LDPC_sum_product_BP.md"
---
# T8.5 LDPC 和积置信传播
## 本节学习目标

本节讲低密度奇偶校验码的经典软判决译码算法：置信传播（Belief Propagation, BP）和和积算法（Sum-Product Algorithm, SPA）。T8.4 已经把奇偶校验矩阵 $H$ 画成 Tanner 图；本节继续讲图上每条边如何传递对数似然比消息，变量节点如何合并来自多个校验节点的外信息，校验节点为什么需要 $\tanh$ 和 $\operatorname{atanh}$ 这样的非线性函数，以及 syndrome 检查在迭代停止中的位置。
...
如果 $\tanh$ 和 $\operatorname{atanh}$ 还陌生，先把它们理解成“把校验方程的概率约束转换成 LLR 消息的函数”。本节会解释它们为什么出现，T8.6 再讲工程上如何避开昂贵函数。

本节的概率底座来自 T1.4/T1.5。变量节点把多份近似独立证据相乘，转到 LLR 域后成为加法；校验节点要在满足偶校验的所有未知变量组合上求和，这就是边缘化；posterior LLR 再由概率比值转成一个带符号可靠度。若直接背 SPA 公式而不理解这些来源，很容易把外信息排除项或 LLR 符号写错。
...
| TS 38.212 §5.4.2.1 | `TS_36.212_36212-j30_content.md` 行 1175-1309 |
...
这说明校验节点输出的符号来自其他输入符号的组合，输出幅度受其他输入中最不可靠证据的限制。严格的 SPA 公式不是简单取最小值，而是用 $\tanh/\operatorname{atanh}$ 精确表达概率求和；T8.6 的 Min-Sum 正是对这个精确公式的近似。
...
| $\tanh/\operatorname{atanh}$ | 若保留 SPA，用查找表（Look-Up Table, LUT）或分段线性近似；若资源紧，转 T8.6 的 Min-Sum/NMS/OMS。 |
...
实际 NR LDPC 高吞吐实现通常不会采用朴素 flooding SPA，因为 message memory 和非线性函数代价高。常见路线是 layered schedule 加 Min-Sum 或归一化/偏移 Min-Sum；这些是 T8.6 和 T8.7 的主题。本节仍然重要，因为 Min-Sum 是从 SPA 校验节点公式近似出来的，不理解式 (9)，就很难理解 Min-Sum 为什么“符号相乘、幅度取最小”。
...
| 结论 | 协议证据 | 本地核验 |
|:---|:---|:---|
| NR LDPC 的 $H$ 由 BG、$Z_c$ 和 shift table 构造 | TS 38.212 §5.3.2 | `TS_36.212_36212-j30_content.md` 行 948-989；T8.3 已重建 Table 5.3.2-1/2/3。 |
| LDPC 编码在 GF(2) 中执行 | TS 38.212 §5.3.2 | `TS_36.212_36212-j30_content.md` 行 948-989；T8.4 已用 syndrome 手算解释。 |
| BP 输入位于 LDPC 母码位置是接收端实现推论 | TS 38.212 §5.4.2.1 | `TS_36.212_36212-j30_content.md` 行 1175-1309 描述发送侧 LDPC circular buffer、rate matching output 和 RV 起点；由此可推得接收机若要按 $H$ 的边运行 BP，需要把接收 LLR 恢复到相同的 $H$ 列位置。 |
| BP/SPA 不是 3GPP 规范指定的唯一译码算法 | TS 38.212 只定义编码、rate matching 和结构 | 本节作为接收机算法理论讲义，明确协议/实现边界。 |
## 执行与证据记录

| 项目 | 记录 |
|:---|:---|
| 路线图卡片 | `2026-06-19-lte-nr-decoding-learning-roadmap.md` T8.5，Prompt/产出/验收/3GPP 证据已覆盖。 |
| 讲义文件 | `docs/L2/T8.5_LDPC_sum_product_BP.md`。 |
| 协议来源 | TS 38.212 Rel-19 `38212-j30` §5.3.2、§5.4.2.1。 |
| 图表资产 | `tools/figures/render_ldpc_bp_spa_round.py` 生成 `docs/L2/assets/T8.5_LDPC_BP_SPA_one_round.png`。 |
| Python 校验 | 已执行正文 toy BP 片段，覆盖 $q^{(0)}$、$r$、$q^{(1)}$、posterior、hard decision 和 syndrome，输出 `T8.5_TOY_BP_OK`。 |
| 资产文件检查 | `python3` 打开 `docs/L2/assets/T8.5_LDPC_BP_SPA_one_round.png` 输出 `IMAGE_OK ... (1600, 1090) RGB`。 |
| 单篇审计 | `python3 tools/audit_lesson_terms.py docs/L2/T8.5_LDPC_sum_product_BP.md` 输出 `LESSON_TERM_AUDIT_OK`；`python3 tools/audit_markdown_headings.py docs/L2/T8.5_LDPC_sum_product_BP.md` 输出 `MARKDOWN_HEADING_AUDIT_OK`；`python3 tools/audit_lesson_depth.py --strict docs/L2/T8.5_LDPC_sum_product_BP.md` 输出 `LESSON_DEPTH_AUDIT_OK`；`python3 tools/audit_latex_render.py docs/L2/T8.5_LDPC_sum_product_BP.md` 输出 `LATEX_RENDER_AUDIT_OK formulas=196`。 |
