...
**5 篇 L2 讲义**（L2 新增模块 M12，编号 T12.1-T12.5，T12 系列编号空闲）。每篇 400-600 行，含 1 个内嵌 numpy 验证代码块。定时同步/相干带宽时间不做独立篇目（L1 T2.x + 概念笔记已覆盖），在 T12.1 衔接说明。
...
- **模板**：沿用 L2 讲义模板（范本 `docs/L2/T8.1_NR_LDPC_decoder_chain_overview.md`）：
  ...
  - 内容章节：公式 LaTeX（编号 `\tag{n}`）、表格、生活类比、图（如需，用 mermaid 或 SVG——SVG 需按 CLAUDE.md 强制视觉验证）
  - `## 小结`：收束本节 + 指向下一篇
...
1. 5 篇按模板完成（学习目标 bullets 可检验、前置知识检查表格、内容章节、小结）
2. 每篇内嵌 Python 代码块实际运行，输出记录在实施报告，数值与讲义断言一致（功率守恒/分集概率/检测器 SINR 对比/剪枝一致性/MSE 曲线）
3. 无死链（全部 wikilink 可解析）
4. T12.1-12.5 编号唯一（不与现有编号冲突）
5. L2 入口 M12 章节更新完成
6. 合规红线抽查通过（Rule 10/16/20/8）
7. git 提交
