...
## 追加批次五：dB 分贝概念笔记（2026-08-24 grill 锁定）

_Locked via grill — by Claude + AuroraYaus（2026-08-24，1 问收敛）_

## Goal

用户问"dB 在不同领域的计算方法"——检索确认知识库无 dB/分贝专门笔记（术语总表无 dB 条目，仅散见于 T2.9/T13.x 等讲义使用），命中"无匹配"。新建六段式概念笔记 `docs/concepts/dB_分贝.md`：10log/20log 双公式、通信（dBm/dBW/dBc/dBi/路径损耗/Eb-N0）/声学（SPL 20µPa）/数字音频（dBFS）/光学对照、速算规则与常见误解；完成术语登记（dB + dBm 两行）、图谱挂载、排名表、CSI_SINR 与 Information_Theory 反向链接、README 计数同步。

## Approach

1. 新建 `docs/concepts/dB_分贝.md`（六段式，frontmatter `type: definition` + `queries: 1`）：协议锚点标注"非 3GPP 标准项（通用度量标度）"+ 本地 dBm 用法锚点 TS 36.133 §A.9.3（`TS_36.133_36133-j50_sA.9-XX/full.md:12266`）。
2. 术语总表：缩写表加 dB/dBm 两行（SNR 行后）+ 概念笔记索引加 1 行（数学与信息论组）+ 计数 109→110 篇。
3. 概念图谱入口「数学基础」区加 `[[dB_分贝]]`；排名表加行（dB 分贝/1/2026-08-24）。
4. 反向链接：CSI_SINR、Information_Theory 图谱关联各加 `[[dB_分贝]]`（仅机械加链）。
5. README 三语 + 口径线计数同步：概念 109→110；术语 245→247 + 概念索引 109→110（实测口径，承接批次四校准）。
6. 审计闭环 + 提交双推。

## Key decisions & tradeoffs（1 问）

| 决策点 | 结论 | 理由 |
|:---|:---|:---|
| 落点 | 新建独立 dB_分贝 概念笔记 | dB 是 CSI_SINR/Eb-N0/dBm/整形增益等十余篇笔记的共同地基，独立成篇可被多笔记引用；扩充既有笔记会把标度埋进单一主题 |

## Out of scope

- 讲义层内容改动（仅机械 wikilink）；SVG 配图（沿用批次四不配图先例）；声学/光学标准原文考证（仅通用公式 + 参考值，不引具体标准条文）。
