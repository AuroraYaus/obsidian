...
20. **LaTeX 修复**（1 处）：`T1.4:333` 拆为两个独立 `$$` 块，各自保留 `\tag{17}`/`\tag{18}`；全库同类扫描（grep `aligned`/`gather` 内 `\tag` 模式）确认无存量；`audit_latex_render.py` 全库 PASS。
...
| LaTeX 修法 | 拆两个独立 `$$` 块保留 `\tag`；Rule 20 固化禁令 | 编号语义不丢、审计连续性检查继续有效；工具已能检出无需新工具 |
