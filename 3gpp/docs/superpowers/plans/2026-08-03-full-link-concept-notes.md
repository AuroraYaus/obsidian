...
- **图谱关联**：`[[概念图谱入口]]`、`[[LLR_对数似然比]]`、`[[Fixed_Point_Numbers_定点数]]`、`[[Soft_Demodulation_软解调]]`、`[[Detector_Comparison_检测器对比]]`；**必须保留现有 图谱关联 条目** `[[T2.16_LLR_clipping_scaling_quantization]]`、`[[T18.1_fixed_point_decoder_requirements]]`；关系语义：LLR 量化是软解调到译码器的格式转换；裁剪阈值决定动态范围预算，±31 同时吸收 MMSE 归一化放大。
