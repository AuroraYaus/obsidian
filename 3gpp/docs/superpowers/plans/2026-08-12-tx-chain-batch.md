...
发送端处理链（TX Chain）是数据从 MAC 层到天线的完整加工流水线：传输信道处理（CRC（循环冗余校验，Cyclic Redundancy Check）附着→码块分段→信道编码→速率匹配→码块级联）把 TB（传输块，Transport Block）变成编码比特流；物理信道处理（加扰→调制映射→层映射→预编码→RE 映射→OFDM 生成）把比特流变成时域波形。它是整个知识库「接收端译码链路」的镜像——理解发送端每一环，才能理解接收端为什么那样逆着做（T0.1 的「发送端顺序与接收端逆序」）。
...
```
TB → CRC 附着（TB CRC）→ 码块分段（+CB CRC）→ 信道编码（Turbo/LDPC/Polar）
→ 速率匹配 → 码块级联 → 编码比特流
```

每环的接收端镜像：CRC 校验（T7.4/T9.5）、CB 切分（T3.2-T3.5）、译码（T6/T8/T10）、速率恢复（T7/T9）、LLR 拼接（T2.6）——发送端编码链的每一环在接收端都有一个逆操作。
...
接收端镜像：解扰（T2.6 的逆）、软解调 LLR（T2.13/T2.14）、MIMO 检测（T12/T2.12）、解映射、FFT/同步（T2.7/T2.8）。
...
| 物理信道处理只有比特→符号 | 还有层映射/预编码/RE 映射/OFDM——完整五环 |
...
- 物理信道处理：TS 38.211（Rel-19 j30）§5-§7（调制/层映射/预编码/RE 映射/OFDM），本地 `TS_38.211_38211-j30`。
...
- [[Precoding_预编码]]
- [[RE_Mapping_资源元素映射]]
- 关系语义：发送端处理链是全链路的上半身——传输信道处理（编码链）与物理信道处理（波形链）两段共 12 环，每环都有接收端镜像（译码链路/软解调/MIMO 检测），与 T0.1 的「发送端规则→接收端逆流程」翻译表互为表里，至此「发送端→接收端」全链路知识闭环。
```
...
```bash
cd /home/yys/AGENT/obsidian && git add "3gpp/docs/concepts/TX_Chain_发送端处理链总览.md" && git commit -m "docs(concepts): 新增 TX Chain 发送端处理链总览概念笔记（12 环全景，阶段 1 收官）
...
| [[TX_Chain_发送端处理链总览]] | 发送端 12 环处理链全景（阶段 1 收官）。 |
```
