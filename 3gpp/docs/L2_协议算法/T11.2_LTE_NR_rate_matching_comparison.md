...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/T11.2_LTE_NR_rate_matching_comparison.md"
---
# T11.2 LTE/NR 速率匹配与速率恢复对比
## 本节学习目标

本节从接收端视角对比三类速率匹配和速率恢复。这里先统一单篇缩写：Turbo（Turbo 码，Turbo Code）在 LTE 数据传输中使用三路编码流；低密度奇偶校验码在 NR 数据传输中使用稀疏校验矩阵和母码位置；Polar（极化码，Polar Code）在 NR 控制信息中使用极化母码和 frozen mask；对数似然比是接收端在速率恢复中搬运和合并的软信息；混合自动重传请求负责重传时保留、补充和合并软信息；循环冗余校验用于译码后判定候选结果是否可信；传输块和码块分别表示上层交付的数据块和译码器逐块处理对象；正交相移键控是每个 symbol 承载 2 个 bit 的调制；正交幅度调制的调制阶数影响 NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）的 bit interleaving；块错误率用于后续链路性能验收；寄存器传输级和专用集成电路用于描述硬件实现边界。
...
- 说明重复 LLR（对数似然比，Log-Likelihood Ratio）的累加和定点饱和风险。
- 说明 RTL（寄存器传输级，Register Transfer Level）中 address generator、mask RAM、soft buffer 可以抽象复用，但协议规则和交织表不能混用。
- 列出验证 checklist：索引、mask、LLR 累加、饱和、CRC（循环冗余校验，Cyclic Redundancy Check）/syndrome/path metric。
...
| T9.1/T9.2/T9.3 | 知道 NR LDPC rate recovery 包含 bit deinterleaving、RV（冗余版本，Redundancy Version）/k0 地址扫描、soft buffer 合并和 mask 初始化。 |
...
| HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） | 知道重传不是简单替换前一次接收结果，而是补充新的软证据。 |
...
| RTL address generator 和 mask RAM 抽象 | “RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射” | 区分可共享抽象和不可共享协议规则。 |
| 三条并排流程图且标出输入输出 | “三条协议链路的正向与反向关系” | 用 Mermaid、字段表和循环缓存例子列出 LTE Turbo、NR LDPC、NR Polar 的输入输出和工程检查点。 |

本节是对比讲义，不重新推导 LTE Turbo 子块交织、NR LDPC BG（基图，Base Graph）/Zc、NR Polar reliability sequence 的全部细节；这些分别由 T7、T8/T9、T10 承接。本节必须完成的是“同名概念背后的坐标系和协议语义对比”。
...
| NR Polar | Polar codeword position 的 LLR，也就是 SC（逐次消除，Successive Cancellation）/SCL（连续消除列表译码，Successive Cancellation List）tree 输入坐标。 |
...
换句话说，LTE Turbo rate recovery 的闭环是：先在 circular buffer 坐标上把 RV 选出的 LLR 写回并做 HARQ soft combining，再按 bit collection 反向拆成系统流、第一校验流和第二校验流，最后分别做 sub-block deinterleaving，把三路 LLR 交给两个 SISO（软入软出，Soft-Input Soft-Output）译码器。这个流程和 NR LDPC 的 base graph/lifting 母码坐标、NR Polar 的 bit selection/frozen mask 坐标都不同；共享软件或 RTL 模块时只能共享“地址轨迹生成和 mask 事件”这种抽象接口，不能共享具体表、起点公式或 `<NULL>`/punctured/shortened 的初始化语义。
...
LDPC 的 `k0` 表值本节不复现。当前本地 `table_0021.csv/html` 只保留了表头和 RV 行，数值为空；因此正文不使用具体 `k0` 数字。工程实现若需要 bit-exact，应把 `k0` 表作为配置 ROM（只读存储器，Read-Only Memory）或常量表，并用协议重建结果生成。

NR LDPC 还要额外关注高阶调制的 bit-channel 不均衡。QPSK（正交相移键控，Quadrature Phase Shift Keying）每个 symbol 只有 2 个 bit lane，16QAM、64QAM、256QAM 和 1024QAM（1024 阶正交幅度调制，1024 Quadrature Amplitude Modulation）则有更多 bit lane；在 Gray-mapped QAM（正交幅度调制，Quadrature Amplitude Modulation）中，不同 lane 的 LLR 可靠性通常不同。TS 38.212 §5.4.2.2 的 bit interleaving 与 $Q_m$ 绑定，目的之一就是让 rate-matched bits 按协议定义的 lane 顺序进入调制。接收端若漏做 bit deinterleaving，不只是顺序错，还会把“某个 lane 的可靠度统计”错放到另一个 LDPC 母码位置。高 SNR（信噪比，Signal-to-Noise Ratio）时这种错误尤其隐蔽：LLR 幅度很大，LDPC core 会强烈相信错位证据，表现为 syndrome 停滞、CB（码块，Code Block）CRC 稳定失败或 HARQ 合并后更难恢复。详细公式、QPSK/16QAM 手算例子和接口顺序风险见 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]]；本节只强调横向对比中的结论：LTE Turbo 没有 NR LDPC 这种 $Q_m$ bit deinterleaver，NR Polar 的 coded-bit interleaving 又是另一套规则，三者不能复用同一张置换表。
...
5. 输出 SC/SCL/CA-SCL（CRC 辅助 SCL，CRC-aided SCL）decoder 的 $N$ 个 codeword LLR。
...
| `mask` | `<NULL>` mask、valid_rx_mask。 | filler/shortened/unknown mask、CBG（码块组，Code Block Group）schedule mask。 | punctured/shortened/repeated/frozen 相关 mask。 | 可共享 mask RAM 形态，不共享生成规则。 |
...
| LTE 分段 filler | LTE 码块分段输入侧 | 取决于分段/编码位置，属于构造约束 | 按分段规则处理，码块重组时删除对应占位。 | 若留进 TB（传输块，Transport Block），CRC 或业务比特边界错。 |
...
浮点验证先不要直接追求完整 BLER（块错误率，Block Error Rate）曲线，而要先证明索引轨迹正确：
...
| 单元 | 可共享内容 | 不可共享内容 |
|:---|:---|:---|
| LLR ingress | FIFO、DMA（直接存储器访问，Direct Memory Access）、长度计数、valid/ready。 | 不同 codec 的 $E$ 解释和 CB/candidate 边界。 |
...
| 用 LTE 表处理 Polar sub-block | 控制信息 CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）失败，且路径 PM 看似正常。 | 分离 interleaver table id，禁止跨 codec 复用。 |
...
| 项目 | 记录 |
|:---|:---|
| Roadmap Prompt | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` [[T11.2_LTE_NR_rate_matching_comparison|T11.2]]：要求从接收端视角对比 LTE Turbo、NR LDPC 和 NR Polar 的速率匹配/反匹配，聚焦循环缓存、交织、RV 和反向映射。 |
| 台账要求 | 已覆盖 `docs/audits/lte_nr_decoding_remaining_work_register.md` [[T11.2_LTE_NR_rate_matching_comparison|T11.2]] 的共同目标、空洞语义、统一字段表、小循环缓存例子、定点饱和、RTL 抽象、流程图和验证 checklist。 |
...
| 审计命令 | 本轮 E3 收尾重新运行：`python3 tools/audit_lesson_terms.py docs/L2_协议算法/T11.2_LTE_NR_rate_matching_comparison.md`；`python3 tools/audit_markdown_headings.py docs/L2_协议算法/T11.2_LTE_NR_rate_matching_comparison.md`；`python3 tools/audit_lesson_depth.py --strict docs/L2_协议算法/T11.2_LTE_NR_rate_matching_comparison.md`；`python3 tools/audit_latex_render.py docs/L2_协议算法/T11.2_LTE_NR_rate_matching_comparison.md`。结果见本轮最终审计输出。 |
...
3. `docs/L2_协议算法/T7.1_LTE_Turbo_de_rate_matching_overview.md` and `docs/L2_协议算法/T7.2_LTE_subblock_deinterleaver_circular_buffer.md`.
4. `docs/L2_协议算法/T9.1_NR_LDPC_rate_recovery_overview.md`, `docs/L2_协议算法/T9.2_NR_LDPC_circular_buffer_states.md`, and `docs/L2_协议算法/T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0.md`.
5. `docs/L2_协议算法/T10.7_NR_Polar_rate_recovery.md`.
