...
| 资源分配类型 | 位宽 | 编码方式 |
|:---|:---|:---|
| Type 0（位图） | $N_{\text{RBG}}$ 位（配置） | 每个 RBG（资源块组，Resource Block Group）1 位；$N_{\text{RBG}} = \lceil (N_{\text{BWP,start}} + N_{\text{BWP,size}})/P \rceil$，名义 RBG 大小 $P$ 查表 5.1.2.2.1-1（带宽 37-72 PRB 时 $P=4$） |
...
0_0 的字段全部固定位宽（除 FDRA 随 BWP 大小变化）——这正是回退格式的设计哲学：**任何配置状态下的 UE 都能解析**。
...
| `mcs-Table = qam1024`（Rel-19） | Table 5.1.3.1-4（支持 1024QAM，$Q_m = 10$） |
| SI-RNTI / P-RNTI / RA-RNTI 调度的 1_0 | 恒用 Table 5.1.3.1-1，且协议要求 $Q_m \le 2$ |

Table 5.1.3.1-1 的部分行（本地核对）：$I_{MCS}=10 \to (Q_m=4, R=340/1024)$、$I_{MCS}=16 \to (Q_m=4, R=658/1024)$、$I_{MCS}=18 \to (Q_m=6, R=466/1024)$、$I_{MCS}=26 \to (Q_m=6, R=873/1024)$。注意 **$I_{MCS}=28$ 对应"保留"（reserved）**：在某些表/场景下它表示 TB 禁用或特殊处理（如双码字下 $I_{MCS}=26$ 且 $rv=1$ 时禁用该 TB，§5.1.3.2）。
