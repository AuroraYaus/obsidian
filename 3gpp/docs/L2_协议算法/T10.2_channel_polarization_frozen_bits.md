...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/T10.2_channel_polarization_frozen_bits.md"
---
# T10.2 信道极化、冻结位与信息位
## 本节学习目标

本节从零基础解释 NR Polar（极化码，Polar Code）的理论核心：信道极化（channel polarization，编码变换后不同 bit 位置可靠性向两端分化的现象，[[Channel_Polarization_信道极化]]）、bit-channel、冻结位（frozen bit）和信息位。这里的“信道极化”不是物理天线极化，也不是调制星座点的正负方向，而是编码变换后不同 bit 位置的可靠性分化。某些位置更适合承载真实信息，某些位置更不可靠，适合放发送端和接收端都知道的固定值。

本节使用的单篇技术缩写先统一说明：对数似然比是译码器输入软信息；逐次消除译码（Successive Cancellation, SC）是 Polar 的基础译码方式；循环冗余校验在后续 CA-SCL（CRC 辅助 SCL，CRC-aided SCL）中辅助路径选择；寄存器传输级和专用集成电路用于描述硬件实现边界。
...
| LLR（对数似然比，Log-Likelihood Ratio） | 知道正负表示更像 0 或 1，绝对值表示可靠度。 |
| T1.6 信息论直觉 | 知道信道有可靠和不可靠之分。 |
| T10.1 | 知道 Polar 接收链路需要 rate recovery、frozen mask、SCL（连续消除列表译码，Successive Cancellation List）/CRC（循环冗余校验，Cyclic Redundancy Check）selector。 |
...
| T10.1 | `docs/L2_协议算法/T10.1_NR_Polar_decoder_chain_overview.md` |

边界说明：本节是理论基础课，不复现完整 NR reliability sequence，也不处理 rate matching 后的 puncturing/shortening/repetition 如何改变信息集合。完整可靠性序列、索引筛选和 NR construction 规则由 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 承接；Polar rate recovery 由 [[T10.7_NR_Polar_rate_recovery|T10.7]] 承接。本节不省略协议证据，只是不把后续章节的表格提前展开。
...
本节采用教学约定：$\mathbf{u}$ 按 $u_0,u_1,u_2,u_3$ 的自然顺序写，$G_4$ 使用式 (6)，输出按 $x_0,x_1,x_2,x_3$ 解释。NR 的可靠性序列、interleaving、rate matching 和具体顺序筛选由 [[T10.3_NR_Polar_reliability_sequence|T10.3]]/[[T10.7_NR_Polar_rate_recovery|T10.7]] 处理。工程实现必须在 descriptor 中固定顺序约定，至少 dump：

| 字段 | 目的 |
|:---|:---|
| `index_base` | 说明表格/日志使用 0-based 还是 1-based。 |
| `info_set_order` | 说明 information set 是按 bit index 升序还是可靠性顺序存储。 |
| `frozen_mask_hash` | 快速比对 RTL（寄存器传输级，Register Transfer Level）和 golden model 是否使用同一 mask。 |
...
| 对象 | 定点风险 | 后续承接 |
|:---|:---|:---|
| LLR | f/g 函数饱和或符号约定错。 | [[T10.4_Polar_SC_decoding|T10.4]]、[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]]。 |
| path metric | SCL 排序方向和累加位宽错。 | [[T10.5_Polar_SCL_decoding|T10.5]]、[[T10.6_CRC_aided_SCL_control_reliability|T10.6]]。 |
| mask | bit pack 后下标错，导致 frozen/information 反。 | 本节验证字段，[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] 边界案例。 |
...
| 结论 | 协议依据 | 本地证据 |
|:---|:---|:---|
| Polar 编码使用 $F^{\otimes n}$ 并在 GF(2) 中执行 | TS 38.212 §5.3.1.2 | `content.md:873-943` |
| Polar sequence 按可靠性升序排列 | TS 38.212 §5.3.1.2 | `content.md:873-879`; `tables/table_0012.csv/html` |
| information/frozen 位置集合来自 Polar sequence 和 rate matching 相关规则 | TS 38.212 §5.3.1.2/§5.4.1.1 | `content.md:879-883`; `content.md:1041-1079` |

## 图示
![图 1：NR Polar N=4 transform frozen mask](assets/T10.2_NR_Polar_N4_transform_frozen_mask.svg)

## 执行与证据记录

| 项 | 记录 |
|:---|:---|
| 写作范围 | 新增 `docs/L2_协议算法/T10.2_channel_polarization_frozen_bits.md`。 |
...
| 自动审计 | `python3 tools/audit_lesson_terms.py docs/L2_协议算法/T10.2_channel_polarization_frozen_bits.md` 输出 `LESSON_TERM_AUDIT_OK`；`python3 tools/audit_markdown_headings.py docs/L2_协议算法/T10.2_channel_polarization_frozen_bits.md` 输出 `MARKDOWN_HEADING_AUDIT_OK`；`python3 tools/audit_lesson_depth.py --strict docs/L2_协议算法/T10.2_channel_polarization_frozen_bits.md` 输出 `LESSON_DEPTH_AUDIT_OK`；`python3 tools/audit_latex_render.py docs/L2_协议算法/T10.2_channel_polarization_frozen_bits.md` 输出 `LATEX_RENDER_AUDIT_OK formulas=63`。 |
| 审核经理 | 待审核。 |
...
3. 本项目 [[T10.1_NR_Polar_decoder_chain_overview|T10.1]]：`docs/L2_协议算法/T10.1_NR_Polar_decoder_chain_overview.md`。
