...
## 最小手算模型：三种译码器的面积-吞吐-存储换算

**本节主角解决什么问题**：比较三种译码器的硬件代价时，最容易被名词带偏——Turbo 公式多、LDPC 图大、Polar 排序难，这些定性印象无法直接指导面积与吞吐决策。本节给出一个最小换算模型，解决一个具体问题：**把面积、吞吐、存储三种代价放进同一把尺子**，让"哪个译码器在什么口径下更便宜"变成可手算的数值结论，而不是形容词结论。

换算模型解决的是口径问题：不同译码器工作在不同块长、迭代次数与列表深度下，直接比"核心面积"或"峰值周期数"都会失真；先把每个译码器的代价归一成"每单位吞吐的面积"，再比，结论才稳定。这正是下文「工程决策矩阵」给定性、本节给定量的分工。本节的三个口径与正文其余各节的衔接：吞吐口径接「数值例子：教学级周期估算」，面积与存储口径接「工程决策矩阵」，带宽换算接「三类硬件数据通路」的存储瓶颈行。

### 白话直觉：运输成本的类比

比硬件代价像比运输成本：不能只比车价（核心面积），也不能只比最高时速（峰值吞吐），要看"每公里运费"（单位吞吐面积）与"油箱多大"（存储）。一辆车再便宜，跑得慢、每公里油费高，综合成本未必低；一辆快车如果靠一个巨大油箱撑着，成本也未必好看。译码器同理：核心逻辑面积小不等于代价低，存储往往反超逻辑，口径必须统一。

这个直观类比还提示第三件事：**短途与长途要分开计价**——Polar 的短块效率不能直接当作长块数据面的效率，正如市内配送车的时速不能用来估算长途货运的运力。这正是下文第 4 步的口径修正。统一口径的另一个好处是评审可复算：同一张表任何人都能用四则运算重算一遍，结论不依赖"行业经验"。这套换算同样适用于任意工艺与时钟——把 0.0012 mm²/KB 与 500 MHz 换成目标值重算即可。

### 手算例子：同一工艺、同一 500 MHz 时钟下的三行换算

教学假设（不代表真实芯片）：同一工艺、同一 500 MHz 时钟；SRAM（静态随机存取存储器，Static Random-Access Memory）密度取 1 KB ≈ 0.0012 mm²；bit/cycle 数取自本讲义「数值例子：教学级周期估算」一节的教学值。所有 mm² 与 KB 数值均为教学量级，仅用于演示换算次序。先列三行原始数据：

| 译码器 | 核心逻辑面积 | 关键存储 | 教学吞吐（bit/cycle） | 500 MHz 数据面吞吐 |
|:---|:---|:---|:---|:---|
| Turbo | 0.35 mm² | 640 KB（外信息 + $\alpha/\beta$ 度量） | 0.076 | $0.076\times500=38$ Mbit/s |
| LDPC | 0.30 mm² | 1024 KB（LLR + 消息） | 0.255 | $0.255\times500=127.5$ Mbit/s |
| Polar SCL | 0.25 mm² | 256 KB（路径 + 部分和，$L=8$） | 0.217（128 bit 块口径） | $0.217\times500=108.5$ Mbit/s（仅短块） |

逐步手算三步换算：

1. 吞吐换算（bit/cycle × 时钟频率）：Turbo $0.076 \times 500 = 38$ Mbit/s，LDPC $0.255 \times 500 = 127.5$ Mbit/s——同频同工艺下 LDPC 数据面吞吐约为 Turbo 的 3.4 倍。
2. 单位吞吐核心面积（面积效率口径）：核心面积 ÷ 吞吐。Turbo $0.35 / 38 \approx 0.0092$ mm²/(Mbit/s)，LDPC $0.30 / 127.5 \approx 0.0024$ mm²/(Mbit/s)——每 Mbit/s 的核心逻辑代价，LDPC 约为 Turbo 的 1/4。
3. 存储面积换算（KB × SRAM 密度）：Turbo $640 \times 0.0012 = 0.77$ mm²，LDPC $1024 \times 0.0012 = 1.23$ mm²，Polar $256 \times 0.0012 = 0.31$ mm²——LDPC 的存储面积（1.23 mm²）反超其核心逻辑（0.30 mm²）4 倍，"存储主导面积"不是形容词而是换算结果。
4. Polar 的块长口径修正：108.5 Mbit/s 是 128 bit 块内口径，控制块总载荷小，不能当作数据面吞吐外推。当列表深度 $L$ 从 8 增到 16，路径、部分和与复制网络的存储约翻倍（$0.31 \to 0.62$ mm²）——Polar 的代价主要随 list size 的存储复制增长，而不是随块长增长。

把三步换算汇总成表：

| 译码器 | 核心面积 | 存储面积 | 总面积 | 数据面吞吐 | 单位吞吐总面积 |
|:---|:---:|:---:|:---:|:---:|:---:|
| Turbo | 0.35 | 0.77 | 1.12 mm² | 38 Mbit/s | $1.12/38\approx0.0295$ mm²/(Mbit/s) |
| LDPC | 0.30 | 1.23 | 1.53 mm² | 127.5 Mbit/s | $1.53/127.5=0.0120$ mm²/(Mbit/s) |
| Polar SCL | 0.25 | 0.31 | 0.56 mm² | 108.5 Mbit/s（短块口径） | $0.56/108.5\approx0.0052$ mm²/(Mbit/s)（不可外推） |

按单位吞吐总面积读表：LDPC（0.0120）约为 Turbo（0.0295）的 40%——差距主要来自吞吐（3.4 倍），而不是面积（LDPC 总面积 1.53 mm² 反而比 Turbo 的 1.12 mm² 更大，存储主导）。Polar 的 0.0052 数字最优，但建立在 128 bit 短块口径上，不能当作数据面结论——**口径必须与块长绑定**，这正是本换算模型的第一课。

### 存储带宽的换算

面积只是存储的一半代价，另一半是带宽——存储里的每个 bit 每轮迭代都要被读写。用「吞吐和周期估算示例」的周期数换算每周期平均访问量：

1. LDPC：1024 KB 存储 = $1024 \times 1024 \times 8 = 8388608$ bit；每轮迭代读 + 写各一次，8 轮迭代共 $8388608 \times 2 \times 8 = 134217728$ 次 bit 访问；除以总周期 33120，得每周期约 4053 次 bit 访问。
2. Turbo：640 KB 存储 = $640 \times 1024 \times 8 = 5242880$ bit；6 次迭代读 + 写各一次，共 $5242880 \times 2 \times 6 = 62914560$ 次；除以总周期 81101，得每周期约 776 次。
3. Polar：路径复制发生在每个信息位的分裂处——每个信息位要为候选路径复制 partial sum 与路径状态，访问量约等于（每路径状态 bit 数 × 候选数 $2L$）÷ 每信息位周期数，随 $L$ 线性增长。
4. 位宽放大：若消息位宽从 8 bit 加到 12 bit，上述访问量同比例放大 1.5 倍——位宽是带宽的乘法器，定点位宽决策直接影响存储带宽预算。
5. 双端口代价：读改写链条要求存储支持同周期读 + 写，双端口 SRAM 的面积约为单端口的 1.3-1.5 倍（教学量级）——带宽需求的代价最终回到面积列。

每周期数千次 bit 访问，意味着 SRAM 必须多 bank 并行供给——这就是 bank conflict（LDPC）、交织随机访问（Turbo）、路径复制（Polar）会成为瓶颈的数值根源。若每个 bank 每周期可服务 32 次 bit 访问（32 bit 字宽、每周期 1 次），LDPC 的 4053 次/周期约需 127 个 bank 并行供给（$4053/32 \approx 127$），再计入读改写双端口与冲突余量还要上浮——存储带宽预算是按这个量级反推的，不是拍脑袋。这个换算的另一面是面积与带宽同源：bank 数越多面积越大，bank 数越少停等越多，面积与带宽在存储上不可兼得——这正是「工程决策矩阵」"存储访问"行的量化背景。把带宽需求与吞吐并列看：吞吐红利的前提是存储带宽跟得上，跟不上的部分就变成 $C_{\text{stall}}$（式 (3)）。

### 功耗与延迟的口径说明

本换算模型有意不列功耗与延迟两列，原因值得说明：功耗依赖活动因子、时钟门控与电压域这类工艺与实现细节，教学尺度上给不出可信的单值换算；延迟则已经隐含在吞吐口径里（吞吐 = $B/C_{\text{decode}}$，$C_{\text{decode}}$ 即延迟的周期形式）。两者的定性结论留在「工程决策矩阵」的对应行——功耗看迭代轮数与存储读写，延迟看依赖形状与排序关键路径。**模型的边界必须声明：能换算的给数值，不能换算的给定性，混在一起才是失真。**

### 正式定义

三个口径的正式定义：

- 吞吐口径：有效 bit 数 ÷ 总周期数（bit/cycle），必须与块长、迭代/列表深度绑定声明；换算成 Mbit/s 时乘时钟频率。
- 面积效率（单位吞吐核心面积）：核心逻辑面积 ÷ 数据面吞吐（mm²/(Mbit/s)），越小越好——它是"每单位吞吐的硬件价格"。
- 统一时钟假设：三个换算步骤全部基于同一 500 MHz 时钟——真实系统中三类译码器可能分处不同时钟域，换算前必须先把吞吐归一到同一频率。
- 存储面积换算：存储量 × 工艺 SRAM 密度（mm²）——把"多少 KB"翻译成"多少 mm²"，才能与核心面积相加比较。

符号约定：$f_{\text{clk}}$ 为时钟频率，$C_{\text{decode}}$ 为总译码周期，$B$ 为有效 bit 数；吞吐 = $B / C_{\text{decode}} \times f_{\text{clk}}$。三者缺一不可：只看面积漏掉吞吐，只看吞吐漏掉存储，只看存储漏掉延迟。三个口径之间可以互相换算验证：吞吐 × 单位吞吐面积 = 核心面积/周期比，任何一组数字自相矛盾都能被四则运算查出。本讲义「吞吐和周期估算示例」给出 $C_{\text{decode}}$ 的周期模型（式 (2)-(4)），本节只做"周期 → 吞吐 → 面积"的三步换算。

### 常见误解

| 误解 | 正确理解 |
|:---|:---|
| 算得快 = 吞吐高 | 吞吐 = 有效 bit/总周期，受存储带宽与依赖形状约束——Turbo 0.076 的瓶颈是迭代与交织搬运，不是乘加器慢 |
| 核心面积小 = 代价低 | LDPC 核心 0.30 mm² 但存储 1.23 mm²，按单位吞吐总面积（0.0120）反而优于 Turbo |
| Polar 短块率可外推为数据面吞吐 | 108.5 Mbit/s 是 128 bit 块口径，控制块总载荷小；外推会高估其数据面能力 |
| SRAM 是配角 | LDPC 存储面积是核心逻辑的 4 倍，存储带宽往往先于计算阵列成为瓶颈 |
| 三类译码器共用一套存储 | 访问模式不同（随机交织 / 规则移位 / 路径复制），核心存储不宜共享（见「统一译码子系统」节） |
| 教学数字可以直接用于芯片决策 | 本表是教学量级换算，真实决策还要叠加工艺、电压、时钟域与误块率（Block Error Rate, BLER）目标 |
| 面积、吞吐、存储三项够全了 | 还需延迟（吞吐口径隐含）与功耗（活动因子决定）两个定性维度，见「工程决策矩阵」 |

### 接收端后果

- 面积预算按"单位吞吐面积"分配而不是按"核心面积"分配——误判口径会导致芯片面积超预算或吞吐不达标。
- 存储密度与带宽决定译码器上限：bank conflict（LDPC）、交织随机访问（Turbo）、路径复制（Polar）各自把存储压力转成延迟或停等。
- 周期模型里的 $C_{\text{stall}}$ 类项（式 (3)）直接吃掉换算表里的吞吐红利——RTL 实测低于纸面估算时，先查存储而不是先查计算阵列。
- 存储带宽预算：按上面每周期数千 bit 访问的带宽需求反推 SRAM bank 数——bank 不足时计算阵列被迫停等，面积再省也换不回吞吐。
- 周期口径：本模型所有 Mbit/s 均按 500 MHz 教学时钟换算，真实时钟域与频率规划会整体缩放这些数字，但不改变相对结论。
- 验证视角：三个口径都可以成为 dump 校验点——吞吐对周期计数器、面积对综合报告、存储对 bank/容量占用，任何一项对不上都指向换算假设错误而不是协议错误。
- 迭代收敛口径：吞吐数字按收敛后的平均迭代/列表深度换算，最坏情况（不收敛、满迭代）下的吞吐会低于表中教学值。
- 复用口径：统一译码子系统里共享的 SRAM allocator 与 bank 监控不计入单个译码器面积，分码类成本归集时要把共享部分摊回各核。
- 教学边界：本模型刻意把功耗与延迟留给定性维度（见「功耗与延迟的口径说明」），任何试图给功耗套单值换算的估算都应在报告中声明假设。
- 验收映射：换算表的三个口径可以直接映射为验收项——吞吐验收对周期预算、面积验收对综合与布局报告、存储验收对 bank 占用与 dump 容量。
- 工程后果：本表把三类译码器的代价差异量化到同一把尺子上，架构决策（选哪类译码器做数据面、哪类做控制面）才有数值依据；下一节「任务边界与 Prompt 覆盖」的各行要求都可以在本模型的三个口径上找到落脚点。

### 换算结论的口径声明

把本节的换算浓缩成三句口径声明：(1) 数据面吞吐比面积更先决定优劣——LDPC 的单位吞吐总面积约为 Turbo 的 40%；(2) 存储面积可以反超核心逻辑——LDPC 的 1.23 mm² 对 0.30 mm²；(3) 短块效率不可外推——Polar 的数字只在 128 bit 块口径内成立。这三句结论全部来自同一组可复算的数字，任何一句被单独引用而脱离口径都会失真。这正是本节解决的最初那个问题：硬件代价对比必须建立在统一口径的数值换算上，而不是名词与印象上。

### 本节的定位

本节是「工程决策矩阵」与「数值例子：教学级周期估算」的换算底座：矩阵给定性维度，周期估算给 $C_{\text{decode}}$，本节把两者换算成面积与吞吐口径的数值结论。三个口径的顺序不能颠倒：先周期、再吞吐、后面积——颠倒顺序会把 $C_{\text{stall}}$ 遗漏在换算之外。手算例子的全部中间值（38/127.5/108.5 Mbit/s、0.77/1.23/0.31 mm²、0.0092/0.0024/0.0052 mm²/(Mbit/s)）都来自前述公式的逐步代入，建议读者独立重算一遍。
