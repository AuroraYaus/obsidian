...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/T11.3_HARQ_soft_buffer_comparison.md"
---
# T11.3 LTE/NR HARQ 与软缓存对比
## 本节学习目标

本节从接收端译码器状态的角度对比 LTE 和 NR 的混合自动重传请求软缓存。这里先统一单篇缩写：Turbo（Turbo 码，Turbo Code）是 LTE 数据业务的主要信道编码；低密度奇偶校验码是 NR 数据业务的主要信道编码；对数似然比是 soft buffer 保存和合并的软信息；传输块是 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）反馈和最终交付的基本数据块；码块是分段后送入译码器的处理块；CBG（码块组，Code Block Group）是 NR 为部分重传引入的 CB（码块，Code Block）分组；码块组传输信息（Code Block Group Transmission Information, CBGTI）指示本次哪些 CBG 被传输；码块组刷新信息（Code Block Group Flushing Out Information, CBGFI）说明下行 CBG 重传能否与旧实例合并；新数据指示（New Data Indicator, NDI）区分新 TB（传输块，Transport Block）和旧 TB 重传；冗余版本（Redundancy Version, RV）决定本次从速率匹配循环缓存的哪个窗口取得编码比特；循环冗余校验决定 soft buffer 是释放还是保留；传输块大小影响 CB 分段和本次可用比特数；上行共享信道是 LTE/NR 上行数据承载入口；下行控制信息携带 RV、NDI 和 CBG 相关控制字段；块错误率用于后续链路性能验收；静态随机存取存储器是硬件 soft buffer 的常见存储形态；寄存器传输级/专用集成电路实现要把这些字段落成地址生成、软缓存读改写、饱和加法、mask 和释放控制。（CBGTI/CBGFI 的配置与分组语义详见 [[T9.8_CBG_code_block_group|T9.8]]，增量写缓存操作详见 [[T9.7_CB_incremental_write|T9.7]]——本节只在对比语境引用。）

学完本节后，应能做到：

- 解释 HARQ soft buffer 为什么是译码器状态，不只是媒体接入控制层的调度状态。
- 说明 LTE 与 NR 的共同点：失败后保留同一编码位置的 LLR（对数似然比，Log-Likelihood Ratio），重传时按 RV 地址执行 soft combining。
- 说明 LTE 与 NR 的关键差异：LTE Turbo 主线按 TB/CB 处理；NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）可以通过 CBG 只重传和更新部分 CB 组。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| T4.3 HARQ soft combining 基础 | 知道独立观测在 LLR 域可以相加，CRC（循环冗余校验，Cyclic Redundancy Check）fail 后软信息仍有价值。 |
| T7.3 LTE HARQ 软缓存与冗余版本 | 知道 LTE Turbo RV 是同一 circular buffer 上的窗口选择。 |
| T9.3 NR LDPC HARQ soft buffer、RV k0（速率匹配起点，Rate matching starting position）与 CBG | 知道 NR LDPC RV 通过 `k0` 进入 circular buffer 地址，CBG mask 会改变哪些 CB 本次被更新。 |
...
| 讲译码控制影响 | “译码器状态机”到“RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射” | 只保留会影响 soft buffer 的调度字段。 |
...
| 最小 dump 包 | “验证方法与最小 dump 包” | LTE 查 RV/k0/Ncb，NR 额外查 CBG mask/BG（基图，Base Graph）/Zc/CBGFI。 |
...
本节依据本地 Rel-19 抽取资料。协议分工要先讲清：TS 36.212/TS 38.212 定义信道编码和速率匹配轨迹；TS 36.213/TS 38.214 提供调制、RV、TBS（传输块大小，Transport Block Size）、CBG 等物理层过程字段；TS 36.321/TS 38.321 描述 MAC（媒体接入控制层，Medium Access Control）HARQ entity/process 怎样把新传输、重传、soft buffer 合并和 ACK/NACK 连接起来。接收端 soft buffer 的 SRAM（静态随机存取存储器，Static Random-Access Memory）bank、LLR 位宽、饱和阈值和内部 pipeline 不是 3GPP 逐拍规定，但必须保持同一编码位置软信息合并这一语义。
...
- 本节不复现 TS 38.212 Table 5.4.2.1-2 的具体 `k0` 数值。当前本地抽取表存在公式/数值空洞；[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 已给出人工核验后的公式形式。本节只使用 `k0` 作为“RV 到 circular buffer 起点”的抽象字段，例子使用 toy 地址。
- 本节不展开 TS 36.213/TS 38.214 的全部调度、HARQ-ACK timing、RRC（无线资源控制，Radio Resource Control）配置树和 DCI（下行控制信息，Downlink Control Information）格式细节。只保留会进入译码 descriptor 或 soft buffer 状态机的字段。
...
CBG 不改变 LDPC 校验矩阵，也不改变单个 CB 的 Min-Sum 或 BP（置信传播，Belief Propagation）译码公式。它改变的是 HARQ 重传和 soft buffer 更新控制。TS 38.214 对 PDSCH（物理下行共享信道，Physical Downlink Shared Channel）/PUSCH（物理上行共享信道，Physical Uplink Shared Channel）CBG 都说明：CBGTI 的 bit 从最高有效位开始映射到 CBG#0，`0` 表示对应 CBG 不传输，`1` 表示传输。PDSCH 还可能有 CBGFI：`0` 表示早前同 CBG 实例可能损坏，不能简单合并；`1` 表示可合并。
...
浮点模型的目标不是做完整 BLER（块错误率，Block Error Rate）曲线，而是验证 soft buffer 状态转移和 LLR 合并语义。建议最小方案如下：
...
5. 本节不展开搜索空间、PUCCH（物理上行控制信道，Physical Uplink Control Channel）资源、完整 HARQ-ACK timing 等不会直接改变 soft buffer 的细节；保留 `ndi`、`rvidx`、CBGTI、CBGFI、TBS/CB 分段、CRC 结果等会决定清空、写回、合并、保持或释放的字段。

## 协议证据表

| 结论 | 协议证据 | 本地路径 | 证据状态 |
|:---|:---|:---|:---|
| LTE Turbo rate matching 使用 circular buffer、`Ncb`、`E`、`rvidx` | TS 36.212 §5.1.4.1.2 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md:817-965` | 已核验文本；公式抽取有空白，[[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]] 已补主线公式证据。 |
...
| LTE UL-SCH（上行共享信道，Uplink Shared Channel） RV/TBS 字段来自物理层过程 | TS 36.213 §8.6/§8.6.1 | `3GPP_Rel19/processed/TS_36.213_36213-j30_s08-s09/content.md:1227-1395` | 已核验文本；本节只作字段来源边界。 |
...
| 审计命令 | `python3 tools/audit_lesson_terms.py docs/L2_协议算法/T11.3_HARQ_soft_buffer_comparison.md` -> `LESSON_TERM_AUDIT_OK`；`python3 tools/audit_markdown_headings.py docs/L2_协议算法/T11.3_HARQ_soft_buffer_comparison.md` -> `MARKDOWN_HEADING_AUDIT_OK`；`python3 tools/audit_lesson_depth.py --strict docs/L2_协议算法/T11.3_HARQ_soft_buffer_comparison.md` -> `LESSON_DEPTH_AUDIT_OK`；`python3 tools/audit_latex_render.py docs/L2_协议算法/T11.3_HARQ_soft_buffer_comparison.md` -> `LATEX_RENDER_AUDIT_OK formulas=13`；`python3 tools/audit_reference_rebuilds.py docs/L2_协议算法/T11.3_HARQ_soft_buffer_comparison.md` -> `REFERENCE_REBUILD_AUDIT_CANDIDATES`，候选项已在正文边界中说明：本节不使用 TS 38.212 Table 5.4.2.1-2 具体 `k0` 表值。 |
