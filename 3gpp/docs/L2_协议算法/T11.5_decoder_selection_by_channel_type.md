...
本节把长期分散在 LTE、NR、数据链路、控制链路和硬件 descriptor 中的“译码器选择”统一起来。先统一本篇缩写和术语：Turbo（Turbo 码，Turbo Code）是 LTE 数据传输主线使用的译码器家族；低密度奇偶校验码是 NR 数据传输主线使用的译码器家族；Polar（极化码，Polar Code）是 NR 控制信息主线使用的译码器家族；混合自动重传请求决定数据软缓存和重传上下文；下行共享信道承载下行数据传输块；上行共享信道承载上行数据传输块；上行控制信息承载 HARQ 确认（HARQ Acknowledgement, HARQ-ACK）、信道状态信息（Channel State Information, CSI）、调度请求（Scheduling Request, SR）等上行控制比特；下行控制信息承载调度、功控、资源指示等下行控制比特；传输信道（Transport Channel, TrCH）是 TS 36.212/TS 38.212 中描述编码方案的协议对象；循环冗余校验是错误检测边界；无线网络临时标识（Radio Network Temporary Identifier, RNTI）会参与 DCI（下行控制信息，Downlink Control Information）CRC（循环冗余校验，Cyclic Redundancy Check）背景；传输块是媒体接入控制层与物理层之间的数据块；码块是译码器内部处理粒度；CBG（码块组，Code Block Group）是 NR 中可用于部分重传的 CB（码块，Code Block）分组；对数似然比是接收端软信息；调制与编码方案和传输块大小影响数据任务的码率、块长和译码负载；块错误率用于链路验收；寄存器传输级和专用集成电路用于描述硬件实现边界。

学完本节后，应能做到：

- 从 TS 36.212 和 TS 38.212 的协议表解释为什么“信道/信息类型”先于算法偏好决定译码器家族。
- 给 LTE DL-SCH（下行共享信道，Downlink Shared Channel）/UL-SCH（上行共享信道，Uplink Shared Channel）选择 Turbo 译码器，并知道 LTE 控制信息不是 NR Polar 分支。
- 给 NR DL-SCH/UL-SCH/PCH 选择 LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）译码器，并知道 BG（基图，Base Graph）、Zc、HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）、CBG 是 LDPC descriptor 的关键上下文。
- 给 NR DCI 选择 Polar 译码器，并说明 CRC/RNTI 背景对盲检和最终路径选择的影响。
- 给 NR UCI（上行控制信息，Uplink Control Information）选择 Polar 或 small-block 分支，并说明 UCI on PUSCH（物理上行共享信道，Physical Uplink Shared Channel）时数据和控制不是同一个译码任务。
- 设计接收端 `decoder_type` 选择伪代码，避免跨制式、跨信息类型误分支。
- 为验证准备协议向量分类、descriptor 分支覆盖、错误注入和 status code。

本节的核心原则是：**接收端先根据协议对象识别任务，再选择译码器家族；不能因为输入都是 LLR（对数似然比，Log-Likelihood Ratio），就把数据、控制、LTE、NR 的译码规则混在一起。**
...
| LTE DL-SCH/UL-SCH 到 Turbo | “LTE 数据传输的选择” | 解释 TB（传输块，Transport Block）/CB/HARQ/Turbo core 的接收端后果。 |
...
| 接收端选择逻辑 | “接收端选择伪代码” | 给可直接转 RTL（寄存器传输级，Register Transfer Level）/firmware 的判定结构。 |
...
本节是协议映射和接收端系统设计课。它复用 [[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 已复现的 TS 36.212 Table 5.1.3-1/2 和 TS 38.212 Table 5.3-1/2，不重复完整重建大表；本节新增的是接收端选择逻辑、descriptor 字段、边界案例和验证方法。
...
- 本节讨论译码器家族选择，不展开调度器、搜索空间、PDCCH（物理下行控制信道，Physical Downlink Control Channel）/PUCCH（物理上行控制信道，Physical Uplink Control Channel）/PUSCH 资源映射或 TS 38.213/38.214 的完整控制信道流程。
- 本节使用“TBCC”表示尾咬卷积码（Tail Biting Convolutional Code）类 LTE 控制译码分支。它不是 Turbo，也不是 NR Polar。
- NR UCI 的 “Block code / Polar” 边界依赖 UCI 比特数和对应 §6.3 分支；本节给选择框架，具体小负载处理详见 [[T10.1_NR_Polar_decoder_chain_overview|T10.1]]/[[T10.8_NR_Polar_decoder_edge_cases|T10.8]]。
- NR BCH 在 TS 38.212 Table 5.3-1 中使用 Polar，但本路线图重点是 LTE/NR 数据和控制译码，因此仅在速查表中列边界，不展开 PBCH（物理广播信道，Physical Broadcast Channel）课程。
...
这三个问题决定接收端后续对象完全不同。数据 TB 需要 TB CRC、CB segmentation、HARQ soft buffer、rate recovery、CB CRC/TB CRC 汇总和 MAC（媒体接入控制层，Medium Access Control）上交。控制信息更短，通常没有数据 TB 那样的 HARQ soft buffer 生命周期；DCI 还涉及盲检和 RNTI 背景，UCI 还涉及 PUCCH/PUSCH 背景和 HARQ-ACK/CSI/SR 等不同 payload 组合。
...
| NR TS 38.212 Table 5.3-1 | DL-SCH/UL-SCH/PCH 数据 | `decoder_type=LDPC`，需要 BG、Zc、Ncb/k0/RV（冗余版本，Redundancy Version）、HARQ 和 CBG 上下文。 |
| NR TS 38.212 Table 5.3-2 | DCI / UCI 等控制 | DCI 进入 Polar/CA-SCL（CRC 辅助 SCL，CRC-aided SCL）并检查 CRC/RNTI；UCI 按 bit length 和物理承载进入 Polar 或 small-block。 |
...
| 字段 | 为什么需要 | 典型来源 |
|:---|:---|:---|
| `BG` | 选择 LDPC base graph 1 或 2。 | TS 38.212 §6.2.2/§7.2.2 和 MCS（调制与编码方案，Modulation and Coding Scheme）/TBS（传输块大小，Transport Block Size）背景。 |
| `Zc` | 决定 QC-LDPC（准循环 LDPC，Quasi-Cyclic LDPC）lifting 和地址生成。 | TS 38.212 §5.2.2/§5.3.2。 |
...
UCI 的 small-block 例外也必须尊重。若信息比特数极小，协议可能使用 small-block coding，而不是 Polar SCL（连续消除列表译码，Successive Cancellation List）。工程实现中应让 `control_context` 明确记录 UCI payload type、bit length、PUCCH/PUSCH 背景和 chosen coding branch。
...
| `harq_context` | 数据任务 | HARQ process、NDI（新数据指示，New Data Indicator）、RV、soft buffer 地址、CBG mask。 |
...
下面用四个极小 descriptor 走一遍 selector。这个例子不计算 BLER（块错误率，Block Error Rate），也不计算译码结果，只训练“协议对象到译码器家族”的判定。
...
| 分支 | 定点关键点 | RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射 |
|:---|:---|:---|
...
| NR LDPC | channel/message/posterior LLR 位宽，NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum）系数，soft combining 饱和。 | LDPC CN/VN array、message memory、LLR RAM、bank conflict guard。 |
| NR Polar | LLR、path metric、partial sum、CRC state，PM tie-break。 | SC（逐次消除，Successive Cancellation）/SCL tree controller、sorter、path memory、CRC/RNTI selector。 |
...
| 结论 | 协议证据 | 本地路径 | 证据状态 |
|:---|:---|:---|:---|
| LTE TrCH 编码方案由 Table 5.1.3-1 给出，DL-SCH/UL-SCH 使用 Turbo | TS 36.212 §5.1.3, Table 5.1.3-1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md:683-705`; `tables/table_0007.csv/html` | 已核验；[[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 已复现表格语义，本节复用。 |
...
| NR TrCH 编码方案由 Table 5.3-1 给出，DL-SCH/UL-SCH/PCH 使用 LDPC，BCH 使用 Polar | TS 38.212 §5.3, Table 5.3-1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:807-813`; `tables/table_0009.csv/html` | 已核验；[[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 已复现表格语义，本节复用。 |
| NR 控制信息编码方案由 Table 5.3-2 给出，DCI 使用 Polar，UCI 使用 Block code / Polar | TS 38.212 §5.3, Table 5.3-2 | `content.md:807-813`; `tables/table_0010.csv/html` | 已核验；[[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 已复现表格语义，本节复用。 |
| NR UL-SCH code block individually LDPC encoded | TS 38.212 §6.2.4 | `content.md:1391-1401` | 已核验文本。 |
| NR UCI 分 PUCCH/PUSCH 背景 | TS 38.212 §6.3.1/§6.3.2 | `content.md:2035-2105`; `content.md:2863-2889` | 已核验文本；small-block/Polar 详细条件由 [[T10.1_NR_Polar_decoder_chain_overview|T10.1]]/[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] 承接。 |
...
| 项目 | 记录 |
|:---|:---|
| 路线图卡片 | `2026-06-19-lte-nr-decoding-learning-roadmap.md` [[T11.5_decoder_selection_by_channel_type|T11.5]]。 |
| 本地协议 | TS 36.212 `36212-j30`；TS 38.212 `38212-j30`。 |
| 协议表复用 | TS 36.212 Table 5.1.3-1/2 已由 [[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 复现；TS 38.212 Table 5.3-1/2 已由 [[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 复现；本节复用并补 selector 逻辑。 |
...
| 审计结果 | `LESSON_TERM_AUDIT_OK`；`MARKDOWN_HEADING_AUDIT_OK`；`LESSON_DEPTH_AUDIT_OK`；`LATEX_RENDER_AUDIT_OK formulas=0`；引用重建审计输出 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，候选项已在正文标注为复用 [[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 已复现协议表、仅列边界不展开 PBCH/调度器，或本节新增自绘 selector 图。 |
