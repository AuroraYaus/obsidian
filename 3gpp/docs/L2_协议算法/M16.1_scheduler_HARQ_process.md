...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/M16.1_scheduler_HARQ_process.md"
---
# M16.1 调度与 HARQ 进程：MAC 层的资源分配与重传状态机
...
画出"测量 → 调度 → 译码 → 反馈"的完整闭环（含讲义编号）。

**参考答案**：SRS 测量（T15.3）→ 调度器（本节）→ DCI（T14.2）→ 译码（T9）→ HARQ-ACK（T14.3）→ 调度器更新（重传优先）。
...
print(f"M16.1_OK n_slots={n_slots} new={stats['new']} retx={stats['retx']} "
...
```text
M16.1_OK n_slots=40 new=24 retx=16 ACK=29 NACK=11 retx_fail_rate~0.15 < new_fail_rate~0.3 (soft-combine gain)
```
...
下一节 M16.2 讲调度的空间维度延伸：**波束管理**——高频段下调度与波束绑定（SSB/CSI-RS 波束测量、TCI/QCL、BFR）。

## 本节在 M16 系列中的位置

M16 系列四站：M16.1（调度/HARQ，时频资源 + 重传状态机）→ M16.2（波束管理，空间维度）→ M16.3（CA/BWP，多载波与带宽适配）→ M16.4（MAC 层映射，信道映射全景收束）。本节是"调度"主题的核心——后续三篇是调度的空间/多载波/信道映射延伸。

## 执行与证据记录

| 项目 | 记录 |
|:---|:---|
| 新增文件 | `docs/L2_协议算法/M16.1_scheduler_HARQ_process.md`。 |
