...
统一译码子系统可以共享 DMA（直接内存访问，Direct Memory Access）、输入 LLR buffer、CRC checker、寄存器配置和中断/status 框架，但三类核心的内部计算单元通常不能简单共享：Turbo 的 SISO/trellis、LDPC 的 CN/VN message update、Polar 的 sorter/path memory 是不同数据通路。
