...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/T11.1_Turbo_LDPC_Polar_algorithm_comparison.md"
---
# T11.1 Turbo、LDPC、Polar 译码算法对比
## 本节学习目标

本节是对比课，不重新推导 T6、T8、T10 已经展开过的全部公式，而是把三类译码器放到同一个接收端视角下比较。先统一本篇缩写：Turbo（Turbo 码，Turbo Code）是 LTE 数据传输主线使用的并行级联卷积码；低密度奇偶校验码是 NR 数据传输主线使用的稀疏校验矩阵码；Polar（极化码，Polar Code）是 NR 控制信息主线使用的短块编码方案；对数似然比是三类译码器共同消费的软信息；循环冗余校验用于检查候选比特是否满足错误检测规则；块错误率用于链路性能验证；媒体接入控制层是物理层译码结果上交或反馈的上层边界；传输块是最终交付给 MAC（媒体接入控制层，Medium Access Control）的业务块；码块是译码核心逐块处理的对象；混合自动重传请求负责重传和软信息保留；上行共享信道、下行共享信道、上行控制信息和下行控制信息用于标识协议使用位置；寄存器传输级和专用集成电路用于描述硬件实现边界。

学完本节后，应能做到：

- 说明 Turbo、LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）、Polar 各自的名称来源、基本思想和接收端译码对象。
- 从协议事实解释 LTE 数据传输使用 Turbo、NR 数据传输使用 LDPC、NR 控制信息使用 Polar 的背景。
- 区分三种图模型：Turbo trellis、LDPC Tanner graph（Tanner 图，Tanner Graph）、Polar decoding tree。
- 区分三种译码行为：Turbo SISO（软入软出，Soft-Input Soft-Output）迭代、LDPC message passing、Polar SC（逐次消除，Successive Cancellation）/SCL（连续消除列表译码，Successive Cancellation List）path search。
- 区分软信息语义：channel LLR（对数似然比，Log-Likelihood Ratio）、extrinsic information、check/variable messages、path metric。
- 比较停止和选择边界：Turbo CRC（循环冗余校验，Cyclic Redundancy Check）/迭代、LDPC syndrome/CRC/迭代、Polar CRC-aided selection。
...
| T10.6 CRC-aided SCL | 知道 Polar SCL 保留路径列表，再用 CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）做最终选择。 |
...
| 验证方法 | “验证方法与最小 dump 包” | 覆盖 BLER（块错误率，Block Error Rate）、bit-exact、latency、memory trace、CRC/syndrome/path metric。 |
...
|:---|:---|:---|
| UL-SCH（上行共享信道，Uplink Shared Channel） | Turbo coding | 1/3 |
| DL-SCH（下行共享信道，Downlink Shared Channel） | Turbo coding | 1/3 |
...
|:---|:---|:---|
| DCI（下行控制信息，Downlink Control Information） | Tail biting convolutional coding | 1/3 |
| CFI | Block code | 1/16 |
| HI | Repetition code | 1/3 |
| UCI（上行控制信息，Uplink Control Information） | Block code / Tail biting convolutional coding | variable / 1/3 |
...
这两张表给出本节最重要的协议事实：NR 数据传输主线使用 LDPC，NR 控制信息主线使用 Polar。具体 UCI 何时走 block code、何时走 Polar，由 [[T10.1_NR_Polar_decoder_chain_overview|T10.1]]/[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] 的控制信息分支继续解释；本节只保留“控制短块与 Polar 译码器”的对比主线。
...
三类译码器的性能曲线都可能出现瀑布区和高 SNR（信噪比，Signal-to-Noise Ratio）失败，但机制不同。对比时不能只看最终 BLER 数字，还要看失败发生在什么内部对象上。
...
| LDPC | Tanner 图消息逐轮让 hard decision 满足更多校验。 | trapping set、短环、NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum）参数不合适、LLR/消息饱和。 | syndrome weight 曲线、CN/VN message 饱和、layer trace。 |
...
容量和编码增益也要按同一口径比较。Turbo、LDPC、Polar 的曲线若使用不同块长、码率、调制、SNR 定义或停止条件，就不能直接比较“谁离容量更近”。[[T1.6_information_theory_minimum_for_decoding|T1.6]] 提供容量和容量差距语言，[[T4.5_decoder_performance_metrics|T4.5]]/[[T17.5_BER_BLER_curve_reporting|T17.5]] 负责把瀑布区、错误平层和置信区间写进报告。
...
式 (3) 回答的问题是：某条校验方程根据其他变量的证据，对目标变量给出什么约束。它保留 SPA（和积算法，Sum-Product Algorithm）校验节点的符号乘积和最小幅度结构，硬件上可做 min1/min2 优化。
...
|:---|:---|:---|:---|
| Turbo | 系统位/校验位三路流和交织地址。 | SISO 在 trellis 上做前向/后向递推，输出外信息；两个 decoder 迭代交换。 | `extrinsic[bit_id]`、iteration posterior、CB（码块，Code Block）CRC 状态。 |
| LDPC | rate recovery 后的 LDPC 母码位置。 | 作为变量节点 channel LLR，和 CN/VN 消息一起更新 posterior。 | edge messages、posterior LLR、syndrome weight、CB/TB（传输块，Transport Block）CRC 状态。 |
...
| HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）与 IR | LTE Turbo 通过 circular buffer、RV（冗余版本，Redundancy Version）和 soft combining 支持增量冗余。 | LDPC mother code、limited buffer、RV/k0 和 puncturing 更自然地在大母码坐标上做覆盖。 | 两者都支持 IR-HARQ；差异在地址空间、母码结构和重传覆盖粒度。 |
...
因此，LTE 接收机设计文档应把 Turbo 当作自己的数据链路主角：先按 TS 36.212 恢复三路 Turbo 软输入，再用 BCJR（BCJR 算法，Bahl-Cocke-Jelinek-Raviv Algorithm）/Log-MAP 系列算法做迭代译码，最后用 CB/TB CRC 交付结果。NR 接收机的数据主线才切换到 TS 38.212 LDPC rate recovery 和 LDPC core。这个边界可以避免两类错误：一是把 LTE Turbo 写成 NR LDPC 的过渡注脚；二是把 NR 选择 LDPC 误读成 Turbo 在 LTE 场景中“不重要”。
...
| 典型错误 | 外信息重复计数、交织地址错、tail 边界错。 | BG（基图，Base Graph）/Zc/RV 地址错、message memory/bank conflict、syndrome/CRC 混淆。 | frozen mask 错、PM 方向错、CRC/RNTI 边界错。 |
...
| 存储 | extrinsic RAM、interleaver/deinterleaver 地址、软缓存。 | edge message memory、posterior LLR、soft buffer、shift table。 | path LLR memory、partial sum memory、PM list、frozen/reliability ROM（只读存储器，Read-Only Memory）。 |
...
统一译码子系统可以共享 DMA（直接存储器访问，Direct Memory Access）、输入 LLR buffer、CRC checker、寄存器配置和中断/status 框架，但三类核心的内部计算单元通常不能简单共享：Turbo 的 SISO/trellis、LDPC 的 CN/VN message update、Polar 的 sorter/path memory 是不同数据通路。
...
| `harq_id/rvidx` | rate recovery/HARQ 需要 | rate recovery/HARQ/CBG（码块组，Code Block Group）需要 | 控制分支通常不按同一数据 HARQ 语义处理 | soft buffer 污染。 |
...
[[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]] 的浮点仿真不要求重新实现完整 Turbo/LDPC/Polar golden model，但要建立可比指标：
...
| `memory_trace_hash` | 对比 RTL（寄存器传输级，Register Transfer Level）和 golden model 的地址轨迹。 |
...
3. 为什么 NR 控制信息适合 Polar SCL/CA-SCL（CRC 辅助 SCL，CRC-aided SCL），而不是简单复用 LDPC 数据译码器？
...
| 项目 | 记录 |
|:---|:---|
| Roadmap Prompt | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` [[T11.1_Turbo_LDPC_Polar_algorithm_comparison|T11.1]]：要求从图模型、迭代/列表行为、复杂度、存储、延迟和数据/控制信道适配性对比 Turbo、LDPC、Polar。 |
...
| 审计命令 | 单篇审计已运行：`python3 tools/audit_lesson_terms.py docs/L2_协议算法/T11.1_Turbo_LDPC_Polar_algorithm_comparison.md` 输出 `LESSON_TERM_AUDIT_OK`；`python3 tools/audit_markdown_headings.py ...` 输出 `MARKDOWN_HEADING_AUDIT_OK`；`python3 tools/audit_lesson_depth.py --strict ...` 输出 `LESSON_DEPTH_AUDIT_OK`；`python3 tools/audit_latex_render.py ...` 输出 `LATEX_RENDER_AUDIT_OK formulas=22`；`python3 tools/audit_reference_rebuilds.py ...` 输出 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，作为候选清单处理。 |
| C1 补写 | 已新增“NR 数据主码从 Turbo 转向 LDPC 的动机”，覆盖吞吐、并行度、QC/IR-HARQ、错误平层、功耗和协议场景边界；明确 LTE Turbo 仍是 LTE 数据链路主角。 |
| 证据边界 | 本节是算法/协议/工程对比，不重新复现完整 Turbo interleaver 表、LDPC BG/shift 表、Polar reliability sequence 或 rate matching 表；这些已由 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]]/[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]/[[T10.3_NR_Polar_reliability_sequence|T10.3]]/[[T10.7_NR_Polar_rate_recovery|T10.7]] 等章节承接。 |
