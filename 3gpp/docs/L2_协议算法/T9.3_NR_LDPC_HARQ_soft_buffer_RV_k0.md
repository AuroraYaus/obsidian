...
- 列出最小调试 dump：CBGTI mask、CBGFI、RV/k0、CB status、TB CRC（循环冗余校验，Cyclic Redundancy Check）、soft buffer 写前写后摘要和 saturation count。
...
| 要求来源 | 原子要求 | 正文覆盖位置 |
|:---|:---|:---|
| roadmap T9.3 | 讲解 NR LDPC RV、`k0`、circular buffer 起点 | “RV、k0（速率匹配起点，rate matching starting position）与 circular buffer 起点”。 |
...
| 协议补充边界 | 说明 PDSCH（物理下行共享信道，Physical Downlink Shared Channel）CBGFI 的 flush/combine 语义 | “CBG partial retransmission 的接收端语义”“接收端伪代码”“验证方法与最小 dump 包”。 |
...
- 本节不会展开 TS 38.214 的全部调度器规则、HARQ-ACK 时序和 RRC（无线资源控制，Radio Resource Control）配置树。只讲它们如何影响译码器 descriptor。
...
CBG 的关键不是给 LDPC 增加一种新码。CBG 不改变 LDPC 校验矩阵，也不改变单个 CB 的 BP（置信传播，Belief Propagation）/Min-Sum 译码算法。它改变的是 HARQ 重传和 soft buffer 更新的控制边界：某个 CBG 本次没有被传输时，组内 CB 的 soft buffer 不应写入新 LLR；某个 CBG 被传输时，组内每个 CB 按本次 RV 进行 rate recovery 和 soft combining。

TS 38.214 §5.1.7.2 对 PDSCH 重传说明 CBGTI 指示哪些 CBG 出现在传输中，`0` 表示对应 CBG 不传输，`1` 表示传输。同一段还定义 CBGFI：若该字段存在，`CBGFI=0` 表示被重传的相同 CBG 早前接收实例可能已损坏，接收端不能简单把旧实例当作可合并证据；`CBGFI=1` 表示本次重传 CBG 可与早前实例合并。§6.1.5.2 对 PUSCH（物理上行共享信道，Physical Uplink Shared Channel）也给出 CBGTI 对应规则，并说明 bit order 从 MSB 映射到 CBG#0 往后。译码器实现必须把这些 bit order 和 flush/combine 语义固化到 descriptor 解析中，否则最常见错误就是“本该保持的 CBG 被覆盖，本该更新的 CBG 没有新证据”，或“本该 flush 的旧 CBG 仍被累加”。
...
| 协议参数层 | TS 38.212 真实 BG（基图，Base Graph）、`Ncb`、`E`、RV | RV0/1/2/3 地址覆盖统计、unknown/known/repeated 计数。 |
