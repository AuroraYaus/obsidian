...
本节使用的单篇技术缩写先统一说明：低密度奇偶校验码是 NR 数据业务采用的信道编码；基图（Base Graph, BG）是 NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）校验矩阵的模板，协议使用 BG1/BG2；提升大小（Lifting Size, Zc）把 BG 中的一个元素扩展为 $Z_c \times Z_c$ 子矩阵；对数似然比是译码器输入软信息；码块是逐块 LDPC 译码对象；CBG（码块组，Code Block Group）是 NR 部分重传的 CB（码块，Code Block）分组；传输块是最终交付给媒体接入控制层的业务块；循环冗余校验用于检查 CB/TB（传输块，Transport Block）候选比特；混合自动重传请求根据失败状态决定软缓存保留和重传；冗余版本（Redundancy Version, RV）决定 rate recovery 的 circular buffer 起点；调制与编码方案和传输块大小提供分段、码率和资源边界；下行控制信息携带 RV、CBG 等控制字段；上行共享信道和下行共享信道是本节引用的数据信道；寄存器传输级/专用集成电路实现要能 dump descriptor、地址轨迹、LLR（对数似然比，Log-Likelihood Ratio）范围、syndrome 曲线和 CRC（循环冗余校验，Cyclic Redundancy Check）边界。

本节不是再讲一个新算法，而是把 T8/T9 的协议、算法和工程边界串成一套排查清单。NR LDPC 链路失败时，最危险的习惯是马上怀疑 Min-Sum 或 BP（置信传播，Belief Propagation）核心。实际工程中，更多失败来自 descriptor、rate recovery、filler、RV、CBG、拼接顺序或 CRC 输入边界。
...
- 给出最小 dump 包，能让软件 golden model、RTL（寄存器传输级，Register Transfer Level）波形和协议 descriptor 对齐。
...
| CRC 通过但上层组包失败 | “案例总表” | MAC（媒体接入控制层，Medium Access Control）/TB id/codeword/context 字段。 |
...
正文额外补充 Python 诊断分类片段、RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射、验证方法和自测答案。
...
边界说明：本节不会复现全部协议表值。BG 选择和 Zc 表值由 [[T8.2_NR_LDPC_base_graph_selection|T8.2]]/[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 负责；RV/k0 由 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 负责；本节引用这些章节的结论，用于组织失败排查。

## 排查顺序原则

失败排查应按协议链路从外到内推进：

1. Descriptor：BG、Zc、C、K、E、Qm（调制阶数，Modulation Order）、Ncb、RV、CBG、NDI（新数据指示，New Data Indicator）、HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）id 是否来自同一个调度上下文。
...
| 现象 | 更可能的假设 | 暂时不优先的假设 |
|:---|:---|:---|
| 多个 CB 同时 fail，且都在边界 TBS（传输块大小，Transport Block Size）/MCS（调制与编码方案，Modulation and Coding Scheme）附近 | BG/Zc/segmentation descriptor 错 | 单个 CN/VN 算法 bug。 |
...
这说明 $\hat{\mathbf{x}}$ 满足 toy LDPC 校验。但如果业务 TB candidate 应该按 `[bit0, bit1, bit2, bit3]` 输入 CRC，而实现错误地按 `[bit2, bit1, bit0, bit3]` 拼接，LDPC syndrome 仍然是 0，CRC 却可能失败。这个例子解释了为什么 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 的 bit order、[[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]] 的 concat order 和本节的 CRC boundary 必须单独 dump。
...
这个数值例子解释了为什么 [[T9.6_NR_LDPC_decoder_edge_cases|T9.6]] 的最小 dump 包必须包含 `addr_trace_hash`、`unknown_count`、`repeat_count`、`new_count` 和 `rvidx/k0/Ncb`。只看最终 CRC fail，无法判断是信道真的差，还是重传证据被写到了错误坐标。
...
|:---|:---|:---|:---|:---|:---|
| BG 选择边界 | $A/R$ 靠近 BG1/BG2 分支边界，或重传复用旧 descriptor | syndrome 长期不收敛；CB CRC fail；H 尺寸与 soft buffer 不匹配 | `A`, `R`, `BG`, `MCS`, `TBS`, `NDI` | 对照 [[T8.2_NR_LDPC_base_graph_selection|T8.2]] 重算 BG；检查重传是否沿用初传 BG | 修正 descriptor 生成和 HARQ 上下文继承。 |
| Zc 选择边界 | $K'$ 刚好跨过 lifting size set 门限 | 矩阵尺寸、layer count、地址深度错；可能越界或 syndrome 异常 | `Zc`, `iLS`, `Kb`, `K`, `filler_count`, table id | 对照 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] Table 5.3.2-1/2/3 复算 | 修正 Zc 选择和 shift table 列选择。 |
...
| limited buffer | `limitedBufferRM` 启用，$N_{\mathrm{cb}}$ 小于母码长度 | RV 地址落点和覆盖率异常 | `ILBRM`, `N`, `Ncb`, `E`, `rv` | 对照 [[T9.1_NR_LDPC_rate_recovery_overview|T9.1]]/[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 复算 circular buffer 长度 | descriptor 明确 limited buffer，地址生成器使用 Ncb。 |
| RV mismatch | DCI（下行控制信息，Downlink Control Information）RV、HARQ state 或 k0（速率匹配起点，Rate matching starting position）表选择错 | soft buffer 命中区域异常；重复/新覆盖统计不对 | `rvidx`, `k0`, `BG`, `Zc`, `addr_trace` | 固定 RV pattern，比较地址轨迹 | 修正 RV 解析、k0 表和 HARQ process 更新。 |
...
| LLR saturation | 输入缩放过大、HARQ 合并未饱和保护、位宽过窄 | syndrome 曲线停滞；posterior 大量夹在最大/最小值 | `llr_min/max`, `sat_count`, `scale`, `iteration` | 统计每轮饱和比例和失败 CB | 调整缩放、位宽、饱和加法和 NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum）参数。 |
...
| CRC pass but upper assembly fail | PHY TB CRC pass，但 MAC/RLC（无线链路控制层）组包失败 | PHY 交付 pass，上层序列号/长度异常 | `tb_id`, `cw_id`, `harq_id`, `MAC length`, `LCID` | 比对 MAC PDU 长度、TB mapping 和多 TB 上下文 | 修正 PHY-to-MAC 交付元数据和上下文绑定。 |
...
2. 按 [[T8.2_NR_LDPC_base_graph_selection|T8.2]] 的 BG 选择规则离线重算 BG。
...
- 完整 H 的尺寸错误。
- Layer loop 次数或 shift ROM（只读存储器，Read-Only Memory）读列错误。
...
2. 对照 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] Table 5.3.2-1 重算 `Zc` 和 `iLS`。
...
2. 用 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 的 k0 公式生成 expected address trace。
...
触发条件：CBGTI 从 LSB 开始解析、CBG-to-CB map 与初传不一致、PDSCH（物理下行共享信道，Physical Downlink Shared Channel）CBGFI=0 时仍合并旧实例。
...
3. 对照 [[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]] 检查 `concat_order` 和 `tb_crc_input_len`。
4. 对照 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 检查 bit deinterleaving 是否导致信息位 lane 顺序错。
...
4. 查 hard bits、filler ranges、CB CRC ranges、payload ranges、concat_order、tb_crc_input_len、[[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] bit order。
...
| NR LDPC H 来自 BG、Zc 和 shift table | TS 38.212 §5.3.2 | `content.md:948-989`；[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 已复现 Table 5.3.2-1/2/3 |
...
3. 本项目 [[T8.2_NR_LDPC_base_graph_selection|T8.2]]：`docs/L2_协议算法/T8.2_NR_LDPC_base_graph_selection.md`。
4. 本项目 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]：`docs/L2_协议算法/T8.3_NR_LDPC_lifting_QC_matrix.md`。
5. 本项目 [[T8.8_NR_LDPC_decoder_numeric_walkthrough|T8.8]]：`docs/L2_协议算法/T8.8_NR_LDPC_decoder_numeric_walkthrough.md`。
6. 本项目 [[T9.1_NR_LDPC_rate_recovery_overview|T9.1]]-[[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]]。
