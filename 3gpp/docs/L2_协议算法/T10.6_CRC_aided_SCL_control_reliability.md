...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/T10.6_CRC_aided_SCL_control_reliability.md"
---
# T10.6 CRC 辅助 SCL 与控制信道可靠性
## 本节学习目标

本节讲循环冗余校验辅助逐次消除列表译码（CRC-aided Successive Cancellation List, CA-SCL）。[[T10.5_Polar_SCL_decoding|T10.5]] 的 SCL（连续消除列表译码，Successive Cancellation List）能输出多条候选路径和路径度量（Path Metric, PM），但控制信息译码不能只看 PM。CRC（循环冗余校验，Cyclic Redundancy Check）会随 payload 进入 Polar（极化码，Polar Code）的 information set，译码后用来判断候选路径是否满足检错关系。对下行控制信息，CRC 还与无线网络临时标识（Radio Network Temporary Identifier, RNTI）存在边界；对上行控制信息，CRC 长度和是否附加 CRC 与 UCI（上行控制信息，Uplink Control Information）payload 条件有关。
...
- 区分 UCI CRC、DCI（下行控制信息，Downlink Control Information）CRC 和 DCI RNTI 边界。
- 给出 final selector 伪代码、多路径 CRC checker 的 RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射和常见错误。
...
证据边界：`content.md` 对部分公式符号抽取不完整，因此本节不重新复现 CRC 多项式细节；CRC 多项式家族由 [[T3.1_LTE_NR_CRC_families|T3.1]] 复现。本节只精读影响 CA-SCL 选择逻辑的协议事实：UCI/ DCI 是否带 CRC、CRC 长度、DCI 的 RNTI 边界，以及 CRC 后进入 Polar 编码链路。
...
| 组成 | 含义 | 解决的问题 |
|:---|:---|:---|
| SC（逐次消除，Successive Cancellation） | Successive Cancellation，逐次消除。 | 沿 Polar 树逐位判决，但只保留一条路径。 |
...
| 误解 | 正确理解 |
|:---|:---|
| PM 最小就一定正确。 | PM 只衡量候选与 LLR（对数似然比，Log-Likelihood Ratio）的一致性，CRC fail 的路径不能输出为有效控制信息。 |
...
| 控制信息 | 典型内容 | 译码错误后果 |
|:---|:---|:---|
| UCI | HARQ-ACK、SR、CSI（信道状态信息，Channel State Information）等上行控制。 | 基站可能错误理解终端反馈或信道状态。 |
| DCI | 调度、调制与编码方案、RV（冗余版本，Redundancy Version）、HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）process、资源分配。 | 终端可能按错误资源收发，或把不属于自己的 DCI 当成有效。 |
...
本节沿用 [[T10.5_Polar_SCL_decoding|T10.5]] 的约定：PM 越小越好。每条路径还会生成 CRC 检查结果：
...
| 项 | UCI Polar 分支 | DCI |
|:---|:---|:---|
| 协议入口 | TS 38.212 §6.3.1.2.1，PUSCH（物理上行共享信道，Physical Uplink Shared Channel）UCI 对应 §6.3.2.2.1 调用。 | TS 38.212 §7.3.2。 |
...
这段代码故意不实现 CRC 多项式计算，因为 [[T3.1_LTE_NR_CRC_families|T3.1]] 已复现 CRC 家族，本节关注 CA-SCL final selector 的决策边界。工程实现中 `crc_ok` 必须由真实 CRC checker 产生，`rnti_ok` 必须来自 DCI 的 RNTI context 检查。
...
| 对象 | 策略 | 风险 |
|:---|:---|:---|
| PM | 沿用 [[T10.5_Polar_SCL_decoding|T10.5]] 的 PM 位宽、饱和和 tie-break。 | PM 饱和导致多个 pass 路径并列，若 tie-break 不稳定则输出不复现。 |
...
| CRC bit 顺序错 | 所有路径 CRC fail 或误通过率异常。 | 对比 [[T3.1_LTE_NR_CRC_families|T3.1]] CRC golden 和 path bit extraction。 |
...
| PUCCH（物理上行控制信道，Physical Uplink Control Channel）UCI Polar 分支可使用 6/11 bit CRC，并有无 CRC 小负载边界 | TS 38.212 §6.3.1.2.1 | `content.md:2325-2335` |
...
| 项 | 记录 |
|:---|:---|
| 写作范围 | 新增 `docs/L2_协议算法/T10.6_CRC_aided_SCL_control_reliability.md`。 |
...
2. 本项目 [[T3.1_LTE_NR_CRC_families|T3.1]]：`docs/L1_基础/T3.1_LTE_NR_CRC_families.md`。
