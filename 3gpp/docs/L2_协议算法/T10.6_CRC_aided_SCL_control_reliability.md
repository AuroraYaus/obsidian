...
本节讲循环冗余校验辅助逐次消除列表译码（CRC-aided Successive Cancellation List, CA-SCL）。[[T10.5_Polar_SCL_decoding|T10.5]] 的 SCL 能输出多条候选路径和路径度量（Path Metric, PM），但控制信息译码不能只看 PM。CRC 会随 payload 进入Polar的 information set，译码后用来判断候选路径是否满足检错关系。对下行控制信息，CRC 还与无线网络临时标识（Radio Network Temporary Identifier, RNTI）存在边界；对上行控制信息，CRC 长度和是否附加 CRC 与 UCI payload 条件有关。
