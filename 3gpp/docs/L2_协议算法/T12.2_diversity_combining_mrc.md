...
答案在于分集（Diversity）：如果接收端有多根天线，每根天线经历的衰落是独立随机的，那么"所有天线同时深衰落"的概率就是"单根天线深衰落"概率的 N 次方量级——概率不是除以 N，而是指数级变小。多份独立拷贝合在一起，深衰落被"平均"掉，这是 MIMO 接收对抗衰落的第一重收益。本节（[[T12.2_diversity_combining_mrc|T12.2]]）是 MIMO 接收机与检测器系列的第二篇，任务是把"分集为什么有效、怎么合并、能带来多少收益"这一整条逻辑链用数学推出来。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T2.15_fading_channel_LLR_reliability]] 衰落信道与 LLR | 知道 $y=hx+n$、$h$ 是复增益、深衰落时 LLR 可靠度下降；本节的向量 $\mathbf{h}$ 是它的升级版。 |
...
2. **深衰落 RE 的可靠度由 CSI 携带**。MMSE-MRC 输出符号估计的同时给出可靠度（csi = 1 + SINR），软解调用它对 LLR 加权（衔接 [[T2.15_fading_channel_LLR_reliability|T2.15]] 与 [[T12.1_mimo_receiver_chain_overview|T12.1]] 的"软解调+CSI 加权"环节）——正则化保证深衰落 RE 的 LLR 权重被压低，而不是以虚假的高置信度进入译码器。
