...
式 (7) 回答的问题是：干扰清零的账单由谁付——由噪声放大付。ZF 输出噪声是 $\sigma^2[(\mathbf{H}^H\mathbf{H})^{-1}]_{\ell\ell}$，而单层标量情形（T2.15 的 $y=hx+n$ 除 $h$）的噪声是 $\sigma^2/|h|^2$——式 (7) 正是它的矩阵版：$[(\mathbf{H}^H\mathbf{H})^{-1}]_{\ell\ell}$ 扮演 $1/|h|^2$ 的角色。ZF 的输出 SINR 因此是：
