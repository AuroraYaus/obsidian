...
本节从解调器已经输出软信息的位置开始，建立 NR 数据业务低密度奇偶校验码接收侧译码链路。这里的软信息用对数似然比表示：LLR（对数似然比，Log-Likelihood Ratio）的符号表示更像 0 还是更像 1，绝对值表示接收机有多可靠。LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）译码器不会只吃一串硬判决 0/1，它需要这些可靠度数字在校验关系里反复传播。

学完本节后，应能做到：

- 解释 LDPC 的中文名、英文全称、缩写、基本思想和“低密度”的含义。
- 画出从软解调器（demapper）LLR 到速率恢复（rate recovery）、LDPC 译码、码块循环冗余校验、CB（码块，Code Block）拼接、TB（传输块，Transport Block）CRC（循环冗余校验，Cyclic Redundancy Check）和混合自动重传请求反馈的接收侧链路。
- 把每个接收端步骤映射到 TS 38.212 Rel-19 `38212-j30` 的章节，而不是只记一个协议目录。
- 说明调制与编码方案、冗余版本、传输块大小和 CBG（码块组，Code Block Group）为什么来自 TS 38.214 的调度背景，而不是 LDPC 算法内部。
...
本节只做总览，不把基图选择、提升大小、准循环矩阵、Belief Propagation、Min-Sum 和 layered schedule 的细节一次性讲完。那些内容分别由 [[T8.2_NR_LDPC_base_graph_selection|T8.2]] 到 [[T8.8_NR_LDPC_decoder_numeric_walkthrough|T8.8]] 承接。本节的目标是先建立正确地图：每个后续算法细节究竟在接收链路的哪个位置工作。
...
| HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） | 知道 HARQ 重传时接收端会保留旧软信息，并和新传输做软合并。 |
...
| 讲 LDPC 译码 | “LDPC 译码器在链路中的契约” | 说明输入、输出、BG（基图，Base Graph）/Zc/H 的关系和 syndrome/iteration 的总览。 |
...
| 讲可选 CBG 处理 | “CBG 在本节中的位置” | 说明 CBG 是 TS 38.214 的部分重传粒度，[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 详细展开。 |
...
这张表只表示最低覆盖。正文会继续补充背景、对象模型、数值例子、失败模式和实现边界，避免把 [[T8.1_NR_LDPC_decoder_chain_overview|T8.1]] 写成协议索引。
...
LTE 数据业务主要使用 Turbo（Turbo 码，Turbo Code），而 NR 数据业务主要使用 LDPC。这里不要把它理解成“新标准随便换了一个码”。NR 面向更宽带宽、更高峰值速率、更灵活的码率和更多并行接收场景，译码器不能只关注误码性能，还要关注吞吐、延迟、存储访问和可并行实现。
...
| HARQ 增量冗余 | rate matching circular buffer 可选择不同 RV（冗余版本，Redundancy Version）区域。 | soft buffer 保存各母码位置 LLR，重传时合并。 |
...
NR 里不能把“LDPC”泛化成“所有信道编码”。TS 38.212 Table 5.3-1/5.3-2 和 §6.2/§7.2 的链路说明共同给出边界：LDPC 主线服务数据传输信道，控制链路另有 Polar（极化码，Polar Code）或 small block coding 分支。

| 链路对象 | 物理信道 | 是否属于本章 NR LDPC 主线 | 接收端含义 |
|:---|:---|:---:|:---|
| 上行共享信道 | PUSCH（物理上行共享信道，Physical Uplink Shared Channel） | 是 | 走 TB CRC、BG 选择、CB segmentation、LDPC、rate matching 和 CB concatenation。 |
| 下行共享信道 | PDSCH（物理下行共享信道，Physical Downlink Shared Channel） | 是 | 与 UL-SCH（上行共享信道，Uplink Shared Channel）同类，方向不同导致 MCS（调制与编码方案，Modulation and Coding Scheme）/RV/CBG 调度来源不同。 |
| 寻呼信道（Paging Channel, PCH） | PDSCH | 是 | TS 38.212 §7.2 将 DL-SCH（下行共享信道，Downlink Shared Channel）/PCH 放在同一 LDPC 数据链路中。 |
| 下行控制信息 | PDCCH（物理下行控制信道，Physical Downlink Control Channel） | 否 | DCI（下行控制信息，Downlink Control Information）属于控制信息，走 Polar 主线，不进入 T8/T9 的 LDPC descriptor。 |
| 广播信道（Broadcast Channel, BCH）/PBCH | PBCH | 否 | BCH/PBCH 使用 Polar 相关控制链路，不使用本章 LDPC 数据链路。 |
| 上行控制信息 | PUCCH（物理上行控制信道，Physical Uplink Control Channel）/PUSCH multiplexing | 否 | UCI（上行控制信息，Uplink Control Information）有 Polar 或 small block coding 分支；即使复用到 PUSCH，也不是 UL-SCH LDPC CB 本身。 |
...
边界要明确：TS 38.212 主要规定编码链路和比特序列如何生成，接收端必须实现等价逆过程；TS 38.214 提供 MCS、TBS（传输块大小，Transport Block Size）、RV、CBG 等调度和过程背景。3GPP 不规定某个芯片内部 LLR 位宽、静态随机存取存储器bank 数、layered schedule 流水深度、最大迭代次数、early-stop 阈值、RTL（寄存器传输级，Register Transfer Level）状态机或 ASIC（专用集成电路，Application-Specific Integrated Circuit）面积目标。这些属于实现策略，但必须保持协议 bit-exact 的输入输出关系。
...
式 (2) 回答的问题是：第 $j$ 个收到的 LLR 应放回 LDPC 母码缓冲区的哪个地址。$a(j)$ 是由 `rvidx`、$N_{\mathrm{cb}}$、bit interleaving、CBG 是否发送、filler/shortening/puncturing 规则共同决定的地址。[[T9.1_NR_LDPC_rate_recovery_overview|T9.1]] 和 [[T9.2_NR_LDPC_circular_buffer_states|T9.2]] 会详细推导 $a(j)$；本节先说明它在链路中的位置。
...
| TB CRC | 所有 CB 去除可选 CB CRC/filler 后按序拼接得到的 TB candidate | 判断整个 TB 是否可信 | 通过才交付 MAC（媒体接入控制层，Medium Access Control），并产生成功反馈边界。 |
...
本节只建立 CBG 的接收端位置：CBG 不改变 LDPC 的校验矩阵，不改变单个 CB 的译码算法；它改变的是“本次哪些 CB 或 CBG 有新的 LLR、哪些 soft buffer 保持旧值、哪些旧实例可以合并”。具体 CBG mask、部分重传和 soft buffer key 会在 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 详细展开。
...
| `Zc` | TS 38.212 §5.2.2/§5.3.2 | 决定 QC-LDPC（准循环 LDPC，Quasi-Cyclic LDPC）展开尺寸和地址周期 | 地址、bank、矩阵尺寸错。 |
...
[[T8.1_NR_LDPC_decoder_chain_overview|T8.1]] 的浮点仿真不要求实现完整 NR conformance receiver，而是验证链路对象和边界是否正确。
...
| CB CRC/TB CRC 层级 | 用 toy CRC 或已有 [[T3.1_LTE_NR_CRC_families|T3.1]] CRC 函数 | 有 CB CRC 时单 CB 失败阻断 TB 交付；无 CB CRC 时不生成伪 CB pass/fail，最终由 TB CRC 判断交付。 |
...
| LDPC message | check-to-variable 和 variable-to-check 消息单独规划位宽 | 消息过窄导致性能损失，过宽增加 SRAM（静态随机存取存储器，Static Random-Access Memory）和功耗。 |
...
从 RTL/ASIC 角度看，[[T8.1_NR_LDPC_decoder_chain_overview|T8.1]] 的链路可以拆成几类硬件对象：

| 硬件对象 | 功能 | 关键验证点 |
|:---|:---|:---|
| LLR input FIFO | 接收 demapper 输出并按 CB 切片 | $E_r$ 边界、backpressure、Qm（调制阶数，Modulation Order）对齐。 |
...
[[T8.1_NR_LDPC_decoder_chain_overview|T8.1]] 的验证分三层：
...
| 错误 | 现象 | 定位方式 |
|:---|:---|:---|
| 把 demapper 输出当硬比特 | BLER（块错误率，Block Error Rate）性能极差，迭代没有收益 | 检查 decoder 输入是否只有 0/1，而不是 LLR。 |
| BG 选择错 | syndrome 长期不收敛；有 CB CRC 时 CB CRC fail | dump `BG`、payload size、coding rate，并对照 [[T8.2_NR_LDPC_base_graph_selection|T8.2]]。 |
| Zc 错 | 地址周期、矩阵尺寸、bank 访问全错 | dump `Zc`、lift set、BG table index，并对照 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]。 |
...
| 项目 | 记录 |
|:---|:---|
| 路线图卡片 | `2026-06-19-lte-nr-decoding-learning-roadmap.md` [[T8.1_NR_LDPC_decoder_chain_overview|T8.1]]：Prompt 要求从 demapper LLR 开始，总览 rate recovery、LDPC、CB CRC、CB concat、TB CRC 和可选 CBG。 |
| 本地协议检索 | 使用 `rg`、`nl -ba` 和 `sed` 定位 TS 38.212 §5.1、§5.2.2、§5.3.2、§5.4.2、§6.2、§7.2，以及 TS 38.214 §5.1.7 和 §6.1.5。 |
| 协议表/公式边界 | 本节是链路总览，不复现 TS 38.212 Table 5.3.2-1/2/3、Table 5.4.2.1-1/2 或 TS 38.214 MCS/TBS 表；本节不依据这些表给出任何规范数值结论，教学例子中的 `BG1/Zc=32/E=1200/E=1184` 不是规范向量；这些表在 [[T8.2_NR_LDPC_base_graph_selection|T8.2]]/[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]/T9/T11 使用具体值时重建。 |
