...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/T8.4_LDPC_Tanner_graph_message_passing.md"
---
# T8.4 LDPC Tanner 图与消息传递
## 本节学习目标

本节讲低密度奇偶校验码（Low-Density Parity-Check, LDPC，校验矩阵绝大多数位置为 0、依靠稀疏图结构迭代译码的信道编码，[[LDPC_低密度奇偶校验码]]）译码器最核心的图模型：Tanner 图。[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 已经说明 NR 如何用 `BG`、`Zc`、`iLS` 和 shift table 生成奇偶校验矩阵 $H$；本节继续回答：矩阵 $H$ 如何变成图，图上的变量节点和校验节点分别代表什么，syndrome 为什么能判断硬判决是否满足 LDPC 约束，以及迭代消息传递的第一轮信息流如何发生。
...
本节是 [[T8.5_LDPC_sum_product_BP|T8.5]] 置信传播公式推导的准备课。这里重点是“图和消息方向”；校验节点的 $\tanh/\operatorname{atanh}$ 公式、Min-Sum 近似和 layered schedule 分别留到 [[T8.5_LDPC_sum_product_BP|T8.5]]、[[T8.6_LDPC_MS_NMS_OMS|T8.6]] 和 [[T8.7_layered_LDPC_decoding_schedule|T8.7]] 展开。
...
| T8.3 QC-LDPC（准循环 LDPC，Quasi-Cyclic LDPC） | 知道真实 NR 的完整 $H$ 由基图和循环移位表生成。 |
...
| 讲解变量节点 | “变量节点与校验节点” | 定义 variable node，说明它对应候选码字位置和 LLR（对数似然比，Log-Likelihood Ratio）。 |
...
| 按弹性审计清单取舍并拓展 | 全文 | 补协议证据、系统/校验/打孔/filler 影响、Python、定点、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和常见错误。 |
...
| 类别 | 协议地位 | 本节处理方式 |
|:---|:---|:---|
| $H$ 的来源、BG1/BG2 维度、shift table | TS 38.212 §5.3.2 的规范内容 | 作为协议精读对象，引用本地 `content.md`、表格和 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 重建图。 |
| filler bits、码块分段和 $Z_c$ 入口 | TS 38.212 §5.2.2 的规范内容 | 说明这些位置如何影响译码输入和输出剥离。 |
| rate matching、circular buffer、RV（冗余版本，Redundancy Version）起点和 `<NULL>` 跳过 | TS 38.212 §5.4.2.1 的规范内容 | 说明未发送位置与 filler/shortening 位置在接收端 LLR 语义上不同。 |
| Tanner 图、syndrome、message passing | LDPC 理论和工程实现对象 | 由协议 $H$ 推出，不是 3GPP 对具体译码算法的强制规定。 |
| BP（置信传播，Belief Propagation）/SPA（和积算法，Sum-Product Algorithm）、Min-Sum、layered schedule | 接收机实现策略 | 本节只讲消息方向和接口，公式推导留到后续小节。 |

本地证据如下：

| 协议位置 | 本地证据路径 |
| :--- | :--- |
| TS 38.212 §5.2.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md` 行 717-805 |
| TS 38.212 §5.3.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md` 行 948-989 |
| TS 38.212 §5.4.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md` 行 1175-1309 |
| Table 5.3.2-1 | `tables/table_0013.csv/html` |
| Table 5.3.2-2 | `tables/table_0014.csv/html` |
| Table 5.3.2-3 | `tables/table_0015.csv/html` |

抽取边界要明确：`content.md` 对部分公式变量名有丢失，本节使用协议语义重建公式，并把表格 CSV/HTML、[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 图表和原始 Word 抽取路径作为复核证据。本文的 $3 \times 6$ 矩阵是教学例子，不是 NR 一致性测试向量；真实 NR 的 $H$ 尺寸来自 BG1 的 $46Z_c \times 68Z_c$ 或 BG2 的 $42Z_c \times 52Z_c$。
...
这张表的重点不是具体函数形式，而是排除自身消息。例如 $c_0$ 给 $v_0$ 回消息时，只能看 $v_1$ 和 $v_3$ 在同一条校验方程上的消息，不能把 $v_0$ 刚发给 $c_0$ 的消息又带回 $v_0$。具体 $r_{i\to j}$ 如何由这些输入计算，[[T8.5_LDPC_sum_product_BP|T8.5]] 会用 BP/SPA 公式推导。
...
| 短环相关性 | 同一信道证据被多条路径重复使用 | 违反独立消息近似，造成过度自信 | 高 SNR（信噪比，Signal-to-Noise Ratio）下可能表现为停滞、震荡或固定错误子图。 |
| QC lifting 影响 | shift value 决定 lifted graph 的环结构 | 同一 BG（基图，Base Graph）不同 $Z_c/i_{\mathrm{LS}}$ 的短环分布不同 | shift 方向或表列错会改变 cycle 结构。 |

这里不能把“短环多”简单等同于“协议错误”。TS 38.212 固定 BG 和 shift 表，接收端必须按表实现；cycle/girth 是分析译码行为和调试失败的语言。若工程中出现高 SNR 仍失败，除了检查 LLR 符号、rate recovery 和 CRC（循环冗余校验，Cyclic Redundancy Check）边界，还应 dump 错误比特诱导子图，看它是否集中在某些短环或 trapping set 周围。
...
| CB（码块，Code Block）CRC | 码块解出的信息位和 CB CRC 位 | 码块级完整性 | 该 CB 不可信 | 该 CB 通过码块完整性检查 |
| 传输块 CRC | 传输块重组后的信息位和 TB（传输块，Transport Block）CRC 位 | 传输块级完整性 | 混合自动重传请求反馈通常走失败边界 | 整个 TB 可交付上层 |
...
Tanner 图上的迭代译码不是一次性求逆，而是让变量节点和校验节点沿边交换软信息。完整 BP/SPA 公式留到 [[T8.5_LDPC_sum_product_BP|T8.5]]，本节先建立消息方向。
...
| 步骤 | 输入 | 动作 | 输出 |
|:---|:---|:---|:---|
| 1 | demapper LLR、HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）soft buffer | rate recovery，把接收软信息放回 LDPC 编码比特空间 | LDPC input LLR array |
...
下面的伪代码强调结构和边界，不展开 [[T8.5_LDPC_sum_product_BP|T8.5]] 的具体 check-node 公式。
...
或按实现方向使用等价的反向公式生成地址。这里的 `mod Zc` 地址生成会直接影响 memory bank。若多个边在同一周期访问同一 bank，就会产生 bank conflict；[[T8.7_layered_LDPC_decoding_schedule|T8.7]] 的 layered schedule 和 [[T18.5_SIMD_memory_layout_decoders|T18.5]] 的 SIMD/memory layout 会继续展开这个问题。
...
这部分内容解释了为什么 [[T8.4_LDPC_Tanner_graph_message_passing|T8.4]] 的 Tanner 图会自然连接到硬件访存：图上的每条边最终都要变成一次消息读写；稀疏存储格式决定地址生成器、banking、冲突处理和 trace dump 的可验证性。
...
| 结论 | 协议证据 | 本地核验 |
|:---|:---|:---|
| LDPC 编码在 GF(2) 中执行 | TS 38.212 §5.3.2 | `content.md` 行 948-989 提到 parity bits 生成和 GF(2) 编码。 |
| BG1 的 $H$ 基矩阵为 46 行 68 列 | TS 38.212 §5.3.2 | `content.md` 行 971-977；[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 已复核。 |
| BG2 的 $H$ 基矩阵为 42 行 52 列 | TS 38.212 §5.3.2 | `content.md` 行 971-977；[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 已复核。 |
| $H$ 由基图元素替换为全零矩阵或循环置换矩阵得到 | TS 38.212 §5.3.2 | `content.md` 行 977-989；[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 已展开公式和图。 |
| BG1/BG2 的非零位置和 shift value 来自 Table 5.3.2-2/3 | TS 38.212 §5.3.2 | `tables/table_0014.csv/html`、`tables/table_0015.csv/html`；[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 已完整表格化复现。 |
| filler bits 在 LDPC code block segmentation 中插入 | TS 38.212 §5.2.2 | `content.md` 行 747-805；本节用于说明接收端 mask 和输出剥离。 |
| LDPC rate matching 使用 circular buffer、RV 起点和 `<NULL>` 跳过 | TS 38.212 §5.4.2.1 | `content.md` 行 1175-1309；本节用于说明 unknown/punctured 位置与 filler/shortened 位置的 LLR 语义差异。 |

## 图示
![图 1：LDPC Tanner syndrome toy](assets/T8.4_LDPC_Tanner_syndrome_toy.svg)

## 执行与证据记录

| 项目 | 记录 |
|:---|:---|
| 路线图卡片 | `2026-06-19-lte-nr-decoding-learning-roadmap.md` [[T8.4_LDPC_Tanner_graph_message_passing|T8.4]]，Prompt/产出/验收/3GPP 证据已覆盖。 |
| 讲义文件 | `docs/L2_协议算法/T8.4_LDPC_Tanner_graph_message_passing.md`。 |
| 协议来源 | TS 38.212 Rel-19 `38212-j30` §5.2.2、§5.3.2、§5.4.2.1、Table 5.3.2-1/2/3。 |
| 本地表格证据 | [[T8.4_LDPC_Tanner_graph_message_passing|T8.4]] 不重复长表正文；复用 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 已核验的 `table_0013.csv/html`、`table_0014.csv/html`、`table_0015.csv/html` 和表格化重建。 |
| 图表资产 | `docs/L2_协议算法/assets/T8.4_LDPC_Tanner_syndrome_toy.svg`（手绘 SVG，替代原 `tools/archive_python_drawing/figures/render_ldpc_tanner_syndrome.py` 生成的 PNG）。 |
| 单篇审计 | `python3 tools/audit_lesson_terms.py docs/L2_协议算法/T8.4_LDPC_Tanner_graph_message_passing.md` 输出 `LESSON_TERM_AUDIT_OK`；`python3 tools/audit_markdown_headings.py docs/L2_协议算法/T8.4_LDPC_Tanner_graph_message_passing.md` 输出 `MARKDOWN_HEADING_AUDIT_OK`；`python3 tools/audit_lesson_depth.py --strict docs/L2_协议算法/T8.4_LDPC_Tanner_graph_message_passing.md` 输出 `LESSON_DEPTH_AUDIT_OK`；`python3 tools/audit_latex_render.py docs/L2_协议算法/T8.4_LDPC_Tanner_graph_message_passing.md` 输出 `LATEX_RENDER_AUDIT_OK formulas=195`。 |
...
| 资产文件检查 | SVG 几何审计通过（text 完整落盒、不与宿主外形状相交、文字不超芯片、text-text 无重叠、箭头终点落盒边五项）；`cairosvg` 渲染验证通过。 |
