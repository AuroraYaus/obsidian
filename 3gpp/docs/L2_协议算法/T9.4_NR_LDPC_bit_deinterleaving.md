...
本节使用的单篇技术缩写先统一说明：低密度奇偶校验码是 NR 数据业务的信道编码；对数似然比是 demapper 输出并送入译码链路的软信息；正交相移键控每个调制符号承载 2 个 bit；二进制相移键控在本节只作为“更低阶调制或简化测试”的对照；正交幅度调制可以让每个符号承载更多 bit，本节用 16QAM 举例；调制与编码方案决定调制阶数；下行控制信息是下行调度字段来源；上行共享信道和下行共享信道是本节主要关心的数据承载信道；码块是 LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）译码的逐块对象；循环冗余校验用于发现解交织或放置错误；传输块是多个 CB（码块，Code Block）拼接后对应业务数据和 TB（传输块，Transport Block）CRC（循环冗余校验，Cyclic Redundancy Check）的对象；传输块大小决定待承载 bit 数；混合自动重传请求让接收端在重传间复用软信息；块错误率是链路仿真中统计块译码失败的指标；静态随机存取存储器是硬件 reorder buffer 常用存储资源；寄存器传输级/专用集成电路实现需要把反交织落成 reorder buffer、地址生成器和 bank 访问。

本节讲一个很容易被忽略的接收端细节：TS 38.212 §5.4.2.2 在发送端把 rate matching 后的 bit 序列按调制阶数 $Q_m$ 交织后再调制。接收端从 demapper 得到的是“调制符号顺序”的 LLR（对数似然比，Log-Likelihood Ratio），如果不先做 bit deinterleaving，后面的 RV（冗余版本，Redundancy Version）/k0 rate recovery 会把 LLR 放到错误的 LDPC 编码位置。

学完本节后，应能做到：

- 解释为什么 bit interleaving 与调制阶数 $Q_m$ 相关。
- 写出发送端 interleaving 和接收端 deinterleaving 的索引关系。
- 用 QPSK（正交相移键控，Quadrature Phase Shift Keying）的 $Q_m=2$ 例子，把每个 symbol 两个 LLR 反排成 rate recovery 输入。
...
- 解释 bit order 错误为什么可能在高 SNR（信噪比，Signal-to-Noise Ratio）下仍然 CRC fail。
- 给出 RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）中 reorder buffer、permutation address generator 和 bank conflict 的实现风险。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| T2.13/T2.14 软解调 | 知道 QPSK 每个 symbol 输出 2 个 LLR，16QAM 每个 symbol 输出 4 个 LLR。 |
| T9.1 速率恢复总览 | 知道 bit deinterleaving 位于 demapper LLR 和 bit selection 反操作之间。 |
| T9.3 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）soft buffer | 知道 deinterleaving 后的 LLR 还要按 RV/k0 写入 soft buffer。 |
| T5.2 memory banking | 知道 reorder buffer 的读写模式会影响 SRAM（静态随机存取存储器，Static Random-Access Memory）bank conflict。 |

本节不要求掌握完整星座映射表。星座点如何产生每个 bit 的 LLR 在 [[T2.13_BPSK_QPSK_soft_demapping|T2.13]]/[[T2.14_QAM_Max_Log_MAP_demapping|T2.14]] 讲；这里关心的是这些 LLR 进入 LDPC rate recovery 前的顺序。

## 任务边界与 Prompt 覆盖

当前执行依据是 `docs/audits/lte_nr_decoding_remaining_work_register.md` 的 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 条目：NR LDPC 解交织与 LLR 放置。需要说明一个计划错位：roadmap 文件中的 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 是 CBG（码块组，Code Block Group）handling，已经在 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 中覆盖了 HARQ/CBG 的主线，后续仍可单独深化；本节按登记表执行 bit deinterleaving。
...
- 本节不复现 TS 38.214 MCS（调制与编码方案，Modulation and Coding Scheme）表值。MCS 表如何把索引变成 $Q_m$ 在模块 11 或系统配置章节再展开；本节只要求 descriptor 给出的 $Q_m$ 正确。
...
错误的 bit deinterleaving 不只是“数组顺序错”。在高 SNR 下，错误位置上的 LLR 绝对值可能很大，LDPC decoder 会非常相信这些错放证据，导致 syndrome 停滞或 CRC 稳定失败。定位时应回链 [[T2.14_QAM_Max_Log_MAP_demapping|T2.14]] 的 QAM（正交幅度调制，Quadrature Amplitude Modulation）Max-Log-MAP demapping：先确认 bit lane 定义和 LLR 符号，再确认本节的 $Q_m$ 反交织，最后才检查 RV/k0 rate recovery。
...
| 检测点 | 必须检查的内容 | 失败定位 |
|:---|:---|:---|
| Qm（调制阶数，Modulation Order）来源 | TS 38.214 MCS 决定 `Qm`；descriptor 不能写死。 | 同一代码在 QPSK 通过、16QAM/64QAM/256QAM 失败时，先查 `Qm` 是否来自当前调度上下文。 |
...
浮点仿真不要只跑随机 BLER（块错误率，Block Error Rate），应先跑固定 pattern：
...
- 若 reorder buffer 使用更窄 SRAM，需要在 [[T2.16_LLR_clipping_scaling_quantization|T2.16]]/[[T5.1_fixed_point_numbers_for_LLR|T5.1]] 定点预算中显式评估 BLER 损失。
...
| PDSCH（物理下行共享信道，Physical Downlink Shared Channel）从 DCI（下行控制信息，Downlink Control Information）/MCS 确定 modulation order $Q_m$ | TS 38.214 §5.1.3 | `3GPP_Rel19/processed/TS_38.214_38214-j30/content.md:1157-1163` |
| PUSCH（物理上行共享信道，Physical Uplink Shared Channel）读取或确定 modulation order、RV、TBS（传输块大小，Transport Block Size） | TS 38.214 §6.1.4 | `3GPP_Rel19/processed/TS_38.214_38214-j30/content.md:6829-6841` |
...
| Prompt 对齐 | 按登记表 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 覆盖 $Q_m$、QPSK、16QAM、LLR 放置、流程图、reorder buffer、address generator、bank conflict、伪代码、验证和常见错误；roadmap [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 的 CBG handling 已在 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 承接主线，后续可继续深化。 |
...
3. 本项目 [[T2.13_BPSK_QPSK_soft_demapping|T2.13]]：`docs/L1_基础/T2.13_BPSK_QPSK_soft_demapping.md`。
4. 本项目 [[T2.14_QAM_Max_Log_MAP_demapping|T2.14]]：`docs/L1_基础/T2.14_QAM_Max_Log_MAP_demapping.md`。
