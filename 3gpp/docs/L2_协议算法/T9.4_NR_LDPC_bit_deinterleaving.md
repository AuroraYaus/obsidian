...
当前执行依据是 `docs/audits/lte_nr_decoding_remaining_work_register.md` 的 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 条目：NR LDPC 解交织与 LLR 放置。需要说明一个计划错位：roadmap 文件中的 [[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 是 CBG handling，已经在 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 中覆盖了 HARQ/CBG 的主线，后续仍可单独深化；本节按登记表执行 bit deinterleaving。
...
错误的 bit deinterleaving 不只是“数组顺序错”。在高 SNR 下，错误位置上的 LLR 绝对值可能很大，LDPC decoder 会非常相信这些错放证据，导致 syndrome 停滞或 CRC 稳定失败。定位时应回链 [[T2.14_QAM_Max_Log_MAP_demapping|T2.14]] 的 QAM Max-Log-MAP demapping：先确认 bit lane 定义和 LLR 符号，再确认本节的 $Q_m$ 反交织，最后才检查 RV/k0 rate recovery。
