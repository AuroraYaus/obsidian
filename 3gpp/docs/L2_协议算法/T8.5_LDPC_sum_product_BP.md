...
- 区分 channel LLR（对数似然比，Log-Likelihood Ratio）、variable-to-check message、check-to-variable message、posterior LLR 和 extrinsic message。
...
本节是算法理论课，但仍要保持协议边界：TS 38.212 规定 NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）的矩阵结构和 rate matching，不规定接收端必须使用 BP、SPA、Min-Sum 或 layered schedule。本文讲的是从协议定义的 $H$ 出发，接收机可以采用的一类主流软判决译码算法。
...
| 包括 syndrome 检查 | “syndrome early stop 与 CRC（循环冗余校验，Cyclic Redundancy Check）边界” | 说明 syndrome 全零、CRC 边界和最大迭代。 |
...
这张表只表示最低覆盖。正文还会对比 Turbo（Turbo 码，Turbo Code）的 Bahl-Cocke-Jelinek-Raviv（BCJR）/最大后验概率（Maximum A Posteriori, MAP）译码和 LDPC BP 的共同点与差异，帮助把 T6/T8 的算法线索连起来。
...
本节的 toy 矩阵、LLR 数字和 Python 片段都是教学例子，不是 NR conformance vector。真实 NR 接收机的 $H$ 来自 BG1/BG2、$Z_c$、`iLS` 和 shift table，message schedule 通常按 QC-LDPC（准循环 LDPC，Quasi-Cyclic LDPC）结构隐式生成。
...
| 工程检测点 | 外信息不得带回自身证据；$\operatorname{atanh}$ 接近 1 时需 LUT（查找表，Look-Up Table）/裁剪/饱和；syndrome 可做 LDPC 早停，但 CB（码块，Code Block）/TB（传输块，Transport Block）是否可信由 CRC 边界决定。 |
...
| CB/TB CRC | LDPC core 之后 | 检查 CB 或 TB 的完整性 | 决定交付、重组和混合自动重传请求反馈边界；完整 NR 链路还可能有可选 CBG（码块组，Code Block Group）级反馈粒度。 |
...
| 局部约束 | RSC（递归系统卷积码，Recursive Systematic Convolutional Code）编码器状态转移 | 奇偶校验方程 |
...
密度进化（Density Evolution, DE）和外信息转移图（Extrinsic Information Transfer, EXIT chart）是分析 LDPC 迭代译码收敛的方法，不是 TS 38.212 的协议输出。3GPP 规定 BG（基图，Base Graph）、$Z_c$、shift table、rate matching 和 interleaving；它不规定某个接收机的 DE 阈值、EXIT 曲线或 $\alpha/\beta$ 最优值。没有本项目仿真日志、随机种子、脚本和输出时，本章只能写方法和 toy 递推，不能声称 NR LDPC 在某个 SNR（信噪比，Signal-to-Noise Ratio）下达到某个性能结论。
...
| $\tanh/\operatorname{atanh}$ | 若保留 SPA，用查找表（Look-Up Table, LUT）或分段线性近似；若资源紧，转 [[T8.6_LDPC_MS_NMS_OMS|T8.6]] 的 Min-Sum（最小和）/NMS（归一化最小和）/OMS（偏移最小和）。 |
...
6. syndrome early stop 检查候选硬判决是否满足 LDPC 校验矩阵 $H$；CRC 检查 CB/TB 业务数据完整性。syndrome 可用于 decoder 内部早停，CRC 决定交付和 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）反馈边界。
...
| BP 输入位于 LDPC 母码位置是接收端实现推论 | TS 38.212 §5.4.2.1 | `content.md` 行 1175-1309 描述发送侧 LDPC circular buffer、rate matching output 和 RV（冗余版本，Redundancy Version）起点；由此可推得接收机若要按 $H$ 的边运行 BP，需要把接收 LLR 恢复到相同的 $H$ 列位置。 |
