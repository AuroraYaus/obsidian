---
type: spec
aliases:
  - L2 协议入口
  - 译码协议与算法入口
tags:
  - 3gpp
  - docs
  - l2
  - index
source_spec: "3gpp/docs/L2_协议算法"
---

# L2 协议与算法入口

## 概念图谱

- [[3gpp/docs/concepts/概念图谱入口]]

## M6 LTE Turbo 译码协议与算法

- [[T6.1_LTE_Turbo_decoder_chain_overview]]
- [[T6.2_RSC_code_foundation]]
- [[T6.3_LTE_Turbo_encoder_trellis_termination]]
- [[T6.4_LTE_Turbo_internal_interleaver]]
- [[T6.5_BCJR_MAP_decoding_intuition]]
- [[T6.6_Log_MAP_Max_Log_MAP_Turbo]]
- [[T6.7_Turbo_iteration_extrinsic_stopping]]
- [[T6.8_LTE_Turbo_decoder_numeric_walkthrough]]

## M7 LTE 接收侧译码链路

- [[T7.1_LTE_Turbo_de_rate_matching_overview]]
- [[T7.2_LTE_subblock_deinterleaver_circular_buffer]]
- [[T7.3_LTE_HARQ_soft_buffer_RV]]
- [[T7.4_LTE_code_block_reassembly_TB_CRC]]
- [[T7.5_LTE_DL_UL_decoding_differences]]
- [[T7.6_LTE_Turbo_decoder_edge_cases]]

## M8 NR LDPC 译码协议与算法

- [[T8.0_TS38212_chapter5_decoder_side_map]]
- [[T8.1_NR_LDPC_decoder_chain_overview]]
- [[T8.2_NR_LDPC_base_graph_selection]]
- [[T8.3_NR_LDPC_lifting_QC_matrix]]
- [[T8.4_LDPC_Tanner_graph_message_passing]]
- [[T8.5_LDPC_sum_product_BP]]
- [[T8.6_LDPC_MS_NMS_OMS]]
- [[T8.7_layered_LDPC_decoding_schedule]]
- [[T8.8_NR_LDPC_decoder_numeric_walkthrough]]

## M9 NR LDPC 接收侧译码链路

- [[T9.0_TS38214_MCS_TBS_decoder_descriptor]]、[[T9.7_CB_incremental_write]]、[[T9.8_CBG_code_block_group]]
- [[T9.1_NR_LDPC_rate_recovery_overview]]
- [[T9.2_NR_LDPC_circular_buffer_states]]
- [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0]]
- [[T9.4_NR_LDPC_bit_deinterleaving]]
- [[T9.5_NR_LDPC_reassembly_TB_CRC]]
- [[T9.6_NR_LDPC_decoder_edge_cases]]

## M10 NR Polar 译码协议与算法

- [[T10.1_NR_Polar_decoder_chain_overview]]
- [[T10.2_channel_polarization_frozen_bits]]
- [[T10.3_NR_Polar_reliability_sequence]]
- [[T10.4_Polar_SC_decoding]]
- [[T10.5_Polar_SCL_decoding]]
- [[T10.6_CRC_aided_SCL_control_reliability]]
- [[T10.7_NR_Polar_rate_recovery]]
- [[T10.8_NR_Polar_decoder_edge_cases]]

## M11 LTE/NR 译码对比

- [[T11.1_Turbo_LDPC_Polar_algorithm_comparison]]
- [[T11.2_LTE_NR_rate_matching_comparison]]
- [[T11.3_HARQ_soft_buffer_comparison]]
- [[T11.4_decoder_hardware_tradeoff_comparison]]
- [[T11.5_decoder_selection_by_channel_type]]

## M12 MIMO 接收机与检测器

- [[T12.1_mimo_receiver_chain_overview]]
- [[T12.2_diversity_combining_mrc]]
- [[T12.3_linear_detectors_mf_zf_mmse]]
- [[T12.4_sphere_detection_detector_selection]]
- [[T12.5_channel_estimation_llr_reliability]]
- [[T12.6_reference_signal_family_CSI_RS_PTRS_TRS|T12.6 参考信号体系（CSI-RS/PTRS/TRS）]]

## M13 概率整形算法

- [[T13.1_probabilistic_shaping_overview_shaping_gain]]
- [[T13.2_mb_distribution_entropy_power_tradeoff]]
- [[T13.3_ess_enumerative_sphere_shaping]]
- [[T13.4_ps_embedding_four_access_points_bit_organization]]
- [[T13.5_ps_receiver_llr_prior]]
- [[T13.6_tbs_matching_system_cost_roi]]

## M14 控制信道族（阶段 2 深化）

控制面从"开机找小区"到"数据调度与反馈"的完整闭环：下行控制（盲检/DCI）、上行回执（UCI/PUCCH）、接入与广播（小区搜索/PBCH）、LTE 控制译码（TBCC）。

- [[T14.1_PDCCH_blind_decoding|T14.1 PDCCH 盲检]]
- [[T14.2_DCI_format_detailed|T14.2 DCI 格式精读]]
- [[T14.3_PUCCH_UCI_formats|T14.3 PUCCH 与 UCI]]
- [[T14.4_PBCH_cell_search_system_info|T14.4 小区搜索与系统信息]]
- [[T14.5_TBCC_decoding|T14.5 TBCC 译码]]

## M15 上行链路（阶段 3）

上行链路：波形（物理地基，并入 T15.5 深化）、接入（PRACH）、探测（SRS）、功控（功率管理）、NR DL/UL 差异（系统收束）——与 M14 控制信道族构成上下行完整闭环。

- [[T15.2_PRACH_random_access|T15.2 随机接入（PRACH）]]
- [[T15.3_SRS_sounding|T15.3 探测参考信号（SRS）]]
- [[T15.4_uplink_power_control|T15.4 上行功率控制]]
- [[T15.5_NR_DL_UL_differences|T15.5 NR 上下行差异专篇]]
- [[T15.6_DFT_sOFDM_uplink_waveform|T15.6 DFT-s-OFDM 上行波形]]

## M16 调度与系统体系（阶段 5）

调度决策（MAC 层）、空间维度（波束）、多载波与带宽（CA/BWP）、信道组织（MAC 映射）——M14/M15 之后调度的完整画像。

- [[T16.1_scheduler_HARQ_process|T16.1 调度与 HARQ 进程]]
- [[T16.2_beam_management|T16.2 波束管理]]
- [[T16.3_CA_BWP|T16.3 载波聚合与 BWP]]
- [[T16.4_MAC_layer_mapping|T16.4 MAC 层映射]]
- [[T16.5_CSI_reporting_link_adaptation|T16.5 CSI 报告与链路自适应]]
- [[T16.6_random_access_rrc_connection_setup|T16.6 随机接入与 RRC 连接建立]]
