...
本节使用的单篇技术缩写先统一说明：低密度奇偶校验码是 NR 数据业务采用的信道编码；对数似然比是 LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）译码前的软信息，本节只在解释 CBG（码块组，Code Block Group）重传输入来源时提到；传输块是媒体接入控制层交给物理层的业务数据单元；码块是 TB（传输块，Transport Block）经分段后逐块 LDPC 译码的对象；CBG是 NR 为部分重传引入的 CB（码块，Code Block）分组；循环冗余校验用于发现译码或重组错误；混合自动重传请求根据译码状态保留或释放软缓存；下行共享信道和上行共享信道是本节主要讨论的数据承载信道；下行控制信息携带 CBG transmission information 等控制字段；寄存器传输级/专用集成电路实现需要把 CB 状态、拼接地址和 CRC（循环冗余校验，Cyclic Redundancy Check）输入边界做成可验证的硬件控制。

本节回答 NR LDPC 接收链路中一个很靠后的问题：每个 CB 已经完成速率恢复、LDPC 迭代和硬判决以后，接收端怎样把这些 CB 重新变成一个 TB，并决定本次 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）是通过还是失败。
...
- 给出 RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）中 CB status RAM、concat buffer、CRC checker 和 HARQ feedback 的验证观察点。

如果只记一件事：**LDPC decoder 输出的是一个个 CB 的候选结果；真正能交付给 MAC（媒体接入控制层，Medium Access Control）的是按协议顺序去除 filler/可选 CB CRC 后拼接出来、并通过 TB CRC 的 TB candidate。**
...
| T9.3 NR HARQ/CBG | 知道 CBG mask 可能让某些 CB 本次没有新 LLR（对数似然比，Log-Likelihood Ratio），历史状态需要保持。 |
| T3.1 CRC 家族 | 知道 CRC 是 GF(2) 多项式余数检查，CRC pass 不等于所有上下文一定正确。 |

本节不重新推导 BG（基图，Base Graph）、Zc、lifting size 或 LDPC 迭代算法。那些属于 [[T3.4_NR_LDPC_segmentation_rules|T3.4]]、[[T8.2_NR_LDPC_base_graph_selection|T8.2]]、[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]、[[T8.5_LDPC_sum_product_BP|T8.5]]-[[T8.8_NR_LDPC_decoder_numeric_walkthrough|T8.8]]。这里关心的是译码后的比特边界和接收端状态机。
...
| punctured/unknown | rate matching/RV（冗余版本，Redundancy Version）未观测 | LLR 初始化为未知或低置信，不在重组阶段删除。 |
...
对于 PDSCH（物理下行共享信道，Physical Downlink Shared Channel），若 DCI（下行控制信息，Downlink Control Information）中存在 CBGFI，还要按 TS 38.214 §5.1.7.2 判断旧实例是否可合并。`CBGFI=0` 表示早前相同 CBG 实例可能损坏，接收端不能盲目把旧 LLR 当成可合并证据；`CBGFI=1` 表示可与早前实例合并。[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 已讲 soft buffer 合并，本节只强调重组时要使用正确的 CB result/status。
...
“required CB” 在 CBG 场景中很关键。未在本次传输中的 CBG 不等于不 required；它可能 required 但使用历史有效结果。是否可使用历史结果，取决于 HARQ/CBG 状态、NDI（新数据指示，New Data Indicator）、CBGFI 和实现策略。
...
| 结论 | 协议依据 | 本地证据 |
|:---|:---|:---|
| UL-SCH（上行共享信道，Uplink Shared Channel）transport block 有 TB CRC | TS 38.212 §6.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:1361-1375` |
...
| DL-SCH（下行共享信道，Downlink Shared Channel）/PCH transport block 有 TB CRC | TS 38.212 §7.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:3737-3753` |
