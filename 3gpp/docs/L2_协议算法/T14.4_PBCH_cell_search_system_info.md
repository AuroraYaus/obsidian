...
Lmax=8 的小区，SSB 索引的 3 bit 怎么携带？

**提示**：Lmax=8 > 4 → DM-RS 序列携带 3 LSB；Lmax=8 无 MSB（索引 0-7 全在 3 bit 内）——对照 38.213 §4.1 的"Lmax=4 用 2 位、Lmax>4 用 3 位"规则，3 bit 已覆盖 0-7 全部索引。（答案：全部由 DM-RS 序列序号携带。）
...
LTE PBCH 的编码链是什么？为什么 NR 改用 Polar？

**参考答案**：MIB 24 bit → CRC16 → TBCC（K=7，码率 1/3）→ 速率匹配 480 bit → QPSK；40 ms 内 4 次重复。NR 改用 Polar 是因为 Polar 在短码长下性能优于 TBCC（约 0.5-1 dB），且与 NR PDCCH 共用译码器硬件（T10 框架复用）。

### 习题 5：SSB 索引与 Lmax

Lmax=64 的小区，UE 检测到 PBCH DM-RS 序号为 3（3 bit，即低 3 位 = 3），PBCH 载荷附加位 aA+5..aA+7 = 0b101（5）。SSB 索引是多少？

**参考答案**：低 3 位 = 3（DM-RS），高 3 位 = 5（载荷）→ 索引 = 5×8 + 3 = 43（0-63 内）。

### 习题 6：GSCN 全链

0-3000 MHz 段，某小区 SSB 在 GSCN=1800（N=600, M=3）。该位置在 100 kHz 信道栅格上对应哪个 ARFCN 网格点？

**参考答案**：f = 720.15 MHz → 最接近的 100 kHz 整数倍是 720.1 或 720.2 MHz（ARFCN = 720100 或 720200 kHz 网格）。GSCN 与 ARFCN 不必重合——SSB 只需在同步栅格上，载波中心在信道栅格上。

### 习题 7：重选场景

UE 在小区 A 驻留后移动到小区 B 覆盖边缘，B 的 SSB 在另一 GSCN。UE 需要做什么？

**参考答案**：按系统信息里的邻区频率列表对 GSCN 做异频小区搜索（重复本节全流程），比较 S 准则（q-RxLevMin 等）后重选。**重选 = 小区搜索的重复调用**——本节流程是重选、切换的公共原语。

### 习题 8：层级串联

把"UE 开机 → 解出 SIB1"的完整链条按序写出，标出每步的信道/信号。

**参考答案**：同步栅格扫频（无信道，纯搜索）→ PSS/SSS（同步信号）→ 小区 ID 合成（无信道，纯计算）→ PBCH（广播信道，Polar 译码）→ MIB（高层信息）→ CORESET 0 盲检 PDCCH（控制信道，SI-RNTI）→ DCI 1_0（调度信息）→ PDSCH 译码（共享信道，LDPC）→ SIB1（系统信息）。
