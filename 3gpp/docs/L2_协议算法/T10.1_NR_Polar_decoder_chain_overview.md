...
本节从控制信道解调器输出的软信息开始，建立 NR Polar（极化码，Polar Code）控制信息接收侧译码链路。这里先展开本节第一次作为主角出现的术语：极化码是一类利用信道极化现象构造的纠错码；上行控制信息承载混合自动重传请求的确认信息、调度请求和信道状态信息等上行控制内容；下行控制信息承载调度、资源分配、冗余版本和 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）process 等下行控制命令；对数似然比是译码器输入软信息；循环冗余校验用于检错并辅助列表路径选择；无线网络临时标识（Radio Network Temporary Identifier, RNTI）参与 DCI（下行控制信息，Downlink Control Information）CRC（循环冗余校验，Cyclic Redundancy Check）边界；逐次消除译码（Successive Cancellation, SC）是 Polar 的基础树译码；逐次消除列表译码（Successive Cancellation List, SCL）保留多条候选路径；CRC 辅助 SCL（CRC-aided SCL, CA-SCL）用 CRC 从候选路径中选择最终控制比特；低密度奇偶校验码是 NR 数据业务主线；Turbo（Turbo 码，Turbo Code）是 LTE 数据业务主线；传输块、码块、上行共享信道、下行共享信道、调制与编码方案、媒体接入控制层、寄存器传输级和专用集成电路会在本文作为对比或工程边界出现。

学完本节后，应能做到：

- 解释 Polar 的中文名、英文名、基本思想和它适合 NR 短控制块的原因。
- 画出从 demapper LLR（对数似然比，Log-Likelihood Ratio）到速率恢复、子块解交织、Polar 译码、CRC 辅助路径选择和控制比特输出的接收链路。
- 区分 UCI（上行控制信息，Uplink Control Information）与 DCI 在本节中的协议边界：UCI 可能在 PUCCH（物理上行控制信道，Physical Uplink Control Channel）或 PUSCH（物理上行共享信道，Physical Uplink Shared Channel）上承载，DCI 在 PDCCH（物理下行控制信道，Physical Downlink Control Channel）上承载，本节只讲影响 Polar 译码输入输出的部分。
...
本节只做总览，不把信道极化、可靠性序列、SC/SCL 公式、CA-SCL path metric 或 Polar rate recovery 的每个索引一次性写完。[[T10.2_channel_polarization_frozen_bits|T10.2]] 到 [[T10.7_NR_Polar_rate_recovery|T10.7]] 会逐项展开；[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] 会把边界案例整理成调试清单。
...
| T4.2 图模型 | 知道 Polar 译码常用树模型理解，不同于 LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）Tanner 图和 Turbo trellis。 |
...
| 说明后续章节展开范围 | “本节与 [[T10.2_channel_polarization_frozen_bits|T10.2]]-[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] 的分工” | 明确各小节承接内容。 |

这张表是最低覆盖。正文额外补充协议证据、接收端伪代码、浮点/定点/RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射、验证方法、自测和执行记录。

## 资料与协议边界

本节使用 3GPP TS 38.212 Rel-19 `38212-j30`，本地处理目录为 `3GPP_Rel19/processed/TS_38.212_38212-j30`。涉及表格时，本节只复现总览需要的短表和子表，不把 [[T10.3_NR_Polar_reliability_sequence|T10.3]]/[[T10.7_NR_Polar_rate_recovery|T10.7]] 的完整协议表提前展开。
...
证据边界：`content.md` 对若干公式符号抽取不完整，因此本节不声称复现了 §5.2.1、§5.3.1、§5.4.1 的全部 bit-exact 公式。涉及完整可靠性序列、sub-block interleaver pattern、puncturing/shortening/repetition 索引和 CA-SCL 路径度量时，分别由 [[T10.3_NR_Polar_reliability_sequence|T10.3]]、[[T10.7_NR_Polar_rate_recovery|T10.7]]、[[T10.4_Polar_SC_decoding|T10.4]]-[[T10.6_CRC_aided_SCL_control_reliability|T10.6]] 复现和推导。
...
| `tables/table_0012.csv` | `5df7ed346272365183c74ca5e832d94e651223738f05185a40c752d68a9f6a95` | 本节只复现前 16 项作为读表示例，完整表留 [[T10.3_NR_Polar_reliability_sequence|T10.3]]。 |
...
Polar 码由 Erdal Arikan 在 2009 年提出。它在理论上重要，是因为它给出了构造可达容量码的一条明确路径；它在 NR 控制信道中重要，是因为短块控制信息需要低时延、高可靠和强检错边界。短控制块不像大数据 TB（传输块，Transport Block）那样可以依赖大块长并行 LDPC 迭代；它更看重较小码长下的路径搜索、CRC 辅助选择和可控延迟。

NR 控制信息与数据业务的差异如下：

| 对象 | 典型内容 | 编码家族 | 接收端关注点 |
|:---|:---|:---|:---|
| 数据业务 | UL-SCH（上行共享信道，Uplink Shared Channel）/DL-SCH（下行共享信道，Downlink Shared Channel）payload、MAC（媒体接入控制层，Medium Access Control）PDU | LDPC | 大吞吐、CB（码块，Code Block）并行、HARQ 软合并、TB CRC。 |
| UCI | HARQ-ACK、SR、CSI（信道状态信息，Channel State Information） | block code 或 Polar | payload 短、资源紧、CRC 长度与 UCI 类型相关。 |
| DCI | 调度、资源、MCS（调制与编码方案，Modulation and Coding Scheme）、RV（冗余版本，Redundancy Version）、HARQ process | Polar | 盲检、24 bit CRC、RNTI 归属、误通过风险。 |
...
这张表的含义很有限但很关键：DCI 进入 Polar；UCI 可能进入小块 block code，也可能进入 Polar。本节只讲 UCI encoded by Polar code 和 DCI encoded by Polar code 的接收主链路。UCI 小块 block code 不属于本模块主线，后续若比较 UCI 小负载边界，会在 [[T10.8_NR_Polar_decoder_edge_cases|T10.8]] 中作为“无 CRC 小负载”或“非 Polar 分支”说明。
...
本节先不推导 SC 的 $f/g$ 函数，也不展开 $N=4/N=8$ 的完整树遍历。[[T10.2_channel_polarization_frozen_bits|T10.2]] 会从这个直觉继续讲 frozen bit、information bit 和 Polar 变换；[[T10.4_Polar_SC_decoding|T10.4]] 会完整推导 SC。
...
| parity-check bit | TS 38.212 §5.3.1.2 中 Polar 编码内部的辅助校验位概念。 | 本节只定位，具体选择规则留 [[T10.3_NR_Polar_reliability_sequence|T10.3]]/[[T10.6_CRC_aided_SCL_control_reliability|T10.6]]。 |
...
读法：越靠前表示可靠性越低，越靠后表示可靠性越高。真实 NR 不能只用前 16 项；[[T10.3_NR_Polar_reliability_sequence|T10.3]] 会复现本项目需要的可靠性序列子集或完整结构化表，并给出 information set/frozen set 生成伪代码。
...
DCI 的重点不是“路径 PM 最小就一定正确”。若 PM 最优路径的 CRC/RNTI 检查失败，而次优路径通过，CA-SCL 应选择通过 CRC/RNTI 的候选。[[T10.6_CRC_aided_SCL_control_reliability|T10.6]] 会完整展开该案例。
...
该 toy 只验证 selector 逻辑：CRC/RNTI 失败路径即使 PM 更好也不能输出；多个通过者按 PM 选择。真实 CRC 多项式和 RNTI scrambling 由 [[T3.1_LTE_NR_CRC_families|T3.1]]/[[T3.5_NR_Polar_segmentation_crc|T3.5]]/[[T10.6_CRC_aided_SCL_control_reliability|T10.6]] 承接。
...
| SC/SCL | [[T10.4_Polar_SC_decoding|T10.4]]/[[T10.5_Polar_SCL_decoding|T10.5]] 完整实现前，本节只检查 descriptor 和 selector。 |
...
本节不规定具体位宽。[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 会规划 NR Polar 定点模型，[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] 会规划 RTL 微架构；本节只要求 descriptor 和日志字段足够支持 bit-exact 对比。
...
Polar 硬件瓶颈通常不在一个大矩阵乘法，而在树遍历控制、路径复制、排序、partial-sum 访问和低延迟 selector。[[T10.5_Polar_SCL_decoding|T10.5]]/[[T10.6_CRC_aided_SCL_control_reliability|T10.6]]/[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] 会继续细化。
...
## 本节与 [[T10.2_channel_polarization_frozen_bits|T10.2]]-[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] 的分工
...
5. [[T10.1_NR_Polar_decoder_chain_overview|T10.1]] 为什么不完整复现可靠性序列和 rate matching 表？
...
5. 因为本节是总览课，完整可靠性序列和 rate matching 表需要逐表核验和更细索引推导，分别由 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 和 [[T10.7_NR_Polar_rate_recovery|T10.7]] 承接；总览课只复现必要子集并说明证据边界。
...
| 表格证据 | 已核验 `table_0010.csv` SHA-256 并在正文复现；`table_0011.csv`/`table_0012.csv` 已记录 SHA-256，完整复现留 [[T10.3_NR_Polar_reliability_sequence|T10.3]]/[[T10.7_NR_Polar_rate_recovery|T10.7]]。 |
...
3. 本项目 [[T3.5_NR_Polar_segmentation_crc|T3.5]]：`docs/L1_基础/T3.5_NR_Polar_segmentation_crc.md`。
4. 本项目 [[T4.2_graphs_trellises_trees_for_decoding|T4.2]]：`docs/L1_基础/T4.2_graphs_trellises_trees_for_decoding.md`。
