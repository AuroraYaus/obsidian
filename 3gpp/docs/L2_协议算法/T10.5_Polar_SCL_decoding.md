...
本节讲 Polar（极化码，Polar Code）的逐次消除列表译码（Successive Cancellation List, SCL）。[[T10.4_Polar_SC_decoding|T10.4]] 的 SC（逐次消除，Successive Cancellation）每个 information bit 只保留一个判决；一旦早期判错，后续 $g$ 函数会把错误 partial sum 带入右子树。SCL 的核心改进是：在 information bit 位置不急着只选 0 或 1，而是把多个候选路径保留下来，再用路径度量（Path Metric, PM）排序剪枝。

本节使用的工程缩写先统一说明：对数似然比是路径度量更新的输入软信息；循环冗余校验在本节只作为后续 [[T10.6_CRC_aided_SCL_control_reliability|T10.6]] 的最终选择边界，不参与本节 toy 例子的路径剪枝；寄存器传输级和专用集成电路用于描述硬件代价；静态随机存取存储器用于描述路径存储、LLR（对数似然比，Log-Likelihood Ratio）存储和 partial sum 存储。
...
- 说明剪枝时必须同步复制路径 bit、LLR state、partial sum 和 CRC（循环冗余校验，Cyclic Redundancy Check）state。
...
| 排序对硬件影响 | “RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射” | sorter、path memory、copy network、partial sum memory。 |
...
| CRC state | 若做在线 CRC 更新，记录路径的 CRC 寄存器。 | [[T10.6_CRC_aided_SCL_control_reliability|T10.6]] 使用，剪枝时必须同步。 |
...
列表大小增加带来的不是单一乘法关系。$f/g$ 计算、LLR state、partial sum、路径 bit、PM、CRC state 都要按路径维度管理。若 $L$ 翻倍，排序候选数、复制带宽和 SRAM（静态随机存取存储器，Static Random-Access Memory）读写压力也会增加。
...
这一排序表是纯 PM 选择。若进入 [[T10.6_CRC_aided_SCL_control_reliability|T10.6]] 的 CRC-aided SCL，最终选择还要检查 CRC；PM 最小路径不一定是最终输出路径。
...
| CRC 接口 | 保存候选路径和 PM。 | [[T10.6_CRC_aided_SCL_control_reliability|T10.6]] final selector 能继续读取候选。 |
...
| L=1 不等于 SC | SCL 基础状态机错误。 | 用 [[T10.4_Polar_SC_decoding|T10.4]] directed case 做等价测试。 |

## 与 CRC-aided SCL 的边界

本节到剪枝后保留 $L$ 条候选为止。若所有 information bit 都处理完，基础 SCL 可以选择 PM 最小路径作为输出。但 NR 控制信息通常还要利用 CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）边界进行最终选择。CRC-aided SCL 的规则不是“无条件选 PM 最小”，而是在候选路径中优先选择 CRC 通过的路径，再按 PM 或实现约定处理多个通过候选。

因此，本节的 PM 排序是候选管理机制；[[T10.6_CRC_aided_SCL_control_reliability|T10.6]] 的 CRC-aided selector 是最终输出机制。两者不能混淆。
...
4. 本项目 [[T10.4_Polar_SC_decoding|T10.4]]：`docs/L2_协议算法/T10.4_Polar_SC_decoding.md`。
