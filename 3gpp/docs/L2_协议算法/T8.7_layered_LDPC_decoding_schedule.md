...
- 说明 layer 与 TS 38.212 LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）base graph row group 的关系。
...
| T8.5 BP（置信传播，Belief Propagation）/SPA（和积算法，Sum-Product Algorithm） | 知道变量节点和校验节点沿边交换对数似然比消息。 |
| T8.6 Min-Sum | 知道工程上常用 Min-Sum 类 check-node unit 代替精确 SPA。 |
| 存储读写 | 知道同一个变量节点 LLR（对数似然比，Log-Likelihood Ratio）可能被多个 layer 读取和写回。 |
...
| 工程取舍 | “RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）映射”和“验证方法” | 补 bank conflict、pipeline、parallelism、日志字段。 |

这张表只表示最低覆盖。正文还会补 syndrome/CRC（循环冗余校验，Cyclic Redundancy Check）边界、Zc 地址公式、伪代码和常见错误。
...
| edge schedule ROM（只读存储器，Read-Only Memory） | 保存 row group、column group、shift | 整个 decoder 配置期间只读。 |
...
syndrome early stop 和 CRC early stop 常被混用，必须分开。这里先明确边界对象：传输块是协议交付给物理层编码链路的大块数据，码块是在 TB（传输块，Transport Block）分段后进入 LDPC 译码器的单个处理单元。

| 对象 | 检查位置 | 检查内容 | 工程用途 |
|:---|:---|:---|:---|
| syndrome early stop | LDPC decoder core 内部 | $H\hat{\mathbf{x}}^T=\mathbf{0}$ | 判断当前硬判决是否满足 LDPC 码约束，可提前停止迭代。 |
| CB（码块，Code Block） CRC | LDPC 译码、信息位抽取和 filler removal 之后 | 条件性 CB 数据完整性 | 只有当本 TB 被分成多个 CB 并附加 CB CRC 时才检查；单 CB 场景不应假设必有 CB CRC。 |
...
**浮点仿真**应以 [[T8.5_LDPC_sum_product_BP|T8.5]]（SPA/BP）和 [[T8.6_LDPC_MS_NMS_OMS|T8.6]]（MS（最小和，Min-Sum）/NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum））的浮点模型为 baseline，将 layered schedule 作为独立变量对比：

- 同一条 BG、$Z_c$、码率和 Eb/N0 下，对比 flooding SPA、flooding MS、layered SPA 和 layered MS 的 BLER（块错误率，Block Error Rate）、FER（帧错误率，Frame Error Rate）、平均迭代次数和 syndrome stop rate。
...
5. syndrome early stop 检查 LDPC 码约束 $H\hat{\mathbf{x}}^T=\mathbf{0}$，用于 decoder 内部停止；CRC 检查 CB/TB 数据完整性，用于交付和 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）反馈边界。CB CRC 是条件性的，只有分段并附加 CB CRC 时才存在。
