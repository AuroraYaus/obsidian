...
[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 讲了 NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）的软缓存、RV（冗余版本，Redundancy Version）/k0 和 CBG（码块组，Code Block Group）部分重传的协议语义；[[T4.3_HARQ_soft_combining_basics|T4.3]] 讲了 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）软合并的 LLR（对数似然比，Log-Likelihood Ratio）证据相加原理。本节把两者在**译码侧缓存操作**这个工程视角上收拢：UE 的 HARQ CB（码块，Code Block）缓存面对新数据和重传，分别执行**覆盖写（Overwrite）**与**增量写（Incremental Write）**两种操作。这是基带/硬件开发中高频出现的术语，也是面试常考点——本节给出从协议触发条件到缓存操作、再到硬件实现的完整闭环。

学完本节后，应能做到：

- 用一句话定义 CB 增量写，并说清它与覆盖写的本质区别。
- 从 DCI（下行控制信息，Downlink Control Information）字段（NDI（新数据指示，New Data Indicator）、CBGTI）推导"本次传输对某个 CB 执行覆盖写还是增量写"。
...
| 缓存管理 | 软缓存按 CB 独立管理——NR R15 引入 CBG 后，缓存操作粒度从 TB（传输块，Transport Block）细化到 CB |
...
gNB 发送 TB 的全部 CB（含 TB CRC（循环冗余校验，Cyclic Redundancy Check）与各 CB CRC），UE：
...
| 增量写适用于所有信道 | 仅 PDSCH（物理下行共享信道，Physical Downlink Shared Channel）/PUSCH（物理上行共享信道，Physical Uplink Shared Channel）的 LDPC 信道存在；控制信道（Polar（极化码，Polar Code））无此机制（控制信息不做 IR 软合并）。 |
...
| TS 38.321 §5.3.2.2 | HARQ 信息组成：NDI、TBS（传输块大小，Transport Block Size）、RV、HARQ process ID；NDI toggled 判定 | `3GPP_Rel19/processed/TS_38.321_38321-j20/full.md` |
...
5. 靠同一套坐标（RV、k0（速率匹配起点，Rate matching starting position）、Ncb、E）；错位会导致累加位置错误，软合并增益变损耗。
...
| 维度 | 归属 | 依据 |
|:---|:---|:---|
| HARQ 软合并（IR）语义与 NDI toggled 判定 | 协议规定 | TS 38.321 §5.3.2.2 |
| CBG 分组与重传粒度（$M = \min(N, C)$、CBGTI 位图、CBGFI 反馈粒度） | 协议规定 | TS 38.214 §5.1.7 |
| RV 速率匹配起点 $k_0$ 与逐 CB 循环缓存 | 协议规定 | TS 38.212 §5.4.2 |
| "增量写/增量读"的措辞 | 实现域（协议无此字样，本节已标注） | 工程术语，描述软合并语义 |
| LLR 累加的定点位宽、饱和策略、量化前后顺序 | 实现域 | 接收机自选，精度/面积权衡 |
| 缓存架构（banking、覆盖状态标记、descriptor 接口字段） | 实现域 | 接收机自选 |

接收端义务边界：NDI 翻转必须清空整个 HARQ process 的 CB 缓存；NDI 未翻转且 CBGTI 命中，必须把本次 LLR 与历史 LLR 按同一 $k_0$ 坐标累加；CBGTI 未命中的 CBG 缓存必须保持不动；反馈按 CBGFI 的粒度执行。除此之外——累加用几位定点、缓存分几个 bank、译码器如何调度——协议都不干涉。
