...
本节从接收机视角对比 LTE 下行共享信道与上行共享信道的 Turbo（Turbo 码，Turbo Code）译码差异。这里的重点不是证明“下行有一个 Turbo 算法、上行有另一个 Turbo 算法”，而是讲清同一个 LTE Turbo 译码核心放在不同接收系统中时，协议链路、参数来源、混合自动重传请求上下文、软缓存索引、反馈时限和验证重点如何改变。

本节后文使用以下缩写：传输块、码块、循环冗余校验、对数似然比、调制与编码方案、传输块大小、块错误率、加性白高斯噪声、静态随机存取存储器、寄存器传输级和专用集成电路。


- 指出 TS 36.212 Rel-19 中 UL-SCH（上行共享信道，Uplink Shared Channel）和 DL-SCH（下行共享信道，Downlink Shared Channel）的处理链路位置，并解释它们如何复用通用 CRC（循环冗余校验，Cyclic Redundancy Check）、分段、Turbo 编码、速率匹配和码块串接规则。
- 区分“Turbo 核心算法可复用”和“接收端上下文不能混用”。
- 列出 LTE Turbo 译码器从物理层控制、调度上下文和媒体接入控制层/HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）状态获得的关键参数。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| LTE Turbo 链路 | 知道接收端大体要做解速率匹配、软合并、Turbo 译码、CB（码块，Code Block）CRC、CB 重组和 TB（传输块，Transport Block）CRC。 |
| DL-SCH/UL-SCH | 知道 DL-SCH 是基站到终端的数据共享信道，UL-SCH 是终端到基站的数据共享信道。 |
| HARQ | 知道 HARQ 用 CRC 成功/失败结果驱动重传，重传软信息需要与旧软信息合并。 |
| LLR | 知道 LLR（对数似然比，Log-Likelihood Ratio）是译码器输入软信息，符号和幅度都会影响译码。 |
| MAC（媒体接入控制层，Medium Access Control）/PHY 边界 | 知道调度、HARQ 进程、反馈和资源位置不是 Turbo 核心算法内部产生的。 |
...
协议边界必须明确：TS 36.212 决定 TB 如何附 CRC、如何分段、如何 Turbo 编码、如何速率匹配、如何 CB 串接，因此它直接决定译码器的反向 bit 流程。TS 36.213/36.321 决定很多控制参数和时序背景，例如 HARQ 进程、反馈期限、资源与 MCS（调制与编码方案，Modulation and Coding Scheme）/RV（冗余版本，Redundancy Version）来源；这些会影响译码器 descriptor 和状态机，但本节不把调度器或 MAC HARQ entity 设计写成 Turbo 译码算法。

## 下行与上行的概念来源

“下行”和“上行”是无线链路方向，不是编码算法名称。下行指基站发送、终端接收；上行指终端发送、基站接收。放到 LTE Turbo 译码器里，方向首先决定“译码器运行在哪一端”：

- DL-SCH 译码器运行在用户设备（User Equipment, UE）接收机中，面对的是基站发来的 PDSCH（物理下行共享信道，Physical Downlink Shared Channel）软信息。
- UL-SCH 译码器运行在演进型基站（evolved NodeB, eNB）接收机中，面对的是 UE 发来的 PUSCH（物理上行共享信道，Physical Uplink Shared Channel）软信息。
...
| `tb_size` | 必需 | 必需 | 调度/TBS（传输块大小，Transport Block Size）计算 | 分段、CRC 输入长度和重组错。 |
...
沿用前文 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]] 的分段结论，假设某个 TB 经过 TB CRC 后进入分段的长度为 $B=10001$，可得到一个教学分段结果：
...
| LLR 生成 | PDSCH demapper 输出 DL-SCH LLR | PUSCH demapper 输出 UL-SCH LLR | LLR count、Qm（调制阶数，Modulation Order）、layer/codeword mapping |
...
|:---|:---|:---|:---|:---|
| DL RV 错 | UE 锁存的 RV 与本次传输不一致 | 重传后 BLER（块错误率，Block Error Rate）不降反升 | `direction`、`harq_id`、`rvidx`、$k_0$、soft before/after | RV 必须进入地址生成，不只写日志。 |
...
浮点仿真先用无噪声向量确认协议地址和 descriptor，再加入 AWGN（加性白高斯噪声，Additive White Gaussian Noise）噪声或真实链路 LLR 看性能曲线。若无噪声都 fail，说明不是信道噪声问题。
...
| 软缓存 | UE category 和 $N_{soft}$ 限制更显著 | 多 UE 共享大 SRAM（静态随机存取存储器，Static Random-Access Memory），key 错风险更高 | key 显式包含 direction 和 UL UE context。 |
...
建议 RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）结构把 Turbo core 和上下文控制分开：
...
| TS 36.212 §5.1.2 | 分段、CB CRC、filler 和 CB 编号顺序是 UL/DL 共同基础。 | 详细分段手算承接 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T7.4_LTE_code_block_reassembly_TB_CRC|T7.4]]。 |
| TS 36.212 §5.1.3.2 | UL/DL 共享 Turbo coding 规则。 | Turbo 内核推导承接 T6。 |
| TS 36.212 §5.1.4.1 | Turbo coded transport channels 的 rate matching 是 DL/UL 共同译码反操作基础。 | RV/soft buffer 细节承接 [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]]/[[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]。 |
...
| 项目 | 记录 |
|:---|:---|
| 路线图卡片 | 使用 [[T7.5_LTE_DL_UL_decoding_differences|T7.5]]。 |
...
| 图表资产 | `docs/L2_协议算法/assets/T7.5_LTE_DL_UL_decoder_context.svg`（手绘 SVG，替代原 `tools/archive_python_drawing/figures/render_lte_dl_ul_decoder_context.py` 生成的 PNG）；已按全局视觉审计要求重画并目检：箭头垂直、文本框尺寸协调、说明框不压边。 |
| 审计结果 | T7 全量已通过 `LESSON_TERM_AUDIT_OK`、`MARKDOWN_HEADING_AUDIT_OK`、`LESSON_DEPTH_AUDIT_OK`、`LATEX_RENDER_AUDIT_OK formulas=434`；[[T7.5_LTE_DL_UL_decoding_differences|T7.5]] 单篇 LaTeX 审计 `LATEX_RENDER_AUDIT_OK formulas=46`。 |
