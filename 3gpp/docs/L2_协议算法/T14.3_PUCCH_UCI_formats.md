...
### 练习 4：format 0 的循环移位计算

给定：format 0，1 bit HARQ-ACK，资源的基础循环移位 $m_0 = 2$。发送 ACK 时发送端用的循环移位 $m_{\text{cs}}$ 是多少？NACK 呢？

**提示**：$m_{\text{cs}} = (m_0 + m_{\text{cs}}') \bmod 12$；1 bit 时 $m_{\text{cs}}' \in \{0, 6\}$——ACK 用 6，NACK 用 0。算出两个值（答案：ACK 8，NACK 2）。
...
负载 O = 400 bit 的 CSI 上报，走什么编码？CRC 多少位？

**参考答案**：O > 11 → Polar；O = 400 > 360 → CRC 11 bit（gCRC11 系）。

### 习题 5：两级资源分配

UE 收到 DCI 的 pucchResourceIndicator = 5（3 bit），PUCCH resource set 含 8 个 resource。指出两级机制的每一级在这次上报中的角色。

**参考答案**：第一级——RRC 配置的 set（按 UCI 负载段选择，含 8 个 resource 的定义：PRB/符号/序列/OCC/功控）；第二级——DCI 3 bit 索引 5 选中 set 内第 6 个 resource。两级合并确定"在哪、用什么结构、多大功率"上报。

### 习题 6：codebook 与 format 衔接

Type-2 codebook 下，DAI 指示本调度机会内共 4 个 PDSCH 需要反馈（4 bit HARQ-ACK），另有 1 bit SR。PUCCH 资源为 2 符号。选什么 format？UCI 编码走哪条路（RM 还是 Polar）？

**参考答案**：总 UCI = 4 + 1 = 5 bit > 2 → 符号树 2 < 4 短格式 → format 2。编码：5 ≤ 11 → RM(32, 5)，无 CRC。若负载增加到 12 bit（> 11）则切换 Polar（≤ 1706 单段，CRC 6 bit 因 O ≤ 360）。
