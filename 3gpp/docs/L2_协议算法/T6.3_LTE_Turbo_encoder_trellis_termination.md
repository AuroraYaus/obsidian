...
tags:
  ...
  - l2
  - lesson
source_spec: "docs/L2_协议算法/T6.3_LTE_Turbo_encoder_trellis_termination.md"
---
# T6.3 LTE Turbo 编码器与网格终止
## 本节学习目标

本节讲 LTE Turbo（Turbo 码，Turbo Code）编码器的协议结构和网格终止。上一节已经用玩具递归系统卷积码（Recursive Systematic Convolutional code, RSC）解释了状态、寄存器和校验输出；本节把这些概念放回 TS 36.212 Rel-19 `36212-j30` 的 Figure 5.1.3-2 和 §5.1.3.2.2。
...
| LLR（对数似然比，Log-Likelihood Ratio） | 知道它是 Turbo 译码器的软输入。 |
| CB（码块，Code Block） | 知道每个 CB 独立进行 Turbo 编码和译码。 |
| 传输块 | 知道 TB（传输块，Transport Block）可能被拆成多个 CB。 |
...
边界声明：TS 36.212 规定编码器输入输出和尾比特排列；它不规定 Turbo 译码器采用 MAP（最大后验概率，Maximum A Posteriori）、Log-MAP 还是 Max-Log-MAP，也不规定迭代次数。后者是实现选择，但无论采用何种算法，都必须使用与协议编码器一致的网格和尾比特顺序。
...
从零基础角度看，终止解决的是“故事结尾是否确定”的问题。没有结尾约束时，最后几步路径会出现多个解释；有结尾约束时，不满足终态的解释会被压低。这个定义直接带来工程后果：Turbo 译码器的后向递推、尾比特 LLR、解速率匹配母码长度和 CRC（循环冗余校验，Cyclic Redundancy Check）早停都必须共享同一个终止边界。推导上，$D=K+4$ 不是任意补长，而是协议把终止输出复用到三路母码流后的每路长度；实现若跳过这一步，后续公式即使写对也会在数组边界处失效。
...
本次阶段 4 已新增 `docs/L2_协议算法/assets/T6.3_TS36.212_Figure_5.1.3-2_turbo_encoder_rebuild.svg`（手绘 SVG），把协议 Figure 5.1.3-2 的主体结构和终止虚线路径重建为图表。后续若实现 bit-exact Turbo encoder，还应进一步把开关位置转成可执行状态转移，并明确每个尾部输出位进入 $d^{(0)}$、$d^{(1)}$、$d^{(2)}$ 的索引。
...
| 协议结构 | RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）对应模块 | 设计关注点 |
|:---|:---|:---|
| 两个 8 状态组成编码器 | 两个半迭代 SISO（软入软出，Soft-Input Soft-Output）核或一个复用 SISO 核 | 面积和吞吐折中。 |
...
## 图示
![图 1：TS36.212 Figure 5.1.3-2 turbo encoder rebuild](assets/T6.3_TS36.212_Figure_5.1.3-2_turbo_encoder_rebuild.svg)
...
| 图证据 | Figure 5.1.3-2 通过 Word XML 邻近段落和 relationship `rId87 -> media/image79.wmf` 定位；已重建为 `docs/L2_协议算法/assets/T6.3_TS36.212_Figure_5.1.3-2_turbo_encoder_rebuild.svg`（手绘 SVG，替代原 PIL 渲染 PNG 与 `tools/archive_python_drawing/figures/render_lte_turbo_encoder_structure.py`），并目检无文字遮挡。 |
...
| Turbo 编码器为 PCCC，含两个 8 状态组成编码器和一个内部交织器 | §5.1.3.2.1，Figure 5.1.3-2 | `sections.jsonl` paragraph 403，`content.md` lines 721-745，`media/image79.wmf`；本地重建图 `docs/L2_协议算法/assets/T6.3_TS36.212_Figure_5.1.3-2_turbo_encoder_rebuild.svg` |
