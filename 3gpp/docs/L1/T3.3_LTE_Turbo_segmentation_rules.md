...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1/T3.3_LTE_Turbo_segmentation_rules.md"
---
...
| 协议位置 | 本地证据 |
| :--- | :--- |
| TS 36.212 Rel-19 `36212-j30` §5.1 Generic procedures | `TS_36.212_36212-j30_content.md`，`sections.jsonl` |
| TS 36.212 §5.1.1 CRC calculation | `TS_36.212_36212-j30_content.md` |
| TS 36.212 §5.1.2 Code block segmentation and code block CRC attachment | `TS_36.212_36212-j30_content.md`，`sections.jsonl` |
| TS 36.212 §5.1.3.2 Turbo coding | `TS_36.212_36212-j30_content.md`，`sections.jsonl` |
| TS 36.212 Table 5.1.3-3 | `tables/table_0009.csv`，`tables/table_0009.html` |
| TS 36.212 §5.2.2.1-§5.2.2.3 | `TS_36.212_36212-j30_content.md` |
| TS 36.212 §5.3.2.1-§5.3.2.3 | `TS_36.212_36212-j30_content.md` |

协议前向链路可以压缩成下面的顺序：

```text
MAC交付 TB
  -> 传输块 CRC 附加，通常使用 24 bit CRC
  -> 码块分段与码块 CRC 附加
  -> 每个 CB 独立 Turbo 编码
  -> 速率匹配与物理信道映射
```

接收端不是重新发明规则，而是按相反方向恢复：

```text
LLR 与速率恢复
  -> 按 CB 描述符切分软信息
  -> 对每个 CB 执行 Turbo 译码
  -> 若存在 CB CRC，则执行 CB CRC 检查
  -> 删除 CB CRC 与 filler 位置
  -> 按协议顺序拼接 CB payload
  -> 执行 TB CRC 检查
  -> 交付 TB 或声明译码失败
```

这里的关键是“码块边界是协议边界”。译码器可以在工程上并行处理多个 CB，但 CB 的数量、长度、填充位和 CRC 类型不能由硬件随意决定。
...
TS 36.212 Table 5.1.3-3 给出 Turbo 内部交织器支持的 $K$ 值集合，以及每个 $K$ 对应的内部交织器参数 $f_1$ 和 $f_2$。本地逗号分隔值文件（Comma-Separated Values, CSV）`table_0009.csv` 与超文本标记语言（HyperText Markup Language, HTML）`table_0009.html` 已核验。先解释每一列：

表格复现图的视觉审查说明也作为正文保留：例外：左上角 i 为短索引字段，可低于主体说明密度；其余正文、表头、首列和证据说明均已提升到 24px。
...
TS 36.212 Table 5.1.3-3 的正文复现按 2x2 分区组织，避免 188 行长表把阅读路径打断。表格覆盖全部 188 个表项，并高亮本节手算例子用到的 `K=4992`、`K=5056` 和最大块长 `K=6144`；执行记录只保留可复现证据，不作为正文阅读入口。


| 内容 | 说明 |
|:---|:---|
| 表格来源 | TS 36.212 Table 5.1.3-3；本地结构化证据为 `table_0009.csv/html`，本节图表中按 2x2 分区展示全部 188 个 Turbo interleaver 参数项。 |
...
这张表是 TS 36.212 Table 5.1.3-3 的正文复现：它覆盖全部合法 `K/f1/f2` 组合，并把本节手算用到的关键行单独列在下方，避免读者只记住 `K` 而漏掉配套的二次置换参数。
...
举一个专门用来观察顺序的边界例子。若 $B=6145$，则 $C=2$、$L=24$、$B'=6193$，查 Table 5.1.3-3 得到 $K_-=3072$、$K_+=3136$、$C_-=1$、$C_+=1$、$F=15$。这时 CB0 是 3072 bit 的短块，CB1 是 3136 bit 的长块；CB0 开头还有 15 个 filler 位置。接收端不能把 CB1 当成第一个块，也不能把两个块按译码完成时间拼回去。
...
第三，TS 36.212 §5.1.3.2 进一步涉及第 0 个码块存在 filler 时 Turbo 编码器输入和部分输出的处理。当前 `TS_36.212_36212-j30_content.md` 里相关符号抽取不完整，因此正文记录“存在专门处理规则”这一事实；具体输出流下标由 T6.3 在核验原始 Word XML、图 5.1.3-2 和公式 XML 后逐符号展开。
...
| 证据意识 | 能说明本节已核验并表格化复现 Table 5.1.3-3，但未逐符号复现 Turbo 编码器 filler 输出规则。 |
...
| TS 36.212 3GPP_译码知识库入口 | 说明 `TS_36.212_36212-j30_content.md`、`sections.jsonl`、`tables/*.html/csv`、`equations/*.xml` 的用途和边界。 | 复杂公式应优先核验 XML 或原始 Word。 |
| TS 36.212 §5.1.2 | 使用 $Z=6144$、分段时 CB CRC `L=24`、filler 放在第一个 block 开头、filler 为 `<NULL>`、CRC 计算时 filler 按 0。 | `TS_36.212_36212-j30_content.md` 对部分公式变量抽取不完整，本节用可核验规则和表格重建尺寸逻辑。 |
| TS 36.212 Table 5.1.3-3 | 使用 `table_0009.csv/html` 提取完整 Turbo 内部交织器参数表，包含 188 组 `i/K/f1/f2`，正文以 Markdown 表格和关键行说明承接。 | 本节只使用 $K$ 列完成分段尺寸计算；$f_1/f_2$ 的交织地址公式会在 T6.4 精读。 |
...
| 协议清单 | 检查 `3GPP_Rel19/manifest.csv`、`3GPP_Rel19/processed/manifest.json` 和 TS 36.212 3GPP_译码知识库入口。 |
| 本地协议检索 | 使用 `rg`、`sed` 检索 TS 36.212 `TS_36.212_36212-j30_content.md` 与 `sections.jsonl`，定位 §5.1.2、§5.1.3.2、§5.2.2.2、§5.3.2.2。 |
| 表格核验 | 查看 `tables/table_0009.csv` 和 `tables/table_0009.html`，确认 Turbo 合法 $K$ 集合和 $f_1/f_2$ 参数可由本地表格抽取；使用 `tools/figures/render_lte_turbo_interleaver_table.py` 生成 `docs/L1/assets/T3.3_TS36.212_Table_5.1.3-3.png` 并插入正文。 |
