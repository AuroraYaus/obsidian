...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1/T4.2_graphs_trellises_trees_for_decoding.md"
---
# T4.2 因子图、Tanner 图与网格图：译码约束的三种可视化语言
## 本节学习目标

译码器面对的不是一串孤立比特，而是一组有约束的比特。图的作用是把“哪些比特互相约束、消息沿哪里传递、状态怎样演化”画出来。对零基础学习者来说，图不是抽象装饰，而是后续理解 Turbo、低密度奇偶校验码和Polar译码器硬件结构的地图。

本节讲四种图语言：

- **因子图（factor graph）**：通用图语言，用变量节点和函数/约束节点表示概率或约束分解。
- **Tanner 图（Tanner graph）**：LDPC 常用图语言，用变量节点和校验节点表示奇偶校验矩阵。
- **网格图（trellis diagram）**：卷积码和 Turbo 译码常用图语言，用状态随时间变化表示码约束。
- **树形图（tree diagram）**：Polar SC/SCL 译码常用直觉，用递归分裂和路径保留表示逐位判决。
...
| 循环冗余校验 | 知道 CRC 通常用于译码结果验收，不是图上每条边的消息。 |
...
| 协议位置 | 本地证据 |
| :--- | :--- |
| LTE TS 36.212 Rel-19 `36212-j30` §5.1.3.2 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md` |
| NR TS 38.212 Rel-19 `38212-j30` §5.3.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md` |
| NR TS 38.212 Rel-19 `38212-j30` §5.3.1.2 | `TS_36.212_36212-j30_content.md`，`tables/table_0012.csv/html` |
| NR TS 38.212 Rel-19 `38212-j30` §6.2/§7.2/§7.3 | `TS_36.212_36212-j30_content.md` |
...
NR TS 38.212 §5.3.2 不直接给一个完整巨大 $\mathbf{H}$ 矩阵，而是给 base graph、lifting size 和循环置换规则。接收端可以把这些协议参数展开成实际 Tanner 图。T3.4 已经复现了 NR LDPC 的 lifting sets 和 base graph 非零位置；T4.2 只解释“为什么这些表最终会变成图上的边”。
...
这两条公式是算法教学公式，不是 3GPP 规范公式。协议给出 $\mathbf{H}$ 的构造，接收机厂商选择 BP、Min-Sum、Normalized Min-Sum、Offset Min-Sum 或 layered schedule。
...
Polar 树和 LDPC Tanner 图的思维方式不同。LDPC 是在同一个稀疏图上反复传消息；Polar SC/SCL 是按递归结构逐步判决。T3.5 已经解释 CRC-aided SCL：SCL 产生多条候选路径，CRC 帮助选择通过路径中 path metric 最优者。

## 三类图的对比

| 图语言 | 适配编码 | 节点含义 | 边含义 | 接收端典型算法 | 硬件关注点 |
|:---|:---|:---|:---|:---|:---|
| 网格图 | 卷积码、Turbo constituent code | 状态和时间 | 输入 bit 导致的状态转移 | BCJR/MAP、Max-Log-MAP、Viterbi | 前向/后向度量存储、交织访问。 |
...
| LDPC toy message | 在两校验图上跑一轮 Min-Sum | 消息方向、目标排除和符号与 T4.1 一致。 |
...
| 图语言 | 硬件模块 | 存储形态 | 关键瓶颈 |
|:---|:---|:---|:---|
| Tanner 图 | variable-node/check-node update units | 边消息静态随机存取存储器、layer schedule ROM | bank conflict、读写带宽、层间依赖。 |
...
寄存器传输级设计中，图结构会变成地址生成器和调度器。专用集成电路面积不只来自计算单元，更多时候来自消息 SRAM、跨 bank 连接和排序网络。
...
本节使用 TS 36.212 和 TS 38.212 作为编码结构背景，不复现协议原图 Figure 5.1.3-2，也不复现完整 NR LDPC base graph 表格，因为 T3.4 已经对 LDPC 表格做了表格化重建。正文 Mermaid 图均为教学图，用于解释接收端算法结构。
