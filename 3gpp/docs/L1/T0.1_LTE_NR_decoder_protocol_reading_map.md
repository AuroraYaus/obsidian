...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1/T0.1_LTE_NR_decoder_protocol_reading_map.md"
---
...
全项目重复使用的术语、缩写和简要解释集中在 [译码讲义术语总表](../L0/L0_terminology_glossary.md)。本节和后续讲义默认直接使用这些简称，正文只在需要讲解概念本身时补充上下文。
...
| TS 36.211/38.211 | `3GPP_Rel19/processed/TS_36.211_*`，`TS_38.211_38211-j30` | 调制和物理映射背景；本项目只保留影响 LLR 顺序、Qm 和控制信息承载的入口。 |
...
| MCS/TBS/RV/CBG 调度 | 本次传输发多少、用什么 Qm/R/RV、哪些 CB/CBG 被调度 | 生成 descriptor，决定 decoder 输入规模和 soft buffer 行为 | T9.0、T14.6、T15.2 |
...
EVI["证据闭环<br/>本地 TS_36.212_36212-j30_content.md、sections.jsonl<br/>tables/html/csv、equations/media<br/>audit logs"]
...
| 证据归档 | processed TS_36.211_36211-j30_s06-s08_content/tables/equations/media | evidence manifest | T12-T15 | 把模板或候选清单写成真实通过 |
...
| HARQ / soft buffer | 旧 LLR 是否保留？本次是 Chase 还是 incremental redundancy？soft buffer key 是否匹配？ | TS 36.213/36.321；TS 38.214/38.321 | T4.3、T7.3、T9.3、T11.3、T14.5 |
| MCS/TBS | Qm、R、PRB、RE、层数和 TBS 如何影响 decoder 输入规模？ | TS 38.214 §5.1.3、§6.1.4 | T9.0、T14.6、T15.2 |
| Descriptor | 哪些字段是协议派生，哪些是调度/MAC/RRC 上下文，哪些是实现策略？ | TS 36.212/38.212 + TS 36.213/38.214 + MAC/RRC | T4.6、T14.4、T14.6 |
| 验证证据 | 这是本地协议表复现、教学公式、仿真模板，还是真实工具输出？ | `processed/*/TS_36.212_36212-j30_content.md`、`tables/*.html/csv`、`equations/*.xml`、审计日志 | T12-T15、`docs/audits/*` |
...
| 项目 | 记录 |
|:---|:---|
| 新增文件 | `docs/L1/T0.1_LTE_NR_decoder_protocol_reading_map.md`。 |
| 新增证据资产 | `docs/L1/assets/T0.1_LTE_NR_decoder_protocol_reading_map.png`。 |
