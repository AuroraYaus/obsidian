...
1. **协议事实**：TS 38.211 §4.4.4.1 直接规定 resource block 是频域连续 $N_{sc}^{RB}=12$ 个子载波。LTE TS 36.211 §5.2.3 也采用 $N_{sc}^{RB}=12$，15 kHz SCS 下一个 RB 的频域跨度是 180 kHz。
2. **不是 OFDM 正交性的数学必然**：子载波正交性来自 T2.1 的 $\Delta f = 1/T_u$，不是来自"每 12 个子载波成一组"。理论上 8、10、16 个子载波也可以组成某种资源分配粒度，只要子载波间隔满足正交条件。
3. **工程上为什么 12 合理**：12 继承 LTE 的 RB 粒度；$12=2\times2\times3$，便于在一个 PRB 内设计 2、3、4、6 等梳状或分组参考信号图样；同时它在调度灵活性和 DCI/RBG 开销之间取得折中。更小的粒度会增加调度指示开销，更大的粒度会降低频域调度精度。
...
![图 3：不同 SCS 的 PRB 频域嵌套 — 同一 720 kHz 频段内，4 个 15 kHz PRB = 2 个 30 kHz PRB = 1 个 60 kHz PRB](assets/T2.3_prb_nesting.svg)
