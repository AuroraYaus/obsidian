...
| 带宽部分 | Bandwidth Part (BWP) | 给定载波、给定 Numerology 下连续 CRB 的子集；UE 的常规收发以当前激活的 DL/UL BWP 为工作窗口。 |
...
2. **"只在激活 BWP 上工作"分下行和上行说**：下行侧，UE 不期望在 active BWP 外接收 PDSCH、PDCCH 或 CSI-RS（RRM 测量例外）；上行侧，UE 不应在 active BWP 外发送 PUSCH、PUCCH，活动小区内由 SRS-Resource 配置的 SRS 也不应发到 active BWP 外。
...
| 项目 | 记录 |
|:---|:---|
| 本地协议检索 | 使用 `rg` 检索 TS 38.211 的 §4.4.1–§4.4.5 定义入口，并核对 §4.4.5 中 BWP 的连续 CRB 定义、active DL/UL BWP 限制和 RRM 例外。 |
