...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1/T2.3_NR_frequency_resource_grid.md"
---
# T2.3 NR 频域资源网格：RE、PRB、CRB、Point A 与 BWP
## 本节学习目标

T2.2 建立了 NR 的时域层级——从 μ 出发推导无线帧、子帧、时隙和 OFDM 符号。现在补上频域维度，构成完整的二维资源网格。NR 频域资源不是简单的"第几个子载波"，而是一条五级概念链。

先区分本节的核心对象：

| 术语 | 英文 | 先这样理解 |
|:---|:---|:---|
| 资源粒子 | Resource Element (RE) | 时频二维的最小资源粒度——频域 1 个子载波 × 时域 1 个 OFDM 符号。 |
| 物理资源块 | Physical Resource Block (PRB) | 频域连续 12 个子载波，NR 中与时隙解耦的纯频域概念。 |
| 公共资源块 | Common Resource Block (CRB) | 从 Point A 开始对整个载波带宽的绝对 PRB 编号。 |
| 带宽部分 | Bandwidth Part (BWP) | 一个载波内连续 CRB 的子集，UE 只在激活 BWP 上工作。 |

- 解释 RE 的二维地址 $(k, l)_{\mu}$ 中每个符号的含义。
- 说明 PRB 为什么定义为 12 子载波且与 SCS 无关——这简化了跨 Numerology 的参考信号设计。
- 理解 Point A 作为整个载波频域公共参考原点的角色。
- 区分 CRB（绝对编号）和 PRB（BWP 内局部编号）——$n_{PRB}^\mu = n_{CRB}^\mu - N_{BWP,i}^{start,\mu}$。
- 解释为什么不同 BWP 的 UE 在相同物理频率位置生成相同的 DM-RS 序列。
...
| 问题 | 需要达到的程度 |
|:---|:---|
| 子载波间隔 Δf 的含义是什么？ | 能说出 $\Delta f = 15 \cdot 2^\mu$ kHz，不同 μ 对应不同 SCS（T2.2）。 |
| OFDM 符号是什么？ | 时域上一个 CP + N 点 IFFT 输出的时间片段（T2.1）。 |
| 时隙的结构是什么？ | 普通 CP 下 14 个 OFDM 符号（T2.2）。 |
...
其中 $k$ 是频域子载波索引，$l$ 是时域 OFDM 符号索引，下标 $\mu$ 表示该 RE 属于哪个 Numerology 的网格。一个 RE 承载一个复数调制符号（modulation symbol）——在 QPSK 下为 2 个比特，在 64QAM 下为 6 个比特。
...
为什么是 12？关键原因是简化参考信号设计。DM-RS、CSI-RS 等参考信号在一个 PRB 内的时频图样（pattern）对所有 Numerology 通用——设计一套图样，适用于所有 SCS。如果 PRB 的子载波数随 μ 变化，参考信号图样需要为每个 μ 单独设计，协议复杂度会急剧膨胀。
...
![图 3：不同 SCS 的 PRB 频域嵌套 — 1 个 15 kHz PRB (180 kHz) = 2 个 30 kHz PRB = 4 个 60 kHz PRB](assets/T2.3_prb_nesting.svg)

## Point A：频域公共参考原点

Point A 是整个载波的频域公共参考原点，位于系统带宽的最低可用频率边界处。它是 NR 频域资源编排的"零点"——所有绝对位置都从这里开始数。

Point A 由 RRC 高层信令配置：
- SIB1 中的 `offsetToPointA` 字段——指示 Point A 与 SSB（同步信号块）最低子载波之间的频域偏移
...
BWP 是 CRB 范围内的连续子集。BWP 内的 PRB 使用局部编号：
...
![图 2：Point A → CRB → BWP → PRB 频域偏移关系 — CRB 为绝对编号，BWP 内的 PRB 为局部编号](assets/T2.3_pointA_chain.svg)
...
交织映射的工程动机是频率分集——将连续的逻辑资源分散到不连续的物理频率位置，对抗频率选择性衰落。对译码器而言，这意味着 LLR 序列中的相邻比特可能来自频谱上相距甚远的 RE——它们的信道质量可能差异很大。译码器不直接处理 VRB/PRB 映射，但接收端的 LLR 顺序取决于发送端的 VRB→PRB 映射方式。
...
一个 CORESET 被划分为多个 CCE（Control Channel Element，控制信道元素），每个 CCE = 6 个 REG（Resource Element Group）= 6 × 12 = 72 个 RE（未扣除 DM-RS）。CCE 的编号顺序由交织器决定，而非简单的频率递增。
...
| TS 38.331 Rel-19 | `3GPP_Rel19/processed/TS_38.331_38331-j30/` | BWP RRC 配置入口已定位；ASN.1 不在本节展开。 |
...
2. NR 为什么把 PRB 固定为 12 子载波且不与 SCS 耦合？
...
2. 12 子载波作为跨 Numerology 常数，主要动机是简化参考信号设计——DM-RS/CSI-RS 在一个 PRB 内的时频图样对所有 SCS 通用，无需为每个 μ 单独设计。
...
| PRB 定义 | 能说明 PRB = 12 子载波、NR 中与时隙解耦，解释 12 的设计理由。 |
| Point A | 能解释其角色和 RRC 配置方式。 |
| CRB vs PRB | 能写出 $n_{PRB} = n_{CRB} - N_{BWP}^{start}$ 并解释场景。 |
| BWP | 能说明每 UE 最多 4 BWP、1 个激活、不同 BWP 可用不同 Numerology。 |
...
| T2.2 NR Numerology | Δf 取值和时隙结构见 T2.2。 |
| T2.4 LTE 帧结构 | NR 与 LTE PRB 定义对比见 T2.4。 |

## 执行与证据记录

| 项目 | 记录 |
|:---|:---|
| 本地协议检索 | 使用 `rg` 检索 TS 38.211 的 §4.4.1–§4.4.5 定义入口。 |
...
| 使用技能/流程 | 使用 `3gpp-expert` 技能确认协议证据边界；使用 `verification-before-completion` 规则验证。 |
...
- [3GPP, 2026] 3GPP TS 38.331, Rel-19, `38331-j30`, Radio Resource Control (RRC) protocol specification. Local path: `3GPP_Rel19/processed/TS_38.331_38331-j30`.
