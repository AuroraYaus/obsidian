...
type: spec
aliases:
  - T17.4 NR Polar float sim plan
tags:
  - 3gpp
  - docs
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md"
---
# T17.4 NR Polar 浮点仿真计划：从可靠性序列、速率恢复到 CA-SCL 列表扫描
## 本节学习目标

本节定义NRPolar的浮点仿真计划。Polar（极化码，Polar Code）在本项目中主要服务于控制信息译码，例如上行控制信息和下行控制信息。与 NR 低密度奇偶校验码数据链路不同，Polar 仿真更关心短块、可靠性序列、冻结位、速率恢复、逐次消除（Successive Cancellation, SC）、逐次消除列表（Successive Cancellation List, SCL）、循环冗余校验辅助 SCL（CRC-aided Successive Cancellation List, CA-SCL）、加性白高斯噪声信道和控制信息误检风险。


- 按3GPP TS 38.212 的 Polar code block segmentation、reliability sequence、Polar encoding、rate matching、UCI（上行控制信息，Uplink Control Information）/DCI（下行控制信息，Downlink Control Information）调用链，定义一套可审计的浮点仿真配置。
- 解释 $A$、$K$、$E$、$N$、$L$、CRC（循环冗余校验，Cyclic Redundancy Check） type、无线网络临时标识（Radio Network Temporary Identifier, RNTI）和 rate recovery 状态为什么必须写入 descriptor。
- 说明可靠性序列如何决定 information set/frozen set，速率恢复如何把空口对数似然比放回 Polar codeword order。
- 比较 SC、SCL 和 CA-SCL 的仿真口径，设计 list-size sweep，并输出块错误率、BER（比特错误率，Bit Error Rate）、误检、CRC 通过率和延迟代理指标。
- 规划随机种子、输出目录、`metrics.csv`、`frames.jsonl`、失败 replay、图形结果、定点化承接和寄存器传输级/专用集成电路验证证据。

本节的核心不是“写一个能跑 CA-SCL 的脚本”这么简单，而是把协议构造、接收端算法、随机样本、list size、CRC/RNTI 选择和失败样本都绑定到同一份可复现证据。否则 list size 变大后 BLER（块错误率，Block Error Rate）改善，到底来自 CA-SCL 本身、CRC 选择、rate recovery 地址、可靠性序列，还是随机样本不同，就无法判断。

## 前置知识检查

| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T10.3_NR_Polar_reliability_sequence|T10.3]] NR Polar 可靠性序列 | 知道 TS 38.212 Table 5.3.1.2-1 给出可靠性升序序列，选 information set 时从当前 $N$ 子序列末尾取 $K$ 个更可靠位置。 |
| [[T10.6_CRC_aided_SCL_control_reliability|T10.6]] CRC 辅助 SCL | 知道 SCL 产生多条候选路径，CA-SCL 在 CRC/RNTI 通过的候选中选路径度量（Path Metric, PM）最优者。 |
| [[T10.7_NR_Polar_rate_recovery|T10.7]] NR Polar 速率恢复 | 知道 sub-block deinterleaving、bit selection reverse、puncturing、shortening、repetition 和 coded-bit deinterleaving 的接收侧语义。 |
...
| LLR（对数似然比，Log-Likelihood Ratio）、BER/BLER 基础 | 知道本项目默认 `POS_ZERO`，即正 LLR 倾向 bit 0；知道 BER/BLER 是工程统计指标，不是 TS 38.212 直接给出的 decoder 合格线。 |
...
| Prompt | 请定义 NR Polar 浮点仿真计划：可靠性序列、速率恢复、SC/SCL/CA-SCL、列表大小扫描、CRC 检查、延迟代理指标、随机种子、输出和阈值。写作时参考本文“单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
本节是工程仿真计划，不把 TS 38.212 的 Polar sequence 1024 项表和 sub-block interleaver pattern 重新排满全文。上游 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 已完整表格化复现 TS 38.212 Table 5.3.1.2-1，[[T10.7_NR_Polar_rate_recovery|T10.7]] 已复现 Table 5.4.1.1-1，并说明 puncturing/shortening/repetition 的接收端 LLR 语义。本节把这些协议资产接入一次可运行、可审计、可回放的 CA-SCL 浮点实验。
...
T17.4 NR Polar Floating-Point Simulation Plan 覆盖 reliability sequence、rate recovery、SC/SCL/CA-SCL、list-size sweep、latency proxy 和 replay evidence。正文中的仿真链路如下：

![图 1：NR Polar 浮点仿真链路——Protocol Config 固定协议证据与参数，经 UCI/DCI Builder、Polar Constructor、Polar Encoder、Rate Match 生成发送侧参考，AWGN + LLR 生成接收端软信息，Decoder Sweep 比较 SC/SCL/CA-SCL 与列表长度，CRC/RNTI + Metrics 输出选路与 BLER，失败回放归档保存复现证据](assets/T17.4_NR_Polar_float_sim_flow.svg)
...
| AWGN（加性白高斯噪声，Additive White Gaussian Noise） + LLR | 扫描 Eb/N0，记录 effective rate、demapper LLR、`POS_ZERO` 符号约定和 archived seeds。 |
| Decoder Sweep | 比较 SC、SCL and CA-SCL，并在 $L=1/2/4/8/16$ 上统计 PM 和 node counts。 |
| CRC/RNTI + Metrics | 对候选执行 candidate CRC、RNTI check，记录 selected path、BLER、FER（帧错误率，Frame Error Rate）、avg list ops 和 latency proxy。 |
...
| edge | SCL | puncture/short/repeat，即 puncturing、shortening、repetition 三类 rate recovery 边界 | fixed SNR（信噪比，Signal-to-Noise Ratio） | full decoder dump |
...
| NR 下行共享信道/上行共享信道LDPC（低密度奇偶校验码，Low-Density Parity-Check Code） 数据链路 | 不属于本节，由 [[T17.3_NR_LDPC_float_sim_plan|T17.3]] 处理；本节只在对比时说明 LDPC 与 Polar rate recovery 不可混用。 |
| 长期演进Turbo（Turbo 码，Turbo Code）数据链路 | 不属于本节，由 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 处理；本节不使用 LTE Turbo interleaver 或 circular buffer 规则。 |
...
T17.4 的核心实验是 list-size sweep。必须让 $L=1,2,4,8,16$ 等配置使用同一批 payload seed、noise seed、协议 descriptor 和信噪比（Signal-to-Noise Ratio, SNR）点，只有 decoder list size 和与之直接相关的资源统计不同。
...
| `rate_recovery_edge` | 验证 puncturing、shortening、repetition 和 interleaving。 | 人工构造边界 $E<N$、$E>N$、coded-bit interleaving on/off。 | LLR 初始化和地址 trace 与 [[T10.7_NR_Polar_rate_recovery|T10.7]] toy 规则一致。 |
...
继承 [[T17.1_python_golden_model_project_layout|T17.1]] 的 `canonical_json_sha256_u64le_v1` seed 派生规则。payload、CRC/RNTI perturbation、noise、rate recovery edge injection 和 tie-breaker 必须使用不同 stage seed。
...
若某个 DCI case 要求误通过风险，报告不能只写 `false_pass_rate=0`。当观测到 0 次 false pass 时，仍要写样本数和上界，例如 `false_pass_count=0/50000, false_pass_upper_bound_95pct=6.0e-5`。具体上界算法由 [[T17.5_BER_BLER_curve_reporting|T17.5]] 统一定义；T17.4 只要求字段和验收口径必须存在。
...
| 可靠性序列 | `Q_sequence_hash/info_set_hash/frozen_set_hash` 与 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 资产一致。 |
...
| RTL/ASIC 对象 | T17.4 输出来源 |
|:---|:---|
| Descriptor 寄存器 | `A/K/E/N/L`、`crc_type`、`rnti_context`、`decoder_mode`。 |
| Reliability ROM（只读存储器，Read-Only Memory） | `Q_sequence_hash`、`info_set_hash`、`frozen_set_hash`、`pc_set_hash`。 |
...
| 错误 | 现象 | 定位方法 |
|:---|:---|:---|
| 可靠性序列方向反 | 高 SNR 仍大量 fail。 | 对比 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 的 $Q_N$ 和 `info_set_hash`。 |
...
| :--- | :--- | :--- | :--- |
| CRC calculation | TS 38.212 `38212-j30` §5.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:637-657` | 已核验锚点；CRC 多项式由 [[T3.1_LTE_NR_CRC_families|T3.1]] 复现。 |
| Polar segmentation and CRC attachment | TS 38.212 §5.2.1 | `content.md:665-715` | 已核验锚点；公式 XML 不在本节重转写。 |
| Polar reliability sequence and encoding | TS 38.212 §5.3.1.2、Table 5.3.1.2-1 | `content.md:873-883`；`tables/table_0012.csv/html` | 上游 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 已复现完整表图。 |
| Polar rate matching | TS 38.212 §5.4.1、§5.4.1.1-§5.4.1.3、Table 5.4.1.1-1 | `content.md:1019-1125`；`tables/table_0019.csv/html` | 上游 [[T10.7_NR_Polar_rate_recovery|T10.7]] 已复现 pattern 表。 |
...
| 候选资产 | 本节处理 | 关闭条件 |
|:---|:---|:---|
| TS 38.212 Table 5.3.1.2-1 | 上游 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 已完整表格化复现，本节只消费 `Q_sequence_hash`。 | 若 T17.4 实现直接解析原表，必须把 [[T10.3_NR_Polar_reliability_sequence|T10.3]] 的 CSV/HTML、脚本和表格资产纳入 evidence archive，并验证 rank 覆盖 0..1023。 |
| TS 38.212 Table 5.4.1.1-1 | 上游 [[T10.7_NR_Polar_rate_recovery|T10.7]] 已复现 32 项 pattern，本节只消费 pattern hash。 | 若实现内置 pattern ROM，必须跑 sub-block interleaver round-trip 单测并记录 ROM hash。 |
| UCI payload field tables | 本节只规划 Polar decoder 仿真，不复现所有 CSI（信道状态信息，Channel State Information）/混合自动重传请求ACK field size 表。 | 若后续从高层 UCI 配置自动生成 $A$，必须复现实际使用的 TS 38.212/38.213/38.214 表格子集。 |
...
| 写入文件 | `docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T17.4。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 阶段 11 与 T17.4 条目。 |
| 前置依据 | 已读取 `docs/L2_协议算法/T10.3_NR_Polar_reliability_sequence.md`、`docs/L2_协议算法/T10.6_CRC_aided_SCL_control_reliability.md`、`docs/L2_协议算法/T10.7_NR_Polar_rate_recovery.md`、`docs/L3_工程实现/T17.1_python_golden_model_project_layout.md`。 |
...
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md`：`LATEX_RENDER_AUDIT_OK formulas=94`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选来自上游已重建协议表、PDCCH blind detection/UCI payload field 边界和正文工程公式引用，已在“未复现协议资产与关闭条件”中逐项说明。 |
| 只读复核 | 审核经理 Popper 首轮 `Critical=0`、`Important=2`、`Minor=2`；已修复阈值可执行例子、缩写首现、SC/SCL/CA-SCL 二维命令矩阵，标题保持 `## 本节学习目标` 以满足当前深度审计脚本；横向复核确认 T17.4 Prompt 覆盖充分，剩余问题为台账/矩阵同步而非正文覆盖缺口。 |
...
- [上游讲义] `docs/L2_协议算法/T10.3_NR_Polar_reliability_sequence.md`，NR Polar 可靠性序列。
