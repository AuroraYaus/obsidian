...
| NR 下行共享信道/上行共享信道LDPC 数据链路 | 不属于本节，由 [[T17.3_NR_LDPC_float_sim_plan|T17.3]] 处理；本节只在对比时说明 LDPC 与 Polar rate recovery 不可混用。 |
| 长期演进Turbo数据链路 | 不属于本节，由 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 处理；本节不使用 LTE Turbo interleaver 或 circular buffer 规则。 |
...
## 图示
![图 1：NR Polar float sim flow](assets/T17.4_NR_Polar_float_sim_flow.svg)
...
| 新增证据资产 | `docs/L3_工程实现/assets/T17.4_NR_Polar_float_sim_flow.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t12_4_nr_polar_float_sim_flow.py`）。 |
