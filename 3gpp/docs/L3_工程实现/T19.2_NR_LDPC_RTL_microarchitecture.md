...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T5.2_memory_banking_buffering_basics|T5.2]] 存储分 bank 与缓存基础 | 知道静态随机存取存储器、寄存器文件、多 bank、bank conflict、stall、bypass 和 crossbar 的含义。 |
| [[T5.4_RTL_state_machine_handshake_basics|T5.4]] RTL 状态机与握手 | 知道 `valid/ready`、start/done/error、同步复位、pipeline stall 和状态转移条件。 |
| [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] NR LDPC 提升大小与 QC 矩阵构造 | 知道 BG、$Z_c$、$i_{\mathrm{LS}}$、shift value 和 $P^p$ 的含义。 |
| [[T8.6_LDPC_MS_NMS_OMS|T8.6]] LDPC MS/NMS/OMS | 知道 CN 输出符号、min1/min2、归一化 Min-Sum（Normalized Min-Sum, NMS）和偏移 Min-Sum（Offset Min-Sum, OMS）。 |
| [[T8.7_layered_LDPC_decoding_schedule|T8.7]] Layered LDPC 调度 | 知道 layered read-modify-write 公式和 syndrome/CRC early-stop 边界。 |
| [[T9.1_NR_LDPC_rate_recovery_overview|T9.1]]-[[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]] NR LDPC rate recovery 与 HARQ | 知道 rate recovery 后的 LLR 如何进入 LDPC core，RV、$k_0$、$N_{\mathrm{cb}}$ 和 CBG mask 是 descriptor 上下文。 |
| [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] NR LDPC 定点模型计划 | 知道整数位宽、CN message、posterior LLR、layer trace、saturation 和 C/C++ fixed checkpoint。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] Bit-Exact 回归框架 | 知道 RTL 输出必须和 Python/C++ fixed trace 在 metadata/hash/checkpoint 上逐项比较。 |

若尚未掌握“CN、VN 和 syndrome 是什么”，先回看 [[T8.4_LDPC_Tanner_graph_message_passing|T8.4]]-[[T8.8_NR_LDPC_decoder_numeric_walkthrough|T8.8]]。本节默认你已经理解 LDPC 是在 Tanner graph 上交换软信息；这里要解决的是：这张由协议表定义的大图，如何变成 ASIC 中每拍可读写的 banked memory、地址生成器、比较器树和 stop controller。
...
| Table 5.3.2-1 | `tables/table_0013.csv/html` | 本节引用表语义，完整表已由 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]/[[T3.4_NR_LDPC_segmentation_rules|T3.4]] 表格化复现。 |
...
| [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] | `docs/L2_协议算法/T8.3_NR_LDPC_lifting_QC_matrix.md` | T19.2 把它映射到 RTL 地址和 bank。 |
| [[T8.7_layered_LDPC_decoding_schedule|T8.7]] | `docs/L2_协议算法/T8.7_layered_LDPC_decoding_schedule.md` | T19.2 扩展为微架构和验证字段。 |
| [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] | `docs/L3_工程实现/T18.3_NR_LDPC_fixed_point_model_plan.md` | T19.2 把对象变成 RTL 端口、SRAM、counter 和 trace。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] | `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md` | 本节只定义 RTL 侧怎么输出这些 checkpoint。 |
...
| `trace_status` | checkpoint signals | trace/status/error counters | 输出 bit-exact trace、first mismatch 对齐字段和 counters。 | [[T18.6_bit_exact_regression_harness|T18.6]] schema 约束可回放性。 |
...
RTL 位宽必须继承 [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] 的 fixed requirement。最低字段：
...
| Trace/status | first mismatch、counter、failure bundle | 与 [[T18.6_bit_exact_regression_harness|T18.6]] `checkpoint_manifest` 对齐。 |
...
| 测试 | 输入 | 检查点 |
|:---|:---|:---|
| QC address smoke | 小 $Z_c$、BG1/BG2 各一个 row group | `var_addr` 与 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] toy/golden matrix 一致。 |
...
| Bit-exact replay | [[T18.6_bit_exact_regression_harness|T18.6]] vector + C++ fixed trace | `quantized_llr/cn_update/layer_trace/final` 全一致。 |
...
示例数值是结构示例，真实向量必须从上游协议向量和 [[T18.6_bit_exact_regression_harness|T18.6]] manifest 生成，并记录表格 hash、rate recovery hash、bitwidth profile hash 和 RTL commit hash。
...
- `docs/L3_工程实现/assets/T19.2_NR_LDPC_RTL_microarchitecture.svg`（手绘 SVG，2026-08-04 替代原 PIL 渲染 PNG；原 `tools/archive_python_drawing/figures/render_t14_2_nr_ldpc_rtl_microarchitecture.py` 与 `T14.2_NR_LDPC_RTL_microarchitecture.png` 已删除，图示语义与数据不变）
...
引用重建审计输出为候选清单：候选主要来自 TS 38.212 Table 5.3.2-1/2/3 的上游已复现回链、§5.3.2 协议公式转写、本文工程估算公式和正文说明。本节未新增复制协议长表，完整表由 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]/[[T3.4_NR_LDPC_segmentation_rules|T3.4]] 的表格化复现资产承接；若后续 RTL 生成脚本直接解析 Table 5.3.2-2/3，应把 `tables/table_0014.csv/html` 和 `tables/table_0015.csv/html` 的 hash、解析脚本、使用行列和 schedule hash 纳入 T20 evidence archive。
