...
本节设计NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）译码器的寄存器传输级微架构。先把本篇会反复使用的术语列清：基图（Base Graph, BG）是协议给出的稀疏连接模板；提升大小（lifting size）$Z_c$ 把模板中的一个连接扩展成 $Z_c$ 条规律连接；准循环 LDPC（Quasi-Cyclic LDPC, QC-LDPC）表示矩阵由循环移位单位矩阵拼成；校验节点（Check Node, CN）对应奇偶校验方程；变量节点（Variable Node, VN）对应编码比特；对数似然比是接收端软信息；循环冗余校验用于码块或传输块完整性检查；码块是 LDPC core 一次译码的对象；传输块是多个 CB（码块，Code Block）重组后的交付对象；CBG（码块组，Code Block Group）是混合自动重传请求部分重传可操作的 CB 集合；块错误率是性能验收指标；专用集成电路实现要关注面积、功耗、吞吐、时序和验证证据。


- 从3GPP TS 38.212 §5.3.2 的 BG、$Z_c$、lifting set 和 shift table 推出 RTL（寄存器传输级，Register Transfer Level）的 edge schedule、地址生成器和 syndrome checker 输入。
- 画出 layered NR LDPC decoder 的主数据通路：descriptor、QC table ROM（只读存储器，Read-Only Memory）、layered controller、CN min1/min2 unit、VN read-modify-write、LLR（对数似然比，Log-Likelihood Ratio）/message RAM bank、syndrome/CRC（循环冗余校验，Cyclic Redundancy Check）gate 和 trace/status 输出。
...
- 区分 syndrome early stop、CB CRC 和 TB（传输块，Transport Block） CRC 的边界，避免把局部 layer 检查误写成完整 CB 通过。
- 规划 bit-exact（比特精确，bit-exact） 验证：`quantized_llr`、`cn_update`、`layer_trace`、`posterior_snapshot`、`syndrome_crc` 等检查点必须能和 C/C++ fixed model 对齐。
...
| [[T8.6_LDPC_MS_NMS_OMS|T8.6]] LDPC MS（最小和，Min-Sum）/NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum） | 知道 CN 输出符号、min1/min2、归一化 Min-Sum（Normalized Min-Sum, NMS）和偏移 Min-Sum（Offset Min-Sum, OMS）。 |
| [[T8.7_layered_LDPC_decoding_schedule|T8.7]] Layered LDPC 调度 | 知道 layered read-modify-write 公式和 syndrome/CRC early-stop 边界。 |
| [[T9.1_NR_LDPC_rate_recovery_overview|T9.1]]-[[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]] NR LDPC rate recovery 与 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） | 知道 rate recovery 后的 LLR 如何进入 LDPC core，RV（冗余版本，Redundancy Version）、$k_0$、$N_{\mathrm{cb}}$ 和 CBG mask 是 descriptor 上下文。 |
...
若尚未掌握“CN、VN 和 syndrome 是什么”，先回看 [[T8.4_LDPC_Tanner_graph_message_passing|T8.4]]-[[T8.8_NR_LDPC_decoder_numeric_walkthrough|T8.8]]。本节默认你已经理解 LDPC 是在 Tanner graph 上交换软信息；这里要解决的是：这张由协议表定义的大图，如何变成 ASIC（专用集成电路，Application-Specific Integrated Circuit）中每拍可读写的 banked memory、地址生成器、比较器树和 stop controller。
...
| [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] | `docs/L3_工程实现/T18.3_NR_LDPC_fixed_point_model_plan.md` | T19.2 把对象变成 RTL 端口、SRAM（静态随机存取存储器，Static Random-Access Memory）、counter 和 trace。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] | `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md` | 本节只定义 RTL 侧怎么输出这些 checkpoint。 |

协议前因后果是：NR 使用 LDPC 处理数据传输的大码块。TS 38.212 不把完整奇偶校验矩阵 $H$ 一项项写出，而是用 BG 表、lifting size set 和 shift value 生成结构化稀疏矩阵。这样做的工程后果非常直接：硬件不需要保存完整 $H$，只需要保存 row group、column group、shift 和 local index 规则；但硬件必须保证生成出的连接和协议表完全一致，否则 syndrome、CRC 和 BLER（块错误率，Block Error Rate）都会出错。
...
![图 1：NR LDPC RTL 微架构——Protocol Descriptor 锁存任务身份，QC Table ROM 提供 H 矩阵边调度，Layered Controller 逐拍控制，CN Unit 计算 sign/min1/min2，VN RMW 执行 layered 更新，Banked Memory 承担并行访问，Stop & Trace 在完整 H 检查点收口](assets/T19.2_NR_LDPC_RTL_microarchitecture.svg)
