...
| [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] NR LDPC 定点模型 | 知道 LDPC layered schedule、edge message、posterior LLR、min1/min2 和 syndrome check。 |
| [[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] NR Polar 定点模型 | 知道 Polar node LLR、partial sum、path state、path metric、list pruning 和 CRC/RNTI selector。 |
...
## 图示
![图 1：SIMD memory layout decoders](assets/T18.5_SIMD_memory_layout_decoders.svg)
...
| 新增证据资产 | `docs/L3_工程实现/assets/T18.5_SIMD_memory_layout_decoders.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t13_5_simd_memory_layout_decoders.py`）。 |
