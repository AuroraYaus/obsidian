...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md"
---
# T18.5 C/C++ 译码器 SIMD 与内存布局：从协议顺序到缓存友好数组
## 本节学习目标

本节讲LTE Turbo（Turbo 码，Turbo Code）、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）和 NR Polar（极化码，Polar Code）译码器的 C/C++ 内存布局。这里的主角不是新的译码算法，而是单指令多数据（Single Instruction, Multiple Data, SIMD）、缓存局部性、数组对齐、访存步长和 profiling。前面 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]、[[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]]、[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 已经把对数似然比、外信息、LDPC message、Polar path metric 等对象定成整数模型；本节继续回答一个工程问题：这些整数应该按什么顺序放在内存里，中央处理器（Central Processing Unit, CPU）或后续寄存器传输级/专用集成电路才能连续读取、并行计算、稳定回放。
...
- 说明 `alignas(64)`、64-byte cache line、SIMD load/store 宽度、tail mask、padding、stride 和 false sharing 对吞吐、延迟和 bit-exact（比特精确，bit-exact） replay 的影响。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T4.6_decoder_interface_contracts|T4.6]] 译码器接口契约 | 知道 descriptor、input、output、status、error、debug/trace 分层。 |
| [[T5.2_memory_banking_buffering_basics|T5.2]] 内存分 bank 与 buffering | 知道连续访问、bank conflict、ping-pong buffer 和静态随机存取存储器映射的基本含义。 |
| [[T5.3_throughput_latency_parallelism|T5.3]] 吞吐、时延与并行度 | 知道吞吐瓶颈可能来自计算、访存、分支或同步，而不只是算法迭代次数。 |
| [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] LTE Turbo 定点模型 | 知道 Turbo 三路码流、交织器地址、alpha/beta、extrinsic 和 max-log 检查点。 |
| [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] NR LDPC 定点模型 | 知道 LDPC layered schedule、edge message、posterior LLR（对数似然比，Log-Likelihood Ratio）、min1/min2 和 syndrome check。 |
| [[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] NR Polar 定点模型 | 知道 Polar node LLR、partial sum、path state、path metric、list pruning 和 CRC（循环冗余校验，Cyclic Redundancy Check）/RNTI（无线网络临时标识，Radio Network Temporary Identifier）selector。 |

如果你还没有形成“数组就是接口的一部分”的概念，本节会有点反直觉。C/C++ 定点模型不是只要算出同一个结果就行；它还要为后续 SIMD、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、失败 replay 和性能归因留下可测量的结构。
...
| 资料 | 本地路径 | 边界 |
| :--- | :--- | :--- |
| [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]]/[[T7.5_LTE_DL_UL_decoding_differences|T7.5]]/[[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]] LTE 与混合自动重传请求/Turbo 边界 | `docs/L2_协议算法/T7.*.md` | 不在本节重新精读 TS 36.213/TS 36.321 时序。 |
...
| T12 浮点仿真 | `docs/L3_工程实现/T17.*.md` | 本节不生成真实性能曲线。 |
| [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 定点模型 | `docs/L3_工程实现/T18.2_*`, `T18.3_*`, `T18.4_*` | 本节只重排内存，不改变数值语义。 |
...
![图 1：SIMD 与缓存——64 byte cache line 装 32 个 int16 LLR；256-bit SIMD 16 个 lane 中后 4 个为 padding；好布局（连续 load、lane 全有效、hot/cold 分离）与坏布局（gather、padding、false sharing）对比](assets/T18.5_SIMD_memory_layout_decoders.svg)
...
具体例子：设某个 Turbo CB（码块，Code Block）的 `sys[]` 输入有 $K=34$ 个 `int16_t` LLR，目标平台使用 256-bit SIMD。由式 (3) 得到每个 vector 有 16 个 lane，由式 (4) 得到
...
| Bank conflict | LDPC two edges hit the same SRAM（静态随机存取存储器，Static Random-Access Memory） bank in one cycle; pipeline stalls. |
...
LTE Turbo 的输入来自 rate recovery 后三类软信息：systematic、parity 0、parity 1。协议和 T7/[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 固定的是码块长度、内部交织器、tail bit、冗余版本和 `<NULL>`/puncturing 语义；实现布局要决定这些流怎样进入 Turbo core。
...
| 方向 | 方法 | 适用场景 | 风险 |
|:---|:---|:---|:---|
| 多 CB 并行 | SIMD lane 0..15 分别处理不同码块同一 trellis 位置。 | 小码块很多、HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）batch 多。 | 不同 `K` 需要 bucket；tail mask 复杂。 |
...
当 $Z_c$ 大于 lane 数时，按 `z_block` 分块；当 $Z_c$ 小于 lane 数时，可以合并多个 edge 或多个 code block，但这会增加调度复杂度。[[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] 中的 min1/min2、argmin、sign product 都适合 SoA：每个数组连续保存同一类量，便于 vector compare、min、blend 和 sign XOR。
...
Polar SCL（连续消除列表译码，Successive Cancellation List）的内存问题与 Turbo/LDPC 不同。它的瓶颈不一定是单个 $f/g$ 计算，而是 path split/prune 后的状态复制。若 list size 为 $L$，每条 path 都保存 node LLR、partial sum、path metric、CRC state 和 hard bits；每个 information bit 可能产生 $2L$ 个候选，再剪回 $L$ 条。
...
这些是计划级命令模板，不表示本节已经在仓库中实现了真实 `decoder_fixed` 二进制。真实命令将在 [[T18.6_bit_exact_regression_harness|T18.6]] bit-exact 回归框架中落地。
...
| lazy copy | Polar CA-SCL（CRC 辅助 SCL，CRC-aided SCL）实现策略。 | path remap trace 与 selected path 一致。 |
...
| 本节产出 | `docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md`。 |
| 协议边界 | 本节不直接精读新的 TS 条文；使用上游 T7/T8/T9/T10/T12/[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 的协议向量、descriptor 和 evidence hash。 |
...
| 文档审计 | `python3 tools/audit_lesson_terms.py docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md` -> `LESSON_TERM_AUDIT_OK`；`python3 tools/audit_markdown_headings.py docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md` -> `MARKDOWN_HEADING_AUDIT_OK`；`python3 tools/audit_lesson_depth.py --strict docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md` -> `LESSON_DEPTH_AUDIT_OK`；`python3 tools/audit_latex_render.py docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md` -> `LATEX_RENDER_AUDIT_OK formulas=36`；`python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md` -> `REFERENCE_REBUILD_AUDIT_NO_CANDIDATES`。 |
...
3. `docs/L3_工程实现/T18.2_LTE_Turbo_fixed_point_model_plan.md`，LTE Turbo 定点模型计划。
4. `docs/L3_工程实现/T18.3_NR_LDPC_fixed_point_model_plan.md`，NR LDPC 定点模型计划。
5. `docs/L3_工程实现/T18.4_NR_Polar_fixed_point_model_plan.md`，NR Polar 定点模型计划。
