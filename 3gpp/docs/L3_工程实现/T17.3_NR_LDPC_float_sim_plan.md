...
本节定义NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）的浮点仿真计划。浮点仿真的目标是先把协议参数、软信息、消息传递算法、循环冗余校验和块错误率统计做成可复现的黄金参考，再交给后续定点 C/C++、寄存器传输级和专用集成电路阶段。
...
- 输出 BLER（块错误率，Block Error Rate）/BER（比特错误率，Bit Error Rate）曲线、迭代次数、syndrome、CRC（循环冗余校验，Cyclic Redundancy Check）、失败 dump、可重跑命令和协议证据。

本节的重点不是实现最快的 decoder，而是让每个 BLER 点、每个失败 CB（码块，Code Block）、每个算法变体都能回到同一份协议 descriptor、同一组随机种子和同一套输入 LLR（对数似然比，Log-Likelihood Ratio）。
...
| [[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]] NR LDPC 码块重组与 TB（传输块，Transport Block）CRC | 知道 TB 可分成多个 CB，CBG（码块组，Code Block Group）、filler、CB CRC 和 TB CRC 的接收端边界。 |
...
| 编号 | T17.3 |
| 前置 | T8.8, T9.5, T17.1 |
| Prompt | 请定义 NR LDPC 浮点仿真计划：基图选择、提升、速率匹配/恢复、Min-Sum 变体、CRC 检查、BLER 曲线、随机种子、输出和阈值。写作时参考本文“单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
NR LDPC 的协议链路主要来自 TS 38.212，调制阶数、目标码率、TBS（传输块大小，Transport Block Size）和 CBG 调度语义来自 TS 38.214。本节只把这些协议结论作为仿真参数来源；BP/MS/NMS/OMS 的具体迭代调度、归一化因子、偏移量、早停策略和 BLER 阈值属于接收端工程策略。
...
| TS 38.212 §6.2、§7.2 | `content.md` 上行共享信道/下行共享信道chain | UL-SCH（上行共享信道，Uplink Shared Channel）/DL-SCH（下行共享信道，Downlink Shared Channel）调用 TB CRC、BG selection、CB segmentation、LDPC coding、rate matching 和 CB concatenation。 | 仿真要区分 channel/direction，并记录 UL/DL 调用链。 |
| TS 38.214 §5.1.3、§6.1.4 | `content.md` PDSCH（物理下行共享信道，Physical Downlink Shared Channel）/PUSCH（物理上行共享信道，Physical Uplink Shared Channel）MCS（调制与编码方案，Modulation and Coding Scheme）/TBS sections | MCS、$Q_m$、target code rate、RV 和 TBS 的系统输入来源。 | 若使用系统级输入，必须记录 MCS table、$Q_m$、target rate、TBS 和下行控制信息/RV 来源。 |
...
| TS 38.212 §5.4.2.1、§6.1.2.1.1、§7.3.1.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:1257-1289`；`3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:4489`；`3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:6097-6103` | LDPC bit selection 中未被 CBGTI 调度的 CB、PUSCH/PDSCH DCI（下行控制信息，Downlink Control Information）中 CBGTI/CBGFI 字段存在性。 | rate recovery 必须把 `scheduled_cb_count`、`held_cb_count`、CBGTI bit width 和 CBGFI present 写入 descriptor。 |

协议前因后果是：媒体接入控制层交给物理层 TB；物理层先做 TB CRC，然后按 BG 规则和最大 CB size 分段，选 $Z_c$ 并展开 QC-LDPC（准循环 LDPC，Quasi-Cyclic LDPC）矩阵；编码后再按 RV 和 rate matching 选择发送比特。接收端要反向恢复：demapper 给 LLR，bit deinterleaver 和 circular buffer 回填到 LDPC codeword 域，decoder 在同一 $H$ 上迭代，syndrome/CRC 给停止和验收证据，最后 CB 重组和 TB CRC 决定交付。
...
| 内容 | 说明 |
|:---|:---|
| Protocol Config | TB/CB Builder；BG + Zc Select；LDPC Encoder；Rate Match + RV；AWGN（加性白高斯噪声，Additive White Gaussian Noise） + LLR |
| LDPC Decoder Variants | Flooding or layered BP, MS, NMS and OMS with syndrome and CRC stopping records. |
| Trace + Metrics | BG, Zc, rv, k0（速率匹配起点，Rate matching starting position）, E, Ncb, iter, syndrome, CRC, BLER, BER, avg_iter. |
...
| Polar（极化码，Polar Code）控制信道 | 不属于本节，由 [[T17.4_NR_Polar_float_sim_plan|T17.4]] 处理。 |
| 小块编码 | 不属于本节主线；TS 38.212 small block coding 不使用 LDPC decoder。 |
| MIMO（多输入多输出，Multiple-Input Multiple-Output）、信道估计、均衡 | 不属于本节；本节从理想 AWGN 或已得到的 demapper LLR 开始。 |
...
| `reference` | 生成正式 BLER 曲线。 | BP 或 NMS，多 SNR（信噪比，Signal-to-Noise Ratio）点，固定 min frames/error blocks。 | 曲线随 $E_b/N_0$ 升高下降。 |
...
4. 不等于。syndrome zero 只说明 hard bits 满足 LDPC parity-check equations；CB/TB CRC 仍是接收端交付和 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）反馈的关键 gate。
