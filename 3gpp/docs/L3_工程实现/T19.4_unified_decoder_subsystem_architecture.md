...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T19.4_unified_decoder_subsystem_architecture.md"
---
# T19.4 LTE/NR统一译码子系统架构：从三类译码引擎到共享 DMA、软缓存和状态中断
## 本节学习目标

本节设计一个统一译码子系统。这里的“统一”不是把LTE Turbo（Turbo 码，Turbo Code）、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）和 NR Polar（极化码，Polar Code）的算法揉成一个核心，而是把三类译码引擎放进同一个寄存器传输级/专用集成电路子系统：上层用同一种任务描述符、DMA（直接存储器访问，Direct Memory Access）、软缓存、配置寄存器、状态、中断请求（Interrupt Request, IRQ）和 trace/failure bundle 管理它们；每个引擎仍保留自己的算法状态和可独立测试边界。
...
| 传输块大小 | 调度和分段前后的 TB（传输块，Transport Block）尺寸上下文。 |
...
| Engine-local state | 只属于某个引擎的状态，例如 Turbo alpha/beta/extrinsic、LDPC CN/VN/message RAM、Polar path/partial-sum/PM/CRC（循环冗余校验，Cyclic Redundancy Check） state。 |
...
- 规划 RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）验证：每个引擎能用 mock DMA、私有 trace 和固定 descriptor 单独跑，也能在统一子系统中做混合任务调度回归。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T4.6_decoder_interface_contracts|T4.6]] 译码器接口契约 | 知道 descriptor、input、output、status、error、debug/trace 六类对象，以及 soft buffer key 为什么不能只用 RV（冗余版本，Redundancy Version）。 |
| [[T5.2_memory_banking_buffering_basics|T5.2]] 存储分 bank 与缓存基础 | 知道 SRAM（静态随机存取存储器，Static Random-Access Memory）、bank、端口、软缓存、读改写、bank conflict 和共享仲裁。 |
| [[T5.4_RTL_state_machine_handshake_basics|T5.4]] RTL 状态机与握手 | 知道 `valid/ready`、start/done/error、reset、timeout、backpressure 和 CDC 风险。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] Bit-exact（比特精确，bit-exact） 回归框架 | 知道三类译码器的 checkpoint manifest、failure bundle 和 status compare。 |
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] LTE Turbo RTL 微架构 | 知道 Turbo engine 的 SISO（软入软出，Soft-Input Soft-Output）、alpha/beta、extrinsic、CRC early stop 和 trace 字段。 |
| [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] NR LDPC RTL 微架构 | 知道 LDPC engine 的 BG（基图，Base Graph）/$Z_c$/edge schedule、layered controller、bank conflict、syndrome/CRC 和 trace 字段。 |
| [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] NR Polar RTL 微架构 | 知道 Polar engine 的 SC（逐次消除，Successive Cancellation）/SCL（连续消除列表译码，Successive Cancellation List）、path memory、PM sorter、CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier） selector 和 trace 字段。 |

若尚未掌握 descriptor 和 soft buffer key 的关系，先回看 [[T4.6_decoder_interface_contracts|T4.6]]。本节默认你已经知道三类译码器内部差异；这里解决的是 SoC 级集成：谁接收任务、谁搬 LLR（对数似然比，Log-Likelihood Ratio）、谁拥有软缓存、谁产生中断、谁保留可复现证据。
...
| TS 36.321 | `3GPP_Rel19/processed/TS_36.321_36321-j20` | 本节只记录 soft-buffer/HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）上下文入口。 |
| TS 36.331 | `3GPP_Rel19/processed/TS_36.331_36331-j21` | 不抽取具体 RRC（无线资源控制，Radio Resource Control）信息元素到寄存器。 |
| TS 38.212 | `3GPP_Rel19/processed/TS_38.212_38212-j30` | 不规定接收机内部 DMA、软缓存或寄存器。 |
| TS 38.214 | `3GPP_Rel19/processed/TS_38.214_38214-j30` | 本节只作 context evidence，具体字段映射到 [[T19.6_decoder_register_map_configuration_flow|T19.6]]/T15 核验。 |
| TS 38.321 | `3GPP_Rel19/processed/TS_38.321_38321-j20` | 不把 MAC（媒体接入控制层，Medium Access Control）状态机字段直接声明为硬件寄存器。 |
| TS 38.331 | `3GPP_Rel19/processed/TS_38.331_38331-j20` | 不复现具体 RRC 表格。 |
| [[T4.6_decoder_interface_contracts|T4.6]] | `docs/L1_基础/T4.6_decoder_interface_contracts.md` | 本节把接口契约升到子系统架构。 |
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]]-[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] | `docs/L3_工程实现/T19.1*`、`T19.2*`、`T19.3*` | 本节不重写每个 engine 内部算法。 |
...
![图 1：统一译码器子系统——Config Registers 经 Descriptor Dispatcher 分发到 Input DMA、三类译码引擎与 Output DMA，底部 Soft Buffer Manager 与 DMA Arbiter 管理共享资源，Status/IRQ 汇总任务结果](assets/T19.4_unified_decoder_subsystem_architecture.svg)
...
| Output DMA | Writes decoded payload, CB（码块，Code Block） status, selected path metadata and result descriptors. |
...
| NR LDPC Engine | QC schedule ROM（只读存储器，Read-Only Memory）, layered controller, CN min1/min2, VN RMW, bank conflict counters. |
| NR Polar Engine | SC/SCL tree, LLR and partial-sum state, PM sorter, path remap and CRC/RNTI selector. |
| HARQ Soft Buffer Manager | Keyed by carrier / HARQ / TB（传输块，Transport Block） epoch / CB / family; RV belongs to access transaction and trace. |
...
这个分层的好处是每个 engine 可独立测试：给它 mock descriptor、mock input DMA、私有 soft-buffer stub 和私有 trace sink，就能跑 [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]]-[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] 的单引擎回归；接入统一子系统后，再测试共享资源仲裁、跨任务隔离和中断状态。
...
|:---|:---|:---|:---|
| Turbo | `K`、`f1`、`f2`、`E`、`Ncb`、`rvidx`、`max_iter` | TS 36.212/T7/[[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] 回链。 | 检查合法集合、传给 Turbo engine。 |
| LDPC | `BG`、`Zc`、`iLS`、`K`、`N`、`E`、`Ncb`、`rv`、`k0`、`cbg_mask` | `BG/Zc/iLS/k0` 回链 TS 38.212/T9/[[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]]；`cbg_mask` 是实现 descriptor 字段，语义回链 TS 38.214 CBG（码块组，Code Block Group）调度/反馈上下文和 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]，不是 TS 38.212 强制寄存器字段。 | 检查组合、选择 LDPC engine。 |
| Polar | `N`、`K`、`E`、`crc_type`、`rnti_context`、`list_size`、`mask_hash` | TS 38.212/T10/[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] 回链。 | 检查 mask/hash 和实现配置边界。 |
...
4. Soft buffer manager 根据 NDI（新数据指示，New Data Indicator）/TB epoch 判断清空、命中或 miss。
...
| 3 | NR Polar DCI（下行控制信息，Downlink Control Information） 控制信息 | `family=POLAR`、`control_context=PDCCH`、`N=512`、`E=864`、`list_size=8`、`rnti_context` | 不按数据 TB soft buffer 生命周期处理，运行 Polar engine 并输出控制有效性。 |
...
本节只给逻辑分区，不给真实地址。真实地址、bit 位和可写属性应在 [[T19.6_decoder_register_map_configuration_flow|T19.6]] 或项目 RTL 规范中核验。
...
Integration Rules 固化为六条：3GPP specs define task context, not this register map；Each engine can run with private mock DMA and private trace；Shared resources never own algorithm state；Soft-buffer writes are atomic per transaction；Trace uses [[T18.6_bit_exact_regression_harness|T18.6]] checkpoint names；IRQ summarizes status; debug keeps evidence。中文展开就是：协议定义任务上下文而非寄存器图；每个引擎必须可用私有 mock DMA 和私有 trace 独立运行；共享资源不能拥有算法状态；软缓存写入按事务原子提交；trace 命名沿用 [[T18.6_bit_exact_regression_harness|T18.6]] checkpoint；IRQ 只汇总状态，debug 证据留在 STATUS/ERROR/TRACE/failure bundle 中。
...
| Bit-exact integration | [[T18.6_bit_exact_regression_harness|T18.6]] vector schema。 | output/status/counters/checkpoints 与 fixed model 对齐。 |
...
| Turbo engine 私有结构来自 [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]]。 | [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] 本项目讲义。 | `docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md` | alpha/beta/extrinsic/SISO 是 engine-local。 |
| LDPC engine 私有结构来自 [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]]。 | [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] 本项目讲义。 | `docs/L3_工程实现/T19.2_NR_LDPC_RTL_microarchitecture.md` | QC schedule/message RAM/bank conflict 是 engine-local。 |
| Polar engine 私有结构来自 [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]]。 | [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] 本项目讲义。 | `docs/L3_工程实现/T19.3_NR_Polar_RTL_microarchitecture.md` | path/PM/sorter/CRC selector 是 engine-local。 |
| Trace/failure bundle 命名来自 [[T18.6_bit_exact_regression_harness|T18.6]]。 | [[T18.6_bit_exact_regression_harness|T18.6]] 本项目讲义。 | `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md` | 项目验证要求，不是 3GPP 条款。 |
...
| 项 | 命令或证据 | 结果 |
|:---|:---|:---|
| Roadmap 任务 | `2026-06-19-lte-nr-decoding-learning-roadmap.md:1171` | T19.4 prompt 已全量覆盖并拓展。 |
| 术语审计 | `python3 tools/audit_lesson_terms.py docs/L3_工程实现/T19.4_unified_decoder_subsystem_architecture.md` | `LESSON_TERM_AUDIT_OK`。 |
| 标题审计 | `python3 tools/audit_markdown_headings.py docs/L3_工程实现/T19.4_unified_decoder_subsystem_architecture.md` | `MARKDOWN_HEADING_AUDIT_OK`。 |
| 深度审计 | `python3 tools/audit_lesson_depth.py --strict docs/L3_工程实现/T19.4_unified_decoder_subsystem_architecture.md` | `LESSON_DEPTH_AUDIT_OK`。 |
| LaTeX 渲染审计 | `python3 tools/audit_latex_render.py docs/L3_工程实现/T19.4_unified_decoder_subsystem_architecture.md` | `LATEX_RENDER_AUDIT_OK formulas=5`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T19.4_unified_decoder_subsystem_architecture.md` | `REFERENCE_REBUILD_AUDIT_CANDIDATES`；候选为 TS 36.213/TS 38.331 `待核验` 上下文、实现边界和工程解释，不构成本节阻塞。 |
...
| 接口基础 | `docs/L1_基础/T4.6_decoder_interface_contracts.md`；`docs/L1_基础/T5.2_memory_banking_buffering_basics.md`；`docs/L1_基础/T5.4_RTL_state_machine_handshake_basics.md`。 |
| 微架构前置 | `docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md`；`docs/L3_工程实现/T19.2_NR_LDPC_RTL_microarchitecture.md`；`docs/L3_工程实现/T19.3_NR_Polar_RTL_microarchitecture.md`。 |
| 验证回链 | `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md`。 |
