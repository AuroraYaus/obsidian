...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T4.6_decoder_interface_contracts|T4.6]] 译码器接口契约 | 知道 descriptor、input、output、status、error、debug/trace 六类对象，以及 soft buffer key 为什么不能只用 RV。 |
| [[T5.2_memory_banking_buffering_basics|T5.2]] 存储分 bank 与缓存基础 | 知道 SRAM、bank、端口、软缓存、读改写、bank conflict 和共享仲裁。 |
...
| [[T18.6_bit_exact_regression_harness|T18.6]] Bit-exact 回归框架 | 知道三类译码器的 checkpoint manifest、failure bundle 和 status compare。 |
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] LTE Turbo RTL 微架构 | 知道 Turbo engine 的 SISO、alpha/beta、extrinsic、CRC early stop 和 trace 字段。 |
| [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] NR LDPC RTL 微架构 | 知道 LDPC engine 的 BG/$Z_c$/edge schedule、layered controller、bank conflict、syndrome/CRC 和 trace 字段。 |
| [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] NR Polar RTL 微架构 | 知道 Polar engine 的 SC/SCL、path memory、PM sorter、CRC/RNTI selector 和 trace 字段。 |

若尚未掌握 descriptor 和 soft buffer key 的关系，先回看 [[T4.6_decoder_interface_contracts|T4.6]]。本节默认你已经知道三类译码器内部差异；这里解决的是 SoC 级集成：谁接收任务、谁搬 LLR、谁拥有软缓存、谁产生中断、谁保留可复现证据。
...
| LDPC | `BG`、`Zc`、`iLS`、`K`、`N`、`E`、`Ncb`、`rv`、`k0`、`cbg_mask` | `BG/Zc/iLS/k0` 回链 TS 38.212/T9/[[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]]；`cbg_mask` 是实现 descriptor 字段，语义回链 TS 38.214 CBG 调度/反馈上下文和 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]，不是 TS 38.212 强制寄存器字段。 | 检查组合、选择 LDPC engine。 |
...
## 图示
![图 1：unified decoder subsystem architecture](assets/T19.4_unified_decoder_subsystem_architecture.svg)
...
| 图形资产 | `docs/L3_工程实现/assets/T19.4_unified_decoder_subsystem_architecture.svg` | 手绘 SVG，2026-08-04 替代原 PIL 渲染 PNG（原 `render_t14_4_unified_decoder_subsystem.py` 与 PNG 已删除，图示语义不变）；SVG 几何审计六项检查全部通过，cairosvg 渲染验证无报错。 |
