...
type: spec
aliases:
  - T17.5 BER BLER curve reporting
tags:
  - 3gpp
  - docs
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md"
---
# T17.5 BER/BLER 曲线生成与报告：从 metrics.csv 到可复现性能证据
## 本节学习目标

本节讲解如何把LTE Turbo（Turbo 码，Turbo Code）、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）和 NR Polar（极化码，Polar Code）的浮点仿真结果整理成可复现、可审计、可比较的BER（比特错误率，Bit Error Rate）和块错误率曲线报告。这里的“报告”不是把几条线画到图上，而是让图上每一个点都能追溯到协议对象、随机种子、命令、CSV 原始计数、置信区间、停止原因和失败样本。


- 区分 BER、传输块BLER（块错误率，Block Error Rate）、码块BLER、控制块 BLER、FER（帧错误率，Frame Error Rate）和误通过率，避免把不同分母的指标混在同一条曲线上。
- 解释为什么3GPP TS 36.212/TS 38.212 规定的是循环冗余校验、分段、码块串接和控制信息 CRC（循环冗余校验，Cyclic Redundancy Check）/RNTI（无线网络临时标识，Radio Network Temporary Identifier）语义，而不是直接规定某个仿真 BLER 门限。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T4.5_decoder_performance_metrics|T4.5]] 译码器性能指标 | 知道 BER、BLER、FER、吞吐、延迟和统计波动的基本含义。 |
| [[T17.1_python_golden_model_project_layout|T17.1]] Python Golden Model 工程布局 | 知道 `metrics.csv`、`frames.jsonl`、`seeds.json`、`resolved_config.yaml`、失败 dump 和 evidence archive 的角色。 |
| [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] LTE Turbo 浮点仿真计划 | 知道 LTE Turbo 仿真会输出 TB（传输块，Transport Block）/CB（码块，Code Block） CRC 状态、Log-MAP/Max-Log-MAP 配置、混合自动重传请求/冗余版本（Redundancy Version, RV）和有效码率口径。 |
| [[T17.3_NR_LDPC_float_sim_plan|T17.3]] NR LDPC 浮点仿真计划 | 知道 NR LDPC 仿真会输出基图（Base Graph, BG）、$Z_c$、RV、CBG、syndrome、置信传播（Belief Propagation, BP）/Min-Sum（MS）/归一化 Min-Sum（Normalized Min-Sum, NMS）/偏移 Min-Sum（Offset Min-Sum, OMS）、CB CRC 和 TB CRC。 |
| [[T17.4_NR_Polar_float_sim_plan|T17.4]] NR Polar 浮点仿真计划 | 知道 NR Polar 仿真会输出 $A/K/E/N/L$、连续抵消（Successive Cancellation, SC）/连续抵消列表（Successive Cancellation List, SCL）/CRC 辅助 SCL（CRC-Aided SCL, CA-SCL）、CRC/RNTI、误通过、路径度量和 list-size sweep。 |

若你还没掌握 [[T4.5_decoder_performance_metrics|T4.5]]，本节的 Wilson 区间和早停规则会显得像统计公式；若你还没掌握 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 到 [[T17.4_NR_Polar_float_sim_plan|T17.4]]，本节的 CSV 字段会显得像杂乱日志。正确读法是：先明确“一个曲线点到底统计了什么协议对象”，再谈画图。
...
| Prompt | 请讲解如何生成、保存、绘制和解读 BER/BLER 曲线，包含置信区间、最小帧数、仿真早停、CSV/PNG 命名和失败诊断。写作时参考本文“单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
|:---|:---|:---|:---|
| TS 36.212 §5.1.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md:575-599` | LTE CRC calculation 和零余数语义，支撑 TB/CB CRC pass/fail。 | 不规定 BLER 目标值。 |
| TS 36.212 §5.1.2 | `content.md:601-673` | LTE code block segmentation、CB CRC 和 filler 边界，支撑 CB BLER 分母。 | 不规定最小帧数或早停规则。 |
| TS 36.212 §5.1.5 | `content.md:1025-1035` | LTE CB rate matching 输出串接成 TB 发送序列，支撑 TB 级曲线回链。 | 不规定 CSV 字段。 |
| TS 36.212 §5.2.2.1-§5.2.2.5 | `content.md:1077-1109` | LTE 上行共享信道TB CRC、segmentation、Turbo coding、rate matching、concatenation 调用链。 | 不规定 Log-MAP 或 Max-Log-MAP 实现。 |
| TS 38.212 §5.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:637-657` | NR CRC calculation 总入口，支撑 LDPC 和 Polar CRC pass/fail。 | 不规定仿真曲线门限。 |
| TS 38.212 §5.2.1 | `content.md:665-715` | NR Polar segmentation and CRC attachment，支撑控制信息 BLER 和 CA-SCL final selector。 | 不规定 list size。 |
| TS 38.212 §5.2.2 | `content.md:717-789` | NR LDPC segmentation and CB CRC，支撑 CB BLER、TB BLER 和 CBG 统计对象。 | 不规定 BP/MS/NMS/OMS 阈值。 |
| TS 38.212 §5.5 | `content.md:1335-1351` | NR code block concatenation，支撑 TB 级统计与 CB 输出顺序。 | 不规定报告图样式。 |
| TS 38.212 §6.2.1/§6.2.3/§6.2.6 | `content.md:1361-1415` | NR UL-SCH（上行共享信道，Uplink Shared Channel） TB CRC、CB segmentation、CB concatenation 调用链。 | 不规定仿真停止条件。 |
| TS 38.212 §6.3.1.2.1/§6.3.1.3.1/§6.3.1.4.1 | `content.md:2325-2387` | NR PUCCH（物理上行控制信道，Physical Uplink Control Channel） 上的上行控制信息Polar 的 CRC6/CRC11、Polar channel coding 和 rate matching 边界，支撑 `control_bler` 的控制块对象定义。 | 不规定 PUCCH BLER 目标值。 |
| TS 38.212 §6.3.2.2.1/§6.3.2.3.1/§6.3.2.4.1 | `content.md:2863-3009` | NR PUSCH（物理上行共享信道，Physical Uplink Shared Channel） 上 UCI（上行控制信息，Uplink Control Information） Polar 复用 PUCCH UCI 的 segmentation/CRC/channel coding，并在 §6.3.2.4.1 给出 HARQ-ACK 等 UCI 的 rate matching 输入、输出和长度上下文，支撑 UCI-on-PUSCH 曲线对象。 | 不规定 UCI 仿真最小 trial 数。 |
| TS 38.212 §7.2.1/§7.2.3/§7.2.6 | `content.md:3737-3783` | NR 下行共享信道/PCH TB CRC、CB segmentation、CB concatenation 调用链。 | 不规定性能验收阈值。 |
| TS 38.212 §7.3.2 | `content.md:7177-7192` | NR 下行控制信息CRC24 和 RNTI scrambling 边界，支撑 false-pass、RNTI mismatch 和 DCI（下行控制信息，Downlink Control Information） BLER 统计。 | 不规定 blind-detection 误检目标。 |
...
![图 1：BER/BLER 曲线报告总览——三类仿真输出统一 CSV，Curve Builder 生成报告就绪曲线（横轴 Eb/N0、纵轴 BLER 对数刻度），曲线异常经 SNR row 展开后回放失败样本](assets/T17.5_BER_BLER_curve_reporting.svg)
...
[[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 到 [[T17.4_NR_Polar_float_sim_plan|T17.4]] 默认使用 `payload_over_sum_E`，即：
...
python3 -m decoder_golden.reporting.build_report_bundle \
  ...
  --protocol-refs docs/L3_工程实现/T17.2_LTE_Turbo_float_sim_plan.md docs/L3_工程实现/T17.3_NR_LDPC_float_sim_plan.md docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md \
  --include-failures 20 \
...
| 高 SNR 仍大量失败 | `llr_sign_convention`、`first_failure_path`、`crc_mode` | LLR（对数似然比，Log-Likelihood Ratio） 符号反、rate recovery 地址错、CRC bit order 错。 | replay 第一失败帧，核对 descriptor 和 LLR。 |
...
```text
输入：
  run_dirs：T17.2/T17.3/T17.4 生成的仿真目录
...
| HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） /soft buffer | `rv_idx`、`harq_process_id`、`cbg_state`、`repetition_count`、`saturation_count`。 |
...
| 协议回链 | 每个报告包都包含 TS 36.212/38.212 本地路径和上游 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]]/[[T17.3_NR_LDPC_float_sim_plan|T17.3]]/[[T17.4_NR_Polar_float_sim_plan|T17.4]] 文档路径。 |
...
| :--- | :--- | :--- | :--- |
| LTE CRC calculation | TS 36.212 `36212-j30` §5.1.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md:575-599` | 已核验锚点；公式 XML 不在本节重转写。 |
| LTE segmentation and CB CRC | TS 36.212 §5.1.2 | `content.md:601-673` | 已核验锚点。 |
| LTE CB concatenation | TS 36.212 §5.1.5/§5.2.2.5 | `content.md:1025-1035`；`content.md:1105-1109` | 已核验锚点。 |
| NR CRC calculation | TS 38.212 `38212-j30` §5.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md:637-657` | 已核验锚点。 |
| NR Polar CRC attachment | TS 38.212 §5.2.1 | `content.md:665-715` | 已核验锚点。 |
| NR LDPC segmentation and CB CRC | TS 38.212 §5.2.2 | `content.md:717-789` | 已核验锚点。 |
| NR CB concatenation | TS 38.212 §5.5/§6.2.6/§7.2.6 | `content.md:1335-1351`；`content.md:1409-1415`；`content.md:3777-3783` | 已核验锚点。 |
| NR UCI on PUCCH Polar CRC/channel coding/rate matching | TS 38.212 §6.3.1.2.1/§6.3.1.3.1/§6.3.1.4.1 | `content.md:2325-2387` | 已核验锚点。 |
| NR UCI on PUSCH Polar CRC/channel coding/rate matching | TS 38.212 §6.3.2.2.1/§6.3.2.3.1/§6.3.2.4.1 | `content.md:2863-3009` | 已核验锚点。 |
| NR DCI CRC/RNTI | TS 38.212 §7.3.2 | `content.md:7177-7192` | 已核验锚点。 |

## 未复现协议资产与关闭条件

| 候选资产 | 本节处理 | 关闭条件 |
|:---|:---|:---|
| TS 36.212/38.212 CRC 多项式完整公式 XML | 本节只引用 CRC pass/fail 语义；CRC 多项式复现由 [[T3.1_LTE_NR_CRC_families|T3.1]] 和对应协议讲义承担。 | 若报告脚本自动生成 CRC checker，必须把具体 CRC 多项式、bit order 和 XML/表格证据纳入 evidence archive。 |
| TS 36.212/38.212 码块分段公式 | 本节不重新推导分段公式，只使用上游 T3/T7/T9/[[T17.2_LTE_Turbo_float_sim_plan|T17.2]]/[[T17.3_NR_LDPC_float_sim_plan|T17.3]] 的 descriptor 输出。 | 若曲线报告工具从 TB size 自动生成 CB 分段，必须复用上游 bit-exact 分段单测和协议表图证据。 |
| TS 38.212 DCI blind detection 全流程 | 本节只统计 DCI CRC/RNTI false pass，不展开 PDCCH（物理下行控制信道，Physical Downlink Control Channel） candidate/search-space。 | 若评估真实 blind detection 误检，必须补 PDCCH 搜索空间、aggregation level、RNTI sweep 和候选数分母。 |
| TS 38.212 UCI small-block coding 表格 | 本节聚焦 Polar UCI 曲线报告；small-block UCI 不进入 T17.5 的 NR Polar 主线。 | 若后续报告 UCI 小块码性能，必须新增 `protocol_case=nr_uci_small_block`、重建相应表格和独立 control BLER 证据。 |
...
| 写入文件 | `docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T17.5。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 阶段 11 与 T17.5 条目。 |
| 前置依据 | 已读取 `docs/L1_基础/T4.5_decoder_performance_metrics.md`、`docs/L3_工程实现/T17.2_LTE_Turbo_float_sim_plan.md`、`docs/L3_工程实现/T17.3_NR_LDPC_float_sim_plan.md`、`docs/L3_工程实现/T17.4_NR_Polar_float_sim_plan.md`。 |
...
| README 核验 | 已读取 TS 36.212/38.212 README；TS 36.212 记录 paragraphs `3430`、headings `144`、tables `250`、equations `23`、media `1034`；TS 38.212 记录 paragraphs `4193`、headings `165`、tables `321`、equations `4360`、media `1058`；README 提醒公式以 `equations/*.xml` 为准，表格优先看 `tables/*.html`。 |
| 审核经理复核 | Hegel 审核 T17.5：Critical=0，Important=3，Minor=3；已修复 Important：补齐所有指标 CI/upper-bound 字段、拆分 TB BLER/control BLER/false-pass 命令模板、补 TS 38.212 §6.3 UCI PUCCH/PUSCH 协议锚点；同时修正 `tb_fail` 和通用 `error_count/ci_low/high` 命名。Copernicus 复核：Critical=0，Important=3，Minor=3；已修复 bundle 示例旧 `all_decoders_tb_bler` 残留、`target_*` 诊断别名 schema 落地、PUSCH UCI rate matching 锚点范围扩展到 `content.md:2863-3009`，并处理首现展开和 `NR PUCCH 上上行控制信息` 文本错误。 |
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md`：`LATEX_RENDER_AUDIT_OK formulas=42`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选来自本节明确声明不复现的协议公式/表格边界、DCI blind detection 边界和 README 公式权威性提示，不构成本节失败。 |
...
- [上游讲义] `docs/L1_基础/T4.5_decoder_performance_metrics.md`，译码器性能指标。
