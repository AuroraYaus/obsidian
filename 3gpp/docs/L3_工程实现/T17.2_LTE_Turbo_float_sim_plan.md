...
| [[T7.4_LTE_code_block_reassembly_TB_CRC|T7.4]] LTE 码块重组与 TB CRC | 知道 TB 可分成多个 CB，$K_-$ 短块在前、$K_+$ 长块在后，filler 只在第 0 个 CB 开头删除。 |
...
## 图示
![图 1：LTE Turbo float sim flow](assets/T17.2_LTE_Turbo_float_sim_flow.svg)
...
| 新增证据资产 | `docs/L3_工程实现/assets/T17.2_LTE_Turbo_float_sim_flow.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t12_2_lte_turbo_float_sim_flow.py`）。 |
