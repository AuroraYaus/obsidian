...
本节进入LTE Turbo（Turbo 码，Turbo Code）的浮点仿真规划。这里的“浮点”表示对数似然比、信道噪声、Log-MAP/Max-Log-MAP 递推和外信息交换都先用 `float64` 或同等级浮点数表达，目标是建立算法正确性和协议位序的黄金参考，而不是立即考虑位宽、饱和和硬件吞吐。

本节学习目标：

- 用3GPP TS 36.212 的循环冗余校验、码块分段、Turbo 编码、速率匹配和冗余版本（Redundancy Version, RV）规则生成参考链路。
- 定义加性白高斯噪声信道、二进制相移键控或正交相移键控软解调和 LLR（对数似然比，Log-Likelihood Ratio）符号约定。
- 说明解速率匹配、混合自动重传请求软合并、Log-MAP/Max-Log-MAP 译码、码块CRC（循环冗余校验，Cyclic Redundancy Check）和TB（传输块，Transport Block）CRC 如何组成接收端仿真链路。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T6.7_Turbo_iteration_extrinsic_stopping|T6.7]] Turbo 迭代 | 知道两个软输入软输出（Soft-Input Soft-Output, SISO）译码器交换外信息，最大迭代次数影响最坏延迟，CRC 早停是接收端实现策略。 |
| [[T7.4_LTE_code_block_reassembly_TB_CRC|T7.4]] LTE 码块重组与 TB CRC | 知道 TB 可分成多个 CB（码块，Code Block），$K_-$ 短块在前、$K_+$ 长块在后，filler 只在第 0 个 CB 开头删除。 |
| [[T17.1_python_golden_model_project_layout|T17.1]] Golden Model 工程布局 | 知道 `vector.json`、`input_llr.npy`、`metrics.csv`、`frames.jsonl`、`seeds.json`、失败 replay 和 evidence archive 的职责。 |
| LLR 基础 | 知道正 LLR 倾向比特 0 或 1 必须由工程约定明确，本项目默认使用 `POS_ZERO`。 |
| BER（比特错误率，Bit Error Rate）/BLER（块错误率，Block Error Rate）基础 | 知道BER和 BLER 是统计指标，不是 3GPP 直接给出的译码器合格线。 |

如果前置知识不足，先回看 [[T2.13_BPSK_QPSK_soft_demapping|T2.13]]、[[T4.5_decoder_performance_metrics|T4.5]]、[[T6.7_Turbo_iteration_extrinsic_stopping|T6.7]]、[[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]]、[[T7.4_LTE_code_block_reassembly_TB_CRC|T7.4]] 和 [[T17.1_python_golden_model_project_layout|T17.1]]。本节默认你已经理解“协议位序必须 bit-exact，算法曲线和阈值是工程评估”这个边界。
...
| 编号 | T17.2 |
| 前置 | T6.7, T7.4, T17.1 |
| Prompt | 请定义 LTE Turbo 浮点仿真计划：编码器参考、AWGN（加性白高斯噪声，Additive White Gaussian Noise）信道、解速率匹配、Log-MAP/Max-Log-MAP 译码、CRC 检查、BLER 曲线、随机种子、输出和阈值。写作时参考本文“单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
本节是工程仿真计划，不把 TS 36.212 的全部表格重新复制一遍。已经在上游讲义重建的协议资产继续引用：TS 36.212 Table 5.1.3-3 的合法 $K/f_1/f_2$ 表见 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T6.4_LTE_Turbo_internal_interleaver|T6.4]]，Figure 5.1.3-2 的 Turbo 编码器结构重建见 [[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]]，Table 5.1.4-1 的 32 列子块交织置换见 [[T7.2_LTE_subblock_deinterleaver_circular_buffer|T7.2]]/[[T11.2_LTE_NR_rate_matching_comparison|T11.2]]。本节重点是把这些协议资产接成一次可运行、可审计、可回放的浮点实验。
...
| TS 36.212 §5.1.3.2.1 Turbo encoder | `content.md` around line 721；[[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]] 重建图 | 规定 LTE Turbo 为并行级联卷积码，含两个 8 状态组成编码器和内部交织器，母码率为 1/3。 | 编码器参考必须输出系统流、第一校验流、第二校验流和 trellis termination。 |
...
| TS 36.212 §5.1.4.1.2 Bit collection, selection and transmission | `content.md` around line 827 and 923 | 规定 soft buffer size $N_{\mathrm{cb}}$、输出长度 $E$、`rvidx` 和选择流程。 | HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）重传必须按同一 CB/进程/RV 地址进行软合并。 |
| TS 36.213 MCS（调制与编码方案，Modulation and Coding Scheme）/TBS（传输块大小，Transport Block Size） | `3GPP_Rel19/processed/TS_36.213_*` | `待核验`：用于系统级 MCS/TBS 输入锚点。 | 本节不使用具体 MCS/TBS 表值作为已核验事实，关闭条件是定位精确分册、章节、表格和本地抽取文件。 |
...
![图 1：LTE Turbo 浮点仿真链路——Protocol Config 固定协议证据与参数，经 TB/CB Builder、Turbo Encoder、Rate Matching + RV 生成发送侧参考，AWGN + LLR 生成接收端软信息，Decoder 完成 rate recovery 与 Log-MAP/Max-Log-MAP 译码，输出 trace、BLER/BER 与可复现归档](assets/T17.2_LTE_Turbo_float_sim_flow.svg)

| 内容 | 说明 |
|:---|:---|
| Protocol Config | TB/CB Builder；Turbo Encoder；Rate Matching + RV；AWGN + LLR；Decoder |
| TS 36.212 refs, TB size, CRC, K/f1/f2, RV, max_iter, Log-MAP mode. | Payload, TB CRC, CB segmentation, filler, CB CRC metadata.；RSC（递归系统卷积码，Recursive Systematic Convolutional Code）pair, internal interleaver, systematic/parity/tail streams.；Sub-block interleaving, circular buffer, rvidx, E and received positions.；Eb/N0 sweep, noise seed |
...
| AWGN 信道 | 先用 BPSK（二进制相移键控，Binary Phase Shift Keying）或 QPSK（正交相移键控，Quadrature Phase Shift Keying）等效软解调验证 decoder，不把多径、同步、信道估计作为主线。 |
...
| 射频、同步、均衡、MIMO（多输入多输出，Multiple-Input Multiple-Output）检测 | 不属于本节；本节从已得到的符号软信息或理想 AWGN 软信息开始。 |
...
其中 $\pi$ 是 LTE Turbo 内部交织器。$\mathbf{d}^{(0)}$ 是系统流，直观上保留原信息比特顺序；$\mathbf{d}^{(1)}$ 和 $\mathbf{d}^{(2)}$ 是两个组成编码器产生的校验流。式 (2) 是工程抽象，具体反馈和校验抽头、尾比特路径由 TS 36.212 Figure 5.1.3-2 和 [[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]] 重建图约束。
...
式 (6) 的前提是符号能量归一化为 1，且 $E_b/N_0$ 用 dB 输入。若仿真改用 $E_s/N_0$ 或直接 SNR（信噪比，Signal-to-Noise Ratio），必须另写字段，不能把三个量混用。
...
每个半迭代中，SISO 输出后验 LLR。外信息按 [[T6.7_Turbo_iteration_extrinsic_stopping|T6.7]] 的定义传给另一个 SISO：
...
T17.2 必须继承 [[T17.1_python_golden_model_project_layout|T17.1]] 的归档口径。一次运行输出建议如下：
...
本节必须继承 [[T17.1_python_golden_model_project_layout|T17.1]] 的 seed 派生规则，禁止使用 Python 内置 `hash()`。建议 `seeds.json` 记录：
...
并记录 `confidence_limited=true`。若 $N_{\mathrm{err}}=0$，工程报告可以写上界估计或交给 [[T17.5_BER_BLER_curve_reporting|T17.5]] 的置信区间统一处理。
...
| Interleaver 地址 ROM（只读存储器，Read-Only Memory）验证 | `f1/f2` 和派生 $\pi(k)$ 地址 dump。 |
...
| CB 重组顺序错 | 单个 CB CRC pass，但 TB CRC fail。 | 按 [[T7.4_LTE_code_block_reassembly_TB_CRC|T7.4]] 检查 $K_-$ 在前、$K_+$ 在后和 filler 删除。 |
...
| LTE Turbo encoder | TS 36.212 §5.1.3.2.1、Figure 5.1.3-2 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md`；[[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]] 重建图 `docs/L2_协议算法/assets/T6.3_TS36.212_Figure_5.1.3-2_turbo_encoder_rebuild.svg` | 已重建图。 |
| LTE trellis termination | TS 36.212 §5.1.3.2.2 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md` | 已核验锚点。 |
| LTE Turbo internal interleaver | TS 36.212 §5.1.3.2.3、Table 5.1.3-3 | `tables/table_0009.csv/html`；[[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 重建资产 | 已重建上游资产。 |
