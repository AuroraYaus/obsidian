...
若尚未掌握 LLR 的正负号含义、partial sum 为什么不是普通 bit 缓存，先回看 [[T10.4_Polar_SC_decoding|T10.4]] 和 [[T10.5_Polar_SCL_decoding|T10.5]]。本节默认你已经理解 Polar 树的算法行为；这里解决的是 ASIC 中如何把它做成可定时、可复现、可定位的结构。
...
- 使用 [[T17.4_NR_Polar_float_sim_plan|T17.4]] 规定的 AWGN 信道、调制方案和 BLER 基线，验证浮点 SC/SCL 的 BLER 曲线与 T10 理论推导一致。
