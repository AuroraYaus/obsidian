...
| LTE 协议精读与工程章节 | `docs/L2_协议算法/T7.*.md`, `docs/L3_工程实现/T17.2_*`, `docs/L3_工程实现/T18.2_*` | 本节只汇总回归字段，不重写 T7/T18.2 的完整推导。 |
...
- `docs/L3_工程实现/assets/T13.6_bit_exact_regression_harness.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/figures/render_t13_6_bit_exact_regression_harness.py`）
...
```bash
python3 /tmp/audit_svg_geometry.py docs/L3_工程实现/assets/T13.6_bit_exact_regression_harness.svg
# PASS（text 宿主/相交/宽度、text-text 间距、箭头终点、viewBox 越界全部通过）

python3 -c "import cairosvg; cairosvg.svg2png(url='docs/L3_工程实现/assets/T13.6_bit_exact_regression_harness.svg', write_to='/tmp/T13.6_bit_exact_regression_harness.png', scale=1.5)"
# 渲染无报错
