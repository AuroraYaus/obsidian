...
| NR LDPC 协议精读与工程章节 | `docs/L2_协议算法/T8.*.md`, `docs/L2_协议算法/T9.*.md`, `docs/L3_工程实现/T17.3_*`, `docs/L3_工程实现/T18.3_*` | 本节不重新定义 BG、$Z_c$ 或 lifting 表。 |
...
## 图示
![图 1：bit exact regression harness](assets/T18.6_bit_exact_regression_harness.svg)
...
- `docs/L3_工程实现/assets/T18.6_bit_exact_regression_harness.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t13_6_bit_exact_regression_harness.py`）
...
```bash
python3 /tmp/audit_svg_geometry.py docs/L3_工程实现/assets/T18.6_bit_exact_regression_harness.svg
# PASS（text 宿主/相交/宽度、text-text 间距、箭头终点、viewBox 越界全部通过）

python3 -c "import cairosvg; cairosvg.svg2png(url='docs/L3_工程实现/assets/T18.6_bit_exact_regression_harness.svg', write_to='/tmp/T18.6_bit_exact_regression_harness.png', scale=1.5)"
# 渲染无报错
