...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T18.6_bit_exact_regression_harness.md"
---
# T18.6 Bit-Exact 回归框架：从协议向量到 Python/C++/RTL 同步判定
## 本节学习目标

本节设计 bit-exact 回归框架。为了避免第一次出现的缩写含混，这里先把本节会反复使用的通信和硬件名词列清楚：3GPP规范给出空口编码、速率匹配、码块组织和控制信息规则；LTE在本项目中主要对应 Turbo（Turbo 码，Turbo Code）译码链；NR在本项目中主要对应低密度奇偶校验码和Polar（极化码，Polar Code）译码链。接收机输入的软信息通常写成对数似然比。译码后会检查循环冗余校验，上层对象包括传输块和码块。重传来自混合自动重传请求和冗余版本（Redundancy Version, RV）。后续硬件实现会落到寄存器传输级和专用集成电路。本节也会使用块错误率作为浮点与定点性能预算口径。


- 解释 bit-exact（比特精确，bit-exact） 的含义：在约定检查点上，两个实现输出的整数、有效位、饱和计数、选择顺序和状态字段逐项一致，而不是只看最终 CRC（循环冗余校验，Cyclic Redundancy Check）是否通过。
- 设计统一 vector schema，使 Python 浮点参考、Python 定点参考、C/C++ 定点模型和后续 RTL（寄存器传输级，Register Transfer Level）输出使用同一份协议元数据、随机种子、输入 LLR（对数似然比，Log-Likelihood Ratio）、量化配置和检查点清单。
- 区分“允许容差的比较”和“必须逐整数一致的比较”：浮点参考可以用于数值容差和 BLER（块错误率，Block Error Rate）预算，定点参考到 C/C++、C/C++ 到 RTL 必须 bit-exact。
- 为三类译码器列出检查点：Turbo 的 rate recovery、branch metric、alpha/beta、extrinsic、硬判决和 CRC；LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）的量化 LLR、CN min1/min2、layer trace、posterior、syndrome 和 CRC；Polar 的 rate recovery、f/g、partial sum、path metric、剪枝顺序和 CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）选择。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T4.6_decoder_interface_contracts|T4.6]] 译码器接口契约 | 知道 descriptor、input、output、status、error、debug/trace 分层，理解为什么元数据也是接口的一部分。 |
| T7/T8/T9/T10 协议译码链 | 能说明 LTE Turbo、NR LDPC、NR Polar 的接收侧速率恢复、码块、CRC、HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）/RV 和边界案例来自哪些上游章节。 |
| T12 浮点仿真 | 知道 `vector.json`、seed registry、metrics、failure dump 和 BLER 曲线的作用。 |
| [[T18.1_fixed_point_decoder_requirements|T18.1]] 定点需求 | 知道位宽、Q 格式、缩放、舍入、饱和、符号约定和性能损失预算必须写成需求。 |
| [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 定点模型 | 知道三类译码器各自有哪些内部整数对象和 bit-exact checkpoint。 |
| [[T18.5_SIMD_memory_layout_decoders|T18.5]] SIMD 与内存布局 | 知道 `layout_version`、`protocol_order_hash`、tail mask 和 SIMD lane 不能改变协议逻辑顺序。 |

若你还停留在“最终 CRC 通过就说明实现对了”的层次，本节是一个转折点。真实工程里，最终结果一致但内部检查点不同，可能来自补偿性错误、tie-breaker 差异、饱和策略不同或随机排序不稳定。这些差异在当前样本上未必暴露，换一个 SNR（信噪比，Signal-to-Noise Ratio）、RV、码长或 list size 就可能变成难定位的系统缺陷。
...
| Prompt | 请设计 bit-exact 回归框架，对比 Python 浮点/定点参考、C/C++ 定点模型和后续 RTL 输出。包含向量格式、元数据、种子追踪、容差、通过/失败方案和 CI 命令。写作时参考本文“单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
| LTE 协议精读与工程章节 | `docs/L2_协议算法/T7.*.md`, `docs/L3_工程实现/T17.2_*`, `docs/L3_工程实现/T18.2_*` | 本节只汇总回归字段，不重写 T7/[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 的完整推导。 |
| NR LDPC 协议精读与工程章节 | `docs/L2_协议算法/T8.*.md`, `docs/L2_协议算法/T9.*.md`, `docs/L3_工程实现/T17.3_*`, `docs/L3_工程实现/T18.3_*` | 本节不重新定义 BG（基图，Base Graph）、$Z_c$ 或 lifting 表。 |
| NR Polar 协议精读与工程章节 | `docs/L2_协议算法/T10.*.md`, `docs/L3_工程实现/T17.4_*`, `docs/L3_工程实现/T18.4_*` | 本节不重新选择 reliability sequence 或 list policy。 |
| SIMD/内存布局章节 | `docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md` | 本节不做真实 profiling，只规定回归需要记录的布局证据。 |
...
“exact” 的字面意思是完全一致，但工程上要先问：在哪个对象上完全一致？浮点数天然会受到舍入、库函数、平台和表达式重排影响，因此 Python 浮点参考与定点实现通常不要求每一个中间浮点值逐 bit 一致。它的角色是建立算法趋势、无噪声样本、BER（比特错误率，Bit Error Rate）/BLER 曲线和定点损失预算。
...
![图 1：Bit-Exact 回归流水线——协议向量经 Python Float、Python Fixed、C/C++ Fixed、RTL Output 四层实现，Compare Core 先验 metadata/hash 再比较检查点与最终状态，四类比较规则（Float vs fixed、Py fixed vs C/C++、C/C++ vs RTL、Metadata），CI Gate 自动执行套件、Failure Bundle 保存可回放证据](assets/T18.6_bit_exact_regression_harness.svg)
...
| 译码器 | 必填协议字段 | 作用 |
|:---|:---|:---|
| LTE Turbo | `K`、`D=K+4`、`f1`、`f2`、`E`、`Ncb`、`rv_idx`、`k0`、`null_mask_path`、`tail_bits_present`、`tb_crc_type`、`cb_crc_type` | 复现 TS 36.212 内部交织器、tail bits、rate recovery、`<NULL>` 跳过和 CB（码块，Code Block）/TB（传输块，Transport Block） CRC 检查点。 |
| NR LDPC | `base_graph`、`z_c`、`lifting_set`、`K`、`N`、`E`、`Ncb`、`k0`、`filler_mask_path`、`punctured_mask_path`、`cbg_id`、`cbgti`、`cbgfi` | 复现 TS 38.212 BG/lifting、circular buffer、limited-buffer、filler/puncturing、CBG（码块组，Code Block Group）部分重传和 syndrome/CRC 检查点。 |
...
其中 $H$ 可以是项目固定的哈希派生函数。真正重要的不是哈希算法名字，而是每个派生种子都能从 manifest 里重建。若 Polar SCL（连续消除列表译码，Successive Cancellation List）有两个 path metric 完全相同，排序 tie-breaker 必须由固定规则决定，不能依赖容器遍历顺序。若 LDPC layered schedule 有并行 edge 更新，trace 输出顺序也必须排序稳定。
...
| `ldpc.cn_update` | CN sign、min1、min2、argmin、NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum）参数。 | Min-Sum 族算法最容易在 tie-breaker 和饱和上分叉。 |
...
| `polar.fg` | SC（逐次消除，Successive Cancellation）/SCL 节点的 f/g 输入输出 LLR。 | 定位符号、min-sum、加法和饱和差异。 |
| `polar.partial_sum` | partial sum 更新、path state 映射、lazy-copy 引用。 | 发现 path copy 和节点回写错误。 |
| `polar.pm_prune` | path metric、候选排序、剪枝顺序、tie-breaker。 | SCL/CA-SCL（CRC 辅助 SCL，CRC-aided SCL）最常见不一致来源。 |
...
| 对象 | 协议/本地证据 | 本节如何使用 |
|:---|:---|:---|
| LTE Turbo 协议向量 | TS 36.212 Rel-19 `36212-j30`，路径 `3GPP_Rel19/processed/TS_36.212_36212-j30`；上游 T7/[[T17.2_LTE_Turbo_float_sim_plan|T17.2]]/[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] | vector schema 的 `standard=lte`、Turbo checkpoint、CB/TB CRC、HARQ/RV 和 soft buffer 字段回链。 |
| NR LDPC 协议向量 | TS 38.212 Rel-19 `38212-j30`，路径 `3GPP_Rel19/processed/TS_38.212_38212-j30`；上游 T8/T9/[[T17.3_NR_LDPC_float_sim_plan|T17.3]]/[[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] | vector schema 的 `standard=nr`、LDPC BG/$Z_c$/rate recovery/layered checkpoint 回链。 |
| NR Polar 协议向量 | TS 38.212 Rel-19 `38212-j30`，路径 `3GPP_Rel19/processed/TS_38.212_38212-j30`；上游 T10/[[T17.4_NR_Polar_float_sim_plan|T17.4]]/[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] | vector schema 的 Polar frozen/info set、rate recovery、SCL/CA-SCL、CRC/RNTI selector 回链。 |
| 定点需求 | `docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md` | 提供位宽、舍入、饱和、BLER budget 和 `regression.*` namespace。 |
| C/C++ 定点模型 | `docs/L3_工程实现/T18.2_*`, `docs/L3_工程实现/T18.3_*`, `docs/L3_工程实现/T18.4_*` | 提供各算法内部 checkpoint 和失败字段。 |
| SIMD/布局 | `docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md` | 提供 layout hash、tail mask、protocol order hash 和 scalar/SIMD compare 字段。 |
...
本节规定的回归命令是模板，当前仓库尚未实现 `decoder_regress` 可执行工具；因此本次可执行证据集中在讲义审计、LaTeX 渲染和手绘 SVG 的几何审计与渲染验证。后续若实现真实 regression runner，应把命令输出、suite manifest、failure bundle 和 CI job URL 回写到本节或 [[T20.6_final_decoder_verification_evidence_report|T20.6]] 证据报告。
...
python3 tools/audit_lesson_terms.py docs/L3_工程实现/T18.6_bit_exact_regression_harness.md
# LESSON_TERM_AUDIT_OK

python3 tools/audit_markdown_headings.py docs/L3_工程实现/T18.6_bit_exact_regression_harness.md
# MARKDOWN_HEADING_AUDIT_OK

python3 tools/audit_lesson_depth.py --strict docs/L3_工程实现/T18.6_bit_exact_regression_harness.md
# LESSON_DEPTH_AUDIT_OK

python3 tools/audit_latex_render.py docs/L3_工程实现/T18.6_bit_exact_regression_harness.md
# LATEX_RENDER_AUDIT_OK formulas=13

python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T18.6_bit_exact_regression_harness.md
...
- `docs/L3_工程实现/T17.*.md`，浮点仿真、曲线、向量和失败回放计划。
- `docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md` 至 `docs/L3_工程实现/T18.5_SIMD_memory_layout_decoders.md`，定点需求、三类 C/C++ 定点模型计划和 SIMD/内存布局。
