...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md"
---
# T19.1 LTE TurboRTL 微架构：从 SISO 数据通路到 CRC 门控迭代控制
## 本节学习目标

本节设计LTE Turbo（Turbo 码，Turbo Code）译码器的寄存器传输级微架构。先把本篇会反复使用的术语列清：Turbo 码是 LTE 数据传输主线使用的并行级联卷积码；软输入软输出（Soft-Input Soft-Output, SISO）译码器是 Turbo 迭代译码中的半迭代核心；对数似然比是接收端输入的软信息；循环冗余校验用于判断译码结果是否通过；码块是 Turbo core 一次处理的对象；传输块是多个 CB（码块，Code Block）重组后的交付对象；混合自动重传请求负责重传和软合并上下文；块错误率是后续性能验收指标；静态随机存取存储器保存 LLR（对数似然比，Log-Likelihood Ratio）、alpha/beta 度量和外信息；有限状态机（Finite State Machine, FSM）控制加载、半迭代、CRC（循环冗余校验，Cyclic Redundancy Check）检查和输出；专用集成电路实现还要关注面积、功耗、时序和复位。


- 画出 LTE Turbo RTL（寄存器传输级，Register Transfer Level）的主数据通路：descriptor、rate recovery 输入、SISO core、alpha/beta 存储、外信息存储、交织/解交织地址生成、CRC gate 和 trace/status 输出。
- 解释哪些字段来自3GPP TS 36.212，哪些是接收机内部实现策略。
- 给出 alpha/beta SRAM（静态随机存取存储器，Static Random-Access Memory）、extrinsic ping-pong RAM、LLR buffer 和 mask buffer 的规模估算方法。
- 设计半迭代 FSM：`IDLE -> LOAD -> SISO A -> SISO B -> CRC CHECK -> DONE/ERROR`。
- 说明 CRC 早停为什么只能在完整迭代边界使用，不能在半个 SISO 后随意提交。
- 给出吞吐估算、时钟/复位策略、错误状态码、bit-exact（比特精确，bit-exact） trace 字段和最小验证计划。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T5.2_memory_banking_buffering_basics|T5.2]] 存储与 bank 基础 | 知道 SRAM、寄存器文件、ping-pong buffer、循环缓存和 bank conflict 的含义。 |
| [[T5.4_RTL_state_machine_handshake_basics|T5.4]] RTL 状态机与握手 | 知道 `valid/ready`、start/done/error、同步复位和状态转移条件。 |
| [[T6.4_LTE_Turbo_internal_interleaver|T6.4]] LTE Turbo 内部交织器 | 知道 $K/f_1/f_2$ 生成交织地址，两个 SISO 之间的外信息必须按地址对齐。 |
| [[T6.6_Log_MAP_Max_Log_MAP_Turbo|T6.6]] Log-MAP 与 Max-Log-MAP | 知道 alpha、beta、branch metric、posterior 和 extrinsic 的算法角色。 |
| [[T7.1_LTE_Turbo_de_rate_matching_overview|T7.1]]-[[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]] Rate Recovery 与 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） | 知道速率恢复后进入 Turbo core 的系统位和校验位 LLR，以及重传软合并的边界。 |
| [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] LTE Turbo 定点模型 | 知道整数位宽、归一化、饱和、trace checkpoint 和 C/C++ fixed model 的角色。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] Bit-Exact 回归框架 | 知道 RTL 输出必须和 C/C++ fixed trace 在 checkpoint 上逐整数比较。 |
...
| 编号 | T19.1 |
| 前置 | T5.2, T5.4, T18.2 |
| Prompt | 请设计 LTE Turbo RTL 微架构：SISO 数据通路、alpha/beta 存储、外信息存储、交织/解交织地址生成器、乒乓迭代控制、CRC 早停、时钟/复位方案和吞吐估算。写作时参考本文“单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
本节是工程微架构课，不实现真实 SystemVerilog 源码。正文会给结构、接口、状态机、存储估算、伪代码和验证计划。真实 RTL 编码、testbench、覆盖率和综合会在 T20 系列承接。
...
| TS 36.212 §5.1.3.2.3/Table 5.1.3-3 | `content.md:761-773`，`tables/table_0009.csv/html` | 本节回链 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 的表格复现，不重复完整长表。 |
...
| [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] | `docs/L3_工程实现/T18.2_LTE_Turbo_fixed_point_model_plan.md` | T19.1 把这些字段映射到硬件结构。 |
...
![图 1：LTE Turbo RTL 微架构——Task Descriptor 经 Rate Recovery、SISO Core A/B（乒乓半迭代）、Extrinsic RAM 到 CRC Gate；中部为 RTL 存储与地址对象，底部为乒乓迭代 FSM](assets/T19.1_LTE_Turbo_RTL_microarchitecture.svg)

| 内容 | 说明 |
|:---|:---|
| Task Descriptor | Rate Recovery；SISO Core A/B；Extrinsic RAM；CRC Gate；Status/Trace |
| K, f1, f2, E, Ncb, RV（冗余版本，Redundancy Version）, max_iter, scale and bitwidth profile. | Read soft buffer, place system/parity LLR and valid/null masks.；Branch metric, alpha, beta, posterior and extrinsic update.；Ping-pong apriori/extrinsic storage with pi/depi address maps.；Hard decision, CB CRC, optional e |
...
| Interleaver | f1/f2 polynomial address generator or ROM（只读存储器，Read-Only Memory）；Must align with SISO latency and stall；pi/depi hash, addr_valid |
...
| `crc_gate` | hard bits、crc_type、iter_count | `crc_pass`、`stop_reason` | 完整迭代后检查 CB CRC，决定是否 early stop。 | CRC 多项式由 [[T3.1_LTE_NR_CRC_families|T3.1]]/T7 回链；早停策略是实现策略。 |
| `trace_status` | checkpoint signals | trace/status/error | 输出 bit-exact 对比字段、饱和计数、首个错误位置。 | [[T18.6_bit_exact_regression_harness|T18.6]] schema 约束可回放性。 |
...
如果漏掉 $L_{\mathrm{a},k}$，第二个 SISO 会把自己上一轮给出的证据再听一遍，迭代可能看似更快收敛，但实际是过度自信。RTL 中这通常表现为 extrinsic 饱和计数很高、CRC 偶尔提前通过、换一个 RV 或噪声点后 BLER（块错误率，Block Error Rate）变差。
...
Turbo decoder 的两个 SISO 观察同一批信息位，但第二个 SISO 按内部交织后的顺序观察。TS 36.212 Table 5.1.3-3 给出合法 $K$ 对应的 $f_1$ 和 $f_2$。内部交织地址由上游 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]]/[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 承接，RTL 侧可以有两种实现：
...
T19.1 不假设多时钟域，教学架构使用单个 `clk` 和同步 active-high reset。若 SoC 顶层后来把 soft buffer、DMA（直接存储器访问，Direct Memory Access）或 register bus 放在不同 clock domain，必须在 [[T19.4_unified_decoder_subsystem_architecture|T19.4]]/T20 中引入 CDC 同步和异步 FIFO 证明。
...
RTL 位宽必须继承 [[T18.1_fixed_point_decoder_requirements|T18.1]]/[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 的 fixed requirement，不能在硬件里单独“看起来够用”就改。最低字段：
...
定点最容易错的不是加法器，而是策略不一致：Python fixed 用 nearest rounding，RTL 用 truncation；C++ 每列减最大值，RTL 每 4 列归一化一次；Python 对 extrinsic 先缩放再裁剪，RTL 先裁剪再缩放。这些都必须进入 [[T18.6_bit_exact_regression_harness|T18.6]] 的 `bit_exact_policy_hash`。

## RTL/ASIC 映射

| 结构 | ASIC 关注点 | 设计建议 |
|:---|:---|:---|
| SISO ACS/max-star datapath | 比较器树、修正 LUT（查找表，Look-Up Table）、critical path | Max-Log-MAP 可先落地；Log-MAP 修正 LUT 做可选 mode。 |
...
| Interleaver address | 固定 $K/f_1/f_2$，遍历 $k$ | `pi/depi` 与 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]]/[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] hash 一致。 |
...
| Turbo internal interleaver | TS 36.212 §5.1.3.2.3/Table 5.1.3-3，`content.md:761-773`，`tables/table_0009.csv/html` | 说明 $K/f_1/f_2$ 是地址生成器输入，完整表由 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 复现。 |
...
cairosvg -s 1.5 渲染预览无报错，输出 3600x3210 PNG。
```
...
```bash
python3 tools/audit_lesson_terms.py docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md
# LESSON_TERM_AUDIT_OK

python3 tools/audit_markdown_headings.py docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md
# MARKDOWN_HEADING_AUDIT_OK

python3 tools/audit_lesson_depth.py --strict docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md
# LESSON_DEPTH_AUDIT_OK

python3 tools/audit_latex_render.py docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md
# LATEX_RENDER_AUDIT_OK formulas=55

python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md
# REFERENCE_REBUILD_AUDIT_CANDIDATES
```

引用重建审计输出为候选清单：主要来自 TS 36.212 Table 5.1.3-3 的上游已复现回链、协议边界表和工程公式/图说明；本节未直接复制新的协议长表。若后续 RTL 生成脚本直接解析 Table 5.1.3-3，应把 `tables/table_0009.csv/html` 的 hash、解析脚本和使用行号纳入 T20 evidence archive。

## 参考文献

- 3GPP TS 36.212 Rel-19，本地路径 `3GPP_Rel19/processed/TS_36.212_36212-j30`。
- `docs/L2_协议算法/T3.3_LTE_Turbo_segmentation_rules.md`，LTE Turbo 合法块长和内部交织器表格复现。
- `docs/L2_协议算法/T6.4_LTE_Turbo_interleaver_QPP.md`，QPP interleaver 地址生成。
- `docs/L2_协议算法/T6.6_Log_MAP_Max_Log_MAP_Turbo.md`，Log-MAP、Max-Log-MAP 和外信息推导。
- `docs/L2_协议算法/T7.1_LTE_Turbo_de_rate_matching_overview.md` 至 `docs/L2_协议算法/T7.3_LTE_HARQ_soft_buffer_RV.md`，rate recovery、HARQ/RV 和 soft buffer。
- `docs/L3_工程实现/T18.2_LTE_Turbo_fixed_point_model_plan.md`，LTE Turbo C/C++ 定点模型计划。
- `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md`，bit-exact 回归框架与 failure bundle。
