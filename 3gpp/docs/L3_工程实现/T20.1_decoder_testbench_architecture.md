...
本节设计译码器寄存器传输级验证中的 SystemVerilog 测试平台（testbench，驱动待测设计并与参考模型比对、自动判定通过/失败的验证环境）架构。这里的 testbench 不是“写一个 initial block 跑一下”，而是一套能把 3GPP 协议向量、定点参考模型、统一译码子系统、三类译码引擎和失败证据串起来的验证外壳。DUT 可以是 LTE Turbo 单引擎、NR LDPC 单引擎、NR Polar 单引擎，也可以是 [[T19.4_unified_decoder_subsystem_architecture|T19.4]] 的统一译码子系统。
...
- 把 [[T18.6_bit_exact_regression_harness|T18.6]] 的 `vector.json`、checkpoint manifest 和 failure bundle 映射到 RTL 验证对象。
...
| [[T5.5_decoder_hardware_verification_mindset|T5.5]] 硬件验证思维 | 知道 golden model、bit-exact、directed/random、coverage、波形和 failure replay 的基本目的。 |
...
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] LTE Turbo RTL 微架构 | 知道 Turbo engine 的 rate recovery、SISO、alpha/beta、extrinsic、CRC checkpoint。 |
| [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] NR LDPC RTL 微架构 | 知道 LDPC engine 的 BG、$Z_c$、layered schedule、CN/VN update、syndrome checkpoint。 |
| [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] NR Polar RTL 微架构 | 知道 Polar engine 的 rate recovery、SC/SCL、path metric、sort/prune、CRC/RNTI final selector。 |
| [[T19.4_unified_decoder_subsystem_architecture|T19.4]] 统一译码子系统 | 知道 descriptor dispatcher、input/output DMA、soft buffer manager、status/IRQ/trace 和独立 engine 边界。 |
...
| 为什么 HARQ soft buffer 要测试 reset/abort 中的半提交？ | [[T19.5_soft_buffer_HARQ_memory_architecture|T19.5]]。 |
