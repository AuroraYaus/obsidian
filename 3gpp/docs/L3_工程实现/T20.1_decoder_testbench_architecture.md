...
本节设计译码器寄存器传输级验证中的 SystemVerilog 测试平台（testbench，驱动待测设计并与参考模型比对、自动判定通过/失败的验证环境）架构。这里的 testbench 不是“写一个 initial block 跑一下”，而是一套能把 3GPP 协议向量、定点参考模型、统一译码子系统、三类译码引擎和失败证据串起来的验证外壳。DUT（待测设计，Device Under Test）可以是 LTE Turbo（Turbo 码，Turbo Code）单引擎、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）单引擎、NR Polar（极化码，Polar Code）单引擎，也可以是 [[T19.4_unified_decoder_subsystem_architecture|T19.4]] 的统一译码子系统。
...
- 把 [[T18.6_bit_exact_regression_harness|T18.6]] 的 `vector.json`、checkpoint manifest 和 failure bundle 映射到 RTL（寄存器传输级，Register Transfer Level）验证对象。
- 把 [[T19.6_decoder_register_map_configuration_flow|T19.6]] 的寄存器配置流映射成 testbench driver：写寄存器、锁存 descriptor、启动任务、等待 done/error、读取 status/trace。
- 给出三类译码器的最小 directed tests：LTE Turbo、NR LDPC、NR Polar 分别要抓什么 bug。
- 设计 pass/fail 口径：payload、CRC（循环冗余校验，Cyclic Redundancy Check）/syndrome、status/error/IRQ、descriptor hash、checkpoint、latency、饱和计数和 timeout 都要参与判断。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T5.4_RTL_state_machine_handshake_basics|T5.4]] RTL 状态机与握手 | 理解 `valid/ready`、start/done/error、reset、timeout 和 backpressure。 |
| [[T5.5_decoder_hardware_verification_mindset|T5.5]] 硬件验证思维 | 知道 golden model（黄金模型，Golden Model）、bit-exact（比特精确，bit-exact）、directed/random、coverage、波形和 failure replay 的基本目的。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] Bit-exact 回归框架 | 知道 `vector.json`、checkpoint manifest、descriptor hash、first mismatch 和 failure bundle。 |
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] LTE Turbo RTL 微架构 | 知道 Turbo engine 的 rate recovery、SISO（软入软出，Soft-Input Soft-Output）、alpha/beta、extrinsic、CRC checkpoint。 |
| [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] NR LDPC RTL 微架构 | 知道 LDPC engine 的 BG（基图，Base Graph）、$Z_c$、layered schedule、CN/VN update、syndrome checkpoint。 |
| [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] NR Polar RTL 微架构 | 知道 Polar engine 的 rate recovery、SC（逐次消除，Successive Cancellation）/SCL（连续消除列表译码，Successive Cancellation List）、path metric、sort/prune、CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）final selector。 |
| [[T19.4_unified_decoder_subsystem_architecture|T19.4]] 统一译码子系统 | 知道 descriptor dispatcher、input/output DMA（直接存储器访问，Direct Memory Access）、soft buffer manager、status/IRQ/trace 和独立 engine 边界。 |
| [[T19.6_decoder_register_map_configuration_flow|T19.6]] 寄存器表与配置流 | 知道配置字段、shadow lock、legality check、busy 写保护、error code 和 IRQ 清除。 |

自检问题：

| 问题 | 若回答不出来，先回看 |
|:---|:---|
| 为什么最终 CRC pass 不能替代 bit-exact checkpoint 比较？ | [[T18.6_bit_exact_regression_harness|T18.6]]。 |
| 为什么 BUSY 中写配置必须由断言或 directed test 抓住？ | [[T19.6_decoder_register_map_configuration_flow|T19.6]]。 |
| 为什么 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）soft buffer 要测试 reset/abort 中的半提交？ | [[T19.5_soft_buffer_HARQ_memory_architecture|T19.5]]。 |
| 为什么 Polar SCL 的 path metric tie-breaker 要进入 scoreboard？ | [[T10.5_Polar_SCL_decoding|T10.5]]、[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]]、[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]]。 |
...
| 证据边界 | testbench methodology 不需要直接 3GPP 引用；生成向量必须回链 T7、T9、T10、[[T20.2_protocol_vector_corner_case_suite|T20.2]] 的 Rel-19 证据。 |

本节是工程验证架构课。3GPP 规范不规定你的 SystemVerilog class、interface、mailbox、scoreboard 算法或 timeout cycle 数；协议只规定向量中 TB（传输块，Transport Block）/CB（码块，Code Block）/CBG（码块组，Code Block Group）、CRC、rate matching、HARQ/RV（冗余版本，Redundancy Version）、LDPC BG/$Z_c$、Polar construction 等语义。本节规定如何在 RTL 仿真中证明 DUT 按这些语义工作。
...
| TS 38.212 Rel-19 `38212-j30` | `3GPP_Rel19/processed/TS_38.212_38212-j30` | 不规定 RTL trace 端口或 SVA（系统 Verilog 断言，SystemVerilog Assertion）。 |
...
协议前因后果是：T7/T9/T10 负责回答“这条向量为什么合法、按协议应该输出什么”；[[T18.6_bit_exact_regression_harness|T18.6]] 负责回答“参考模型和 RTL 应该在哪些 checkpoint 一致”；[[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]]-[[T19.6_decoder_register_map_configuration_flow|T19.6]] 负责回答“硬件有哪些接口、状态和错误码”；T20.1 把这些对象放到 SystemVerilog testbench 中，形成可执行验证闭环。
...
| 特点 | 验证后果 |
|:---|:---|
| 输入是长 LLR（对数似然比，Log-Likelihood Ratio）stream，不是一拍数据。 | driver 必须产生 DMA beat、valid/ready、last、mask 和 backpressure。 |
...
![图 1：Testbench 总体架构——Reference Vectors 经 Vector Loader、SV Driver 驱动 Decoder DUT，SV Monitor 采样输出，Scoreboard 对齐比较；Assertions 与 Failure Bundle 支撑验证闭环](assets/T20.1_decoder_testbench_architecture.svg)
...
reference vector loader 是 testbench 的入口。它把 [[T18.6_bit_exact_regression_harness|T18.6]] 的文件包变成 SystemVerilog 可消费的事务对象。
...
真实实现可以拆分成 class 或 interface，但核心字段必须和 [[T19.6_decoder_register_map_configuration_flow|T19.6]] 的配置流对齐。若 `descriptor_hash` 在 loader、driver log、DUT trace 中不同，scoreboard 必须优先报 descriptor mismatch，而不是继续比较 payload。
...
假设 [[T20.2_protocol_vector_corner_case_suite|T20.2]] 后续提供一条 NR LDPC directed vector：`decoder_family=ldpc`，`BG=1`，`Zc=192`，`rv=2`，`cbg_id=1`，`CBGTI=0100`，`CBGFI=1`，HARQ soft buffer 中已经有 RV0 的旧 LLR。这个例子不新增协议表值，只用 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]/[[T19.5_soft_buffer_HARQ_memory_architecture|T19.5]] 已讲过的 CBG/RV 语义展示 testbench 如何工作。
...
| RV/k0 | RV0/1/2/3，固定 `E`。 | `ldpc.rate_recovery.k0_hash`。 | k0（速率匹配起点，Rate matching starting position）公式、Zc 对齐或 circular buffer 起点错。 |
...
本节结构可以映射到 UVM（通用验证方法学，Universal Verification Methodology），但不强制项目当前必须使用 UVM：
...
本节不重新定义浮点仿真。比较链路应继承 [[T18.6_bit_exact_regression_harness|T18.6]]：

| 比较对象 | RTL testbench 中的处理 |
|:---|:---|
| Python float vs Python fixed | 在向量生成阶段完成，产出定点 expected 和性能预算。 |
| Python fixed vs C/C++ fixed | 由 [[T18.6_bit_exact_regression_harness|T18.6]] 回归完成，产出 checkpoint manifest 和 expected trace。 |
...
T20.1 是验证架构，但它反向约束 RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）可观测性：
...
| 协议回链 | 向量生成回链 T7/T9/T10/[[T20.2_protocol_vector_corner_case_suite|T20.2]] 和 Rel-19 本地路径；testbench 方法不冒充 3GPP 规定。 |
...
| 只比较最终 payload | 内部 checkpoint 漏检，CRC fail 难定位。 | 按 [[T18.6_bit_exact_regression_harness|T18.6]] manifest 比较 trace。 |
...
| 结论 | 证据 | 本地路径 | 边界 |
|:---|:---|:---|:---|
| LTE Turbo RTL 向量必须回链 LTE CB segmentation、Turbo interleaver、rate matching 和 CRC。 | TS 36.212 Rel-19 `36212-j30`；项目 T7/[[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]]。 | `3GPP_Rel19/processed/TS_36.212_36212-j30`、`docs/L2_协议算法/T7.*.md`、`docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md` | 本节不重建协议表；[[T20.2_protocol_vector_corner_case_suite|T20.2]] 后续定义 exact vector suite。 |
| NR LDPC RTL 向量必须回链 BG/$Z_c$、rate recovery、RV/k0、CBG 和 CRC/syndrome。 | TS 38.212 Rel-19 `38212-j30`、TS 38.214 Rel-19 `38214-j30`；项目 T8/T9/[[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]]。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`TS_38.214_38214-j30`、`docs/L2_协议算法/T8.*.md`、`docs/L2_协议算法/T9.*.md`、`docs/L3_工程实现/T19.2_NR_LDPC_RTL_microarchitecture.md` | testbench 只消费已生成向量和 descriptor。 |
| NR Polar RTL 向量必须回链 reliability/frozen mask、rate recovery、CRC/RNTI 和 SCL/CA-SCL（CRC 辅助 SCL，CRC-aided SCL）选择语义。 | TS 38.212 Rel-19 `38212-j30`；项目 T10/[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]]。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`docs/L2_协议算法/T10.*.md`、`docs/L3_工程实现/T19.3_NR_Polar_RTL_microarchitecture.md` | list size、PM 近似、sorter 是实现策略。 |
| vector schema、checkpoint manifest、failure bundle 的字段来自项目 bit-exact 框架。 | [[T18.6_bit_exact_regression_harness|T18.6]]。 | `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md` | 工程回归框架，不是 3GPP 条文。 |
| register/status/error/IRQ/trace 驱动对象来自统一子系统和寄存器配置流。 | [[T19.4_unified_decoder_subsystem_architecture|T19.4]]、[[T19.6_decoder_register_map_configuration_flow|T19.6]]。 | `docs/L3_工程实现/T19.4_unified_decoder_subsystem_architecture.md`、`docs/L3_工程实现/T19.6_decoder_register_map_configuration_flow.md` | 真实端口名、总线协议和地址以 RTL 实现为准。 |
...
- [Project, 2026] [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] LTE Turbo RTL microarchitecture, `docs/L3_工程实现/T19.1_LTE_Turbo_RTL_microarchitecture.md`.
- [Project, 2026] [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] NR LDPC RTL microarchitecture, `docs/L3_工程实现/T19.2_NR_LDPC_RTL_microarchitecture.md`.
- [Project, 2026] [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] NR Polar RTL microarchitecture, `docs/L3_工程实现/T19.3_NR_Polar_RTL_microarchitecture.md`.
...
- [Project, 2026] [[T19.6_decoder_register_map_configuration_flow|T19.6]] Decoder register map and configuration flow, `docs/L3_工程实现/T19.6_decoder_register_map_configuration_flow.md`.
