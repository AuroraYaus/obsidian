...
本节进入定点（fixed-point，用有限位宽整数近似实数的数值表示，[[Fixed_Point_Numbers_定点数]]）C/C++ 模型阶段。前面的 T12 模块已经规定了LTE Turbo、NR LDPC和 NR Polar的浮点仿真、随机种子、日志、曲线和失败回放；但浮点模型还不能直接交给寄存器传输级和专用集成电路实现。硬件保存的是有限位宽整数，C/C++ 定点模型必须提前规定这些整数怎样表示、怎样相加、怎样饱和、怎样和浮点基线比较。
...
| [[T17.2_LTE_Turbo_float_sim_plan|T17.2]]-[[T17.5_BER_BLER_curve_reporting|T17.5]] 浮点仿真计划 | 知道浮点基线如何输出 BER/BLER 曲线、置信区间、停止原因和失败样本。 |
...
这只是公共 LLR 需求片段，不是所有译码器的最终位宽。`null` 表示该字段由后续专用章节关闭，不能在实现阶段默默省略。[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 会补 Turbo 的分支度量、alpha/beta 和外信息；[[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] 会补 LDPC 的 CN/VN 消息、min1/min2、NMS/OMS 参数；[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 会补 Polar 的 f/g、路径度量、部分和和列表剪枝；[[T18.5_SIMD_memory_layout_decoders|T18.5]] 会补 SIMD lane、bank 和 packing；[[T18.6_bit_exact_regression_harness|T18.6]] 会补 checkpoint manifest 和 failure bundle。
...
| NR LDPC 协议字段 | 回链 T9/[[T17.3_NR_LDPC_float_sim_plan|T17.3]] 和 TS 38.212 Rel-19 `38212-j30`。 | [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] 必须列 BG、$Z_c$、RV、CBG 等字段来源。 |
| NR Polar 协议字段 | 回链 T10/[[T17.4_NR_Polar_float_sim_plan|T17.4]] 和 TS 38.212 Rel-19 `38212-j30`。 | [[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 必须列 $A/K/E/N/L$、CRC/RNTI 和 rate recovery 来源。 |
| 定点位宽方法 | 非 3GPP 协议内容。 | 不需要协议表格；需要 Python/C/RTL 一致性证据。 |

## 图示
![图 1：fixed point decoder requirements](assets/T18.1_fixed_point_decoder_requirements.svg)
...
| 新增证据资产 | `docs/L3_工程实现/assets/T18.1_fixed_point_decoder_requirements.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t13_1_fixed_point_requirements.py`）。 |
