...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md"
---
# T18.1 定点译码器需求：浮点实验到整数模型规格
## 本节学习目标

本节进入定点（fixed-point，用有限位宽整数近似实数的数值表示，[[Fixed_Point_Numbers_定点数]]）C/C++ 模型阶段。前面的 T12 模块已经规定了LTE Turbo（Turbo 码，Turbo Code）、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）和 NR Polar（极化码，Polar Code）的浮点仿真、随机种子、日志、曲线和失败回放；但浮点模型还不能直接交给寄存器传输级和专用集成电路实现。硬件保存的是有限位宽整数，C/C++ 定点模型必须提前规定这些整数怎样表示、怎样相加、怎样饱和、怎样和浮点基线比较。

本节讲“定点译码器需求”。它不是某个单独算法，而是一份所有后续定点模型必须遵守的工程契约。

- 解释为什么对数似然比不能只写“用 6 bit”而不说明符号、小数位、裁剪、缩放和饱和。
- 为 LTE Turbo、NR LDPC、NR Polar 分别列出输入 LLR（对数似然比，Log-Likelihood Ratio） 位宽、内部消息位宽、路径度量或外信息位宽的需求字段。
- 区分浮点裁剪、整数饱和、舍入模式、符号扩展和溢出回绕。
- 给出块错误率损失预算，并说明为什么预算必须绑定置信区间、样本数和浮点基线。
- 设计 bit-exact 对比方案，说明哪些检查点必须逐整数一致，哪些性能指标允许统计比较。
- 写出一份后续 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]、[[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]]、[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]]、[[T18.6_bit_exact_regression_harness|T18.6]] 可直接复用的 fixed-point requirement 模板。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T2.16_LLR_clipping_scaling_quantization|T2.16]] LLR 裁剪、缩放与量化 | 知道裁剪、缩放、量化、饱和和符号错误的基本含义。 |
| [[T5.1_fixed_point_numbers_for_LLR|T5.1]] LLR 处理中的定点数 | 知道二进制补码、Q 格式、小数位、饱和加法和符号扩展。 |
| [[T4.6_decoder_interface_contracts|T4.6]] 译码器接口契约 | 知道 descriptor、input、output、status、error、debug 六类对象，以及传输块、码块、混合自动重传请求和冗余版本（Redundancy Version, RV）字段。 |
| [[T17.1_python_golden_model_project_layout|T17.1]] Python Golden Model 工程布局 | 知道 `vector.json`、`metrics.csv`、`frames.jsonl`、seed registry、failure dump 和 replay command 的角色。 |
| [[T17.2_LTE_Turbo_float_sim_plan|T17.2]]-[[T17.5_BER_BLER_curve_reporting|T17.5]] 浮点仿真计划 | 知道浮点基线如何输出 BER（比特错误率，Bit Error Rate）/BLER（块错误率，Block Error Rate） 曲线、置信区间、停止原因和失败样本。 |
...
| 编号 | T18.1 |
| 前置 | T5.1, T17.1 |
| Prompt | 请定义定点译码器需求：LLR 位宽、内部消息位宽、饱和规则、舍入、缩放、性能损失预算和 bit-exact 对比方案。写作时参考本文”单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
| 缺失项 | 常见说法 | 实际风险 |
|:---|:---|:---|
| LLR 符号约定 | “输入是 6 bit LLR。” | 正值到底表示比特 `0` 还是 `1` 不清楚，高信噪比下可能系统性 CRC（循环冗余校验，Cyclic Redundancy Check） 失败。 |
...
![图 1：定点译码器位宽需求总览——Float Baseline 经 Quantization Contract、Fixed Model 到 Regression Evidence；五类需求（LLR format、Internal messages、Arithmetic、Loss budget、Compare policy）各含 Required Fields / Evidence / Failure if Missing](assets/T18.1_fixed_point_decoder_requirements.svg)
...
| Soft Buffer | HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） accumulation, saturation count, repeat merge |
...
| 专用扩展 | `turbo.*`、`ldpc.*`、`polar.*`、`simd_memory.*`、`regression.*` | 后续 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.6_bit_exact_regression_harness|T18.6]] 的固定 namespace。公共表不允许把专用字段散落成临时名字。 |

公共字段只解决三类译码器共同拥有的数值契约。后续文档必须在固定 namespace 下继续扩展，而不是重新发明字段名：

| namespace | 后续章节 | 必须预留的字段 |
|:---|:---|:---|
| `turbo.*` | [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] LTE Turbo 定点模型 | `branch_metric_width/frac`、`alpha_width/frac`、`beta_width/frac`、`posterior_width/frac`、`extrinsic_width/frac`、`max_log_mode`、`maxstar_lut_id`、`alpha_norm_policy`、`beta_norm_policy`、`interleaver_addr_policy`、`tail_state_policy`。 |
| `ldpc.*` | [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] NR LDPC 定点模型 | `cn_msg_width/frac`、`vn_msg_width/frac`、`posterior_width/frac`、`min1_width`、`min2_width`、`sign_product_policy`、`nms_alpha_q`、`oms_beta_q`、`layer_schedule_id`、`syndrome_check_policy`、`message_memory_layout`。 |
| `polar.*` | [[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] NR Polar 定点模型 | `fg_llr_width/frac`、`partial_sum_format`、`path_metric_width/frac`、`path_metric_norm_policy`、`list_size`、`list_pruning_policy`、`sorter_compare_width`、`crc_selector_policy`、`rnti_mask_policy`。 |
| `simd_memory.*` | [[T18.5_SIMD_memory_layout_decoders|T18.5]] SIMD 与内存布局 | `lane_count`、`lane_mapping`、`alignment_bytes`、`bank_count`、`bank_conflict_policy`、`transpose_policy`、`packing_order`、`endian_policy`、`dma_burst_bytes`。 |
| `regression.*` | [[T18.6_bit_exact_regression_harness|T18.6]] Bit-Exact 回归 | `checkpoint_manifest`、`trace_format_version`、`first_mismatch_schema`、`golden_hash_policy`、`rerun_command_template`、`curve_budget_ref`、`failure_bundle_layout`。 |

这张扩展表的作用是约束命名和证据粒度。例如 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 如果需要记录 alpha 归一化，就写 `turbo.alpha_norm_policy`；[[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] 如果需要记录 layered schedule，就写 `ldpc.layer_schedule_id`；[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 如果需要记录路径剪枝，就写 `polar.list_pruning_policy`。这样 [[T18.6_bit_exact_regression_harness|T18.6]] 可以用同一个解析器读取需求，而不是为每个算法写一套临时规则。
...
这只是公共 LLR 需求片段，不是所有译码器的最终位宽。`null` 表示该字段由后续专用章节关闭，不能在实现阶段默默省略。[[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 会补 Turbo 的分支度量、alpha/beta 和外信息；[[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] 会补 LDPC 的 CN/VN 消息、min1/min2、NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum） 参数；[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 会补 Polar 的 f/g、路径度量、部分和和列表剪枝；[[T18.5_SIMD_memory_layout_decoders|T18.5]] 会补 SIMD lane、bank 和 packing；[[T18.6_bit_exact_regression_harness|T18.6]] 会补 checkpoint manifest 和 failure bundle。
...
| 饱和高风险处 | BCJR（BCJR 算法，Bahl-Cocke-Jelinek-Raviv Algorithm）/Max-Log-MAP 度量归一化和外信息缩放。 | layered 更新中旧消息扣除、新消息加回、NMS/OMS 后的消息累加。 | SCL（逐次消除列表，Successive Cancellation List） 路径分裂后的 PM 累加和 sorter/pruner 比较。 |
| bit-exact 检查点 | 每半迭代 alpha/beta/extrinsic 摘要、CRC 前硬判决。 | 每层 posterior、syndrome、CB（码块，Code Block） CRC 前硬判决。 | 每个 stage 的 LLR/partial sum、每次 path split 后 PM 和 CRC selector。 |
| 性能指标 | TB（传输块，Transport Block）/CB BLER、平均迭代次数、饱和比例。 | TB/CB BLER、syndrome zero rate、迭代次数、饱和比例。 | control BLER、false-pass rate、list survival、PM 饱和比例。 |

注意：这些字段中的协议身份来自上游讲义和 T12 descriptor，定点位宽本身是实现选择。比如 NR LDPC 的 base graph、$Z_c$、RV、CBG（码块组，Code Block Group） 状态会影响向量和缓存地址，但不会由 3GPP 直接给出“消息必须 7 bit”这种规定。

## 性能损失预算

定点模型必须比浮点模型差一点也可解释，但“差一点”必须量化。推荐从 [[T17.5_BER_BLER_curve_reporting|T17.5]] 的 `metrics.csv` 继承下面字段：
...
在早期没有足够曲线点时，也可以使用同一 SNR（信噪比，Signal-to-Noise Ratio） 下的比值预算：
...
但式 (13) 必须和置信区间一起看。若浮点点位是零错误且只有上界，不能用一个很小的分母得出“定点差 100 倍”的结论；应回到 [[T17.5_BER_BLER_curve_reporting|T17.5]] 的零错误上界和 `confidence_limited` 标记。
...
| 性能层 | BER/BLER、平均迭代、false-pass、饱和比例 | 按 [[T17.5_BER_BLER_curve_reporting|T17.5]] 曲线报告和预算比较，不要求逐帧和浮点完全一致。 |
...
`first_mismatch.json` 至少写 `checkpoint_name`、`index`、`python_value`、`cpp_value`、`expected_width`、`round_mode`、`saturation_mode`、`descriptor_hash`。这样 [[T18.6_bit_exact_regression_harness|T18.6]] 的回归框架可以直接读取。
...
若项目还没有真实 C/C++ 定点模型，T18.1 仍然可以先输出 requirement 模板和 Python reference 函数。后续 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 只要引用同一模板，就不会出现三篇文档各自定义一套位宽语义的情况。
...
硬件阶段不应重新发明定点语义。T14/T15 应从 `fixed_requirement.yaml` 读取或手工同步同名字段；若 RTL 为时序或面积修改位宽，必须生成新的 requirement version，并重新跑 [[T18.6_bit_exact_regression_harness|T18.6]] bit-exact 回归。
...
本节当前只关闭“需求文档级验证”：术语、标题、LaTeX 显示、图形几何、toy 量化函数和模板完整性。上表中的边界整数、符号扩展、Python-vs-C/C++ bit-exact 和 fixed-vs-float 曲线预算是后续模型级验证项目，必须由 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.6_bit_exact_regression_harness|T18.6]] 在真实代码或真实 trace 出现后关闭，不能由 T18.1 代替关闭。
...
| 资料 | 边界 |
| :--- | :--- |
| [[T2.16_LLR_clipping_scaling_quantization|T2.16]] | 本节升级为工程需求模板。 |
| [[T5.1_fixed_point_numbers_for_LLR|T5.1]] | 本节不重复完整基础课推导。 |
| [[T17.1_python_golden_model_project_layout|T17.1]] | 本节不创建真实 `sim/` 包。 |
| [[T17.2_LTE_Turbo_float_sim_plan|T17.2]]-[[T17.4_NR_Polar_float_sim_plan|T17.4]] | 本节不规划各算法专属 C/C++ 数据结构，留给 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]]。 |
| [[T17.5_BER_BLER_curve_reporting|T17.5]] | 本节不生成真实 fixed BLER 曲线。 |
...
| 证据项 | 状态 | 关闭条件 |
| :--- | :--- | :--- |
| LTE Turbo 协议字段 | 回链 T7/[[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 和 TS 36.212 Rel-19 `36212-j30`。 | [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]] 必须列具体本地路径和检查点。 |
| NR LDPC 协议字段 | 回链 T9/[[T17.3_NR_LDPC_float_sim_plan|T17.3]] 和 TS 38.212 Rel-19 `38212-j30`。 | [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]] 必须列 BG（基图，Base Graph）、$Z_c$、RV、CBG 等字段来源。 |
| NR Polar 协议字段 | 回链 T10/[[T17.4_NR_Polar_float_sim_plan|T17.4]] 和 TS 38.212 Rel-19 `38212-j30`。 | [[T18.4_NR_Polar_fixed_point_model_plan|T18.4]] 必须列 $A/K/E/N/L$、CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier） 和 rate recovery 来源。 |
...
| 写入文件 | `docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md` |
...
| 上游讲义 | 已读取 `docs/L1_基础/T2.16_LLR_clipping_scaling_quantization.md`、`docs/L1_基础/T5.1_fixed_point_numbers_for_LLR.md`、`docs/L3_工程实现/T17.1_python_golden_model_project_layout.md`。 |
| Python 验证 | 已执行正文 toy 片段，输出 `T18.1_FIXED_REQUIREMENT_TOY_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md`：`LATEX_RENDER_AUDIT_OK formulas=34`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T18.1_fixed_point_decoder_requirements.md`：`REFERENCE_REBUILD_AUDIT_NO_CANDIDATES`。 |
| 未执行内容 | 本节未实现真实 C/C++ 定点模型，未生成真实 fixed BLER 曲线；边界整数、符号扩展、Python-vs-C/C++ bit-exact 和 fixed-vs-float 曲线预算属于 [[T18.2_LTE_Turbo_fixed_point_model_plan|T18.2]]-[[T18.6_bit_exact_regression_harness|T18.6]] 的模型级验证闭环。 |

## 参考文献

- [上游讲义] `docs/L1_基础/T2.16_LLR_clipping_scaling_quantization.md`，LLR 裁剪、缩放与量化预览。
- [上游讲义] `docs/L1_基础/T5.1_fixed_point_numbers_for_LLR.md`，LLR 处理中的定点数。
- [上游讲义] `docs/L3_工程实现/T17.1_python_golden_model_project_layout.md`，Python Golden Model 工程布局。
- [上游讲义] `docs/L3_工程实现/T17.5_BER_BLER_curve_reporting.md`，BER/BLER 曲线生成与报告。
