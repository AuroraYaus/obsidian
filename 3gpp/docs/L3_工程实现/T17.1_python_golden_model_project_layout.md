...
本节进入 L3 工程实现阶段。前面的 L1/L2 讲义已经把LTE Turbo（Turbo 码，Turbo Code）、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）、NR Polar（极化码，Polar Code）的数学对象、协议链路、速率恢复、混合自动重传请求软缓存、接口契约和边界案例逐步讲清楚；从这一节开始，目标变成把这些知识组织成一个可执行、可复现、可审计的 golden model 工程。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T4.6_decoder_interface_contracts|T4.6]] 译码器接口契约 | 知道 descriptor、input、output、status、error、debug 六类对象，以及对数似然比符号、传输块/码块、HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）process ID、冗余版本（Redundancy Version, RV）和 soft buffer key 的作用。 |
| [[T5.5_decoder_hardware_verification_mindset|T5.5]] 验证思维 | 知道 golden model、bit-exact、边界案例、回归、波形/日志定位的基本关系。 |
| [[T4.5_decoder_performance_metrics|T4.5]] 性能指标 | 知道BER（比特错误率，Bit Error Rate）、块错误率、FER（帧错误率，Frame Error Rate）、迭代次数、延迟代理和统计停止条件不是 3GPP 强制门限，而是工程评估对象。 |
...
本节的执行范围是“工程布局规范”。因此协议依据用于说明向量来源和参数回链，不在本节重新复现 TS 36.212/38.212 的编码、速率匹配或循环冗余校验公式。具体 LTE Turbo 浮点仿真由 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 承接，NR LDPC 由 [[T17.3_NR_LDPC_float_sim_plan|T17.3]] 承接，NR Polar 由 [[T17.4_NR_Polar_float_sim_plan|T17.4]] 承接，BER/BLER（块错误率，Block Error Rate）曲线由 [[T17.5_BER_BLER_curve_reporting|T17.5]] 承接。

## 资料与协议边界

Golden model 工程本身不是 3GPP 规范内容。3GPP 不会规定 Python 包名、配置文件名、日志字段名或随机种子格式。协议规定的是被仿真的对象：CRC（循环冗余校验，Cyclic Redundancy Check）、分段、编码结构、速率匹配、RV、控制/数据信道处理链路和相关参数来源。
...
| TS 36.212 Rel-19 `36212-j30` §5.1.2、§5.1.3.2、§5.1.4.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md`，`sections.jsonl` | LTE CB（码块，Code Block）segmentation、Turbo coding、rate matching/RV 是 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 向量字段来源。 | 具体地址、交织和 RV 恢复由 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 使用。 |
| TS 38.212 Rel-19 `38212-j30` §5.1、§5.2.2、§5.3.2、§5.4.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md`，`sections.jsonl` | NR LDPC CRC、segmentation、base graph/lifting、rate matching 是 [[T17.3_NR_LDPC_float_sim_plan|T17.3]] 向量字段来源。 | CRC 多项式复现见上游讲义；具体 BG（基图，Base Graph）/Zc/k0 查表由 [[T17.3_NR_LDPC_float_sim_plan|T17.3]]/T8/T9 回链。 |
| TS 38.212 Rel-19 `38212-j30` §5.1、§5.2.1、§5.3.1、§5.4.1、§6.3、§7.3 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md`，`sections.jsonl` | NR Polar CRC、CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）、segmentation、encoding、rate matching、上行控制信息/下行控制信息控制信息链路是 [[T17.4_NR_Polar_float_sim_plan|T17.4]] 向量字段来源。 | reliability sequence 和 rate recovery 已由 [[T10.3_NR_Polar_reliability_sequence|T10.3]]/[[T10.7_NR_Polar_rate_recovery|T10.7]] 承接；UCI（上行控制信息，Uplink Control Information）/DCI（下行控制信息，Downlink Control Information）精确 CRC/RNTI 字段由 [[T17.4_NR_Polar_float_sim_plan|T17.4]] 使用时关闭。 |
| TS 38.214 Rel-19 `38214-j30` §5.1.3、§6.1.4 | `3GPP_Rel19/processed/TS_38.214_38214-j30/content.md`，`sections.jsonl` | 调制与编码方案/传输块大小/RV 可作为系统级参数来源。 | 具体 MCS（调制与编码方案，Modulation and Coding Scheme）/TBS（传输块大小，Transport Block Size）表值保留 L3/system bit-exact 条件项，按实际使用范围复现。 |
...
刚开始这很快，但一旦进入三类译码器、多种 SNR（信噪比，Signal-to-Noise Ratio）、多种码率、多种 RV、多种算法开关和随机失败帧，散装脚本会暴露四个问题。
...
| 输出不可比对 | LTE Turbo 输出字段和 NR Polar 输出字段完全不同，没有公共 status/error/debug。 | C/C++ 和 RTL（寄存器传输级，Register Transfer Level）scoreboard 需要为每类模型重写大量胶水代码。 |
...
![图 1：Golden Model 工程流转——输入证据经 config/descriptor、vector builder、model runner、normalized outputs 到 artifact fanout，再分发 metrics、trace/failure bundle、golden vectors 与审计报告，最终服务 C/C++ 定点模型与 RTL scoreboard](assets/T17.1_golden_model_project_layout.svg)
...
| `input_llr.npy` | 接收端输入 LLR（对数似然比，Log-Likelihood Ratio），已经按本节约定的正号语义存储。 | NumPy `.npy` 或压缩数组。 |
...
| `frames` | 已仿真帧数。 |
| `error_blocks` | 错误 TB（传输块，Transport Block）/CB/control block 数。 |
...
| `nr_ldpc` | BG/Zc、QC-LDPC（准循环 LDPC，Quasi-Cyclic LDPC）扩展、BP/MS/NMS/OMS、LDPC rate recovery、syndrome/CRC。 | LTE Turbo trellis、Polar path metric sorter。 |
...
| RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）需要的字段 | T17.1 应如何保存 |
|:---|:---|
...
| 新增证据资产 | `docs/L3_工程实现/assets/T17.1_golden_model_project_layout.svg`（手绘 SVG，2026-08-07 按 T17.1 正文重新梳理绘制，audit_svg_layout.py R1-R11 全过）。 |
