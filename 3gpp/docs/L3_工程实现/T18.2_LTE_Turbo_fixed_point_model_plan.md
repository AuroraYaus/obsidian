...
本节规划LTE Turbo（Turbo 码，Turbo Code）的 C/C++ 定点译码器模型。[[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 已经定义浮点仿真：编码器参考、加性白高斯噪声信道、解速率匹配、Log-MAP/Max-Log-MAP、循环冗余校验、块错误率曲线和失败回放。本节把这些浮点对象约束成整数模型：每条对数似然比怎样量化，分支度量怎样计算，alpha/beta 路径度量怎样归一化和饱和，外信息怎样缩放，交织地址怎样驱动数组访问，以及 C/C++ 输出怎样和 Python 定点参考做 bit-exact 对比。


- 说明 LTE Turbo 定点模型和 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 浮点模型的输入、输出和检查点关系。
- 为分支度量、alpha/beta 度量、后验 LLR（对数似然比，Log-Likelihood Ratio）、外信息和 soft buffer 分配位宽、缩放和饱和策略。
- 区分 Log-MAP、Max-Log-MAP 和修正 Max-Log-MAP 在定点实现中的不同代价。
- 设计 C/C++ 结构体、数组布局、状态字段和 trace 字段。
- 写出分支度量、前向递推、后向递推、外信息扣除和 CRC（循环冗余校验，Cyclic Redundancy Check） gate 的 bit-exact 检查方案。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T6.5_BCJR_MAP_decoding_intuition|T6.5]] BCJR（BCJR 算法，Bahl-Cocke-Jelinek-Raviv Algorithm）/MAP（最大后验概率，Maximum A Posteriori） 直觉 | 知道分支度量、前向度量 alpha、后向度量 beta 和后验 LLR 的含义。 |
| [[T6.6_Log_MAP_Max_Log_MAP_Turbo|T6.6]] Log-MAP 与 Max-Log-MAP | 知道 `max-star` 修正项、Max-Log-MAP 近似和外信息扣除。 |
| [[T6.4_LTE_Turbo_internal_interleaver|T6.4]] LTE Turbo 内部交织器 | 知道 $K/f_1/f_2$ 生成交织/解交织地址，外信息必须按地址对齐。 |
| [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]] LTE 混合自动重传请求软缓存与 RV | 知道冗余版本（Redundancy Version, RV）选择循环缓存窗口，同一编码比特位置的 LLR 要累加或饱和。 |
| [[T18.1_fixed_point_decoder_requirements|T18.1]] 定点译码器需求 | 知道 Q 格式、裁剪、舍入、饱和、性能损失预算和 bit-exact 检查点。 |

本节假设你已经知道 Turbo 译码器由两个软输入软输出（Soft-Input Soft-Output, SISO）译码器迭代交换外信息。若这个概念还不清楚，应先回看 [[T6.7_Turbo_iteration_extrinsic_stopping|T6.7]]；否则定点外信息缩放和交织地址会显得像单纯数组搬运。

本节后半部分还会把模型映射到寄存器传输级和专用集成电路的检查点。这里的 RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit） 不是协议要求，而是为了让 C/C++ 定点模型以后能成为硬件 scoreboard 的黄金参考。
...
| 资料 | 本地路径 | 边界 |
| :--- | :--- | :--- |
| TS 36.212 §5.1.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md:575-599` | CRC 算法由上游 [[T3.1_LTE_NR_CRC_families|T3.1]]/[[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 回链，本节只消费状态。 |
| TS 36.212 §5.1.2 | `content.md:601-673` | 不重新推导分段公式。 |
| TS 36.212 §5.1.3.2.1/§5.1.3.2.2 | `content.md`，[[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]] 重建图 | 不规定接收机必须用 Log-MAP 或 Max-Log-MAP。 |
| TS 36.212 §5.1.3.2.3/Table 5.1.3-3 | `tables/table_0009.csv/html`，[[T6.4_LTE_Turbo_internal_interleaver|T6.4]]/[[T6.6_Log_MAP_Max_Log_MAP_Turbo|T6.6]] 回链 | 本节不复制全表，使用上游已重建资产。 |
...
| [[T6.6_Log_MAP_Max_Log_MAP_Turbo|T6.6]] | `docs/L2_协议算法/T6.6_Log_MAP_Max_Log_MAP_Turbo.md` | 算法文献背景，不是 3GPP 规范公式。 |
| [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] | `docs/L3_工程实现/T17.2_LTE_Turbo_float_sim_plan.md` | 本节不生成真实 BLER（块错误率，Block Error Rate） 曲线。 |
...
![图 1：LTE Turbo 定点数据通路——Protocol Vector 经 LLR Quantizer、SISO Core、Iterative Exchange 到 CRC/Report，五层 bit-exact 比较点（Rate recovery、Branch metric、Forward/backward、Extrinsic、CRC gate），底部为位宽与算法选项](assets/T18.2_LTE_Turbo_fixed_point_model_plan.svg)

| 内容 | 说明 |
|:---|:---|
| Stage | Integer Object；Compare Field；Failure Signal |
| Protocol Vector | LLR Quantizer；SISO Core；Iterative Exchange；CRC/Report |
| K, f1, f2, RV, E, Ncb, filler mask, d0/d1/d2 streams and TS 36.212 evidence. | Apply [[T18.1_fixed_point_decoder_requirements|T18.1]] width, scale, round and saturation to system and parity LLR streams.；Compute branch metric, alpha, beta, posterior and extrinsic with Log-MAP or Max-Log-MAP.；Interleave and deinterleave extrinsic LLR; update  |
...
| CRC gate | hard bits and CB（码块，Code Block）/TB（传输块，Transport Block） CRC；crc_status, iter_used；late first mismatch |
| Log-MAP | max-star correction LUT（查找表，Look-Up Table）, higher accuracy, wider trace checks |
| Max-Log-MAP | max only, hardware friendly, needs BLER loss budget |
本节流程顺序为：Protocol Vector 提供 $K/f_1/f_2$、RV、$E$、$N_{\mathrm{cb}}$ 和三路流；LLR Quantizer 按 [[T18.1_fixed_point_decoder_requirements|T18.1]] 量化系统流与校验流；SISO Core 计算分支度量、alpha、beta、后验和外信息；Iterative Exchange 通过交织/解交织地址交换外信息；CRC/Report 记录硬判决、CB/TB CRC、早停和 replay。中部表格列出四层 compare checkpoint，底部列出 Log-MAP、Max-Log-MAP、度量归一化、外信息缩放和饱和观测。
...
T18.2 的 C/C++ 模型应从 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 的失败包或 run directory 读取对象，而不是临时构造散装数组。
...
这些结构体是计划级接口，不是最终应用二进制接口（Application Binary Interface, ABI）。关键点是：协议 descriptor、定点格式、数组缓冲区和 trace 不能混成一个大结构，否则后续 [[T18.6_bit_exact_regression_harness|T18.6]] 和 RTL scoreboard 很难单独比较。`TurboFixedConfig` 必须能唯一决定 bit-exact 行为：同一个输入向量、同一个配置对象和同一个代码版本，不能因为舍入、饱和、Log-MAP mode、查找表（Look-Up Table, LUT）或归一化策略缺字段而得到两个合法答案。

## 分支度量定点化

[[T6.6_Log_MAP_Max_Log_MAP_Turbo|T6.6]] 给出二进制相移键控/AWGN（加性白高斯噪声，Additive White Gaussian Noise） 教学模型下的分支度量：
...
TS 36.212 §5.1.3.2.3 用 $K/f_1/f_2$ 给出 Turbo 内部交织器的二次置换参数。上游 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 已采用下面的索引约定：
...
生成交织输出。换言之，`j` 是交织后数组的位置，`pi[j]` 是从原始数组读取的位置。这个约定和另一些代码中使用的 $c'_{\Pi(j)}=c_j$ 相反；两种约定本身都可以工作，但 Python、C/C++、RTL 和测试向量必须统一。本节后续所有 `pi/depi` 都沿用 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 的 `c'_j=c_{pi[j]}` 约定。
...
实现错误通常不是数组越界，而是 $\Pi$ 与 $\Pi^{-1}$ 用反，或者把 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 的 `c'_j=c_{pi[j]}` 约定和另一种正向写法混用。测试必须包含：
...
第一版 C/C++ 模型以清晰和 bit-exact 为先，不先追求单指令多数据（Single Instruction, Multiple Data, SIMD）。[[T18.5_SIMD_memory_layout_decoders|T18.5]] 会再处理 SIMD 和缓存布局。本节建议数组采用结构化分离：
...
| Python float | [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 浮点模型 | Python/C fixed | 曲线和检查点趋势比较，不要求整数一致。 |
...
定点模型必须继承 [[T17.5_BER_BLER_curve_reporting|T17.5]] 的曲线报告口径。推荐字段：

| 字段 | 说明 |
|:---|:---|
| `float_baseline_run_id` | [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 浮点 Log-MAP 或 Max-Log-MAP 基线。 |
| `fixed_run_id` | 当前定点模型运行。 |
| `fixed_requirement_id` | 位宽/缩放/饱和规格版本。 |
| `fixed_loss_budget_db` | 例如 0.10 dB。 |
| `bler_ratio_budget` | 同一 SNR（信噪比，Signal-to-Noise Ratio） 点 BLER 比值预算，例如 1.10。 |
...
T18.2 的定点模型会直接变成 [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] RTL 微架构的前置规格。

| C/C++ 对象 | RTL/ASIC 对象 | 风险 |
|:---|:---|:---|
| `sys_llr/par_llr/apriori` | SISO 输入 RAM 和读端口。 | 端口顺序或符号扩展错误。 |
| `alpha/beta` | alpha/beta RAM、滑窗缓存或寄存器阵列。 | 位宽不足和初始化错误。 |
| `maxstar_lut` | 修正项 ROM（只读存储器，Read-Only Memory）。 | LUT scale 与 metric scale 不一致。 |
...
RTL 可以选择滑窗、并行 SISO 或折叠架构；但只要它声明兼容某个 `fixed_requirement_id`，就必须在 [[T18.6_bit_exact_regression_harness|T18.6]]/T15 中通过同一组 bit-exact 检查点。
...
| HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） /RV | 使用 [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]] 风格 RV0/RV2 重传向量。 | soft buffer 合并后检查点一致。 |
| 性能预算 | fixed vs float 曲线。 | 满足 [[T18.1_fixed_point_decoder_requirements|T18.1]]/[[T17.5_BER_BLER_curve_reporting|T17.5]] 预算和置信区间要求。 |
...
| LTE Turbo encoder/trellis | TS 36.212 §5.1.3.2.1/§5.1.3.2.2 | `content.md`；[[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]] 重建图 | 上游已重建图。 |
| LTE internal interleaver | TS 36.212 §5.1.3.2.3/Table 5.1.3-3 | `tables/table_0009.csv/html`；[[T6.4_LTE_Turbo_internal_interleaver|T6.4]]/[[T17.2_LTE_Turbo_float_sim_plan|T17.2]] | 上游已复现。 |
...
| Log-MAP/Max-Log-MAP | 非 3GPP 强制关系式 | [[T6.6_Log_MAP_Max_Log_MAP_Turbo|T6.6]]；Robertson/Hagenauer 文献 | 实现策略。 |

## 未复现协议资产与关闭条件

| 候选资产 | 本节处理 | 关闭条件 |
|:---|:---|:---|
| TS 36.212 Table 5.1.3-3 全表 | 本节回链 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]]/[[T17.2_LTE_Turbo_float_sim_plan|T17.2]]，不重新复制长表。 | 若 T18.2 代码直接解析原表生成地址，必须把 CSV/HTML、脚本和 hash 纳入 fixed evidence archive。 |
| TS 36.213 调制与编码方案/传输块大小 | 本节不使用具体 MCS（调制与编码方案，Modulation and Coding Scheme） /TBS（传输块大小，Transport Block Size） 表值。 | 若 fixed vector 从 MCS index 自动生成 TB size 或 $Q_m$，必须关闭 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 标记的 `待核验` 分册/章节/表格。 |
...
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T18.2_LTE_Turbo_fixed_point_model_plan.md`：`REFERENCE_REBUILD_AUDIT_CANDIDATES`，候选为 TS 36.212 表/关系式回链、[[T6.4_LTE_Turbo_internal_interleaver|T6.4]]/[[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 已重建资产、实现策略边界、论文引用和 TS 36.213 MCS/TBS 条件项；本节未直接使用 MCS/TBS 表值。 |
