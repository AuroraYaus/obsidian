...
本节规划LTE Turbo的 C/C++ 定点译码器模型。[[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 已经定义浮点仿真：编码器参考、加性白高斯噪声信道、解速率匹配、Log-MAP/Max-Log-MAP、循环冗余校验、块错误率曲线和失败回放。本节把这些浮点对象约束成整数模型：每条对数似然比怎样量化，分支度量怎样计算，alpha/beta 路径度量怎样归一化和饱和，外信息怎样缩放，交织地址怎样驱动数组访问，以及 C/C++ 输出怎样和 Python 定点参考做 bit-exact 对比。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T6.5_BCJR_MAP_decoding_intuition|T6.5]] BCJR/MAP 直觉 | 知道分支度量、前向度量 alpha、后向度量 beta 和后验 LLR 的含义。 |
...
| [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] | `docs/L3_工程实现/T17.2_LTE_Turbo_float_sim_plan.md` | 本节不生成真实 BLER 曲线。 |
...
[[T6.6_Log_MAP_Max_Log_MAP_Turbo|T6.6]] 给出二进制相移键控/AWGN 教学模型下的分支度量：
...
| HARQ/RV | 使用 [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]] 风格 RV0/RV2 重传向量。 | soft buffer 合并后检查点一致。 |
...
| TS 36.213 调制与编码方案/传输块大小 | 本节不使用具体 MCS/TBS 表值。 | 若 fixed vector 从 MCS index 自动生成 TB size 或 $Q_m$，必须关闭 [[T17.2_LTE_Turbo_float_sim_plan|T17.2]] 标记的 `待核验` 分册/章节/表格。 |
...
| 新增证据资产 | `docs/L3_工程实现/assets/T18.2_LTE_Turbo_fixed_point_model.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t13_2_lte_turbo_fixed_point_model.py`）。 |
