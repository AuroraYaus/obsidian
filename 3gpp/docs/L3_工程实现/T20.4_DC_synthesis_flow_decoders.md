...
| T19.1-T19.6 | `docs/L3_工程实现/T19.*.md` | 本节只讲如何综合这些 RTL。 |
| T20.1-T20.3 | `docs/L3_工程实现/T20.1*.md`、`T20.2*.md`、`T20.3*.md` | 当前仓库没有真实 RTL 仿真和 DC coverage 数据库。 |
...
## 图示
![图 1：DC synthesis flow decoders](assets/T15.4_DC_synthesis_flow_decoders.svg)
...
| 图形资产 | `docs/L3_工程实现/assets/T15.4_DC_synthesis_flow_decoders.svg` | 手绘 SVG，2026-08-04 替代原 PIL 渲染 PNG（原 `render_t15_4_dc_synthesis_flow_decoders.py` 与 PNG 已删除，图示语义不变）；SVG 几何审计六项检查全部通过，cairosvg 渲染验证无报错。 |
