...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] LTE Turbo RTL 微架构 | 知道 Turbo SISO、alpha/beta、extrinsic、interleaver 和 CRC checkpoint。 |
...
| [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] NR Polar RTL 微架构 | 知道 SC/SCL、path metric、sorter、partial sum、CRC/RNTI selector。 |
...
## 图示
![图 1：DC synthesis flow decoders](assets/T20.4_DC_synthesis_flow_decoders.svg)
...
| 图形资产 | `docs/L3_工程实现/assets/T20.4_DC_synthesis_flow_decoders.svg` | 手绘 SVG，2026-08-04 替代原 PIL 渲染 PNG（原 `render_t15_4_dc_synthesis_flow_decoders.py` 与 PNG 已删除，图示语义不变）；SVG 几何审计六项检查全部通过，cairosvg 渲染验证无报错。 |
