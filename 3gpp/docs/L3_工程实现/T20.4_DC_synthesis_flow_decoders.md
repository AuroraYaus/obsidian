...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T20.4_DC_synthesis_flow_decoders.md"
---
# T20.4 Synopsys Design Compiler 综合流程：从译码 RTL 到时序、面积和功耗报告
## 本节学习目标

本节讲解译码寄存器传输级的 Synopsys Design Compiler 综合流程。Design Compiler 常简称 DC（设计编译器，Design Compiler），它的任务是把硬件描述语言写成的 RTL（寄存器传输级，Register Transfer Level）映射到具体工艺库单元，生成门级网表，并给出时序、面积、功耗、约束违例和结构风险报告。
...
- 识别译码器常见综合关键路径：Turbo（Turbo 码，Turbo Code）ACS/度量更新、LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）check-node min tree、Polar（极化码，Polar Code）SCL（连续消除列表译码，Successive Cancellation List）sorter、soft buffer key compare、DMA（直接存储器访问，Direct Memory Access）arbitration。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]] LTE Turbo RTL 微架构 | 知道 Turbo SISO（软入软出，Soft-Input Soft-Output）、alpha/beta、extrinsic、interleaver 和 CRC（循环冗余校验，Cyclic Redundancy Check）checkpoint。 |
| [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] NR LDPC RTL 微架构 | 知道 LDPC layered schedule、CN/VN、message RAM、bank conflict 和 syndrome checkpoint。 |
| [[T19.3_NR_Polar_RTL_microarchitecture|T19.3]] NR Polar RTL 微架构 | 知道 SC（逐次消除，Successive Cancellation）/SCL、path metric、sorter、partial sum、CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）selector。 |
| [[T19.4_unified_decoder_subsystem_architecture|T19.4]] 统一译码子系统 | 知道 dispatcher、DMA、soft buffer manager、status/error/IRQ 和 trace。 |
| [[T19.6_decoder_register_map_configuration_flow|T19.6]] 寄存器表与配置流 | 知道配置寄存器、start/done/error、timeout、trace mask 和 descriptor hash。 |
| [[T20.1_decoder_testbench_architecture|T20.1]]-[[T20.3_coverage_regression_strategy|T20.3]] | 知道 testbench、protocol vectors、coverage bins、regression tiers 和 sign-off gate。 |

自检问题：

| 问题 | 若回答不出来，先回看 |
|:---|:---|
| 为什么综合脚本不能把 testbench 文件放进 design filelist？ | [[T20.1_decoder_testbench_architecture|T20.1]]。 |
| 为什么 `dc_shell` 没装时不能声称“综合通过”？ | 本节工具边界。 |
| 为什么 `report_timing` 通过不代表译码功能正确？ | [[T18.6_bit_exact_regression_harness|T18.6]]、[[T20.1_decoder_testbench_architecture|T20.1]]。 |
| 为什么 reset 约束写错会让时序报告看起来很好但芯片不可用？ | [[T5.4_RTL_state_machine_handshake_basics|T5.4]]、[[T19.6_decoder_register_map_configuration_flow|T19.6]]。 |
| 为什么 Polar list size 或 LDPC parallelism 会影响面积和关键路径？ | [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]]、[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]]。 |
...
本节是工程方法课。3GPP 协议不规定 DC Tcl、工艺库、时钟周期、compile option、面积单位或功耗模型；协议和上游讲义只影响“综合后应该验证哪些配置和向量”。若综合后要跑 LTE/NR 协议向量或 gate-level sanity，必须回链 [[T20.1_decoder_testbench_architecture|T20.1]]-[[T20.3_coverage_regression_strategy|T20.3]] 和 T7/T9/T10/[[T20.2_protocol_vector_corner_case_suite|T20.2]] 的证据。
...
| [[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]]-[[T19.6_decoder_register_map_configuration_flow|T19.6]] | `docs/L3_工程实现/T19.*.md` | 本节只讲如何综合这些 RTL。 |
| [[T20.1_decoder_testbench_architecture|T20.1]]-[[T20.3_coverage_regression_strategy|T20.3]] | `docs/L3_工程实现/T20.1*.md`、`T20.2*.md`、`T20.3*.md` | 当前仓库没有真实 RTL 仿真和 DC coverage 数据库。 |

协议前因后果是：协议参数决定译码器需要支持哪些 family、块长、RV（冗余版本，Redundancy Version）、CBG（码块组，Code Block Group）、list size 和 soft buffer 行为；这些行为塑造 RTL 结构；RTL 结构再进入 DC 综合。DC 本身只看硬件结构和约束，不理解“TS 38.212 的 CBG 语义”。因此综合通过后仍必须回到协议向量和回归证据，证明被综合的硬件没有破坏译码语义。
...
![图 1：DC 综合流程——RTL Inputs 经 DC Setup、SDC Constraints、Compile 生成 Reports，工具可用性边界显式声明](assets/T20.4_DC_synthesis_flow_decoders.svg)
...
| memory wrapper | SRAM（静态随机存取存储器，Static Random-Access Memory）wrapper 可综合，行为模型隔离。 | 用仿真 readmemh memory 当 ASIC（专用集成电路，Application-Specific Integrated Circuit）SRAM。 |
| testbench 排除 | `tb/`、DPI、class、random 不进入 RTL filelist。 | 把 [[T20.1_decoder_testbench_architecture|T20.1]] testbench class 送进 DC。 |
| 参数固定 | family mask、parallelism、LLR（对数似然比，Log-Likelihood Ratio）width 有明确参数。 | 综合时 parameter 未覆盖，得到默认小配置。 |
...
| `check_timing` | 检查路径是否被约束、clock 是否定义、端点是否可分析。 | unconstrained path、no_clock、multiple_clock 是 STA（静态时序分析，Static Timing Analysis）前置失败。 |
...
路径例外必须写 owner、理由、适用端点和验证方法。release 前，[[T20.6_final_decoder_verification_evidence_report|T20.6]] 最终证据报告应列出所有例外。
...
4. **Subsystem compile**：综合 [[T19.4_unified_decoder_subsystem_architecture|T19.4]] 统一子系统，观察 dispatcher/DMA/soft buffer/status fan-in。
...
综合优化不能替代架构修复。若 Polar sorter 或 LDPC min tree 组合深度本身过长，下一节 [[T20.5_timing_closure_decoder_critical_paths|T20.5]] 应通过 pipeline、tree split、register duplication 或算法近似改结构，而不是只调 DC option。
...
注意最后一行。时序修复会改变 latency、trace cycle、valid alignment 和 scoreboard expectation，必须回到 [[T20.1_decoder_testbench_architecture|T20.1]]/[[T20.3_coverage_regression_strategy|T20.3]] 的回归。
...
PPA（功耗、性能、面积，Power, Performance, Area）表只能作为方法矩阵。没有真实综合、真实 activity 和真实 STA 时，不能写“面积降低 12%”或“功耗降低 20%”。建议每一次优化都记录 `baseline_report_id`、`candidate_report_id`、`changed_rtl_or_constraint`、`functional_regression_set` 和 `accept/reject_reason`。

| 对象 | PPA 压力 | 候选优化 | 必须重验 |
|:---|:---|:---|:---|
| LDPC CNU/min tree | 比较器、mux、符号树和 saturation 形成 timing/area 压力。 | min1/min2 分级、partial parallel CNU、NMS（归一化最小和，Normalized Min-Sum）/OMS（偏移最小和，Offset Min-Sum）系数量化、pipeline。 | syndrome trace、layered RMW、块错误率/fixed loss。 |
...
| SRAM sleep | soft buffer、message RAM、path memory 在 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）process 空闲时睡眠。 | wake-up latency、retention、半提交 transaction。 | sleep_state_trace、wake_cycles、abort/replay tests。 |
...
| 早停功耗 | CRC/syndrome early stop 后关闭迭代 datapath。 | early stop 错误、trace 缺失、统计偏差。 | stop_reason、gated_cycles、BLER（块错误率，Block Error Rate）/false-pass regression。 |
...
| Unified top | DMA arbitration、status/error fan-in、IRQ sticky、trace mux。 | 分区 arbitration、状态寄存、trace 采样降频。 | [[T20.1_decoder_testbench_architecture|T20.1]] monitor 和 [[T20.3_coverage_regression_strategy|T20.3]] coverage event 需更新。 |

[[T20.5_timing_closure_decoder_critical_paths|T20.5]] 会专门展开 timing closure。T20.4 只要求你能从报告中识别这些路径并提出第一步诊断，不要求在本节完成结构修改。
...
| 验证 | 目的 | 回链 |
|:---|:---|:---|
| Netlist sanity | 确认 mapped netlist 可 elaboration，可跑 smoke。 | [[T20.1_decoder_testbench_architecture|T20.1]] testbench smoke。 |
| SDF/gate-level spot | 确认关键路径和时序反标不会破坏 handshake。 | [[T20.1_decoder_testbench_architecture|T20.1]] driver/monitor 和 [[T20.3_coverage_regression_strategy|T20.3]] smoke/commit bins。 |
| Protocol vector replay | 确认综合优化没有破坏协议派生路径。 | [[T20.2_protocol_vector_corner_case_suite|T20.2]] directed vectors，T7/T9/T10 证据。 |
...
| 协议回链 | 文档回链 [[T20.1_decoder_testbench_architecture|T20.1]]-[[T20.3_coverage_regression_strategy|T20.3]]、T7/T9/T10。 | 综合后 smoke/gate-level replay 使用 [[T20.2_protocol_vector_corner_case_suite|T20.2]] 向量。 |
...
| 对象 | 证据 |
| :--- | :--- |
| LTE Turbo 译码配置 | TS 36.212 Rel-19 `36212-j30`，本地路径 `3GPP_Rel19/processed/TS_36.212_36212-j30`，上游 T7/[[T19.1_LTE_Turbo_RTL_microarchitecture|T19.1]]/[[T20.2_protocol_vector_corner_case_suite|T20.2]]。 |
| NR LDPC 译码配置 | TS 38.212 Rel-19 `38212-j30`，TS 38.214 Rel-19 `38214-j30`，上游 T9/[[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]]/[[T20.2_protocol_vector_corner_case_suite|T20.2]]。 |
| NR Polar 译码配置 | TS 38.212 Rel-19 `38212-j30`，上游 T10/[[T19.3_NR_Polar_RTL_microarchitecture|T19.3]]/[[T20.2_protocol_vector_corner_case_suite|T20.2]]。 |
| Testbench/coverage/sign-off | [[T20.1_decoder_testbench_architecture|T20.1]]、[[T20.2_protocol_vector_corner_case_suite|T20.2]]、[[T20.3_coverage_regression_strategy|T20.3]] |
## 执行与证据记录
