...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T20.3_coverage_regression_strategy.md"
---
# T20.3 覆盖率与回归方案：把 directed vectors 变成可签收证据
## 本节学习目标

本节定义译码验证的覆盖率与回归方案。这里的覆盖率不是“跑了多少条测试”，而是回答一个更具体的问题：LTE Turbo（Turbo 码，Turbo Code）、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）和 NR Polar（极化码，Polar Code）的关键协议条件、数值条件、控制条件和实现条件，是否都被测试实际打到，并且失败时能不能复现和分诊。


- 解释功能覆盖率、代码覆盖率、断言覆盖率、交叉覆盖率和回归健康度的区别。
- 把 [[T20.2_protocol_vector_corner_case_suite|T20.2]] 的协议向量、边界案例和 expected-fail policy 转成 coverage bins。
- 给 LTE Turbo、NR LDPC、NR Polar 分别定义算法家族、块长、RV（冗余版本，Redundancy Version）、调制阶数 $Q_m$、CRC（循环冗余校验，Cyclic Redundancy Check）状态、reset/abort、LLR（对数似然比，Log-Likelihood Ratio）饱和等覆盖点。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T5.5_decoder_hardware_verification_mindset|T5.5]] 译码硬件验证思维 | 知道 golden model（黄金模型，Golden Model）、bit-exact（比特精确，bit-exact）、directed/random、coverage、波形和 failure replay 的目的。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] Bit-exact 回归框架 | 知道 `vector.json`、checkpoint manifest、failure bundle、first mismatch 和 CI 命令模板。 |
| [[T20.1_decoder_testbench_architecture|T20.1]] Testbench 架构 | 知道 driver、monitor、scoreboard、assertions、reset/timeout tests 如何产生可观测证据。 |
| [[T20.2_protocol_vector_corner_case_suite|T20.2]] 协议向量与边界案例套件 | 知道 LTE Turbo、NR LDPC、NR Polar 的 directed cases、expected-fail class 和最小 dump 包。 |
| T7/T9/T10 协议边界课 | 知道 LTE HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）/RV、NR LDPC CBG（码块组，Code Block Group）/RV、NR Polar CRC/list/frozen mask 等覆盖点来自哪里。 |

自检问题：

| 问题 | 若回答不出来，先回看 |
|:---|:---|
| 为什么 `CRC_PASS` 不能证明所有 rate recovery 地址都正确？ | [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]]、[[T9.1_NR_LDPC_rate_recovery_overview|T9.1]]、[[T18.6_bit_exact_regression_harness|T18.6]]。 |
| 为什么 `100% code coverage` 仍可能漏掉 NR LDPC CBG hold bug？ | [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]、[[T20.2_protocol_vector_corner_case_suite|T20.2]]。 |
| 为什么 expected-fail vector 的“失败”也要纳入覆盖率？ | [[T20.1_decoder_testbench_architecture|T20.1]]、[[T20.2_protocol_vector_corner_case_suite|T20.2]]。 |
| 为什么 nightly 的随机 seed 必须能 replay？ | [[T17.1_python_golden_model_project_layout|T17.1]]、[[T18.6_bit_exact_regression_harness|T18.6]]。 |
...
| 编号 | T20.3 |
| 前置 | T20.1, T20.2 |
| Prompt | 请定义译码验证的功能覆盖率、代码覆盖率、回归层级、随机种子、夜间运行、失败分诊和 sign-off 标准。覆盖点包括算法家族、块长、RV、Qm（调制阶数，Modulation order）、CRC 状态和复位。 |
...
| 证据边界 | 无需直接 3GPP 引用 for coverage methodology. Coverage bins that depend on protocol parameters must reference upstream Rel-19 evidence from T7/T9/T10/[[T20.2_protocol_vector_corner_case_suite|T20.2]]. |

本节是验证工程课。3GPP 不规定你用什么覆盖率数据库、CI 层级、nightly 脚本或 sign-off 表；协议规定的是被覆盖的对象，例如 TB（传输块，Transport Block）/CB（码块，Code Block）、CRC、RV、rate matching、LDPC BG（基图，Base Graph）/$Z_c$、CBG、Polar CRC/RNTI（无线网络临时标识，Radio Network Temporary Identifier）、list size 压力和 reset 对接收侧状态的影响。因此本节的原则是：coverage 方法属于工程策略，coverage bin 中的协议参数必须能回链到上游 Rel-19 精读讲义或向量套件。
...
| TS 38.214 Rel-19 `38214-j30` | `3GPP_Rel19/processed/TS_38.214_38214-j30` | MCS（调制与编码方案，Modulation and Coding Scheme）/TBS（传输块大小，Transport Block Size）具体表值仍按实际使用范围回链，不在本节复现。 |
| [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]]/[[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]] | `docs/L2_协议算法/T7.3_LTE_HARQ_soft_buffer_RV.md`、`docs/L2_协议算法/T7.6_LTE_Turbo_decoder_edge_cases.md` | 本节只把它们转成覆盖点。 |
| [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]/[[T9.6_NR_LDPC_decoder_edge_cases|T9.6]] | `docs/L2_协议算法/T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0.md`、`docs/L2_协议算法/T9.6_NR_LDPC_decoder_edge_cases.md` | 本节不重写 LDPC 数值推导。 |
| [[T10.5_Polar_SCL_decoding|T10.5]]-[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] | `docs/L2_协议算法/T10.*.md` | 本节不重写 Polar SCL（连续消除列表译码，Successive Cancellation List）推导。 |
| [[T20.1_decoder_testbench_architecture|T20.1]]/[[T20.2_protocol_vector_corner_case_suite|T20.2]] | `docs/L3_工程实现/T20.1_decoder_testbench_architecture.md`、`docs/L3_工程实现/T20.2_protocol_vector_corner_case_suite.md` | 本节将其组织成 coverage/regression plan。 |

协议前因后果可以这样理解：协议定义了“接收端可能遇到哪些合法和非法状态”；[[T20.2_protocol_vector_corner_case_suite|T20.2]] 把这些状态变成向量；T20.3 再把向量覆盖、RTL（寄存器传输级，Register Transfer Level）代码覆盖、断言覆盖、失败分类和夜间回归组织成签收证据。若 coverage bin 没有可观测字段或可复现向量，它只是愿望清单，不是验证计划。
...
式 (1) 中，$b$ 是一个 coverage bin，$v$ 是已经执行过的向量，$\mathrm{observe}(v,b)$ 表示向量 $v$ 的日志、monitor、scoreboard、assertion 或 coverage database 能证明该 bin 被打到。这里的关键不是存在一条测试，而是“可观察”。例如 `rv=2` 写在配置文件里不等于打到 RV2 bin；必须看到 driver 写入、DUT（待测设计，Device Under Test）descriptor hash、rate recovery address trace 或 scoreboard checkpoint 能证明 DUT 真按 RV2 路径执行。
...
| Assertion coverage | SVA（系统 Verilog 断言，SystemVerilog Assertion）或等价断言是否被激活、通过、失败？ | assertion report。 | 断言只写接口，不写协议状态。 |
...
![图 1：覆盖率与回归策略——Protocol Fields 经 Vector Suites 进入 Coverage DB，Regression Tiers 分级执行，Sign-off Gate 汇总 bins 与失败分诊](assets/T20.3_coverage_regression_strategy.svg)
...
功能覆盖率需要先定义对象模型。建议从 [[T20.2_protocol_vector_corner_case_suite|T20.2]] 的向量对象中抽取 coverage event：
...
| Block length | `small`, `boundary`, `large`, `max_supported` | `B/A/K/E/N` 分类 | 具体边界来自 T7/T9/T10/[[T20.2_protocol_vector_corner_case_suite|T20.2]]。 |
...
功能覆盖率的一个常见错误是把 `block_length` 写成真实数字一一覆盖。真实 TB/CB 长度太多，不可能穷尽。工程上应先分类：小块、协议分支边界、表格最大支持、实现最大支持、随机长块。具体到 LTE Turbo，`K` 合法集合来自 TS 36.212 Table 5.1.3-3，上游 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 已复现；T20.3 不重新列全表，只把“合法集合中的最小、边界、最大和随机抽样”定义为 coverage bins。

## 三类译码器的覆盖矩阵

### LTE Turbo 覆盖点

LTE Turbo coverage 必须围绕 T7/[[T20.2_protocol_vector_corner_case_suite|T20.2]] 的接收端风险：

| 覆盖点 | 必选 bins | 为什么重要 | 回链 |
|:---|:---|:---|:---|
| CB segmentation | `C=1`, `C>1`, `F=0`, `F>0`, `K_minus`, `K_plus` | 抓 filler 删除、CB 顺序、重组长度错误。 | [[T3.3_LTE_Turbo_segmentation_rules|T3.3]], [[T7.4_LTE_code_block_reassembly_TB_CRC|T7.4]], [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]。 |
| RV/soft buffer | `rv0`, `rv2`, `rv_overlap`, `Ncb<Kw`, `rv_mismatch` | 抓循环缓存起点、重复累加、limited buffer。 | [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]], [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]。 |
| `<NULL>`/puncture/repetition | `null_skip`, `unknown_llr`, `repeat_sat` | 抓 `<NULL>` 当有效 bit、打孔当强 LLR、重复覆盖。 | [[T7.2_LTE_subblock_deinterleaver_circular_buffer|T7.2]], [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]。 |
| CRC layer | `CB_CRC_PASS`, `CB_CRC_FAIL`, `TB_CRC_PASS`, `TB_CRC_FAIL` | 抓只看 TB CRC 或只看 CB CRC 的错误。 | [[T3.1_LTE_NR_CRC_families|T3.1]], [[T7.4_LTE_code_block_reassembly_TB_CRC|T7.4]], [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]。 |
| Control | `timeout`, `mid_run_reset`, `harq_context_switch` | 抓半提交 soft buffer 和 HARQ 上下文污染。 | [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]], [[T19.5_soft_buffer_HARQ_memory_architecture|T19.5]], [[T20.1_decoder_testbench_architecture|T20.1]]。 |
...
| 覆盖点 | 必选 bins | 为什么重要 | 回链 |
|:---|:---|:---|:---|
| Base graph and lifting | `BG1`, `BG2`, `Zc_min`, `Zc_boundary`, `Zc_max`, `iLS_boundary` | 抓 H 矩阵尺寸、地址生成、banking。 | [[T8.2_NR_LDPC_base_graph_selection|T8.2]], [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]], [[T9.6_NR_LDPC_decoder_edge_cases|T9.6]]。 |
| Rate recovery/RV | `rv0..rv3`, `k0_boundary`, `limited_buffer`, `punctured_systematic` | 抓 k0（速率匹配起点，Rate matching starting position）、unknown LLR、limited buffer 地址。 | [[T9.1_NR_LDPC_rate_recovery_overview|T9.1]], [[T9.2_NR_LDPC_circular_buffer_states|T9.2]], [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]。 |
| CBG | `full_tb`, `partial_cbg`, `held_cbg`, `cbgfi_flush`, `cbgti_order_negative` | 抓部分重传和 CBG mask 位序。 | [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]], [[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]], [[T9.6_NR_LDPC_decoder_edge_cases|T9.6]]。 |
| Stop condition | `syndrome_pass_crc_pass`, `syndrome_pass_crc_fail`, `max_iter`, `timeout` | 抓 syndrome early stop 与 CRC 边界。 | [[T8.4_LDPC_Tanner_graph_message_passing|T8.4]], [[T8.7_layered_LDPC_decoding_schedule|T8.7]], [[T9.6_NR_LDPC_decoder_edge_cases|T9.6]]。 |
| Numeric health | `min_sum`, `nms`, `oms`, `llr_sat`, `cn_sat`, `vn_sat` | 抓定点缩放和饱和。 | [[T8.6_LDPC_MS_NMS_OMS|T8.6]], [[T18.3_NR_LDPC_fixed_point_model_plan|T18.3]], [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]]。 |
...
| 覆盖点 | 必选 bins | 为什么重要 | 回链 |
|:---|:---|:---|:---|
| Context | `DCI`, `UCI_on_PUCCH`, `UCI_on_PUSCH`, `small_no_crc` | 抓控制信息分支和 CRC 长度。 | [[T3.5_NR_Polar_segmentation_crc|T3.5]], [[T10.1_NR_Polar_decoder_chain_overview|T10.1]], [[T10.8_NR_Polar_decoder_edge_cases|T10.8]]。 |
| CRC/RNTI | `CRC6`, `CRC11`, `CRC24`, `RNTI_match`, `RNTI_mismatch` | 抓 CA-SCL（CRC 辅助 SCL，CRC-aided SCL）final selector。 | [[T10.6_CRC_aided_SCL_control_reliability|T10.6]], [[T10.8_NR_Polar_decoder_edge_cases|T10.8]]。 |
| List behavior | `L1_SC`, `L2`, `L4_or_more`, `PM_tie`, `all_crc_fail`, `second_best_crc_pass` | 抓路径复制、剪枝、tie-break 和 CRC-aided 选择。 | [[T10.5_Polar_SCL_decoding|T10.5]], [[T10.6_CRC_aided_SCL_control_reliability|T10.6]], [[T18.4_NR_Polar_fixed_point_model_plan|T18.4]]。 |
| Rate recovery | `puncture`, `shorten`, `repeat`, `subblock_deint`, `coded_bit_deint` | 抓 unknown/strong LLR 初始化和顺序。 | [[T10.7_NR_Polar_rate_recovery|T10.7]]。 |
| Frozen/information mask | `mask_boundary`, `wrong_mask_negative`, `reliability_prefix` | 抓 reliability sequence 和 frozen bit enforcement。 | [[T10.2_channel_polarization_frozen_bits|T10.2]], [[T10.3_NR_Polar_reliability_sequence|T10.3]], [[T10.8_NR_Polar_decoder_edge_cases|T10.8]]。 |
...
| Toggle coverage | SRAM（静态随机存取存储器，Static Random-Access Memory）地址线翻转过。 | 地址生成没跨 bank 冲突或越界。 |

断言覆盖率要和 [[T20.1_decoder_testbench_architecture|T20.1]] 的 assertions 对齐。建议必选断言包括：

| Assertion | 覆盖目标 | 失败含义 |
|:---|:---|:---|
| `no_config_write_when_busy` | busy 写保护被激活并通过。 | driver 或 DUT 配置锁失效。 |
| `stable_when_valid_not_ready` | backpressure 下 stream 稳定。 | DMA（直接存储器访问，Direct Memory Access）handshake 违例。 |
...
| `commit` | merge queue 或 CI | [[T20.2_protocol_vector_corner_case_suite|T20.2]] directed positive/negative、reset/timeout、LLR saturation、family selector。 | required directed vectors 全 pass；expected-fail 分类一致；关键断言无 fail。 |
| `nightly` | 每晚完整运行 | random/stress seeds、交叉覆盖、已知失败 replay、较长 timeout、覆盖率汇总。 | no critical regression；coverage trend 不下降；所有新失败分类。 |
| `weekly` | 每周或 release 前 | 长 seed sweep、BLER（块错误率，Block Error Rate）spot check、代码/断言覆盖、waiver review。 | 覆盖率阈值达标；waiver 有 owner/理由/到期时间。 |
...
随机回归必须可重放。建议使用 [[T17.1_python_golden_model_project_layout|T17.1]] 的分层 seed 派生方式：
...
| `vector_id` | [[T20.2_protocol_vector_corner_case_suite|T20.2]] vector 或 random vector 名称。 |
...
覆盖率计划最终要落到 RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）验证环境。建议有四类硬件可观测点：
...
ASIC sign-off 阶段还要把代码覆盖率和综合可达性分开看。某些低功耗 gating、scan、debug-only trace 在 RTL 仿真中可能不进入功能 sign-off；它们需要 waiver 或单独 DFT（可测试性设计，Design for Test）/low-power 验证口径，不能混入 decoder functional coverage 统计。
...
| Gate | 标准 |
|:---|:---|
| Directed suite | [[T20.2_protocol_vector_corner_case_suite|T20.2]] required positive/negative vectors 100% pass；expected-fail class 与 policy 一致。 |
...
| Coverage event | 从 [[T20.1_decoder_testbench_architecture|T20.1]] monitor/scoreboard summary 生成归一化 event，检查字段完整性。 |
...
| :--- | :--- | :--- |
| LTE Turbo segmentation/filler/RV/CRC | TS 36.212 Rel-19 `36212-j30`；[[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]]/[[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]] | `3GPP_Rel19/processed/TS_36.212_36212-j30`；`docs/L2_协议算法/T7.*.md` |
| NR LDPC BG/$Z_c$/RV/CBG/CRC | TS 38.212 Rel-19 `38212-j30`；TS 38.214 Rel-19 `38214-j30`；T8/T9 | `3GPP_Rel19/processed/TS_38.212_38212-j30`；`docs/L2_协议算法/T9.*.md` |
| NR Polar CRC/list/frozen/rate recovery | TS 38.212 Rel-19 `38212-j30`；T10 | `3GPP_Rel19/processed/TS_38.212_38212-j30`；`docs/L2_协议算法/T10.*.md` |
| Directed vector suite | [[T20.2_protocol_vector_corner_case_suite|T20.2]] | `docs/L3_工程实现/T20.2_protocol_vector_corner_case_suite.md` |
| Testbench observability | [[T20.1_decoder_testbench_architecture|T20.1]] | `docs/L3_工程实现/T20.1_decoder_testbench_architecture.md` |
...
| 引用重建候选解释 | `audit_reference_rebuilds.py` 输出候选清单；候选均为协议证据边界或上游已复现表格回链：TS 36.212 章节用于 coverage bin 来源，不在本节复现公式；TS 36.212 Table 5.1.3-3 已由 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 复现；本节不新增协议公式/表格复现。 |
| 协议证据 | Coverage methodology 为工程策略。协议参数 bins 回链 T7/T9/T10/[[T20.2_protocol_vector_corner_case_suite|T20.2]] 和本地 Rel-19 路径。 |
...
- 本项目 `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md`。
