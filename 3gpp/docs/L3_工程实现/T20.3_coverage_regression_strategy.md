...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T5.5_decoder_hardware_verification_mindset|T5.5]] 译码硬件验证思维 | 知道 golden model、bit-exact、directed/random、coverage、波形和 failure replay 的目的。 |
...
| [[T10.5_Polar_SCL_decoding|T10.5]]-[[T10.8_NR_Polar_decoder_edge_cases|T10.8]] | `docs/L2_协议算法/T10.*.md` | 本节不重写 Polar SCL 推导。 |
...
协议前因后果可以这样理解：协议定义了“接收端可能遇到哪些合法和非法状态”；[[T20.2_protocol_vector_corner_case_suite|T20.2]] 把这些状态变成向量；T20.3 再把向量覆盖、RTL 代码覆盖、断言覆盖、失败分类和夜间回归组织成签收证据。若 coverage bin 没有可观测字段或可复现向量，它只是愿望清单，不是验证计划。
...
| Rate recovery/RV | `rv0..rv3`, `k0_boundary`, `limited_buffer`, `punctured_systematic` | 抓 k0、unknown LLR、limited buffer 地址。 | [[T9.1_NR_LDPC_rate_recovery_overview|T9.1]], [[T9.2_NR_LDPC_circular_buffer_states|T9.2]], [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]。 |
...
| CRC/RNTI | `CRC6`, `CRC11`, `CRC24`, `RNTI_match`, `RNTI_mismatch` | 抓 CA-SCL final selector。 | [[T10.6_CRC_aided_SCL_control_reliability|T10.6]], [[T10.8_NR_Polar_decoder_edge_cases|T10.8]]。 |
...
## 图示
![图 1：coverage regression strategy](assets/T20.3_coverage_regression_strategy.svg)

## 执行与证据记录

| 项目 | 证据 |
|:---|:---|
| 图形资产 | `docs/L3_工程实现/assets/T20.3_coverage_regression_strategy.svg` | 手绘 SVG，2026-08-04 替代原 PIL 渲染 PNG（原 `render_t15_3_coverage_regression_strategy.py` 与 PNG 已删除，图示语义不变）；SVG 几何审计六项检查全部通过，cairosvg 渲染验证无报错。 |
