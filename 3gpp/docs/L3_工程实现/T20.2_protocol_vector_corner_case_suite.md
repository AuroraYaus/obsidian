...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3_工程实现/T20.2_protocol_vector_corner_case_suite.md"
---
# T20.2 协议向量与边界案例套件：从 Rel-19 证据到 directed regression
## 本节学习目标

本节定义译码器验证中的协议向量与边界案例套件。这里的“协议向量”不是随便生成的一组随机比特，而是能从3GPP Release 19 证据回链到本地协议文件、上游讲义、向量生成脚本和预期输出的一条测试样本。LTE Turbo（Turbo 码，Turbo Code）、NR LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）和 NR Polar（极化码，Polar Code）都有自己的协议边界：码块、循环缓存、填充位、冗余版本、码块组、冻结位、列表译码和循环冗余校验/RNTI（无线网络临时标识，Radio Network Temporary Identifier）selector 都可能成为 directed test 的主角。


- 把一条测试样本拆成协议派生字段、实现策略字段、随机种子、输入 LLR（对数似然比，Log-Likelihood Ratio）、预期输出和失败证据。
...
- 把 [[T20.1_decoder_testbench_architecture|T20.1]] 的 testbench loader/scoreboard 和 [[T18.6_bit_exact_regression_harness|T18.6]] 的 bit-exact（比特精确，bit-exact） 回归框架连接起来。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]] LTE Turbo 边界案例 | 知道 filler、`<NULL>`、打孔、重复、RV（冗余版本，Redundancy Version）、LLR 符号、CRC（循环冗余校验，Cyclic Redundancy Check）误通过和 timeout 的区别。 |
| [[T9.6_NR_LDPC_decoder_edge_cases|T9.6]] NR LDPC 边界案例 | 知道 BG（基图，Base Graph）/$Z_c$、filler、punctured systematic bits、limited buffer、RV、CBG（码块组，Code Block Group）、syndrome/CRC 边界。 |
| [[T10.8_NR_Polar_decoder_edge_cases|T10.8]] NR Polar 边界案例 | 知道 small block、CRC 长度、list size、PM tie、puncturing/shortening、frozen mask、RNTI 边界。 |
| [[T18.6_bit_exact_regression_harness|T18.6]] Bit-exact 回归框架 | 知道 vector schema、hash、checkpoint manifest、failure bundle 和比较策略。 |
| [[T20.1_decoder_testbench_architecture|T20.1]] Testbench 架构 | 知道 loader、driver、monitor、scoreboard、assertion、reset/timeout sequence 的角色。 |

自检问题：

| 问题 | 若回答不出来，先回看 |
|:---|:---|
| 为什么 CRC fail 不是根因分类？ | [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]、[[T9.6_NR_LDPC_decoder_edge_cases|T9.6]]、[[T10.8_NR_Polar_decoder_edge_cases|T10.8]]。 |
| 为什么 expected-fail vector 也可能是“测试通过”？ | [[T20.1_decoder_testbench_architecture|T20.1]] 的 pass/fail policy。 |
| 为什么 RV mismatch 必须保存 address trace，而不是只保存 `rv=2`？ | [[T7.3_LTE_HARQ_soft_buffer_RV|T7.3]]、[[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]、[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]、[[T9.6_NR_LDPC_decoder_edge_cases|T9.6]]。 |
| 为什么 Polar list size 不是 3GPP 强制字段？ | [[T10.5_Polar_SCL_decoding|T10.5]]、[[T10.8_NR_Polar_decoder_edge_cases|T10.8]]、[[T18.4_NR_Polar_fixed_point_model_plan|T18.4]]。 |
...
| 资料 | 本地路径 | 边界 |
| :--- | :--- | :--- |
| TS 36.212 Rel-19 `36212-j30` | `3GPP_Rel19/processed/TS_36.212_36212-j30` | 本节不重新复现 Table 5.1.3-3 全表，使用 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]/T6/T7 上游结论。 |
...
| LTE 边界章节 | `docs/L2_协议算法/T7.6_LTE_Turbo_decoder_edge_cases.md` | T20.2 转换为套件清单，不重写 [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]] 全部解释。 |
| NR LDPC 边界章节 | `docs/L2_协议算法/T9.6_NR_LDPC_decoder_edge_cases.md` | T20.2 只汇总为向量矩阵。 |
| NR Polar 边界章节 | `docs/L2_协议算法/T10.8_NR_Polar_decoder_edge_cases.md` | T20.2 不重新推导 SCL（连续消除列表译码，Successive Cancellation List）。 |
| Bit-exact 框架 | `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md` | 工程框架，不是 3GPP 条文。 |
| Testbench 架构 | `docs/L3_工程实现/T20.1_decoder_testbench_architecture.md` | T20.2 输出给 [[T20.1_decoder_testbench_architecture|T20.1]] 消费。 |

协议前因后果可以按接收端反推。发射端按照 TS 36.212 或 TS 38.212 把 TB（传输块，Transport Block）变成可发送的编码比特；接收端拿到的是被资源、调制、RV、CBG、rate matching 和噪声改变后的软信息。协议向量的作用就是把“按协议应该发生什么”固定下来，使 RTL（寄存器传输级，Register Transfer Level）或 C/C++ 实现失败时能判断是协议字段、地址、数值、状态机还是比较策略错。

## 协议向量不是随机样本

随机样本适合统计 BLER（块错误率，Block Error Rate），但不适合证明边界。比如随机生成 10000 个 NR LDPC TB，也不一定正好落在 BG1/BG2 分支边界、$Z_c$ 表切换点、limited buffer、RV 回绕、CBG hold/flush 或 syndrome pass but CRC fail 的组合上。协议向量套件需要显式设计这些点。
...
![图 1：协议向量套件——Rel-19 Evidence 与 Suite Manifest 驱动三条协议 Lane（Turbo/LDPC/Polar），Pass/Fail Policy 规定正负向量判定，输出 Expected Outcome 与 Failure Bundle](assets/T20.2_protocol_vector_corner_case_suite.svg)
...
| LTE Turbo Lane | segmentation, filler, interleaver, RV window, soft combine, CB（码块，Code Block）/TB CRC and timeout. |
...
[[T18.6_bit_exact_regression_harness|T18.6]] 已定义统一 vector schema。T20.2 在它上面增加 suite 分类和 corner-case 语义。建议最小目录如下：
...
expected-fail 的通过条件需要特别强调。若一个 `RV_MISMATCH_NEGATIVE` 向量被 DUT（待测设计，Device Under Test）译成 CRC pass，测试应 fail；若 DUT 报出 `RATE_RECOVERY_OR_RV_MISMATCH` 并保存 `rv/k0/address_trace`，测试才 pass。换言之，negative vector 的正确结果是“按预期失败并可定位”，不是“强行译对”。

## LTE Turbo Directed Vector Suite

LTE Turbo suite 的证据主线来自 TS 36.212 §5.1.1、§5.1.2、§5.1.3.2 和 §5.1.4.1。接收端的测试主线是：分段 descriptor 正确，filler 和 `<NULL>` 语义正确，循环缓存地址正确，HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）软合并正确，CB/TB CRC 和 timeout 判定正确。
...
| `lte_llr_sign_negative` | directed negative | 解调器和译码器 LLR 符号约定反。 | fixed BPSK（二进制相移键控，Binary Phase Shift Keying）vector, `positive_means_bit0`, hard decision | fail class 为 `LLR_SIGN_CONVENTION_MISMATCH`。 |
...
最小 LTE dump 包应直接继承 [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]]：
...
| `ldpc_rv_mismatch_negative` | directed negative | DCI（下行控制信息，Downlink Control Information）RV、HARQ state 或 k0（速率匹配起点，Rate matching starting position）表错。 | `rvidx`, `k0`, `BG`, `Zc`, `Ncb`, address trace | fail class 为 `RATE_RECOVERY_OR_RV_MISMATCH`。 |
...
| `ldpc_crc_pass_upper_assembly_fail` | directed negative | PHY pass 后交付元数据错。 | `tb_id`, `cw_id`, `harq_id`, MAC（媒体接入控制层，Medium Access Control）length, LCID（逻辑信道标识，Logical Channel Identity） | fail class 指向 upper assembly/context，不改 LDPC core。 |
...
NR Polar suite 的证据主线来自 TS 38.212 §5.1、§5.2.1、§5.3.1、§5.4.1、§6.3 和 §7.3。Polar suite 和数据链路 suite 最大差异是：很多错误发生在 selector 层，而不是 SC（逐次消除，Successive Cancellation）/SCL core 本身。CRC 长度、RNTI、small block 分支和 frozen mask 都会改变“候选路径如何解释”。
...
| `polar_small_block_no_crc` | directed positive/negative | small block 分支被误送 CA-SCL（CRC 辅助 SCL，CRC-aided SCL）CRC selector。 | `context_type`, `A`, `small_block_branch`, `crc_len=0` | 走 small block 预期路径；不报 `CRC_FAIL`。 |
| `polar_uci_crc_6_11` | directed positive | UCI（上行控制信息，Uplink Control Information）6/11 bit CRC 长度选择。 | UCI context, `A`, `E`, `crc_len`, CRC input range | selector 使用正确 CRC range。 |
...
|:---|:---|:---|:---|
| `llr_sign_convention_all` | 三类 decoder 对 LLR 符号解释不一致。 | fixed BPSK/QPSK（正交相移键控，Quadrature Phase Shift Keying）snippets, `positive_means_bit0` | 三类 hard decision 和第一层 checkpoint 一致。 |
...
这些向量也为 [[T20.3_coverage_regression_strategy|T20.3]] 的 coverage bins 提供种子。例如 `llr_sign_convention_all` 可以同时覆盖 `decoder_family`、`llr_sign_convention`、`expected_status` 和 `checkpoint_present` 四类 coverage。
...
协议向量最终要驱动 RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）验证，因此 suite 字段必须能映射到硬件可观测点：

| 硬件对象 | 向量字段 | 验证用途 |
|:---|:---|:---|
| descriptor register/shadow | `protocol_fields`, `descriptor_hash` | 证明硬件锁存的协议任务和 reference 一致。 |
| input DMA（直接存储器访问，Direct Memory Access） | `input_llr`, `valid_mask`, `llr_format` | 证明 LLR 符号、宽度、端序和 backpressure 正确。 |
| soft buffer SRAM（静态随机存取存储器，Static Random-Access Memory） | `harq_id`, `rv_idx`, `addr_trace_hash`, `soft_before/after` | 证明 RV、CBG、reset/abort 和饱和合并正确。 |
...
|:---|:---|:---|:---|:---|:---|
| 基带芯片 RTL bring-up | T20.2 smoke + directed positive + directed negative 三层 suite。 | payload/status/checkpoint/failure bundle。 | 三类 engine 共用 DMA、soft buffer 和 status path。 | [[T20.1_decoder_testbench_architecture|T20.1]] scoreboard 对比 C/C++ fixed expected。 | LDPC CBG negative 被错误 CRC pass，暴露 CBGTI bit order 反。 |
...
| 结论 | 证据 | 本地路径 | 边界 |
|:---|:---|:---|:---|
| LTE Turbo vectors 的 CB/TB CRC、分段、filler、Turbo interleaver 和 rate matching 来自 TS 36.212。 | TS 36.212 Rel-19 `36212-j30` §5.1.1、§5.1.2、§5.1.3.2、§5.1.4.1；项目 [[T3.3_LTE_Turbo_segmentation_rules|T3.3]]、T6、T7。 | `3GPP_Rel19/processed/TS_36.212_36212-j30`、`docs/L2_协议算法/T7.6_LTE_Turbo_decoder_edge_cases.md` | 本节不重建完整协议表；exact K/f1/f2 表由上游已生成资产承接。 |
| NR LDPC vectors 的 BG/$Z_c$/filler/rate matching/RV/CRC 来自 TS 38.212。 | TS 38.212 Rel-19 `38212-j30` §5.2.2、§5.3.2、§5.4.2、§6.2、§7.2；项目 T8/T9。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`docs/L2_协议算法/T9.6_NR_LDPC_decoder_edge_cases.md` | CBG 调度上下文需同时回链 TS 38.214；本节不复现 MCS（调制与编码方案，Modulation and Coding Scheme）/TBS（传输块大小，Transport Block Size）表。 |
| NR CBG directed vectors 需要 CBGTI/CBGFI 上下文。 | TS 38.214 Rel-19 `38214-j30` §5.1.7/§6.1.5；项目 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]/[[T9.6_NR_LDPC_decoder_edge_cases|T9.6]]。 | `3GPP_Rel19/processed/TS_38.214_38214-j30` | 向量保存 CBG mask 和 fail class；调度字段解析实现另行验证。 |
...
| vector schema、checkpoint manifest、failure bundle 属于工程回归框架。 | 项目 [[T18.6_bit_exact_regression_harness|T18.6]]、[[T20.1_decoder_testbench_architecture|T20.1]]。 | `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md`、`docs/L3_工程实现/T20.1_decoder_testbench_architecture.md` | 不属于 3GPP 强制流程。 |
...
| 依赖读取 | `docs/L2_协议算法/T7.6_LTE_Turbo_decoder_edge_cases.md`、`docs/L2_协议算法/T9.6_NR_LDPC_decoder_edge_cases.md`、`docs/L2_协议算法/T10.8_NR_Polar_decoder_edge_cases.md`、`docs/L3_工程实现/T18.6_bit_exact_regression_harness.md`、`docs/L3_工程实现/T20.1_decoder_testbench_architecture.md`。 | 已抽取三类 directed cases、最小 dump 字段、vector schema 和 testbench 消费方式。 |
| Python 诊断片段 | 正文 `classify_rate_recovery()` 片段。 | 输出 `T20.2_VECTOR_CLASSIFIER_OK [6, 7]`。 |
| 引用重建候选解释 | `python3 tools/audit_reference_rebuilds.py docs/L3_工程实现/T20.2_protocol_vector_corner_case_suite.md` | 输出候选清单：TS 36.212/TS 38.212 表格候选已在正文明确回链上游 T3/T6/T7/T8/T9/T10 已复现资产或标注本节不重建；“未核验表值”出现在常见错误和协议证据边界，用作防护规则；Project 引用为本项目内部讲义来源，不是外部论文图表缺失。 |
...
- [Project, 2026] [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]] LTE Turbo decoder edge cases, `docs/L2_协议算法/T7.6_LTE_Turbo_decoder_edge_cases.md`.
- [Project, 2026] [[T9.6_NR_LDPC_decoder_edge_cases|T9.6]] NR LDPC decoder edge cases, `docs/L2_协议算法/T9.6_NR_LDPC_decoder_edge_cases.md`.
- [Project, 2026] [[T10.8_NR_Polar_decoder_edge_cases|T10.8]] NR Polar decoder edge cases, `docs/L2_协议算法/T10.8_NR_Polar_decoder_edge_cases.md`.
- [Project, 2026] [[T18.6_bit_exact_regression_harness|T18.6]] Bit-exact regression harness, `docs/L3_工程实现/T18.6_bit_exact_regression_harness.md`.
- [Project, 2026] [[T20.1_decoder_testbench_architecture|T20.1]] Decoder SystemVerilog testbench architecture, `docs/L3_工程实现/T20.1_decoder_testbench_architecture.md`.
