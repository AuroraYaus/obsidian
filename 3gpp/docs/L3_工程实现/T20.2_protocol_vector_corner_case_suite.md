...
- 把 [[T20.1_decoder_testbench_architecture|T20.1]] 的 testbench loader/scoreboard 和 [[T18.6_bit_exact_regression_harness|T18.6]] 的 bit-exact 回归框架连接起来。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T7.6_LTE_Turbo_decoder_edge_cases|T7.6]] LTE Turbo 边界案例 | 知道 filler、`<NULL>`、打孔、重复、RV、LLR 符号、CRC 误通过和 timeout 的区别。 |
| [[T9.6_NR_LDPC_decoder_edge_cases|T9.6]] NR LDPC 边界案例 | 知道 BG/$Z_c$、filler、punctured systematic bits、limited buffer、RV、CBG、syndrome/CRC 边界。 |
...
## 图示
![图 1：protocol vector corner case suite](assets/T20.2_protocol_vector_corner_case_suite.svg)
...
| 图形资产 | `docs/L3_工程实现/assets/T20.2_protocol_vector_corner_case_suite.svg` | 手绘 SVG，2026-08-04 替代原 PIL 渲染 PNG（原 `render_t15_2_protocol_vector_corner_case_suite.py` 与 PNG 已删除，图示语义不变）；SVG 几何审计六项检查全部通过，cairosvg 渲染验证无报错。 |
