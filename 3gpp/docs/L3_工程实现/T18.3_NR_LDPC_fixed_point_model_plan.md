...
- 给出 NMS/OMS 实验口径和 BLER（块错误率，Block Error Rate） 损失比较方法，避免把算法近似损失和量化损失混在一起。

如果只记住一句话：NR LDPC 定点模型不是把浮点 message passing 改成 `int16_t`，而是把协议矩阵身份、整数消息语义、min1/min2 来源、layered 写回、syndrome/CRC（循环冗余校验，Cyclic Redundancy Check） gate 和曲线损失预算一起固定下来。

## 前置知识检查

| 前置项 | 本节需要达到的程度 |
|:---|:---|
| [[T8.6_LDPC_MS_NMS_OMS|T8.6]] LDPC MS/NMS/OMS | 知道 CN 输出符号来自其他输入符号，幅度来自排除自身后的最小值；知道 NMS 的 $\alpha$ 和 OMS 的 $\beta$ 是实现参数。 |
| [[T8.7_layered_LDPC_decoding_schedule|T8.7]] LDPC layered schedule | 知道 layered update 的核心是 $L_j\leftarrow L_j-r_{old}+r_{new}$，后续 layer 读取新 posterior。 |
| [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] NR LDPC 混合自动重传请求soft buffer | 知道 RV、$k_0$、$E$、$N_{\mathrm{cb}}$、CBG（码块组，Code Block Group） 和 soft combining 会决定 LDPC 输入 LLR。 |
| [[T17.3_NR_LDPC_float_sim_plan|T17.3]] NR LDPC 浮点仿真计划 | 知道浮点基线如何输出 BP/MS/NMS/OMS 曲线、syndrome trace、message statistics 和 failure dump。 |
| [[T18.1_fixed_point_decoder_requirements|T18.1]] 定点译码器需求 | 知道 Q 格式、裁剪、舍入、饱和、性能损失预算和 bit-exact 检查点。 |

如果还不清楚 CN、VN、posterior LLR 和 syndrome 的区别，先回看 [[T8.4_LDPC_Tanner_graph_message_passing|T8.4]]-[[T8.8_NR_LDPC_decoder_numeric_walkthrough|T8.8]]。T18.3 默认你已经知道 LDPC 是一张 Tanner graph 上的消息传递译码，本节只把这些消息变成可审计的整数。
...
| 编号 | T18.3 |
| 前置 | T8.6, T9.3, T18.1 |
| Prompt | 请规划 C/C++ 定点 NR LDPC 译码器模型：LLR/消息位宽、最小/次小存储、归一化/偏移、layered 调度、饱和、syndrome 检查和 bit-exact 测试。写作时参考本文”单节讲义弹性审计清单”，按本节性质取舍：基础课重理论概念、解释和推导；协议课重 3GPP 前因后果和接收侧流程；工程课重仿真、定点、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）、验证和证据记录。 |
...
| TS 38.212 §5.3.2 | `content.md`；`tables/table_0013/0014/0015.csv/html` | 本节使用上游 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] 重建资产，不重画完整 BG shift table。 |
| TS 38.212 §5.4.2/§5.4.2.1/§5.4.2.2 | `content.md`；[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]/[[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] 回链 | 接收端 LLR soft buffer 写回和饱和合并是逆向实现策略。 |
| [[T8.6_LDPC_MS_NMS_OMS|T8.6]] | `docs/L2_协议算法/T8.6_LDPC_MS_NMS_OMS.md` | 算法实现，不是 3GPP 强制译码算法。 |
| [[T8.7_layered_LDPC_decoding_schedule|T8.7]] | `docs/L2_协议算法/T8.7_layered_LDPC_decoding_schedule.md` | 调度是实现策略，TS 38.212 不规定 decoder schedule。 |
| [[T17.3_NR_LDPC_float_sim_plan|T17.3]] | `docs/L3_工程实现/T17.3_NR_LDPC_float_sim_plan.md` | 本节不生成真实 fixed BLER 曲线。 |
...
| syndrome trace | hard decision 代入 $H$ 后的校验子。 | early stop 和失败定位。 | syndrome zero 不能替代 TB（传输块，Transport Block） CRC。 |
...
![图 1：NR LDPC 定点数据通路——Protocol Vector 经 LLR Quantizer、CN Unit、Layered Memory 到 Syndrome/Report，五层比较点（Rate recovery、CN update、Layered RMW、VN/posterior、Syndrome/CRC），底部为位宽与算法选项](assets/T18.3_NR_LDPC_fixed_point_model_plan.svg)

| 内容 | 说明 |
|:---|:---|
| Stage | Integer Object；Compare Field；Failure Signal |
| Protocol Vector | LLR Quantizer；CN Unit；Layered Memory；Syndrome/Report |
| BG, Zc, lifting set, RV, k0（速率匹配起点，Rate matching starting position）, E, Ncb, CBG masks and TS 38.212 evidence. | Apply width, Q format, sign convention, clipping and soft-combine saturation.；Compute sign product, min1/min2, MS/NMS/OMS correction and output saturation.；Read old_R, form q=L-old_R, write new_R and updated posterior LL |
...
| Syndrome/CRC | syndrome, CB（码块，Code Block） /TB CRC；stop_status, iter_used；false early stop |
...
C/C++ 定点模型应从 [[T17.3_NR_LDPC_float_sim_plan|T17.3]] run directory 或 failure bundle 读取对象，而不是手写散装数组。
...
这些不是协议常数，只是实验入口。最终选择必须由 [[T17.5_BER_BLER_curve_reporting|T17.5]] 曲线和 [[T18.6_bit_exact_regression_harness|T18.6]] bit-exact regression 证明。
...
[[T18.6_bit_exact_regression_harness|T18.6]] 的 bit-exact compare 应能在 `layer_trace` 中定位第一处 mismatch。

## C/C++ 数组布局计划

第一版 C/C++ 模型以清晰和 bit-exact 为先，不先追求单指令多数据（Single Instruction, Multiple Data, SIMD）。[[T18.5_SIMD_memory_layout_decoders|T18.5]] 会再处理 SIMD lane、cache 和 bank。
...
| Python float | [[T17.3_NR_LDPC_float_sim_plan|T17.3]] 浮点 BP/MS/NMS/OMS | Python/C fixed | 曲线和统计比较，不要求整数一致。 |
...
| 字段 | 说明 |
|:---|:---|
| `float_baseline_run_id` | [[T17.3_NR_LDPC_float_sim_plan|T17.3]] 浮点基线。 |
| `fixed_run_id` | 当前定点运行。 |
| `decoder_variant` | `ms`、`nms`、`oms`。 |
| `nms_alpha_q`、`oms_beta_q` | 定点参数。 |
| `fixed_requirement_id` | 位宽/缩放/饱和规格版本。 |
| `fixed_loss_budget_db` | 例如 0.10 dB。 |
| `bler_ratio_budget` | 同一 SNR（信噪比，Signal-to-Noise Ratio） 点 BLER 比值预算。 |
...
T18.3 的定点模型会直接变成 [[T19.2_NR_LDPC_RTL_microarchitecture|T19.2]] NR LDPC RTL 微架构的前置规格。
...
| `layer_schedule` | edge schedule ROM（只读存储器，Read-Only Memory） 和 address generator。 | BG/Zc/shift 地址错。 |
...
RTL 可以选择不同并行度、bank mapping、layer folding 或 partial parallelism；但只要它声明兼容某个 `fixed_requirement_id`，就必须在 [[T18.6_bit_exact_regression_harness|T18.6]]/T15 中通过同一组 bit-exact 检查点。
...
| HARQ（混合自动重传请求，Hybrid Automatic Repeat Request） /RV | 使用 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 风格 RV0/RV2 重传向量。 | soft buffer 合并和 input LLR checkpoint 一致。 |
...
| :--- | :--- | :--- | :--- |
| NR LDPC segmentation | TS 38.212 `38212-j30` §5.2.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md` | 已由 [[T17.3_NR_LDPC_float_sim_plan|T17.3]]/[[T9.5_NR_LDPC_reassembly_TB_CRC|T9.5]] 回链。 |
| NR LDPC coding and H | TS 38.212 §5.3.2、Table 5.3.2-1/2/3 | `tables/table_0013/0014/0015.csv/html`；[[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] | 上游已复现。 |
| NR LDPC rate matching | TS 38.212 §5.4.2/§5.4.2.1/§5.4.2.2 | `content.md`；[[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]]/[[T9.4_NR_LDPC_bit_deinterleaving|T9.4]] | 使用上游证据回链。 |
| MS/NMS/OMS | 非 3GPP 强制算法 | [[T8.6_LDPC_MS_NMS_OMS|T8.6]]；Fossorier/Chen 文献 | 实现策略。 |
| Layered schedule | 非 3GPP 强制调度 | [[T8.7_layered_LDPC_decoding_schedule|T8.7]]；Hocevar 文献 | 实现策略。 |

## 未复现协议资产与关闭条件

| 候选资产 | 本节处理 | 关闭条件 |
|:---|:---|:---|
| TS 38.212 Table 5.3.2-1/2/3 全表 | 本节回链 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]]，不重新复制长表。 | 若 T18.3 代码直接解析原表生成 layer schedule，必须把 [[T8.3_NR_LDPC_lifting_QC_matrix|T8.3]] CSV/HTML、脚本和 hash 纳入 fixed evidence archive。 |
| TS 38.212 Table 5.4.2.1-2 | 本节只消费 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 的 `rv/k0/Ncb/E` descriptor。 | 若 fixed model 从协议表自动生成 $k_0$，必须纳入 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 的公式证据和 toy 校验。 |
| TS 38.214 调制与编码方案/传输块大小/CBG 表 | 本节不使用具体 MCS（调制与编码方案，Modulation and Coding Scheme） /TBS（传输块大小，Transport Block Size） 表值，只要求 vector 已包含 $E/Q_m/RV/CBGTI$。 | 若 fixed vector 从 MCS index 自动生成 TB size、$Q_m$ 或 CBG mask，必须重建实际使用表格子集。 |
