...
| [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] NR LDPC 混合自动重传请求soft buffer | 知道 RV、$k_0$、$E$、$N_{\mathrm{cb}}$、CBG和 soft combining 会决定 LDPC 输入 LLR。 |
...
| HARQ/RV | 使用 [[T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0|T9.3]] 风格 RV0/RV2 重传向量。 | soft buffer 合并和 input LLR checkpoint 一致。 |
...
## 图示
![图 1：NR LDPC fixed point model](assets/T18.3_NR_LDPC_fixed_point_model.svg)
...
| 新增证据资产 | `docs/L3_工程实现/assets/T18.3_NR_LDPC_fixed_point_model.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t13_3_nr_ldpc_fixed_point_model.py`）。 |
