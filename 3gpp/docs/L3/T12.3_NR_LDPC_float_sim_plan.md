...
type: spec
aliases:
  - T12.3 NR LDPC float sim plan
tags:
  - 3gpp
  - docs
  - l3
  - lesson
source_spec: "docs/L3/T12.3_NR_LDPC_float_sim_plan.md"
---
# T12.3 NR LDPC 浮点仿真计划：从基图、提升与速率恢复到可比较 BLER 曲线
## 本节学习目标
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| T8.8 NR LDPC 译码数值走读 | 知道校验节点（Check Node, CN）、变量节点（Variable Node, VN）、syndrome、MS 一轮更新和硬判决。 |
| T9.5 NR LDPC 码块重组与 TB CRC | 知道 TB 可分成多个 CB，CBG、filler、CB CRC 和 TB CRC 的接收端边界。 |
| T12.1 Python Golden Model 工程布局 | 知道 `vector.json`、`seeds.json`、`metrics.csv`、`frames.jsonl`、失败 replay 和 evidence archive 的职责。 |
| T9.3/T9.4 速率恢复 | 知道 RV、$k_0$、limited-buffer rate matching、bit deinterleaving 和软缓存状态会影响 LLR 放置。 |
...
本节是工程仿真计划，不重复完整复现 TS 38.212 的 BG1/BG2 shift table。上游 T8.3 已表格化重建 TS 38.212 Table 5.3.2-2/3，T9.3 已讨论 Table 5.4.2.1-2 的 $k_0$ 边界，T9.4 已固化 bit interleaving 公式证据。本节把这些协议资产接入浮点实验。
...
|:---|:---|:---|:---|
| TS 38.212 §5.2.2 | `TS_36.212_36212-j30_content.md` around line 717 | LDPC 分段、BG1/BG2 最大 CB size、CB CRC、filler、$K_b$ 和 $Z_c$ 选择来源。 | descriptor 必须记录 BG、$K_b$、$Z_c$、$C$、$K$、$K'$、filler 和 CB CRC present。 |
| TS 38.212 §5.3.2 | `TS_36.212_36212-j30_content.md` around line 945；`tables/table_0013/0014/0015.csv/html` | LDPC 编码、BG1/BG2 矩阵大小、lifting set、shift table 和 parity-check matrix 生成。 | decoder 的 Tanner graph、layer schedule 和 syndrome checker 必须来自同一 $H$。 |
| TS 38.212 §5.4.2、§5.4.2.1、§5.4.2.2 | `TS_36.212_36212-j30_content.md` around line 1175 | LDPC rate matching、bit selection、limited buffer、RV $k_0$ 和 bit interleaving。 | rate recovery 必须回填正确 $N_{\mathrm{cb}}$ 地址并执行 bit deinterleaving。 |
| TS 38.212 §6.2、§7.2 | `TS_36.212_36212-j30_content.md` 上行共享信道/下行共享信道chain | UL-SCH/DL-SCH 调用 TB CRC、BG selection、CB segmentation、LDPC coding、rate matching 和 CB concatenation。 | 仿真要区分 channel/direction，并记录 UL/DL 调用链。 |
| TS 38.214 §5.1.3、§6.1.4 | `TS_36.212_36212-j30_content.md` PDSCH/PUSCH MCS/TBS sections | MCS、$Q_m$、target code rate、RV 和 TBS 的系统输入来源。 | 若使用系统级输入，必须记录 MCS table、$Q_m$、target rate、TBS 和下行控制信息/RV 来源。 |
| TS 38.214 §5.1.7.1、§5.1.7.2 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:2407-2437` | PDSCH CBG 分组、CBGTI bit mapping、CBG 是否传输和 CBGFI 合并/刷新语义。 | 下行 CBG 部分重传测试必须记录 CBGTI/CBGFI、held/updated CB 状态和旧实例是否可合并。 |
| TS 38.214 §6.1.5.1、§6.1.5.2 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:7099-7125` | PUSCH CBG 分组、CBGTI bit order、初传/重传时哪些 CBG 被包含。 | 上行 CBG 部分重传测试必须记录 CBGTI 从 MSB 到 CBG#0 的映射，未调度 CBG 不产生新 LLR。 |
| TS 38.212 §5.4.2.1、§6.1.2.1.1、§7.3.1.2.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:1257-1289`；`3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:4489`；`3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:6097-6103` | LDPC bit selection 中未被 CBGTI 调度的 CB、PUSCH/PDSCH DCI 中 CBGTI/CBGFI 字段存在性。 | rate recovery 必须把 `scheduled_cb_count`、`held_cb_count`、CBGTI bit width 和 CBGFI present 写入 descriptor。 |
...
flowchart LR
N0["Protocol Config"]
N1["LDPC Decoder Variants"]
N2["Failure Replay Archive"]
N3["Case"]
N4["reference"]
N5["Record BG, Zc, lifting set, k0, Ncb, E, limited-buffer state and bit-..."]
N0 --> N1
N1 --> N2
N2 --> N3
N3 --> N4
...
classDef config fill:#e3f2fd,stroke:#263238,color:#102027;
classDef decode fill:#f1f8e9,stroke:#263238,color:#102027;
classDef archive fill:#e8eaf6,stroke:#263238,color:#102027;
classDef matrix fill:#ffffff,stroke:#607d8b,color:#102027;
classDef checks fill:#fffde7,stroke:#607d8b,color:#102027;
class N0 config;
class N1 decode;
class N2 archive;
class N3,N4 matrix;
...
NR LDPC rate matching 后的有效发送码率也必须固定口径。本节沿用 T12.2 的默认口径：
...
继承 T12.1 的 `canonical_json_sha256_u64le_v1` seed 派生规则。payload、noise、CBG mask injection、LLR saturation injection 必须使用不同 stage seed。
...
| 错误 | 现象 | 定位方法 |
|:---|:---|:---|
| BG/Zc 选错 | 高 SNR 仍大量 fail，syndrome 不收敛。 | 比较 descriptor 与 TS 38.212 §5.2.2/T8.2/T8.3。 |
...
| bit deinterleaving 方向反 | 高阶调制下 BLER 异常。 | 用 T9.4 toy pattern 验证 LLR 顺序。 |
...
| :--- | :--- | :--- | :--- |
| LDPC segmentation | TS 38.212 `38212-j30` §5.2.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md` | 已核验锚点；公式 XML 不在本节重转写。 |
| LDPC coding and H | TS 38.212 §5.3.2、Table 5.3.2-1/2/3 | `tables/table_0013/0014/0015.csv/html` | 上游 T8.3 已复现。 |
| LDPC rate matching | TS 38.212 §5.4.2、§5.4.2.1、§5.4.2.2 | `TS_36.212_36212-j30_content.md`、T9.3/T9.4 证据 | 使用上游证据回链。 |
| UL-SCH/DL-SCH chain | TS 38.212 §6.2、§7.2 | `TS_36.212_36212-j30_content.md` | 已核验锚点。 |
| MCS/TBS/RV | TS 38.214 `38214-j30` §5.1.3、§6.1.4 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md` | 本节不复现完整 MCS/TBS 表。 |
| CBG partial retransmission | TS 38.214 §5.1.7.1/§5.1.7.2、§6.1.5.1/§6.1.5.2；TS 38.212 §5.4.2.1、§6.1.2.1.1、§7.3.1.2.1 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:2407-2437`；`3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md:7099-7125`；`3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:1257-1289`；`3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:4489`；`3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:6097-6103` | 本节直接核验本地锚点，同时引用 T9.3/T9.5 的接收端细化。 |
...
| TS 38.212 Table 5.3.2-1/2/3 | 上游 T8.3 已重建 BG lifting set 和 shift table，本节只消费 `base_graph/lifting_set/Zc/shift_table_hash`。 | 若 T12.3 后续实现自己解析原表生成 $H$，必须把 T8.3 资产纳入 evidence archive，并用 `H_hash` 与上游重建结果逐项比对。 |
| TS 38.212 Table 5.4.2.1-2 | 上游 T9.3 已复现 `rvid/BG/k0` 公式形式，本节只要求 dump `rv/k0/Ncb/E`。 | 若仿真实现从协议表自动生成 $k_0$，必须重跑 T9.3 toy 校验并保存 `rv/k0` 地址 trace。 |
...
| 写入文件 | `docs/L3/T12.3_NR_LDPC_float_sim_plan.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T12.3。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 阶段 11 与 T12.3 条目。 |
| 前置依据 | 已读取 `docs/L2/T8.8_NR_LDPC_decoder_numeric_walkthrough.md`、`docs/L2/T9.5_NR_LDPC_reassembly_TB_CRC.md`、`docs/L3/T12.1_python_golden_model_project_layout.md`。 |
...
| 3GPP_译码知识库入口 核验 | 已读取 `TS_38.212_38212-j30/TS_36.213_36213-j30_s00-s05_index.md` 与 `TS_38.214_38214-j30/TS_36.213_36213-j30_s00-s05_index.md`；确认 `TS_36.212_36212-j30_content.md` 不是数学公式权威来源，公式应以 `equations/*.xml` 为准，表格优先看 `tables/*.html`。 |
| 未执行内容 | 本节是仿真计划，未运行真实 NR LDPC BLER 仿真；后续实现必须生成 `metrics.csv`、`frames.jsonl`、失败 dump 和曲线图。 |
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3/T12.3_NR_LDPC_float_sim_plan.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3/T12.3_NR_LDPC_float_sim_plan.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T12.3_NR_LDPC_float_sim_plan.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3/T12.3_NR_LDPC_float_sim_plan.md`：`LATEX_RENDER_AUDIT_OK formulas=104`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T12.3_NR_LDPC_float_sim_plan.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选来自 PCH/MCS/TBS 边界、上游已重建 TS 38.212 表格回链和正文公式引用，不构成本节失败。 |
...
- [上游讲义] `docs/L2/T8.8_NR_LDPC_decoder_numeric_walkthrough.md`，NR LDPC 译码数值走读。
- [上游讲义] `docs/L2/T9.5_NR_LDPC_reassembly_TB_CRC.md`，NR LDPC 码块重组与传输块 CRC。
