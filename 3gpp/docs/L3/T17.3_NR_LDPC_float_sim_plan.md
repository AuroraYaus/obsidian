...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T17.3_NR_LDPC_float_sim_plan.md"
---
...
NR LDPC rate matching 后的有效发送码率也必须固定口径。本节沿用 T17.2 的默认口径：
...
继承 T17.1 的 `canonical_json_sha256_u64le_v1` seed 派生规则。payload、noise、CBG mask injection、LLR saturation injection 必须使用不同 stage seed。
...
| LDPC rate matching | TS 38.212 §5.4.2、§5.4.2.1、§5.4.2.2 | `content.md`、T9.3/T9.4 证据 | 使用上游证据回链。 |
...
| TS 38.212 Table 5.3.2-1/2/3 | 上游 T8.3 已重建 BG lifting set 和 shift table，本节只消费 `base_graph/lifting_set/Zc/shift_table_hash`。 | 若 T17.3 后续实现自己解析原表生成 $H$，必须把 T8.3 资产纳入 evidence archive，并用 `H_hash` 与上游重建结果逐项比对。 |
...
## 图示
![图 1：NR LDPC float sim flow](assets/T12.3_NR_LDPC_float_sim_flow.svg)
...
| 写入文件 | `docs/L3/T17.3_NR_LDPC_float_sim_plan.md` |
| 新增证据资产 | `docs/L3/assets/T12.3_NR_LDPC_float_sim_flow.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/figures/render_t12_3_nr_ldpc_float_sim_flow.py`）。 |
...
| 前置依据 | 已读取 `docs/L2/T8.8_NR_LDPC_decoder_numeric_walkthrough.md`、`docs/L2/T9.5_NR_LDPC_reassembly_TB_CRC.md`、`docs/L3/T17.1_python_golden_model_project_layout.md`。 |
...
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3/T17.3_NR_LDPC_float_sim_plan.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3/T17.3_NR_LDPC_float_sim_plan.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T17.3_NR_LDPC_float_sim_plan.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3/T17.3_NR_LDPC_float_sim_plan.md`：`LATEX_RENDER_AUDIT_OK formulas=104`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T17.3_NR_LDPC_float_sim_plan.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选来自 PCH/MCS/TBS 边界、上游已重建 TS 38.212 表格回链和正文公式引用，不构成本节失败。 |
