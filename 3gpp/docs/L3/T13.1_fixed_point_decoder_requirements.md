...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T13.1_fixed_point_decoder_requirements.md"
---
# T13.1 定点译码器需求：浮点实验到整数模型规格
## 本节学习目标
...
- 写出一份后续 T13.2、T13.3、T13.4、T13.6 可直接复用的 fixed-point requirement 模板。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| T2.5 LLR 裁剪、缩放与量化 | 知道裁剪、缩放、量化、饱和和符号错误的基本含义。 |
...
| T12.1 Python Golden Model 工程布局 | 知道 `vector.json`、`metrics.csv`、`frames.jsonl`、seed registry、failure dump 和 replay command 的角色。 |
| T12.2-T12.5 浮点仿真计划 | 知道浮点基线如何输出 BER/BLER 曲线、置信区间、停止原因和失败样本。 |
...
| 专用扩展 | `turbo.*`、`ldpc.*`、`polar.*`、`simd_memory.*`、`regression.*` | 后续 T13.2-T13.6 的固定 namespace。公共表不允许把专用字段散落成临时名字。 |

公共字段只解决三类译码器共同拥有的数值契约。后续文档必须在固定 namespace 下继续扩展，而不是重新发明字段名：

| namespace | 后续章节 | 必须预留的字段 |
|:---|:---|:---|
| `turbo.*` | T13.2 LTE Turbo 定点模型 | `branch_metric_width/frac`、`alpha_width/frac`、`beta_width/frac`、`posterior_width/frac`、`extrinsic_width/frac`、`max_log_mode`、`maxstar_lut_id`、`alpha_norm_policy`、`beta_norm_policy`、`interleaver_addr_policy`、`tail_state_policy`。 |
| `ldpc.*` | T13.3 NR LDPC 定点模型 | `cn_msg_width/frac`、`vn_msg_width/frac`、`posterior_width/frac`、`min1_width`、`min2_width`、`sign_product_policy`、`nms_alpha_q`、`oms_beta_q`、`layer_schedule_id`、`syndrome_check_policy`、`message_memory_layout`。 |
| `polar.*` | T13.4 NR Polar 定点模型 | `fg_llr_width/frac`、`partial_sum_format`、`path_metric_width/frac`、`path_metric_norm_policy`、`list_size`、`list_pruning_policy`、`sorter_compare_width`、`crc_selector_policy`、`rnti_mask_policy`。 |
| `simd_memory.*` | T13.5 SIMD 与内存布局 | `lane_count`、`lane_mapping`、`alignment_bytes`、`bank_count`、`bank_conflict_policy`、`transpose_policy`、`packing_order`、`endian_policy`、`dma_burst_bytes`。 |
| `regression.*` | T13.6 Bit-Exact 回归 | `checkpoint_manifest`、`trace_format_version`、`first_mismatch_schema`、`golden_hash_policy`、`rerun_command_template`、`curve_budget_ref`、`failure_bundle_layout`。 |

这张扩展表的作用是约束命名和证据粒度。例如 T13.2 如果需要记录 alpha 归一化，就写 `turbo.alpha_norm_policy`；T13.3 如果需要记录 layered schedule，就写 `ldpc.layer_schedule_id`；T13.4 如果需要记录路径剪枝，就写 `polar.list_pruning_policy`。这样 T13.6 可以用同一个解析器读取需求，而不是为每个算法写一套临时规则。
...
这只是公共 LLR 需求片段，不是所有译码器的最终位宽。`null` 表示该字段由后续专用章节关闭，不能在实现阶段默默省略。T13.2 会补 Turbo 的分支度量、alpha/beta 和外信息；T13.3 会补 LDPC 的 CN/VN 消息、min1/min2、NMS/OMS 参数；T13.4 会补 Polar 的 f/g、路径度量、部分和和列表剪枝；T13.5 会补 SIMD lane、bank 和 packing；T13.6 会补 checkpoint manifest 和 failure bundle。
...
定点模型必须比浮点模型差一点也可解释，但“差一点”必须量化。推荐从 T12.5 的 `metrics.csv` 继承下面字段：
...
但式 (13) 必须和置信区间一起看。若浮点点位是零错误且只有上界，不能用一个很小的分母得出“定点差 100 倍”的结论；应回到 T12.5 的零错误上界和 `confidence_limited` 标记。
...
| 性能层 | BER/BLER、平均迭代、false-pass、饱和比例 | 按 T12.5 曲线报告和预算比较，不要求逐帧和浮点完全一致。 |
...
`first_mismatch.json` 至少写 `checkpoint_name`、`index`、`python_value`、`cpp_value`、`expected_width`、`round_mode`、`saturation_mode`、`descriptor_hash`。这样 T13.6 的回归框架可以直接读取。
...
assert sat_add(-25, -12, -31, 31) == -31
print("T13.1_FIXED_REQUIREMENT_TOY_OK")
```
...
若项目还没有真实 C/C++ 定点模型，T13.1 仍然可以先输出 requirement 模板和 Python reference 函数。后续 T13.2-T13.4 只要引用同一模板，就不会出现三篇文档各自定义一套位宽语义的情况。

## RTL/ASIC 映射

T13.1 不是 RTL 微架构设计，但它直接决定后续硬件对象。
...
硬件阶段不应重新发明定点语义。T14/T15 应从 `fixed_requirement.yaml` 读取或手工同步同名字段；若 RTL 为时序或面积修改位宽，必须生成新的 requirement version，并重新跑 T13.6 bit-exact 回归。
...
| 定点工具函数 | 执行上面的 Python 片段。 | 输出 `T13.1_FIXED_REQUIREMENT_TOY_OK`。 |
...
本节当前只关闭“需求文档级验证”：术语、标题、LaTeX 显示、图形几何、toy 量化函数和模板完整性。上表中的边界整数、符号扩展、Python-vs-C/C++ bit-exact 和 fixed-vs-float 曲线预算是后续模型级验证项目，必须由 T13.2-T13.6 在真实代码或真实 trace 出现后关闭，不能由 T13.1 代替关闭。
...
| 可执行验证 | Python 片段输出 `T13.1_FIXED_REQUIREMENT_TOY_OK`。 |
...
| T5.1 | 本节不重复完整基础课推导。 |
| T12.1 | 本节不创建真实 `sim/` 包。 |
| T12.2-T12.4 | 本节不规划各算法专属 C/C++ 数据结构，留给 T13.2-T13.4。 |
| T12.5 | 本节不生成真实 fixed BLER 曲线。 |
...
| 证据项 | 状态 | 关闭条件 |
| :--- | :--- | :--- |
| LTE Turbo 协议字段 | 回链 T7/T12.2 和 TS 36.212 Rel-19 `36212-j30`。 | T13.2 必须列具体本地路径和检查点。 |
| NR LDPC 协议字段 | 回链 T9/T12.3 和 TS 38.212 Rel-19 `38212-j30`。 | T13.3 必须列 BG、$Z_c$、RV、CBG 等字段来源。 |
| NR Polar 协议字段 | 回链 T10/T12.4 和 TS 38.212 Rel-19 `38212-j30`。 | T13.4 必须列 $A/K/E/N/L$、CRC/RNTI 和 rate recovery 来源。 |
...
| 写入文件 | `docs/L3/T13.1_fixed_point_decoder_requirements.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T13.1。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 模块 13 与 T13.1 条目。 |
| 上游讲义 | 已读取 `docs/L1/T2.5_LLR_clipping_scaling_quantization.md`、`docs/L1/T5.1_fixed_point_numbers_for_LLR.md`、`docs/L3/T12.1_python_golden_model_project_layout.md`。 |
| Python 验证 | 已执行正文 toy 片段，输出 `T13.1_FIXED_REQUIREMENT_TOY_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3/T13.1_fixed_point_decoder_requirements.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3/T13.1_fixed_point_decoder_requirements.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T13.1_fixed_point_decoder_requirements.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3/T13.1_fixed_point_decoder_requirements.md`：`LATEX_RENDER_AUDIT_OK formulas=34`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T13.1_fixed_point_decoder_requirements.md`：`REFERENCE_REBUILD_AUDIT_NO_CANDIDATES`。 |
| 未执行内容 | 本节未实现真实 C/C++ 定点模型，未生成真实 fixed BLER 曲线；边界整数、符号扩展、Python-vs-C/C++ bit-exact 和 fixed-vs-float 曲线预算属于 T13.2-T13.6 的模型级验证闭环。 |

## 参考文献

- [上游讲义] `docs/L1/T2.5_LLR_clipping_scaling_quantization.md`，LLR 裁剪、缩放与量化预览。
...
- [上游讲义] `docs/L3/T12.5_BER_BLER_curve_reporting.md`，BER/BLER 曲线生成与报告。
