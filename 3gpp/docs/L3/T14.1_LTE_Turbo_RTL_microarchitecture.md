...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md"
---
# T14.1 LTE TurboRTL 微架构：从 SISO 数据通路到 CRC 门控迭代控制
## 本节学习目标
...
| T13.2 LTE Turbo 定点模型 | 知道整数位宽、归一化、饱和、trace checkpoint 和 C/C++ fixed model 的角色。 |
| T13.6 Bit-Exact 回归框架 | 知道 RTL 输出必须和 C/C++ fixed trace 在 checkpoint 上逐整数比较。 |
...
| 资料 | 本地路径 | 边界 |
| :--- | :--- | :--- |
| TS 36.212 3GPP_译码知识库入口 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_index.md` | 3GPP_译码知识库入口 只证明抽取结构存在，不替代协议精读。 |
| TS 36.212 §5.1.3.2.1 | `TS_36.212_36212-j30_content.md:719-745` | 接收端 SISO/Log-MAP/Max-Log-MAP 是实现策略。 |
| TS 36.212 §5.1.3.2.2 | `TS_36.212_36212-j30_content.md:747-751` | RTL 如何初始化 beta、如何窗口化是实现策略。 |
| TS 36.212 §5.1.3.2.3/Table 5.1.3-3 | `TS_36.212_36212-j30_content.md:761-773`，`tables/table_0009.csv/html` | 本节回链 T3.3/T6.4 的表格复现，不重复完整长表。 |
| TS 36.212 §5.1.4.1 | `TS_36.212_36212-j30_content.md:777-789` | 本节只消费 rate recovery 后的 LLR 和 mask。 |
| TS 36.212 §5.1.4.1.2 | `TS_36.212_36212-j30_content.md:817-923` | RTL soft buffer bank、饱和加法和 replay dump 是实现策略。 |
| T13.2 | `docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md` | T14.1 把这些字段映射到硬件结构。 |
...
| `trace_status` | checkpoint signals | trace/status/error | 输出 bit-exact 对比字段、饱和计数、首个错误位置。 | T13.6 schema 约束可回放性。 |
...
Turbo decoder 的两个 SISO 观察同一批信息位，但第二个 SISO 按内部交织后的顺序观察。TS 36.212 Table 5.1.3-3 给出合法 $K$ 对应的 $f_1$ 和 $f_2$。内部交织地址由上游 T6.4/T13.2 承接，RTL 侧可以有两种实现：
...
T14.1 不假设多时钟域，教学架构使用单个 `clk` 和同步 active-high reset。若 SoC 顶层后来把 soft buffer、DMA 或 register bus 放在不同 clock domain，必须在 T14.4/T15 中引入 CDC 同步和异步 FIFO 证明。
...
RTL 位宽必须继承 T13.1/T13.2 的 fixed requirement，不能在硬件里单独“看起来够用”就改。最低字段：
...
定点最容易错的不是加法器，而是策略不一致：Python fixed 用 nearest rounding，RTL 用 truncation；C++ 每列减最大值，RTL 每 4 列归一化一次；Python 对 extrinsic 先缩放再裁剪，RTL 先裁剪再缩放。这些都必须进入 T13.6 的 `bit_exact_policy_hash`。
...
最小验证不应只看最终 CRC。T14.1 的 RTL testbench 至少要包含：

| 测试 | 输入 | 检查点 |
|:---|:---|:---|
| Smoke block | 小 $K$、无噪声、`max_iter=1` | FSM 顺序、descriptor latch、CRC gate 不提前。 |
| Interleaver address | 固定 $K/f_1/f_2$，遍历 $k$ | `pi/depi` 与 T6.4/T13.2 hash 一致。 |
...
| 对象 | 协议/本地证据 | 本节如何使用 |
|:---|:---|:---|
| Turbo encoder 结构 | TS 36.212 Rel-19 `36212-j30` §5.1.3.2.1，`TS_36.212_36212-j30_content.md:719-745` | 说明两个 8-state constituent encoders 和内部交织器是 SISO/trellis 来源。 |
| Trellis termination | TS 36.212 §5.1.3.2.2，`TS_36.212_36212-j30_content.md:747-751` | 说明 tail bits 和 beta 边界。 |
| Turbo internal interleaver | TS 36.212 §5.1.3.2.3/Table 5.1.3-3，`TS_36.212_36212-j30_content.md:761-773`，`tables/table_0009.csv/html` | 说明 $K/f_1/f_2$ 是地址生成器输入，完整表由 T3.3/T6.4 复现。 |
| Rate matching | TS 36.212 §5.1.4.1，`TS_36.212_36212-j30_content.md:777-789` | 说明三路流和 circular buffer 是 rate recovery 输入来源。 |
| RV/E/Ncb 边界 | TS 36.212 §5.1.4.1.2，`TS_36.212_36212-j30_content.md:817-923` | 说明 `E/Ncb/rv_idx` 属于 descriptor 和 soft buffer key。 |
...
- `docs/L3/assets/T14.1_LTE_Turbo_RTL_microarchitecture.png`

已执行表格生成和图形审计：

```bash
python3 tools/figures/render_t14_1_lte_turbo_rtl_microarchitecture.py
# WROTE docs/L3/assets/T14.1_LTE_Turbo_RTL_microarchitecture.png (2400, 2140)
...
```bash
python3 tools/audit_lesson_terms.py docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md
# LESSON_TERM_AUDIT_OK

python3 tools/audit_markdown_headings.py docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md
# MARKDOWN_HEADING_AUDIT_OK

python3 tools/audit_lesson_depth.py --strict docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md
# LESSON_DEPTH_AUDIT_OK

python3 tools/audit_latex_render.py docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md
# LATEX_RENDER_AUDIT_OK formulas=55

python3 tools/audit_reference_rebuilds.py docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md
...
- `docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md`，LTE Turbo C/C++ 定点模型计划。
- `docs/L3/T13.6_bit_exact_regression_harness.md`，bit-exact 回归框架与 failure bundle。
