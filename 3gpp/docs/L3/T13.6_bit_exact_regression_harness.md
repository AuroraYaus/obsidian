...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T13.6_bit_exact_regression_harness.md"
---
# T13.6 Bit-Exact 回归框架：从协议向量到 Python/C++/RTL 同步判定
## 本节学习目标
...
| T13.1 定点需求 | 知道位宽、Q 格式、缩放、舍入、饱和、符号约定和性能损失预算必须写成需求。 |
| T13.2-T13.4 定点模型 | 知道三类译码器各自有哪些内部整数对象和 bit-exact checkpoint。 |
| T13.5 SIMD 与内存布局 | 知道 `layout_version`、`protocol_order_hash`、tail mask 和 SIMD lane 不能改变协议逻辑顺序。 |
...
| LTE 协议精读与工程章节 | `docs/L2/T7.*.md`, `docs/L3/T12.2_*`, `docs/L3/T13.2_*` | 本节只汇总回归字段，不重写 T7/T13.2 的完整推导。 |
| NR LDPC 协议精读与工程章节 | `docs/L2/T8.*.md`, `docs/L2/T9.*.md`, `docs/L3/T12.3_*`, `docs/L3/T13.3_*` | 本节不重新定义 BG、$Z_c$ 或 lifting 表。 |
| NR Polar 协议精读与工程章节 | `docs/L2/T10.*.md`, `docs/L3/T12.4_*`, `docs/L3/T13.4_*` | 本节不重新选择 reliability sequence 或 list policy。 |
| SIMD/内存布局章节 | `docs/L3/T13.5_SIMD_memory_layout_decoders.md` | 本节不做真实 profiling，只规定回归需要记录的布局证据。 |
...
总图英文标题为 `T13.6 Bit-Exact Regression Harness`，强调本节不是单个测试命令，而是从协议向量、Python float、Python fixed、C/C++ fixed、RTL output、compare core 到 CI gate 和 failure bundle 的完整回归外壳。
...
| 对象 | 协议/本地证据 | 本节如何使用 |
|:---|:---|:---|
| LTE Turbo 协议向量 | TS 36.212 Rel-19 `36212-j30`，路径 `3GPP_Rel19/processed/TS_36.212_36212-j30`；上游 T7/T12.2/T13.2 | vector schema 的 `standard=lte`、Turbo checkpoint、CB/TB CRC、HARQ/RV 和 soft buffer 字段回链。 |
| NR LDPC 协议向量 | TS 38.212 Rel-19 `38212-j30`，路径 `3GPP_Rel19/processed/TS_38.212_38212-j30`；上游 T8/T9/T12.3/T13.3 | vector schema 的 `standard=nr`、LDPC BG/$Z_c$/rate recovery/layered checkpoint 回链。 |
| NR Polar 协议向量 | TS 38.212 Rel-19 `38212-j30`，路径 `3GPP_Rel19/processed/TS_38.212_38212-j30`；上游 T10/T12.4/T13.4 | vector schema 的 Polar frozen/info set、rate recovery、SCL/CA-SCL、CRC/RNTI selector 回链。 |
| 定点需求 | `docs/L3/T13.1_fixed_point_decoder_requirements.md` | 提供位宽、舍入、饱和、BLER budget 和 `regression.*` namespace。 |
| C/C++ 定点模型 | `docs/L3/T13.2_*`, `docs/L3/T13.3_*`, `docs/L3/T13.4_*` | 提供各算法内部 checkpoint 和失败字段。 |
| SIMD/布局 | `docs/L3/T13.5_SIMD_memory_layout_decoders.md` | 提供 layout hash、tail mask、protocol order hash 和 scalar/SIMD compare 字段。 |

## 图示
![图 1：bit exact regression harness](assets/T13.6_bit_exact_regression_harness.png)
...
- `tools/figures/render_t13_6_bit_exact_regression_harness.py`
- `docs/L3/assets/T13.6_bit_exact_regression_harness.png`

本节规定的回归命令是模板，当前仓库尚未实现 `decoder_regress` 可执行工具；因此本次可执行证据集中在讲义审计、LaTeX 渲染、图形几何/可读性审计和 Python 脚本编译。后续若实现真实 regression runner，应把命令输出、suite manifest、failure bundle 和 CI job URL 回写到本节或 T15.6 证据报告。

本次实际运行并记录：

```bash
python3 tools/figures/render_t13_6_bit_exact_regression_harness.py
# WROTE docs/L3/assets/T13.6_bit_exact_regression_harness.png (2300, 2160)

python3 tools/audit_lesson_terms.py docs/L3/T13.6_bit_exact_regression_harness.md
# LESSON_TERM_AUDIT_OK

python3 tools/audit_markdown_headings.py docs/L3/T13.6_bit_exact_regression_harness.md
# MARKDOWN_HEADING_AUDIT_OK

python3 tools/audit_lesson_depth.py --strict docs/L3/T13.6_bit_exact_regression_harness.md
# LESSON_DEPTH_AUDIT_OK

python3 tools/audit_latex_render.py docs/L3/T13.6_bit_exact_regression_harness.md
# LATEX_RENDER_AUDIT_OK formulas=13

python3 tools/audit_reference_rebuilds.py docs/L3/T13.6_bit_exact_regression_harness.md
# REFERENCE_REBUILD_AUDIT_NO_CANDIDATES

python3 tools/audit_figure_geometry.py tools/figures/render_t13_6_bit_exact_regression_harness.py

python3 tools/audit_figure_readability.py tools/figures/render_t13_6_bit_exact_regression_harness.py

python3 -m py_compile tools/figures/render_t13_6_bit_exact_regression_harness.py
# exit 0
```

人工图像复核：T13.6 图的字体与上下边框距离充足，相邻边框间距正常，箭头方向、头部和线宽正常，连线从合理真实边界进入 Compare Core；表格单元格居中，底部规则行留白正常。当前 `decoder_regress`、C/C++ 二进制和 RTL scoreboard 尚未实现，因此 CI 和 replay 命令只作为接口模板，不记录为已执行。
...
- `docs/L2/T7.*.md`，LTE Turbo、HARQ、RV、rate recovery 和边界案例精读。
- `docs/L2/T8.*.md` 与 `docs/L2/T9.*.md`，NR LDPC base graph、lifting、rate recovery、HARQ/CBG 和译码流程精读。
- `docs/L2/T10.*.md`，NR Polar construction、SC/SCL、CA-SCL、rate recovery 和边界案例精读。
- `docs/L3/T12.*.md`，浮点仿真、曲线、向量和失败回放计划。
- `docs/L3/T13.1_fixed_point_decoder_requirements.md` 至 `docs/L3/T13.5_SIMD_memory_layout_decoders.md`，定点需求、三类 C/C++ 定点模型计划和 SIMD/内存布局。
