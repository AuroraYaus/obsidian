...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md"
---
# T14.3 NRPolarRTL 微架构：从 CA-SCL 树遍历到排序器瓶颈
## 本节学习目标

本节设计NRPolar的寄存器传输级微架构。这里的主角不是重新推导 Polar 编码本身，而是把逐次消除译码（Successive Cancellation, SC）、逐次消除列表译码（Successive Cancellation List, SCL）和循环冗余校验辅助 SCL（CRC-aided SCL, CA-SCL）落到硬件可实现的数据通路：树遍历控制、对数似然比存储、部分和（partial sum）存储、路径存储、路径度量（Path Metric, PM）更新、`2L -> L` 排序/剪枝器、CRC/无线网络临时标识（Radio Network Temporary Identifier, RNTI）检查器和低延迟控制状态机。
...
| SC tree | Polar 译码的二叉树遍历结构，通过 $f/g$ 函数从 codeword LLR 推出 $\hat{u}$。 |
...
| Bit-exact trace | Python/C/C++/RTL 在同一 checkpoint 输出相同整数、path id、排序顺序和 CRC 结果的证据。 |
...
- 规划 RTL/专用集成电路验证：`polar.rate_recovery`、`polar.mask`、`polar.fg`、`polar.partial_sum`、`polar.pm_prune`、`polar.final` 必须能和 T13.6 的 bit-exact 框架对齐。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| T5.2 存储分 bank 与缓存基础 | 知道静态随机存取存储器、寄存器文件、bank、端口、stall、bypass 和 crossbar 的含义。 |
| T5.4 RTL 状态机与握手 | 知道 `valid/ready`、start/done/error、pipeline stall、flush、reset 和 timeout。 |
| T10.3 NR Polar 可靠性序列 | 知道 information set、frozen set 和可靠性序列来自 TS 38.212 Table 5.3.1.2-1。 |
| T10.4 Polar SC | 知道 $f/g$ 函数、树遍历、frozen 判决和 partial sum。 |
| T10.5 Polar SCL | 知道 information bit 分裂路径，PM 越小越好，超过列表大小 $L$ 后剪枝。 |
| T10.6 CA-SCL | 知道 final selector 先看 CRC/RNTI pass mask，再在通过路径中选择 PM 最小者。 |
| T10.7 NR Polar rate recovery | 知道 puncturing、shortening、repetition 和 coded-bit deinterleaving 对 decoder input LLR 的影响。 |
| T13.4 NR Polar 定点模型计划 | 知道 LLR 位宽、PM 饱和、path remap、CRC/RNTI selector 和 trace 字段。 |
| T13.6 Bit-exact 回归框架 | 知道 RTL 不能只比较最终 decoded bits，必须比较中间 path/PM/sort/CRC checkpoint。 |

若尚未掌握 LLR 的正负号含义、partial sum 为什么不是普通 bit 缓存，先回看 T10.4 和 T10.5。本节默认你已经理解 Polar 树的算法行为；这里解决的是 ASIC 中如何把它做成可定时、可复现、可定位的结构。
...
| 资料 | 本地路径 | 边界 |
| :--- | :--- | :--- |
| TS 38.212 3GPP_译码知识库入口 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_index.md` | 3GPP_译码知识库入口 只证明抽取结构存在，不替代协议精读。 |
| TS 38.212 §5.3.1 | `TS_36.212_36212-j30_content.md:815-837` | 当前 Markdown 抽取公式符号缺失，精确公式以 Word/tables 和前文复现为准。 |
| TS 38.212 §5.3.1.1 | `TS_36.212_36212-j30_content.md:839-871`; `tables/table_0011.csv/html` | RTL 可预计算 interleaver/mask，不表示协议规定具体硬件结构。 |
| TS 38.212 §5.3.1.2 | `TS_36.212_36212-j30_content.md:873-943`; `tables/table_0012.csv/html` | 完整可靠性序列由 T10.3 复现；本节引用其结果和 hash。 |
| TS 38.212 §5.4.1 | `TS_36.212_36212-j30_content.md:1019-1021` | 本节只关心接收端 rate recovery 后的 RTL 输入契约。 |
| TS 38.212 §5.4.1.1 | `TS_36.212_36212-j30_content.md:1023-1079`; `tables/table_0019.csv/html` | 完整 pattern 已由 T10.7 复现。 |
| TS 38.212 §5.4.1.2 | `TS_36.212_36212-j30_content.md:1081-1109` | 接收侧 LLR 初始化是实现翻译，不是协议逐字公式。 |
| TS 38.212 §5.4.1.3 | `TS_36.212_36212-j30_content.md:1111-1125` | 是否启用和上下文来自对应上行控制信息/下行控制信息链路。 |
| T10.4-T10.7 | `docs/L2/T10.*.md` | 算法和接收端实现策略，不是 TS 38.212 强制 decoder。 |
| T13.4 | `docs/L3/T13.4_NR_Polar_fixed_point_model_plan.md` | T14.3 把这些对象映射到 RTL memory/control/trace。 |
| T13.6 | `docs/L3/T13.6_bit_exact_regression_harness.md` | 本节定义 RTL 侧怎么输出 checkpoint。 |
...
```mermaid
flowchart LR
N0["Protocol Descriptor"]
N1["A, K, E, N, crc_type, UCI/DCI context, RNTI context and list-size con..."]
N2["LLR Memory"]
N3["Partial Sum Memory"]
N4["PM / Path State"]
N5["2L -> L Sorter"]
N6["Latch descriptor, masks and rate-recovered LLR."]
N7["Path-State Memories and Sorter Bottleneck"]
N0 --> N1
N1 --> N2
N2 --> N3
N3 --> N4
...
| A, K, E, N, crc_type, UCI/DCI context, RNTI context and list-size config. | Deinterleaved LLR, info/frozen/PC masks, valid bits and rate-recovery mode.；SC/SCL node order, frozen/information decisions and low-latency schedule.；LLR f and g functions, saturation, partial-sum input and path-local st |
...
式 (3) 是接收端工程翻译：punctured 是未知，shortened 是已知 0，repetition 是同一母码位置的软信息累加。它必须和 T10.7、T13.4、T13.6 的 `polar.rate_recovery` checkpoint 一致。
...
T13.6 的 `polar.partial_sum` checkpoint 至少要包含 `path_id`、`stage_id`、`node_id`、`beta_before`、`beta_after`、`parent_path_id`、`ps_state_id` 和 `lazy_ref`。否则当 RTL 和 fixed model 在某个右子树 LLR 分叉时，无法判断是 $g$ 函数错还是 beta 贴错路径。
...
这个例子暴露两个硬件风险。第一，两个 survivor 都来自同一个 parent P1，path manager 不能只把 P1 原地改名为 P0'，还必须为 P1' 准备独立 state，否则后续某个 path 写 partial sum 会污染另一个 path。第二，P0 的旧状态虽然还在物理 SRAM 里，但 logical path 已被剪掉；若 lazy-copy 引用计数没有减少，后续状态回收会泄漏或复用错误。
...
final selector trace 至少记录 `crc_ok[path]`、`rnti_ok[path]`、`pass_mask`、`selected_path_id`、`selected_pm`、`decode_valid` 和 `fail_reason`。这能直接覆盖 T10.6 的典型风险：PM 最佳路径 CRC fail，而次优路径 CRC pass。
...
- mask 和 reliability ROM 预取，避免译码关键路径动态筛表。
...
ASIC 设计时，`fg_pe_array`、`top_l_sorter` 和 SRAM 端口是三类主要时序压力。经验上，$f/g$ 算子可以通过加 pipeline、降低 fanout 和半并行处理解决；sorter 则容易把 PM add、饱和、比较、mux 和 path remap 串成一条长路径。因此 T14.3 的验收必须让学习者能解释 sorter bottleneck，而不只是画出一个名为 sorter 的框。

## 浮点仿真方案

T14.3 的 RTL bit-exact 目标来自 T12.4 的 NR Polar 浮点仿真计划。浮点 golden model 必须覆盖本节全部微架构模块的等价行为：SC/SCL 树遍历、LLR memory 访存模式、partial-sum 更新、PM 饱和、sorter/pruner 分支和 CRC/RNTI final selection。定点化前必须先在浮点域确认：

- 使用 T12.4 规定的 AWGN 信道、调制方案和 BLER 基线，验证浮点 SC/SCL 的 BLER 曲线与 T10 理论推导一致。
...
| 决策点 | 风险 | 本节策略 |
|:---|:---|:---|
| LLR 输入位宽与缩放 | 过度量化会让 $f/g$ 算子失去软信息分辨能力；位宽过大浪费 SRAM 和 PE 面积 | 采用 T13.4 规定的 $(W, F)$ 格式；rate recovery 输出的 LLR 在进入 `scl_tree_controller` 之前完成缩放和对称饱和 |
| PM（Path Metric）累加与饱和 | PM 随码长单调递增，长码下可能溢出；不同 path 的 PM 差值才是选择依据 | PM 使用 T13.4 的 $\Delta$-PM 相对表示，每次更新后统一减去当前最小 PM；饱和位宽按 $K=1643$ 最差情况设计，并预留 2 bit guard |
...
定点模型验证分三层：(1) Python fixed 与 Python float 的 BLER 损失在 T13.4 预算内；(2) C/C++ fixed 与 Python fixed 的 checkpoint 逐元素一致；(3) RTL 与 C/C++ fixed 在 T13.6 regression harness 下 bit-exact 对齐。

## Bit-exact 验证方法

不能只比较最终 payload 或 CRC。Polar SCL 的危险在于中间 path list 可以短暂分叉，最后又碰巧选到同一结果。T13.6 要求的 checkpoint 在 RTL 侧映射如下：
...
1. 为什么 T14.3 说 sorter 是 bottleneck，而不是 $f/g$ 函数？
...
| 结论 | 证据 | 本地路径 | 边界 |
|:---|:---|:---|:---|
| Polar coding 是 NR TS 38.212 的 channel coding 对象。 | TS 38.212 Rel-19 `38212-j30` §5.3.1。 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md:815-837` | Markdown 抽取公式符号缺失，作为章节锚点使用。 |
| Polar construction 使用 input interleaving、reliability sequence、information/frozen/PC set 和 $G_N=F^{\otimes n}$。 | TS 38.212 §5.3.1.1/§5.3.1.2；Table 5.3.1.1-1/Table 5.3.1.2-1。 | `TS_36.212_36212-j30_content.md:839-943`; `tables/table_0011.csv/html`; `tables/table_0012.csv/html` | 完整可靠性序列由 T10.3 回链复现，本节不重复长表。 |
| Polar rate matching 包括 sub-block interleaving、bit selection 和 coded-bit interleaving。 | TS 38.212 §5.4.1/§5.4.1.1-§5.4.1.3。 | `TS_36.212_36212-j30_content.md:1019-1125`; `tables/table_0019.csv/html` | 接收侧 LLR 初始化是工程反操作。 |
| SC/SCL/CA-SCL、list size、PM 近似、sorter、lazy copy 和 RTL FSM 属于实现策略。 | TS 38.212 没有规定接收机内部 decoder microarchitecture。 | 本节证据边界与 T10.4-T10.7、T13.4、T13.6 回链。 | 禁止写成 3GPP 强制要求。 |
| Bit-exact checkpoint 必须覆盖 rate recovery、mask、$f/g$、partial sum、PM prune 和 final selector。 | T13.6 本项目回归框架。 | `docs/L3/T13.6_bit_exact_regression_harness.md` | 项目验证要求，不是协议条款。 |
## 执行与证据记录

| 项 | 命令或证据 | 结果 |
|:---|:---|:---|
| Roadmap 任务 | `2026-06-19-lte-nr-decoding-learning-roadmap.md:1160` | T14.3 prompt 已全量覆盖并拓展。 |
| 图形生成 | `python3 tools/figures/render_t14_3_nr_polar_rtl_microarchitecture.py` | `WROTE .../docs/L3/assets/T14.3_NR_Polar_RTL_microarchitecture.png (2500, 2500)`。 |
| 脚本编译 | `PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile tools/figures/render_t14_3_nr_polar_rtl_microarchitecture.py` | 退出码 0，无副作用字节码写入。 |
| 图表目视审核 | `view_image docs/L3/assets/T14.3_NR_Polar_RTL_microarchitecture.png` | 四项硬规则通过：字体与上下边框距离、边框间距、箭头形态、连线起止位置均已检查。 |
| 术语审计 | `python3 tools/audit_lesson_terms.py docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md` | `LESSON_TERM_AUDIT_OK`。 |
| 标题审计 | `python3 tools/audit_markdown_headings.py docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md` | `MARKDOWN_HEADING_AUDIT_OK`。 |
| 深度审计 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md` | `LESSON_DEPTH_AUDIT_OK`。 |
| LaTeX 渲染审计 | `python3 tools/audit_latex_render.py docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md` | `LATEX_RENDER_AUDIT_OK formulas=102`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md` | `REFERENCE_REBUILD_AUDIT_CANDIDATES`；候选均为上游已复现表格回链、工程公式/实现策略边界或章节锚点，不构成本节阻塞。 |
...
| 本地协议抽取 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md`；`tables/table_0011.csv/html`；`tables/table_0012.csv/html`；`tables/table_0019.csv/html`。 |
| 前置讲义 | `docs/L2/T10.4_Polar_SC_decoding.md`；`docs/L2/T10.5_Polar_SCL_decoding.md`；`docs/L2/T10.6_CRC_aided_SCL_control_reliability.md`；`docs/L2/T10.7_NR_Polar_rate_recovery.md`。 |
| 定点与验证 | `docs/L3/T13.4_NR_Polar_fixed_point_model_plan.md`；`docs/L3/T13.6_bit_exact_regression_harness.md`。 |
