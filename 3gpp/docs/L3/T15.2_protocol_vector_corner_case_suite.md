...
type: spec
aliases:
  - T15.2 protocol vector corner case suite
tags:
  - 3gpp
  - docs
  - l3
  - lesson
source_spec: "docs/L3/T15.2_protocol_vector_corner_case_suite.md"
---
# T15.2 协议向量与边界案例套件：从 Rel-19 证据到 directed regression
## 本节学习目标
...
- 把 T15.1 的 testbench loader/scoreboard 和 T13.6 的 bit-exact 回归框架连接起来。

如果只记住一句话：T15.2 的套件不是“多跑一些随机帧”，而是把协议规则切换点、数值解释切换点和硬件状态切换点做成可复现的 directed evidence。
...
| T13.6 Bit-exact 回归框架 | 知道 vector schema、hash、checkpoint manifest、failure bundle 和比较策略。 |
| T15.1 Testbench 架构 | 知道 loader、driver、monitor、scoreboard、assertion、reset/timeout sequence 的角色。 |
...
| 为什么 expected-fail vector 也可能是“测试通过”？ | T15.1 的 pass/fail policy。 |
| 为什么 RV mismatch 必须保存 address trace，而不是只保存 `rv=2`？ | T7.3、T7.6、T9.3、T9.6。 |
| 为什么 Polar list size 不是 3GPP 强制字段？ | T10.5、T10.8、T13.4。 |
...
| LTE 边界章节 | `docs/L2/T7.6_LTE_Turbo_decoder_edge_cases.md` | T15.2 转换为套件清单，不重写 T7.6 全部解释。 |
| NR LDPC 边界章节 | `docs/L2/T9.6_NR_LDPC_decoder_edge_cases.md` | T15.2 只汇总为向量矩阵。 |
| NR Polar 边界章节 | `docs/L2/T10.8_NR_Polar_decoder_edge_cases.md` | T15.2 不重新推导 SCL。 |
| Bit-exact 框架 | `docs/L3/T13.6_bit_exact_regression_harness.md` | 工程框架，不是 3GPP 条文。 |
| Testbench 架构 | `docs/L3/T15.1_decoder_testbench_architecture.md` | T15.2 输出给 T15.1 消费。 |
...
T13.6 已定义统一 vector schema。T15.2 在它上面增加 suite 分类和 corner-case 语义。建议最小目录如下：
  ...
  "vectors": [
    {
      ...
      "source_lesson": "docs/L2/T7.6_LTE_Turbo_decoder_edge_cases.md"
    }
  ]
}
```
...
这些向量也为 T15.3 的 coverage bins 提供种子。例如 `llr_sign_convention_all` 可以同时覆盖 `decoder_family`、`llr_sign_convention`、`expected_status` 和 `checkpoint_present` 四类 coverage。
...
print("T15.2_VECTOR_CLASSIFIER_OK", toy["missing_expected_addresses"])
```
...
|:---|:---|:---|:---|:---|:---|
| 基带芯片 RTL bring-up | T15.2 smoke + directed positive + directed negative 三层 suite。 | payload/status/checkpoint/failure bundle。 | 三类 engine 共用 DMA、soft buffer 和 status path。 | T15.1 scoreboard 对比 C/C++ fixed expected。 | LDPC CBG negative 被错误 CRC pass，暴露 CBGTI bit order 反。 |
...
| 结论 | 证据 | 本地路径 | 边界 |
|:---|:---|:---|:---|
| LTE Turbo vectors 的 CB/TB CRC、分段、filler、Turbo interleaver 和 rate matching 来自 TS 36.212。 | TS 36.212 Rel-19 `36212-j30` §5.1.1、§5.1.2、§5.1.3.2、§5.1.4.1；项目 T3.3、T6、T7。 | `3GPP_Rel19/processed/TS_36.212_36212-j30`、`docs/L2/T7.6_LTE_Turbo_decoder_edge_cases.md` | 本节不重建完整协议表；exact K/f1/f2 表由上游已生成资产承接。 |
| NR LDPC vectors 的 BG/$Z_c$/filler/rate matching/RV/CRC 来自 TS 38.212。 | TS 38.212 Rel-19 `38212-j30` §5.2.2、§5.3.2、§5.4.2、§6.2、§7.2；项目 T8/T9。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`docs/L2/T9.6_NR_LDPC_decoder_edge_cases.md` | CBG 调度上下文需同时回链 TS 38.214；本节不复现 MCS/TBS 表。 |
...
| NR Polar vectors 的 CRC、construction、rate matching、UCI/DCI 分支和 RNTI 边界来自 TS 38.212。 | TS 38.212 Rel-19 `38212-j30` §5.1、§5.2.1、§5.3.1、§5.4.1、§6.3、§7.3；项目 T10。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`docs/L2/T10.8_NR_Polar_decoder_edge_cases.md` | list size、PM tie-break、sorter 和 path memory 是实现策略。 |
| vector schema、checkpoint manifest、failure bundle 属于工程回归框架。 | 项目 T13.6、T15.1。 | `docs/L3/T13.6_bit_exact_regression_harness.md`、`docs/L3/T15.1_decoder_testbench_architecture.md` | 不属于 3GPP 强制流程。 |

## 图示
![图 1：protocol vector corner case suite](assets/T15.2_protocol_vector_corner_case_suite.png)

## 执行与证据记录

| 项 | 命令或证据 | 结果 |
|:---|:---|:---|
| Roadmap 任务 | `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T15.2 卡片。 | Prompt 已逐项覆盖并扩展。 |
| 依赖读取 | `docs/L2/T7.6_LTE_Turbo_decoder_edge_cases.md`、`docs/L2/T9.6_NR_LDPC_decoder_edge_cases.md`、`docs/L2/T10.8_NR_Polar_decoder_edge_cases.md`、`docs/L3/T13.6_bit_exact_regression_harness.md`、`docs/L3/T15.1_decoder_testbench_architecture.md`。 | 已抽取三类 directed cases、最小 dump 字段、vector schema 和 testbench 消费方式。 |
| 图形生成 | `python3 tools/figures/render_t15_2_protocol_vector_corner_case_suite.py` | 写出 `docs/L3/assets/T15.2_protocol_vector_corner_case_suite.png (3000, 3060)`。 |
| 脚本编译 | `PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile tools/figures/render_t15_2_protocol_vector_corner_case_suite.py` | 退出码 0。 |
| 资产人工复核 | `view_image docs/L3/assets/T15.2_protocol_vector_corner_case_suite.png` | 首轮发现表格右边界贴近画布和底部说明框接触表格；已缩表宽、加高画布并下移说明框，复检通过。 |
| Python 诊断片段 | 正文 `classify_rate_recovery()` 片段。 | 输出 `T15.2_VECTOR_CLASSIFIER_OK [6, 7]`。 |
| 引用重建候选解释 | `python3 tools/audit_reference_rebuilds.py docs/L3/T15.2_protocol_vector_corner_case_suite.md` | 输出候选清单：TS 36.212/TS 38.212 表格候选已在正文明确回链上游 T3/T6/T7/T8/T9/T10 已复现资产或标注本节不重建；“未核验表值”出现在常见错误和协议证据边界，用作防护规则；Project 引用为本项目内部讲义来源，不是外部论文图表缺失。 |
...
- [Project, 2026] T7.6 LTE Turbo decoder edge cases, `docs/L2/T7.6_LTE_Turbo_decoder_edge_cases.md`.
- [Project, 2026] T9.6 NR LDPC decoder edge cases, `docs/L2/T9.6_NR_LDPC_decoder_edge_cases.md`.
- [Project, 2026] T10.8 NR Polar decoder edge cases, `docs/L2/T10.8_NR_Polar_decoder_edge_cases.md`.
- [Project, 2026] T13.6 Bit-exact regression harness, `docs/L3/T13.6_bit_exact_regression_harness.md`.
- [Project, 2026] T15.1 Decoder SystemVerilog testbench architecture, `docs/L3/T15.1_decoder_testbench_architecture.md`.
