...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md"
---
# T13.2 LTE Turbo 定点模型计划：C/C++ 整数译码器规格
## 本节学习目标

本节规划LTE Turbo的 C/C++ 定点译码器模型。T12.2 已经定义浮点仿真：编码器参考、加性白高斯噪声信道、解速率匹配、Log-MAP/Max-Log-MAP、循环冗余校验、块错误率曲线和失败回放。本节把这些浮点对象约束成整数模型：每条对数似然比怎样量化，分支度量怎样计算，alpha/beta 路径度量怎样归一化和饱和，外信息怎样缩放，交织地址怎样驱动数组访问，以及 C/C++ 输出怎样和 Python 定点参考做 bit-exact 对比。


- 说明 LTE Turbo 定点模型和 T12.2 浮点模型的输入、输出和检查点关系。
...
| T13.1 定点译码器需求 | 知道 Q 格式、裁剪、舍入、饱和、性能损失预算和 bit-exact 检查点。 |
...
| 资料 | 本地路径 | 边界 |
| :--- | :--- | :--- |
| TS 36.212 §5.1.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md:575-599` | CRC 算法由上游 T3.1/T12.2 回链，本节只消费状态。 |
| TS 36.212 §5.1.2 | `TS_36.212_36212-j30_content.md:601-673` | 不重新推导分段公式。 |
| TS 36.212 §5.1.3.2.1/§5.1.3.2.2 | `TS_36.212_36212-j30_content.md`，T6.3 重建图 | 不规定接收机必须用 Log-MAP 或 Max-Log-MAP。 |
| TS 36.212 §5.1.3.2.3/Table 5.1.3-3 | `tables/table_0009.csv/html`，T6.4/T6.6 回链 | 本节不复制全表，使用上游已重建资产。 |
| TS 36.212 §5.1.4.1/§5.1.4.1.2 | `TS_36.212_36212-j30_content.md:777-923`，`tables/table_0010.csv/html` | 接收机 soft buffer 写回、重传合并和饱和是逆向实现策略，不是 TS 36.212 直接规定的内部行为。 |
| T6.6 | `docs/L2/T6.6_Log_MAP_Max_Log_MAP_Turbo.md` | 算法文献背景，不是 3GPP 规范公式。 |
| T12.2 | `docs/L3/T12.2_LTE_Turbo_float_sim_plan.md` | 本节不生成真实 BLER 曲线。 |
...
| K, f1, f2, RV, E, Ncb, filler mask, d0/d1/d2 streams and TS 36.212 evidence. | Apply T13.1 width, scale, round and saturation to system and parity LLR streams.；Compute branch metric, alpha, beta, posterior and extrinsic with Log-MAP or Max-Log-MAP.；Interleave and deinterleave extrinsic LLR; update  |
...
本节流程顺序为：Protocol Vector 提供 $K/f_1/f_2$、RV、$E$、$N_{\mathrm{cb}}$ 和三路流；LLR Quantizer 按 T13.1 量化系统流与校验流；SISO Core 计算分支度量、alpha、beta、后验和外信息；Iterative Exchange 通过交织/解交织地址交换外信息；CRC/Report 记录硬判决、CB/TB CRC、早停和 replay。中部表格列出四层 compare checkpoint，底部列出 Log-MAP、Max-Log-MAP、度量归一化、外信息缩放和饱和观测。
...
T13.2 的 C/C++ 模型应从 T12.2 的失败包或 run directory 读取对象，而不是临时构造散装数组。
...
这些结构体是计划级接口，不是最终应用二进制接口（Application Binary Interface, ABI）。关键点是：协议 descriptor、定点格式、数组缓冲区和 trace 不能混成一个大结构，否则后续 T13.6 和 RTL scoreboard 很难单独比较。`TurboFixedConfig` 必须能唯一决定 bit-exact 行为：同一个输入向量、同一个配置对象和同一个代码版本，不能因为舍入、饱和、Log-MAP mode、查找表（Look-Up Table, LUT）或归一化策略缺字段而得到两个合法答案。
...
本项目建议 T13.2 先采用“吸收比例口径”，即所有分支度量在同一个 `metric_scale` 下保存。若日后引入 Log-MAP 修正 LUT，LUT 的输入输出也必须使用同一 scale。
...
T13.2 不应要求所有 mode 一次完成。推荐先实现 `max_log_map`，用无噪声和小块向量证明位序、交织、外信息和 CRC gate 正确；再引入 LUT 或缩放，比较 `bler_delta_maxlog` 和定点损失。
...
第一版 C/C++ 模型以清晰和 bit-exact 为先，不先追求单指令多数据（Single Instruction, Multiple Data, SIMD）。T13.5 会再处理 SIMD 和缓存布局。本节建议数组采用结构化分离：
...
T13.2 的 bit-exact 不直接对浮点 Log-MAP，而是按三层比较：

| 层级 | 参考 | 被测 | 比较规则 |
|:---|:---|:---|:---|
| Python fixed | Python 定点参考函数 | C/C++ 定点函数 | 所有整数检查点 exact。 |
| Python float | T12.2 浮点模型 | Python/C fixed | 曲线和检查点趋势比较，不要求整数一致。 |
| C/C++ fixed | T13.2 模型 | 后续 RTL | 指定检查点 exact。 |
...
定点模型必须继承 T12.5 的曲线报告口径。推荐字段：

| 字段 | 说明 |
|:---|:---|
| `float_baseline_run_id` | T12.2 浮点 Log-MAP 或 Max-Log-MAP 基线。 |
...
T13.2 的定点模型会直接变成 T14.1 RTL 微架构的前置规格。
...
RTL 可以选择滑窗、并行 SISO 或折叠架构；但只要它声明兼容某个 `fixed_requirement_id`，就必须在 T13.6/T15 中通过同一组 bit-exact 检查点。
...
| 性能预算 | fixed vs float 曲线。 | 满足 T13.1/T12.5 预算和置信区间要求。 |
...
6. T13.2 最少应保存哪四层 compare checkpoint？
...
| :--- | :--- | :--- | :--- |
| LTE CRC | TS 36.212 `36212-j30` §5.1.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md:575-599` | 已核验锚点。 |
| LTE CB segmentation | TS 36.212 §5.1.2 | `TS_36.212_36212-j30_content.md:601-673` | 已核验锚点。 |
| LTE Turbo encoder/trellis | TS 36.212 §5.1.3.2.1/§5.1.3.2.2 | `TS_36.212_36212-j30_content.md`；T6.3 重建图 | 上游已重建图。 |
| LTE internal interleaver | TS 36.212 §5.1.3.2.3/Table 5.1.3-3 | `tables/table_0009.csv/html`；T6.4/T12.2 | 上游已复现。 |
| LTE Turbo rate matching | TS 36.212 §5.1.4.1/§5.1.4.1.2 | `TS_36.212_36212-j30_content.md:777-923`；`tables/table_0010.csv/html` | 已核验锚点；soft buffer 合并/饱和属于实现策略。 |
...
| 候选资产 | 本节处理 | 关闭条件 |
|:---|:---|:---|
| TS 36.212 Table 5.1.3-3 全表 | 本节回链 T6.4/T12.2，不重新复制长表。 | 若 T13.2 代码直接解析原表生成地址，必须把 CSV/HTML、脚本和 hash 纳入 fixed evidence archive。 |
| TS 36.213 调制与编码方案/传输块大小 | 本节不使用具体 MCS/TBS 表值。 | 若 fixed vector 从 MCS index 自动生成 TB size 或 $Q_m$，必须关闭 T12.2 标记的 `待核验` 分册/章节/表格。 |
...
| 写入文件 | `docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T13.2。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 阶段 12 与 T13.2 条目。 |
| 上游讲义 | 已读取 `docs/L2/T6.6_Log_MAP_Max_Log_MAP_Turbo.md`、`docs/L2/T7.3_LTE_HARQ_soft_buffer_RV.md`、`docs/L3/T12.2_LTE_Turbo_float_sim_plan.md`、`docs/L3/T13.1_fixed_point_decoder_requirements.md`。 |
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md`：`LATEX_RENDER_AUDIT_OK formulas=57`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T13.2_LTE_Turbo_fixed_point_model_plan.md`：`REFERENCE_REBUILD_AUDIT_CANDIDATES`，候选为 TS 36.212 表/关系式回链、T6.4/T12.2 已重建资产、实现策略边界、论文引用和 TS 36.213 MCS/TBS 条件项；本节未直接使用 MCS/TBS 表值。 |
...
- [上游讲义] `docs/L2/T6.6_Log_MAP_Max_Log_MAP_Turbo.md`，Log-MAP 与 Max-Log-MAP Turbo 译码。
- [上游讲义] `docs/L2/T7.3_LTE_HARQ_soft_buffer_RV.md`，LTE HARQ 软缓存与冗余版本。
