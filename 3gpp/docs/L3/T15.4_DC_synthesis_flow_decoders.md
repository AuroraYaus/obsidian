...
type: spec
aliases:
  - T15.4 DC synthesis flow decoders
tags:
  - 3gpp
  - docs
  - l3
  - lesson
source_spec: "docs/L3/T15.4_DC_synthesis_flow_decoders.md"
---
# T15.4 Synopsys Design Compiler 综合流程：从译码 RTL 到时序、面积和功耗报告
## 本节学习目标
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| T14.1 LTE Turbo RTL 微架构 | 知道 Turbo SISO、alpha/beta、extrinsic、interleaver 和 CRC checkpoint。 |
| T14.2 NR LDPC RTL 微架构 | 知道 LDPC layered schedule、CN/VN、message RAM、bank conflict 和 syndrome checkpoint。 |
| T14.3 NR Polar RTL 微架构 | 知道 SC/SCL、path metric、sorter、partial sum、CRC/RNTI selector。 |
| T14.4 统一译码子系统 | 知道 dispatcher、DMA、soft buffer manager、status/error/IRQ 和 trace。 |
| T14.6 寄存器表与配置流 | 知道配置寄存器、start/done/error、timeout、trace mask 和 descriptor hash。 |
| T15.1-T15.3 | 知道 testbench、protocol vectors、coverage bins、regression tiers 和 sign-off gate。 |

自检问题：

| 问题 | 若回答不出来，先回看 |
|:---|:---|
| 为什么综合脚本不能把 testbench 文件放进 design filelist？ | T15.1。 |
| 为什么 `dc_shell` 没装时不能声称“综合通过”？ | 本节工具边界。 |
| 为什么 `report_timing` 通过不代表译码功能正确？ | T13.6、T15.1。 |
| 为什么 reset 约束写错会让时序报告看起来很好但芯片不可用？ | T5.4、T14.6。 |
| 为什么 Polar list size 或 LDPC parallelism 会影响面积和关键路径？ | T14.2、T14.3。 |
...
本节是工程方法课。3GPP 协议不规定 DC Tcl、工艺库、时钟周期、compile option、面积单位或功耗模型；协议和上游讲义只影响“综合后应该验证哪些配置和向量”。若综合后要跑 LTE/NR 协议向量或 gate-level sanity，必须回链 T15.1-T15.3 和 T7/T9/T10/T15.2 的证据。
...
| T14.1-T14.6 | `docs/L3/T14.*.md` | 本节只讲如何综合这些 RTL。 |
| T15.1-T15.3 | `docs/L3/T15.1*.md`、`T15.2*.md`、`T15.3*.md` | 当前仓库没有真实 RTL 仿真和 DC coverage 数据库。 |
...
图中英文标题为 `T15.4 Design Compiler Synthesis Flow for Decoders`。图的视觉检查注记也迁移为正文要求：`Visual check: arrows are straight two-point shafts with vector-derived heads; table text is centered and 24px.` 这条注记用于约束后续重绘时的箭头、表格文本和对齐，不属于 Synopsys Design Compiler 本身的功能要求。
...
| testbench 排除 | `tb/`、DPI、class、random 不进入 RTL filelist。 | 把 T15.1 testbench class 送进 DC。 |
...
路径例外必须写 owner、理由、适用端点和验证方法。release 前，T15.6 最终证据报告应列出所有例外。
...
4. **Subsystem compile**：综合 T14.4 统一子系统，观察 dispatcher/DMA/soft buffer/status fan-in。
...
综合优化不能替代架构修复。若 Polar sorter 或 LDPC min tree 组合深度本身过长，下一节 T15.5 应通过 pipeline、tree split、register duplication 或算法近似改结构，而不是只调 DC option。
...
注意最后一行。时序修复会改变 latency、trace cycle、valid alignment 和 scoreboard expectation，必须回到 T15.1/T15.3 的回归。
...
| Unified top | DMA arbitration、status/error fan-in、IRQ sticky、trace mux。 | 分区 arbitration、状态寄存、trace 采样降频。 | T15.1 monitor 和 T15.3 coverage event 需更新。 |

T15.5 会专门展开 timing closure。T15.4 只要求你能从报告中识别这些路径并提出第一步诊断，不要求在本节完成结构修改。
...
print("T15.4_TIMING_TRIAGE_OK", worst["module"], families[worst["module"]], worst["slack_ns"])
```

输出应为：

```text
T15.4_TIMING_TRIAGE_OK polar_sorter NR Polar SCL sorter -0.31
```
...
| 验证 | 目的 | 回链 |
|:---|:---|:---|
| Netlist sanity | 确认 mapped netlist 可 elaboration，可跑 smoke。 | T15.1 testbench smoke。 |
| SDF/gate-level spot | 确认关键路径和时序反标不会破坏 handshake。 | T15.1 driver/monitor 和 T15.3 smoke/commit bins。 |
| Protocol vector replay | 确认综合优化没有破坏协议派生路径。 | T15.2 directed vectors，T7/T9/T10 证据。 |
...
| 协议回链 | 文档回链 T15.1-T15.3、T7/T9/T10。 | 综合后 smoke/gate-level replay 使用 T15.2 向量。 |
...
## 图示
![图 1：DC synthesis flow decoders](assets/T15.4_DC_synthesis_flow_decoders.png)

## 协议证据表

| 对象 | 证据 |
| :--- | :--- |
| LTE Turbo 译码配置 | TS 36.212 Rel-19 `36212-j30`，本地路径 `3GPP_Rel19/processed/TS_36.212_36212-j30`，上游 T7/T14.1/T15.2。 |
| NR LDPC 译码配置 | TS 38.212 Rel-19 `38212-j30`，TS 38.214 Rel-19 `38214-j30`，上游 T9/T14.2/T15.2。 |
| NR Polar 译码配置 | TS 38.212 Rel-19 `38212-j30`，上游 T10/T14.3/T15.2。 |
| Testbench/coverage/sign-off | T15.1、T15.2、T15.3 |
## 执行与证据记录
...
| 本节 Python 片段 | `T15.4_TIMING_TRIAGE_OK polar_sorter NR Polar SCL sorter -0.31`；见 `## 具体数值例子`。 |
