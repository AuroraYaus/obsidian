...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md"
---
# T14.2 NR LDPCRTL 微架构：从 QC 地址生成到 Layered 存储 Bank
## 本节学习目标
...
| T13.3 NR LDPC 定点模型计划 | 知道整数位宽、CN message、posterior LLR、layer trace、saturation 和 C/C++ fixed checkpoint。 |
| T13.6 Bit-Exact 回归框架 | 知道 RTL 输出必须和 Python/C++ fixed trace 在 metadata/hash/checkpoint 上逐项比较。 |
...
本节是工程微架构课，不实现真实 SystemVerilog 源码。正文会给结构、接口、状态机、存储估算、伪代码和验证计划。真实 RTL 编码、testbench、覆盖率、综合约束和时序报告会在 T15 承接。
...
| TS 38.212 §5.3.2 | `TS_36.212_36212-j30_content.md:945-989` | 协议定义编码矩阵，不定义接收端 decoder schedule。 |
...
| TS 38.212 §5.4.2 | `TS_36.212_36212-j30_content.md:1175-1295` | 本节不重写 rate recovery，默认输入 LLR 已放回 LDPC code-bit 位置。 |
| T8.3 | `docs/L2/T8.3_NR_LDPC_lifting_QC_matrix.md` | T14.2 把它映射到 RTL 地址和 bank。 |
| T8.7 | `docs/L2/T8.7_layered_LDPC_decoding_schedule.md` | T14.2 扩展为微架构和验证字段。 |
| T13.3 | `docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md` | T14.2 把对象变成 RTL 端口、SRAM、counter 和 trace。 |
| T13.6 | `docs/L3/T13.6_bit_exact_regression_harness.md` | 本节只定义 RTL 侧怎么输出这些 checkpoint。 |
...
| `trace_status` | checkpoint signals | trace/status/error counters | 输出 bit-exact trace、first mismatch 对齐字段和 counters。 | T13.6 schema 约束可回放性。 |

模块划分的核心原则是“协议身份、算法状态、存储布局和可观测性分层”。`base_graph/z_c/i_ls` 决定 $H$；`nms_alpha_q/oms_beta_q/message_width` 决定算法整数化；`bank_count/layout_version` 决定物理访问；`syndrome_weight/bank_conflict_count/first_mismatch` 决定验证能否定位问题。把这些混在一个无版本大总线里，会让 T15 的 RTL scoreboard 无法证明它和 C/C++ fixed model 正在处理同一个向量。
...
RTL 位宽必须继承 T13.3 的 fixed requirement。最低字段：
...
| Trace/status | first mismatch、counter、failure bundle | 与 T13.6 `checkpoint_manifest` 对齐。 |

## 验证方法

最小验证不应只看最终 CRC。T14.2 的 RTL testbench 至少包含：
...
| Bit-exact replay | T13.6 vector + C++ fixed trace | `quantized_llr/cn_update/layer_trace/final` 全一致。 |
...
示例数值是结构示例，真实向量必须从上游协议向量和 T13.6 manifest 生成，并记录表格 hash、rate recovery hash、bitwidth profile hash 和 RTL commit hash。
...
| Prompt 要求 | 正文证据 | 覆盖状态 |
|:---|:---|:---|
| layered 调度控制器 | `## Layered Decoder FSM 与早停边界`、图 T14.2-1 | 已覆盖。 |
...
| 对象 | 协议/本地证据 | 本节如何使用 |
|:---|:---|:---|
| LDPC §5.3.2 入口 | TS 38.212 Rel-19 `38212-j30`，`TS_36.212_36212-j30_content.md:945-989` | 定位 BG、$Z_c$、Table 5.3.2-1/2/3 和 QC 展开规则。 |
| BG1/BG2 行列组数 | `TS_36.212_36212-j30_content.md:971` | 说明 BG1 为 46 行组、68 列组；BG2 为 42 行组、52 列组。 |
...
| QC 展开规则 | `TS_36.212_36212-j30_content.md:977-989` | 说明有效元素替换为循环移位单位矩阵，其他元素为零矩阵。 |
| Rate recovery context | TS 38.212 §5.4.2，`TS_36.212_36212-j30_content.md:1175-1295` | 说明 `E/Ncb/rv/k0` 是 decoder descriptor 的上游来源。 |
...
- `tools/figures/render_t14_2_nr_ldpc_rtl_microarchitecture.py`
- `docs/L3/assets/T14.2_NR_LDPC_RTL_microarchitecture.png`

已执行表格生成和图形审计：

```bash
python3 tools/figures/render_t14_2_nr_ldpc_rtl_microarchitecture.py
# WROTE docs/L3/assets/T14.2_NR_LDPC_RTL_microarchitecture.png (2500, 2440)

python3 tools/audit_figure_geometry.py tools/figures/render_t14_2_nr_ldpc_rtl_microarchitecture.py

python3 tools/audit_figure_readability.py tools/figures/render_t14_2_nr_ldpc_rtl_microarchitecture.py

python3 -m py_compile tools/figures/render_t14_2_nr_ldpc_rtl_microarchitecture.py
# exit 0
```
...
```bash
python3 tools/audit_lesson_terms.py docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md
# LESSON_TERM_AUDIT_OK

python3 tools/audit_markdown_headings.py docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md
# MARKDOWN_HEADING_AUDIT_OK

python3 tools/audit_lesson_depth.py --strict docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md
# LESSON_DEPTH_AUDIT_OK

python3 tools/audit_latex_render.py docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md
# LATEX_RENDER_AUDIT_OK formulas=85

python3 tools/audit_reference_rebuilds.py docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md
# REFERENCE_REBUILD_AUDIT_CANDIDATES
```

引用重建审计输出为候选清单：候选主要来自 TS 38.212 Table 5.3.2-1/2/3 的上游已复现回链、§5.3.2 协议公式转写、本文工程估算公式和正文说明。本节未新增复制协议长表，完整表由 T8.3/T3.4 的表格化复现资产承接；若后续 RTL 生成脚本直接解析 Table 5.3.2-2/3，应把 `tables/table_0014.csv/html` 和 `tables/table_0015.csv/html` 的 hash、解析脚本、使用行列和 schedule hash 纳入 T15 evidence archive。

## 参考文献

- 3GPP TS 38.212 Rel-19，本地路径 `3GPP_Rel19/processed/TS_38.212_38212-j30`。
- `docs/L2/T8.3_NR_LDPC_lifting_QC_matrix.md`，NR LDPC 提升大小、QC 矩阵构造和 Table 5.3.2-1/2/3 复现。
- `docs/L2/T8.6_LDPC_MS_NMS_OMS.md`，Min-Sum、NMS、OMS、min1/min2 和硬件取舍。
- `docs/L2/T8.7_layered_LDPC_decoding_schedule.md`，layered read-modify-write、syndrome/CRC 边界和硬件风险。
- `docs/L2/T9.1_NR_LDPC_rate_recovery_overview.md` 至 `docs/L2/T9.5_NR_LDPC_reassembly_TB_CRC.md`，rate recovery、RV、CBG、bit deinterleaving 和 TB CRC。
- `docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md`，NR LDPC C/C++ 定点模型计划。
- `docs/L3/T13.6_bit_exact_regression_harness.md`，bit-exact 回归框架和 LDPC checkpoint manifest。
