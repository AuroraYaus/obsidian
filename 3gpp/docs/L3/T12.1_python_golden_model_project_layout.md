...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T12.1_python_golden_model_project_layout.md"
---
# T12.1 Python Golden Model 工程布局：让三类译码仿真可复现、可追踪、可回放
## 本节学习目标
...
- 后续 T12.2-T12.5、T13、T14、T15 如何复用同一套 schema、日志和证据归档。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| T4.6 译码器接口契约 | 知道 descriptor、input、output、status、error、debug 六类对象，以及对数似然比符号、传输块/码块、HARQ process ID、冗余版本（Redundancy Version, RV）和 soft buffer key 的作用。 |
| T5.5 验证思维 | 知道 golden model、bit-exact、边界案例、回归、波形/日志定位的基本关系。 |
| T4.5 性能指标 | 知道BER、块错误率、FER、迭代次数、延迟代理和统计停止条件不是 3GPP 强制门限，而是工程评估对象。 |
...
本节的执行范围是“工程布局规范”。因此协议依据用于说明向量来源和参数回链，不在本节重新复现 TS 36.212/38.212 的编码、速率匹配或循环冗余校验公式。具体 LTE Turbo 浮点仿真由 T12.2 承接，NR LDPC 由 T12.3 承接，NR Polar 由 T12.4 承接，BER/BLER 曲线由 T12.5 承接。
...
|:---|:---|:---|:---|
| TS 36.212 Rel-19 `36212-j30` §5.1.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md`，`sections.jsonl` | LTE CRC calculation 是 LTE Turbo 向量 pass/fail 的协议来源之一。 | 本节只登记向量来源，不重写 CRC 多项式。 |
| TS 36.212 Rel-19 `36212-j30` §5.1.2、§5.1.3.2、§5.1.4.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md`，`sections.jsonl` | LTE CB segmentation、Turbo coding、rate matching/RV 是 T12.2 向量字段来源。 | 具体地址、交织和 RV 恢复由 T12.2 使用。 |
| TS 38.212 Rel-19 `38212-j30` §5.1、§5.2.2、§5.3.2、§5.4.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md`，`sections.jsonl` | NR LDPC CRC、segmentation、base graph/lifting、rate matching 是 T12.3 向量字段来源。 | CRC 多项式复现见上游讲义；具体 BG/Zc/k0 查表由 T12.3/T8/T9 回链。 |
| TS 38.212 Rel-19 `38212-j30` §5.1、§5.2.1、§5.3.1、§5.4.1、§6.3、§7.3 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md`，`sections.jsonl` | NR Polar CRC、CRC/RNTI、segmentation、encoding、rate matching、上行控制信息/下行控制信息控制信息链路是 T12.4 向量字段来源。 | reliability sequence 和 rate recovery 已由 T10.3/T10.7 承接；UCI/DCI 精确 CRC/RNTI 字段由 T12.4 使用时关闭。 |
| TS 38.214 Rel-19 `38214-j30` §5.1.3、§6.1.4 | `3GPP_Rel19/processed/TS_38.214_38214-j30/TS_38.214_38214-j30_content.md`，`sections.jsonl` | 调制与编码方案/传输块大小/RV 可作为系统级参数来源。 | 具体 MCS/TBS 表值保留 L3/system bit-exact 条件项，按实际使用范围复现。 |

协议前因后果要写入每个 vector 的 metadata。一个仿真向量不能只写 `N=1024, K=512`，还应说明这些参数来自哪条链路、哪个 code family、哪个 TS 章节、哪个本地表或哪一节讲义已重建。

本节的协议证据粒度按“工程布局任务”处理：能直接闭环到本节 schema 的内容写成章节级来源和本地路径；涉及具体表、图或公式的 bit-exact 数值，不在本节重复复现，而是回链到已重建讲义或后续 T12.2-T12.4。具体边界如下：

| 证据对象 | 本节是否闭环 | 精细锚点或关闭条件 |
|:---|:---|:---|
| LTE Turbo 合法 $K/f_1/f_2$ | 不在本节重复闭环 | 已由 T3.3/T6.4 使用 TS 36.212 Table 5.1.3-3 的 `table_0009.csv/html` 和正文表格闭环；T12.2 使用时回链。 |
| LTE Turbo 编码器结构 | 不在本节重复闭环 | 已由 T6.3 正文复现 TS 36.212 Figure 5.1.3-2 的 PCCC、内部交织器和终止路径；T12.2 使用时回链。 |
| LTE sub-block interleaver | 不在本节重复闭环 | 已由 T7.2/T11.2 复现 TS 36.212 Table 5.1.4-1 的 `table_0010.csv/html`。 |
| NR LDPC BG/Zc 和 rate matching 表 | 本节只登记来源 | T12.3 使用具体数值时必须回链 T8/T9 已重建资产或在 T12.3 重新列出表/图/公式证据。 |
| NR Polar reliability/rate matching 表 | 本节只登记来源 | T12.4 使用具体数值时必须回链 T10 已重建资产或在 T12.4 重新列出表/图/公式证据。 |
...
因此 T12.1 要先定义工程布局。这个布局不是为了好看，而是为了让失败可以复现、结果可以比较、证据可以归档、后续 RTL 可以接同一套向量。
...
sim/
  pyproject.toml
  TS_36.213_36213-j30_s00-s05_index.md
  configs/
    lte_turbo/
...
T4.6 已经给出中立接口契约。T12.1 需要把它转成仿真配置 schema。最小公共 descriptor 可分成四类字段。
...
`vector.json` 不能只写自然语言说明，至少应有可机器检查的最小 schema。下面字段是后续 T12.2-T12.5 的共同最低要求：
...
T12.1 的浮点仿真不是某个具体算法，而是 runner 和 schema 的 smoke test。建议最小 smoke test 使用 toy decoder：
...
后续 T12.2-T12.4 会把 toy decoder 替换成真实浮点算法。

## 定点化与 Bit-Exact 承接

虽然 T12.1 是浮点仿真布局，但它必须提前服务 T13 的定点模型。
...
T14/T15 会把 Python 输出变成 RTL scoreboard 的输入。为了避免后期重做，T12.1 应提前定义 RTL 需要的证据字段。

| RTL/ASIC 需要的字段 | T12.1 应如何保存 |
|:---|:---|
...
6. T12.1 为什么要记录 3GPP 本地路径，即使本节不复现协议公式？
...
| 后续承接 | 能说明 T12.2-T12.5、T13、T14、T15 如何复用本节目录、向量和日志。 |
...
| NR LDPC CRC/segmentation/coding/rate matching | TS 38.212 Rel-19 `38212-j30` | `3GPP_Rel19/processed/TS_38.212_38212-j30/` | CRC 多项式复现见上游讲义；具体 BG/Zc/k0 查表由 T12.3 和上游 T8/T9 承接。 |
| NR Polar CRC/CRC-RNTI/segmentation/coding/rate matching | TS 38.212 Rel-19 `38212-j30` | `3GPP_Rel19/processed/TS_38.212_38212-j30/` | reliability sequence/rate recovery 由 T10.3/T10.7 承接；UCI/DCI CRC/RNTI 字段由 T12.4 使用时精确闭环。 |
...
| 写入文件 | `docs/L3/T12.1_python_golden_model_project_layout.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T12.1。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 阶段 11 与 T12.1 条目。 |
| 前置依据 | 已读取 `docs/L1/T4.6_decoder_interface_contracts.md`，复用 descriptor/status/error/debug、LLR 符号、HARQ/RV/soft buffer key 口径。 |
...
| 未执行内容 | 未运行真实 BLER 仿真、未生成真实 protocol vectors；具体可执行工程由 T12.2-T12.5 按本节 schema 承接。 |
...
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3/T12.1_python_golden_model_project_layout.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3/T12.1_python_golden_model_project_layout.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T12.1_python_golden_model_project_layout.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3/T12.1_python_golden_model_project_layout.md`：`LATEX_RENDER_AUDIT_OK formulas=4`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T12.1_python_golden_model_project_layout.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选为本节明确声明不重建的协议公式边界和协议来源登记，不构成失败。 |
