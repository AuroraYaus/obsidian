...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T15.1_decoder_testbench_architecture.md"
---
# T15.1 译码器 SystemVerilog Testbench 架构：从协议向量到 RTL 通过/失败判定
## 本节学习目标

本节设计译码器寄存器传输级验证中的 SystemVerilog testbench 架构。这里的 testbench 不是“写一个 initial block 跑一下”，而是一套能把 3GPP 协议向量、定点参考模型、统一译码子系统、三类译码引擎和失败证据串起来的验证外壳。DUT 可以是 LTE Turbo 单引擎、NR LDPC 单引擎、NR Polar 单引擎，也可以是 T14.4 的统一译码子系统。
...
- 把 T13.6 的 `vector.json`、checkpoint manifest 和 failure bundle 映射到 RTL 验证对象。
- 把 T14.6 的寄存器配置流映射成 testbench driver：写寄存器、锁存 descriptor、启动任务、等待 done/error、读取 status/trace。
...
如果只记住一句话：T15.1 的 testbench 是“协议向量和 RTL 行为之间的审判席”。driver 负责把向量变成硬件事务，monitor 负责把硬件行为变成观测记录，scoreboard 负责把两者按同一套 descriptor、hash、checkpoint 和时序对齐后判定。
...
| T13.6 Bit-exact 回归框架 | 知道 `vector.json`、checkpoint manifest、descriptor hash、first mismatch 和 failure bundle。 |
| T14.1 LTE Turbo RTL 微架构 | 知道 Turbo engine 的 rate recovery、SISO、alpha/beta、extrinsic、CRC checkpoint。 |
| T14.2 NR LDPC RTL 微架构 | 知道 LDPC engine 的 BG、$Z_c$、layered schedule、CN/VN update、syndrome checkpoint。 |
| T14.3 NR Polar RTL 微架构 | 知道 Polar engine 的 rate recovery、SC/SCL、path metric、sort/prune、CRC/RNTI final selector。 |
| T14.4 统一译码子系统 | 知道 descriptor dispatcher、input/output DMA、soft buffer manager、status/IRQ/trace 和独立 engine 边界。 |
| T14.6 寄存器表与配置流 | 知道配置字段、shadow lock、legality check、busy 写保护、error code 和 IRQ 清除。 |

自检问题：

| 问题 | 若回答不出来，先回看 |
|:---|:---|
| 为什么最终 CRC pass 不能替代 bit-exact checkpoint 比较？ | T13.6。 |
| 为什么 BUSY 中写配置必须由断言或 directed test 抓住？ | T14.6。 |
| 为什么 HARQ soft buffer 要测试 reset/abort 中的半提交？ | T14.5。 |
| 为什么 Polar SCL 的 path metric tie-breaker 要进入 scoreboard？ | T10.5、T13.4、T14.3。 |
...
| 证据边界 | testbench methodology 不需要直接 3GPP 引用；生成向量必须回链 T7、T9、T10、T15.2 的 Rel-19 证据。 |
...
| LTE 译码协议与工程章节 | `docs/L2/T7.*.md`, `docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md` | 本节不重写 Turbo 算法推导。 |
| NR LDPC 协议与工程章节 | `docs/L2/T8.*.md`, `docs/L2/T9.*.md`, `docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md` | 本节不重写 LDPC 协议表。 |
| NR Polar 协议与工程章节 | `docs/L2/T10.*.md`, `docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md` | 本节不重写 Polar construction。 |
| Bit-exact 回归框架 | `docs/L3/T13.6_bit_exact_regression_harness.md` | 本节把它落到 RTL testbench。 |
| 统一子系统与寄存器配置 | `docs/L3/T14.4_unified_decoder_subsystem_architecture.md`, `docs/L3/T14.6_decoder_register_map_configuration_flow.md` | 真实 RTL 端口名以实现为准。 |

协议前因后果是：T7/T9/T10 负责回答“这条向量为什么合法、按协议应该输出什么”；T13.6 负责回答“参考模型和 RTL 应该在哪些 checkpoint 一致”；T14.1-T14.6 负责回答“硬件有哪些接口、状态和错误码”；T15.1 把这些对象放到 SystemVerilog testbench 中，形成可执行验证闭环。
...
reference vector loader 是 testbench 的入口。它把 T13.6 的文件包变成 SystemVerilog 可消费的事务对象。
...
真实实现可以拆分成 class 或 interface，但核心字段必须和 T14.6 的配置流对齐。若 `descriptor_hash` 在 loader、driver log、DUT trace 中不同，scoreboard 必须优先报 descriptor mismatch，而不是继续比较 payload。
...
假设 T15.2 后续提供一条 NR LDPC directed vector：`decoder_family=ldpc`，`BG=1`，`Zc=192`，`rv=2`，`cbg_id=1`，`CBGTI=0100`，`CBGFI=1`，HARQ soft buffer 中已经有 RV0 的旧 LLR。这个例子不新增协议表值，只用 T9.3/T14.5 已讲过的 CBG/RV 语义展示 testbench 如何工作。
...
本节不重新定义浮点仿真。比较链路应继承 T13.6：

| 比较对象 | RTL testbench 中的处理 |
|:---|:---|
| Python float vs Python fixed | 在向量生成阶段完成，产出定点 expected 和性能预算。 |
| Python fixed vs C/C++ fixed | 由 T13.6 回归完成，产出 checkpoint manifest 和 expected trace。 |
| C/C++ fixed vs RTL | T15.1 scoreboard 的主任务，按 checkpoint 和输出状态 bit-exact 比较。 |
...
T15.1 是验证架构，但它反向约束 RTL/ASIC 可观测性：
...
| 协议回链 | 向量生成回链 T7/T9/T10/T15.2 和 Rel-19 本地路径；testbench 方法不冒充 3GPP 规定。 |
...
| 只比较最终 payload | 内部 checkpoint 漏检，CRC fail 难定位。 | 按 T13.6 manifest 比较 trace。 |
...
| 结论 | 证据 | 本地路径 | 边界 |
|:---|:---|:---|:---|
| LTE Turbo RTL 向量必须回链 LTE CB segmentation、Turbo interleaver、rate matching 和 CRC。 | TS 36.212 Rel-19 `36212-j30`；项目 T7/T14.1。 | `3GPP_Rel19/processed/TS_36.212_36212-j30`、`docs/L2/T7.*.md`、`docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md` | 本节不重建协议表；T15.2 后续定义 exact vector suite。 |
| NR LDPC RTL 向量必须回链 BG/$Z_c$、rate recovery、RV/k0、CBG 和 CRC/syndrome。 | TS 38.212 Rel-19 `38212-j30`、TS 38.214 Rel-19 `38214-j30`；项目 T8/T9/T14.2。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`TS_38.214_38214-j30`、`docs/L2/T8.*.md`、`docs/L2/T9.*.md`、`docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md` | testbench 只消费已生成向量和 descriptor。 |
| NR Polar RTL 向量必须回链 reliability/frozen mask、rate recovery、CRC/RNTI 和 SCL/CA-SCL 选择语义。 | TS 38.212 Rel-19 `38212-j30`；项目 T10/T14.3。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`docs/L2/T10.*.md`、`docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md` | list size、PM 近似、sorter 是实现策略。 |
| vector schema、checkpoint manifest、failure bundle 的字段来自项目 bit-exact 框架。 | T13.6。 | `docs/L3/T13.6_bit_exact_regression_harness.md` | 工程回归框架，不是 3GPP 条文。 |
| register/status/error/IRQ/trace 驱动对象来自统一子系统和寄存器配置流。 | T14.4、T14.6。 | `docs/L3/T14.4_unified_decoder_subsystem_architecture.md`、`docs/L3/T14.6_decoder_register_map_configuration_flow.md` | 真实端口名、总线协议和地址以 RTL 实现为准。 |

## 图示
![图 1：decoder testbench architecture](assets/T15.1_decoder_testbench_architecture.png)

## 执行与证据记录

| 项 | 命令或证据 | 结果 |
|:---|:---|:---|
| Roadmap 任务 | `2026-06-19-lte-nr-decoding-learning-roadmap.md:1203-1212` | T15.1 prompt 已逐条覆盖并扩展。 |
| 图形生成 | `python3 tools/figures/render_t15_1_decoder_testbench_architecture.py` | `WROTE .../docs/L3/assets/T15.1_decoder_testbench_architecture.png (2800, 2380)`。 |
| 脚本编译 | `PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile tools/figures/render_t15_1_decoder_testbench_architecture.py` | 退出码 0。 |
...
- [Project, 2026] T13.6 Bit-exact regression harness, `docs/L3/T13.6_bit_exact_regression_harness.md`.
- [Project, 2026] T14.1 LTE Turbo RTL microarchitecture, `docs/L3/T14.1_LTE_Turbo_RTL_microarchitecture.md`.
- [Project, 2026] T14.2 NR LDPC RTL microarchitecture, `docs/L3/T14.2_NR_LDPC_RTL_microarchitecture.md`.
- [Project, 2026] T14.3 NR Polar RTL microarchitecture, `docs/L3/T14.3_NR_Polar_RTL_microarchitecture.md`.
- [Project, 2026] T14.4 Unified decoder subsystem architecture, `docs/L3/T14.4_unified_decoder_subsystem_architecture.md`.
- [Project, 2026] T14.6 Decoder register map and configuration flow, `docs/L3/T14.6_decoder_register_map_configuration_flow.md`.
