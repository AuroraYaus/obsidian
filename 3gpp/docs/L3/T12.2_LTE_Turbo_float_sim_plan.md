...
type: spec
aliases:
  - T12.2 LTE Turbo float sim plan
tags:
  - 3gpp
  - docs
  - l3
  - lesson
source_spec: "docs/L3/T12.2_LTE_Turbo_float_sim_plan.md"
---
# T12.2 LTE Turbo 浮点仿真计划：从协议比特链路到可复现 BLER 曲线
## 本节学习目标
...
| T12.1 Golden Model 工程布局 | 知道 `vector.json`、`input_llr.npy`、`metrics.csv`、`frames.jsonl`、`seeds.json`、失败 replay 和 evidence archive 的职责。 |
| LLR 基础 | 知道正 LLR 倾向比特 0 或 1 必须由工程约定明确，本项目默认使用 `POS_ZERO`。 |
| BER/BLER 基础 | 知道BER和 BLER 是统计指标，不是 3GPP 直接给出的译码器合格线。 |

如果前置知识不足，先回看 T2.2、T4.5、T6.7、T7.3、T7.4 和 T12.1。本节默认你已经理解“协议位序必须 bit-exact，算法曲线和阈值是工程评估”这个边界。
...
TS 36.212 Rel-19 `36212-j30` 是本节 LTE Turbo 比特处理的主要依据。本地路径为 `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md`、`sections.jsonl`、`tables/` 和 `media/`。本节使用的协议链路如下。

| 协议位置 | 本地证据 | 对浮点仿真的约束 | 接收端后果 |
|:---|:---|:---|:---|
| TS 36.212 §5.1.1 CRC calculation | `TS_36.212_36212-j30_content.md` around line 575 | 定义 CRC24A、CRC24B、CRC16、CRC8 的生成多项式和系统式附加口径。 | TB CRC 和 CB CRC 的 pass/fail 必须使用同一比特顺序和多项式。 |
| TS 36.212 §5.1.2 Code block segmentation and code block CRC attachment | `TS_36.212_36212-j30_content.md` around line 601 | 规定最大码块大小 $Z=6144$、多 CB 时附加 CRC24B、filler 位于第 0 个 CB 开头。 | 仿真 descriptor 必须记录 $C$、$K_r$、$F$、CB CRC 长度和重组顺序。 |
| TS 36.212 §5.1.3.2.1 Turbo encoder | `TS_36.212_36212-j30_content.md` around line 721；T6.3 重建图 | 规定 LTE Turbo 为并行级联卷积码，含两个 8 状态组成编码器和内部交织器，母码率为 1/3。 | 编码器参考必须输出系统流、第一校验流、第二校验流和 trellis termination。 |
| TS 36.212 §5.1.3.2.2 Trellis termination | `TS_36.212_36212-j30_content.md` around line 747 | 规定前 3 个尾比特终止第一组成编码器，后 3 个尾比特终止第二组成编码器。 | 浮点 decoder 的 trellis 边界和参考 encoder tail bits 必须一致。 |
| TS 36.212 §5.1.3.2.3 Turbo internal interleaver | `TS_36.212_36212-j30_content.md` around line 761；`tables/table_0009.csv/html` | 规定 $K$ 对应的 $f_1,f_2$ 二次置换参数。 | SISO-1/SISO-2 外信息交换必须使用同一交织和解交织地址。 |
| TS 36.212 §5.1.4.1 Rate matching for turbo coded transport channels | `TS_36.212_36212-j30_content.md` around line 777 | 规定三路流子块交织、比特收集、循环缓存和速率匹配输出。 | 接收端 rate recovery 必须把收到的 LLR 放回正确循环缓存地址。 |
| TS 36.212 §5.1.4.1.2 Bit collection, selection and transmission | `TS_36.212_36212-j30_content.md` around line 827 and 923 | 规定 soft buffer size $N_{\mathrm{cb}}$、输出长度 $E$、`rvidx` 和选择流程。 | HARQ 重传必须按同一 CB/进程/RV 地址进行软合并。 |
...
T12.2 必须继承 T12.1 的归档口径。一次运行输出建议如下：
...
本节必须继承 T12.1 的 seed 派生规则，禁止使用 Python 内置 `hash()`。建议 `seeds.json` 记录：
...
并记录 `confidence_limited=true`。若 $N_{\mathrm{err}}=0$，工程报告可以写上界估计或交给 T12.5 的置信区间统一处理。

## 浮点仿真验收

单次 T12.2 实验至少满足以下验收项：
...
浮点仿真结束后，T13 会把 LLR 和内部度量转成定点。T12.2 应提前输出这些统计：
...
寄存器传输级和专用集成电路阶段需要从 T12.2 获得可直接进入 testbench 的对象。

| RTL/ASIC 对象 | T12.2 输出来源 |
|:---|:---|
...
1. 为什么 T12.2 需要参考编码器，即使真实接收机不会重新编码？
...
| :--- | :--- | :--- | :--- |
| LTE CRC | TS 36.212 Rel-19 `36212-j30` §5.1.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md` | 已核验锚点。 |
| LTE CB segmentation | TS 36.212 §5.1.2 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md` | 已核验锚点。 |
| LTE Turbo encoder | TS 36.212 §5.1.3.2.1、Figure 5.1.3-2 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md`；T6.3 重建图 `docs/L2/assets/T6.3_TS36.212_Figure_5.1.3-2_turbo_encoder_rebuild.png` | 已重建图。 |
| LTE trellis termination | TS 36.212 §5.1.3.2.2 | `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_content.md` | 已核验锚点。 |
| LTE Turbo internal interleaver | TS 36.212 §5.1.3.2.3、Table 5.1.3-3 | `tables/table_0009.csv/html`；T3.3/T6.4 重建资产 | 已重建上游资产。 |
| LTE Turbo rate matching | TS 36.212 §5.1.4.1-§5.1.4.1.2、Figure 5.1.4-1、Table 5.1.4-1 | `TS_36.212_36212-j30_content.md`、`tables/table_0010.csv/html` | 已核验锚点；完整地址实现由代码阶段承接。 |
...
| 写入文件 | `docs/L3/T12.2_LTE_Turbo_float_sim_plan.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T12.2。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 阶段 11 与 T12.2 条目。 |
| 前置依据 | 已读取 `docs/L2/T6.7_Turbo_iteration_extrinsic_stopping.md`、`docs/L2/T7.4_LTE_code_block_reassembly_TB_CRC.md`、`docs/L3/T12.1_python_golden_model_project_layout.md`。 |
| 协议资料 | 使用 TS 36.212 `36212-j30` §5.1.1-§5.1.4.1 本地抽取物；TS 36.213 MCS/TBS 保持 `待核验`。 |
| 原始 manifest 核验 | `rg -n "36212-j30|36\\.212|TS_36\\.212" 3GPP_Rel19/manifest.csv 3GPP_Rel19/processed/manifest.json 3GPP_Rel19/Rel19_协议下载清单.md` 确认 `manifest.csv` 第 8 行记录 TS 36.212 Rel-19 `36212-j30.zip`、`36212-j30.docx`、源 URL、大小 `1420921` 和 SHA-256 `06a40a3b3214d372b0a6008ee1c885cd025ed30aef6399c9ea76a1d4593a1450`。 |
...
| 3GPP_译码知识库入口 核验 | 已读取 `3GPP_Rel19/processed/TS_36.212_36212-j30/TS_36.212_36212-j30_index.md`；确认该目录包含 `source.docx`、`document.xml`、`TS_36.212_36212-j30_content.md`、`sections.jsonl`、`metadata.json`、`tables/`、`equations/`、`media/`，并记录阅读边界：公式以 `equations/*.xml` 为准，`TS_36.212_36212-j30_content.md` 不是数学公式权威来源。 |
| 未执行内容 | 本节是仿真计划，未运行真实 LTE Turbo BLER 仿真；后续实现必须按本节命令模板生成 `metrics.csv`、`frames.jsonl`、失败 dump 和曲线图。 |
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3/T12.2_LTE_Turbo_float_sim_plan.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3/T12.2_LTE_Turbo_float_sim_plan.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T12.2_LTE_Turbo_float_sim_plan.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3/T12.2_LTE_Turbo_float_sim_plan.md`：`LATEX_RENDER_AUDIT_OK formulas=91`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T12.2_LTE_Turbo_float_sim_plan.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选来自 TS 36.213 MCS/TBS 明确 `待核验`、TS 36.212 表/图已由上游讲义重建的回链，以及正文公式引用，不构成本节失败。 |
...
- [上游讲义] `docs/L2/T6.7_Turbo_iteration_extrinsic_stopping.md`，Turbo 迭代、外信息交换与停止。
- [上游讲义] `docs/L2/T7.4_LTE_code_block_reassembly_TB_CRC.md`，LTE 码块重组与传输块 CRC。
