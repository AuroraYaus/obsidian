...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T17.1_python_golden_model_project_layout.md"
---
...
- 后续 T17.2-T17.5、T13、T14、T15 如何复用同一套 schema、日志和证据归档。
...
本节的执行范围是“工程布局规范”。因此协议依据用于说明向量来源和参数回链，不在本节重新复现 TS 36.212/38.212 的编码、速率匹配或循环冗余校验公式。具体 LTE Turbo 浮点仿真由 T17.2 承接，NR LDPC 由 T17.3 承接，NR Polar 由 T17.4 承接，BER/BLER 曲线由 T17.5 承接。
...
| TS 36.212 Rel-19 `36212-j30` §5.1.2、§5.1.3.2、§5.1.4.1 | `3GPP_Rel19/processed/TS_36.212_36212-j30/content.md`，`sections.jsonl` | LTE CB segmentation、Turbo coding、rate matching/RV 是 T17.2 向量字段来源。 | 具体地址、交织和 RV 恢复由 T17.2 使用。 |
| TS 38.212 Rel-19 `38212-j30` §5.1、§5.2.2、§5.3.2、§5.4.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md`，`sections.jsonl` | NR LDPC CRC、segmentation、base graph/lifting、rate matching 是 T17.3 向量字段来源。 | CRC 多项式复现见上游讲义；具体 BG/Zc/k0 查表由 T17.3/T8/T9 回链。 |
| TS 38.212 Rel-19 `38212-j30` §5.1、§5.2.1、§5.3.1、§5.4.1、§6.3、§7.3 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md`，`sections.jsonl` | NR Polar CRC、CRC/RNTI、segmentation、encoding、rate matching、上行控制信息/下行控制信息控制信息链路是 T17.4 向量字段来源。 | reliability sequence 和 rate recovery 已由 T10.3/T10.7 承接；UCI/DCI 精确 CRC/RNTI 字段由 T17.4 使用时关闭。 |
...
本节的协议证据粒度按“工程布局任务”处理：能直接闭环到本节 schema 的内容写成章节级来源和本地路径；涉及具体表、图或公式的 bit-exact 数值，不在本节重复复现，而是回链到已重建讲义或后续 T17.2-T17.4。具体边界如下：

| 证据对象 | 本节是否闭环 | 精细锚点或关闭条件 |
|:---|:---|:---|
| LTE Turbo 合法 $K/f_1/f_2$ | 不在本节重复闭环 | 已由 T3.3/T6.4 使用 TS 36.212 Table 5.1.3-3 的 `table_0009.csv/html` 和正文表格闭环；T17.2 使用时回链。 |
| LTE Turbo 编码器结构 | 不在本节重复闭环 | 已由 T6.3 正文复现 TS 36.212 Figure 5.1.3-2 的 PCCC、内部交织器和终止路径；T17.2 使用时回链。 |
| LTE sub-block interleaver | 不在本节重复闭环 | 已由 T7.2/T11.2 复现 TS 36.212 Table 5.1.4-1 的 `table_0010.csv/html`。 |
| NR LDPC BG/Zc 和 rate matching 表 | 本节只登记来源 | T17.3 使用具体数值时必须回链 T8/T9 已重建资产或在 T17.3 重新列出表/图/公式证据。 |
| NR Polar reliability/rate matching 表 | 本节只登记来源 | T17.4 使用具体数值时必须回链 T10 已重建资产或在 T17.4 重新列出表/图/公式证据。 |
...
T4.6 已经给出中立接口契约。T17.1 需要把它转成仿真配置 schema。最小公共 descriptor 可分成四类字段。
...
`vector.json` 不能只写自然语言说明，至少应有可机器检查的最小 schema。下面字段是后续 T17.2-T17.5 的共同最低要求：
...
后续 T17.2-T17.4 会把 toy decoder 替换成真实浮点算法。
...
| 后续承接 | 能说明 T17.2-T17.5、T13、T14、T15 如何复用本节目录、向量和日志。 |
...
| NR LDPC CRC/segmentation/coding/rate matching | TS 38.212 Rel-19 `38212-j30` | `3GPP_Rel19/processed/TS_38.212_38212-j30/` | CRC 多项式复现见上游讲义；具体 BG/Zc/k0 查表由 T17.3 和上游 T8/T9 承接。 |
| NR Polar CRC/CRC-RNTI/segmentation/coding/rate matching | TS 38.212 Rel-19 `38212-j30` | `3GPP_Rel19/processed/TS_38.212_38212-j30/` | reliability sequence/rate recovery 由 T10.3/T10.7 承接；UCI/DCI CRC/RNTI 字段由 T17.4 使用时精确闭环。 |
...
## 图示
![图 1：golden model project layout](assets/T12.1_golden_model_project_layout.svg)
...
| 写入文件 | `docs/L3/T17.1_python_golden_model_project_layout.md` |
| 新增证据资产 | `docs/L3/assets/T12.1_golden_model_project_layout.svg`（手绘 SVG，替代原 PIL 渲染 PNG 及 `tools/figures/render_t12_1_golden_model_layout.py`）。 |
...
| 未执行内容 | 未运行真实 BLER 仿真、未生成真实 protocol vectors；具体可执行工程由 T17.2-T17.5 按本节 schema 承接。 |
...
| 审计结果 | `python3 tools/audit_lesson_terms.py docs/L3/T17.1_python_golden_model_project_layout.md`：`LESSON_TERM_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_markdown_headings.py docs/L3/T17.1_python_golden_model_project_layout.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T17.1_python_golden_model_project_layout.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| 审计结果 | `python3 tools/audit_latex_render.py docs/L3/T17.1_python_golden_model_project_layout.md`：`LATEX_RENDER_AUDIT_OK formulas=4`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T17.1_python_golden_model_project_layout.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选为本节明确声明不重建的协议公式边界和协议来源登记，不构成失败。 |
