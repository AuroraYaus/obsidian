...
| T12.3 NR LDPC 浮点仿真计划 | 知道浮点基线如何输出 BP/MS/NMS/OMS 曲线、syndrome trace、message statistics 和 failure dump。 |
| T13.1 定点译码器需求 | 知道 Q 格式、裁剪、舍入、饱和、性能损失预算和 bit-exact 检查点。 |

如果还不清楚 CN、VN、posterior LLR 和 syndrome 的区别，先回看 T8.4-T8.8。T13.3 默认你已经知道 LDPC 是一张 Tanner graph 上的消息传递译码，本节只把这些消息变成可审计的整数。
...
| 资料 | 本地路径 | 边界 |
| :--- | :--- | :--- |
| TS 38.212 §5.2.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md` | 不在本节重新推导分段规则。 |
| TS 38.212 §5.3.2 | `TS_36.212_36212-j30_content.md`；`tables/table_0013/0014/0015.csv/html` | 本节使用上游 T8.3 重建资产，不重画完整 BG shift table。 |
| TS 38.212 §5.4.2/§5.4.2.1/§5.4.2.2 | `TS_36.212_36212-j30_content.md`；T9.3/T9.4 回链 | 接收端 LLR soft buffer 写回和饱和合并是逆向实现策略。 |
| T8.6 | `docs/L2/T8.6_LDPC_MS_NMS_OMS.md` | 算法实现，不是 3GPP 强制译码算法。 |
| T8.7 | `docs/L2/T8.7_layered_LDPC_decoding_schedule.md` | 调度是实现策略，TS 38.212 不规定 decoder schedule。 |
| T12.3 | `docs/L3/T12.3_NR_LDPC_float_sim_plan.md` | 本节不生成真实 fixed BLER 曲线。 |
...
为什么 LDPC 定点模型特别容易出错？因为它的错误不一定立刻体现在最终 hard bits 上。一个 CN 的 `min2` 选择错，可能只让某条边消息偏大；几个 layer 后，posterior LLR 被逐步推向饱和；最后 syndrome 不收敛或 CRC 失败时，根因已经离最终输出很远。因此 T13.3 必须把 CN update、layered read-modify-write、posterior LLR 和 syndrome 分层打点。
...
C/C++ 定点模型应从 T12.3 run directory 或 failure bundle 读取对象，而不是手写散装数组。
...
这些不是协议常数，只是实验入口。最终选择必须由 T12.5 曲线和 T13.6 bit-exact regression 证明。
...
T13.6 的 bit-exact compare 应能在 `layer_trace` 中定位第一处 mismatch。

## C/C++ 数组布局计划

第一版 C/C++ 模型以清晰和 bit-exact 为先，不先追求单指令多数据（Single Instruction, Multiple Data, SIMD）。T13.5 会再处理 SIMD lane、cache 和 bank。
...
T13.3 的 bit-exact 不直接对浮点 BP，而是按三层比较：

| 层级 | 参考 | 被测 | 比较规则 |
|:---|:---|:---|:---|
| Python fixed | Python 定点 LDPC reference | C/C++ fixed | 所有整数检查点 exact。 |
| Python float | T12.3 浮点 BP/MS/NMS/OMS | Python/C fixed | 曲线和统计比较，不要求整数一致。 |
| C/C++ fixed | T13.3 模型 | 后续 RTL | 指定 checkpoint exact。 |
...
| 字段 | 说明 |
|:---|:---|
| `float_baseline_run_id` | T12.3 浮点基线。 |
| `fixed_run_id` | 当前定点运行。 |
...
T13.3 的定点模型会直接变成 T14.2 NR LDPC RTL 微架构的前置规格。
...
RTL 可以选择不同并行度、bank mapping、layer folding 或 partial parallelism；但只要它声明兼容某个 `fixed_requirement_id`，就必须在 T13.6/T15 中通过同一组 bit-exact 检查点。
...
6. T13.3 最少应保存哪五层 compare checkpoint？
...
| :--- | :--- | :--- | :--- |
| NR LDPC segmentation | TS 38.212 `38212-j30` §5.2.2 | `3GPP_Rel19/processed/TS_38.212_38212-j30/TS_38.212_38212-j30_content.md` | 已由 T12.3/T9.5 回链。 |
| NR LDPC coding and H | TS 38.212 §5.3.2、Table 5.3.2-1/2/3 | `tables/table_0013/0014/0015.csv/html`；T8.3 | 上游已复现。 |
| NR LDPC rate matching | TS 38.212 §5.4.2/§5.4.2.1/§5.4.2.2 | `TS_36.212_36212-j30_content.md`；T9.3/T9.4 | 使用上游证据回链。 |
...
| 候选资产 | 本节处理 | 关闭条件 |
|:---|:---|:---|
| TS 38.212 Table 5.3.2-1/2/3 全表 | 本节回链 T8.3，不重新复制长表。 | 若 T13.3 代码直接解析原表生成 layer schedule，必须把 T8.3 CSV/HTML、脚本和 hash 纳入 fixed evidence archive。 |
...
| 写入文件 | `docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md` |
| Roadmap 卡片 | 已读取 `2026-06-19-lte-nr-decoding-learning-roadmap.md` 中 T13.3。 |
| 台账依据 | 已读取 `docs/audits/lte_nr_decoding_remaining_work_register.md` 阶段 12 与 T13.3 条目。 |
| 上游讲义 | 已读取 `docs/L2/T8.6_LDPC_MS_NMS_OMS.md`、`docs/L2/T8.7_layered_LDPC_decoding_schedule.md`、`docs/L3/T12.3_NR_LDPC_float_sim_plan.md`、`docs/L3/T13.1_fixed_point_decoder_requirements.md`。 |
| 术语审计 | `python3 tools/audit_lesson_terms.py docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md`：`LESSON_TERM_AUDIT_OK`。 |
| 标题审计 | `python3 tools/audit_markdown_headings.py docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md`：`MARKDOWN_HEADING_AUDIT_OK`。 |
| 深度审计 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md`：`LESSON_DEPTH_AUDIT_OK`。 |
| LaTeX 审计 | `python3 tools/audit_latex_render.py docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md`：`LATEX_RENDER_AUDIT_OK formulas=55`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T13.3_NR_LDPC_fixed_point_model_plan.md`：`REFERENCE_REBUILD_AUDIT_CANDIDATES`，候选来自上游已重建 TS 38.212 表格回链、实现策略边界、论文引用和 TS 38.214 MCS/TBS 条件项；本节未直接使用 MCS/TBS 表值。 |
...
- [上游讲义] `docs/L2/T8.6_LDPC_MS_NMS_OMS.md`，LDPC Min-Sum、NMS 与 OMS。
- [上游讲义] `docs/L2/T8.7_layered_LDPC_decoding_schedule.md`，LDPC 分层译码调度。
