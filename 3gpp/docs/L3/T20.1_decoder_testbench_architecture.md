...
| LTE 译码协议与工程章节 | `docs/L2_协议算法/T7.*.md`, `docs/L3/T19.1_LTE_Turbo_RTL_microarchitecture.md` | 本节不重写 Turbo 算法推导。 |
| NR LDPC 协议与工程章节 | `docs/L2_协议算法/T8.*.md`, `docs/L2_协议算法/T9.*.md`, `docs/L3/T19.2_NR_LDPC_RTL_microarchitecture.md` | 本节不重写 LDPC 协议表。 |
| NR Polar 协议与工程章节 | `docs/L2_协议算法/T10.*.md`, `docs/L3/T19.3_NR_Polar_RTL_microarchitecture.md` | 本节不重写 Polar construction。 |
...
| 结论 | 证据 | 本地路径 | 边界 |
|:---|:---|:---|:---|
| LTE Turbo RTL 向量必须回链 LTE CB segmentation、Turbo interleaver、rate matching 和 CRC。 | TS 36.212 Rel-19 `36212-j30`；项目 T7/T19.1。 | `3GPP_Rel19/processed/TS_36.212_36212-j30`、`docs/L2_协议算法/T7.*.md`、`docs/L3/T19.1_LTE_Turbo_RTL_microarchitecture.md` | 本节不重建协议表；T20.2 后续定义 exact vector suite。 |
| NR LDPC RTL 向量必须回链 BG/$Z_c$、rate recovery、RV/k0、CBG 和 CRC/syndrome。 | TS 38.212 Rel-19 `38212-j30`、TS 38.214 Rel-19 `38214-j30`；项目 T8/T9/T19.2。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`TS_38.214_38214-j30`、`docs/L2_协议算法/T8.*.md`、`docs/L2_协议算法/T9.*.md`、`docs/L3/T19.2_NR_LDPC_RTL_microarchitecture.md` | testbench 只消费已生成向量和 descriptor。 |
| NR Polar RTL 向量必须回链 reliability/frozen mask、rate recovery、CRC/RNTI 和 SCL/CA-SCL 选择语义。 | TS 38.212 Rel-19 `38212-j30`；项目 T10/T19.3。 | `3GPP_Rel19/processed/TS_38.212_38212-j30`、`docs/L2_协议算法/T10.*.md`、`docs/L3/T19.3_NR_Polar_RTL_microarchitecture.md` | list size、PM 近似、sorter 是实现策略。 |
...
## 图示
![图 1：decoder testbench architecture](assets/T15.1_decoder_testbench_architecture.svg)
...
| 图形资产 | `docs/L3/assets/T15.1_decoder_testbench_architecture.svg` | 手绘 SVG，2026-08-04 替代原 PIL 渲染 PNG（原 `render_t15_1_decoder_testbench_architecture.py` 与 PNG 已删除，图示语义不变）；SVG 几何审计六项检查全部通过，cairosvg 渲染验证无报错。 |
