...
## 图示
![图 1：BER BLER curve reporting](assets/T12.5_BER_BLER_curve_reporting.png)
...
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T12.5_BER_BLER_curve_reporting.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选来自本节明确声明不复现的协议公式/表格边界、DCI blind detection 边界和 README 公式权威性提示，不构成本节失败。 |
