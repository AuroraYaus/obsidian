...
tags:
  ...
  - l3
  - lesson
source_spec: "docs/L3/T19.5_soft_buffer_HARQ_memory_architecture.md"
---
...
| 名称 | 中文含义 |
|:---|:---|
| 低密度奇偶校验码 | NR 数据业务的主要信道编码，T19.5 关注它的 HARQ soft buffer 和 CBG 部分重传。 |
| Turbo | LTE 数据业务的信道编码，T19.5 关注它的 per-CB circular buffer 和 `rvidx`。 |
...
| T19.4 统一译码子系统 | 知道 soft buffer manager 是统一子系统共享资源，engine 不能直接释放整个 HARQ 上下文。 |
...
| 为什么 `harq_id=3, cb=0, rv=0` 和 `harq_id=3, cb=0, rv=2` 应该命中同一个主 soft buffer？ | T7.3、T19.4。 |
| NR 的 CBGTI bit 为 0 时，组内 CB 的 soft buffer 应该怎样处理？ | T9.3。 |
| CBGFI=0 与 CBGFI=1 的工程差别是什么？ | T9.3、TS 38.214 §5.1.7.2。 |
| reset 发生在 soft-buffer 写事务中间时，为什么不能留下半写入状态？ | T5.4、T18.6、T19.4。 |
...
本节是 L3 工程架构课。它会精读协议给出的逻辑身份和 rate-matching/HARQ 语义，但不会给真实芯片寄存器地址、真实 SRAM macro 宽度、真实综合时序或真实软件驱动 ABI。真实寄存器配置流在 T19.6，真实 SystemVerilog testbench、覆盖率和时序签收在 T15 承接。
...
| T7.3 | `docs/L2/T7.3_LTE_HARQ_soft_buffer_RV.md` | T19.5 升级为存储架构，不重复所有 rate matching 推导。 |
| T9.3 | `docs/L2/T9.3_NR_LDPC_HARQ_soft_buffer_RV_k0.md` | T19.5 升级为 bank/transaction/lifecycle 架构。 |
| T19.4 | `docs/L3/T19.4_unified_decoder_subsystem_architecture.md` | T19.5 深化其中 soft buffer manager。 |
...
| 对象 | 推荐策略 | 验证点 |
|:---|:---|:---|
| soft LLR | signed 6-8 bit 起步，范围由 T18.1/T18.2/T18.3 profile 定义。 | 正负饱和、符号约定、zero-is-neutral。 |
...
| `trace_dump` | 输出 address/write/commit/fail evidence。 | T18.6 failure bundle。 |
...
| 统一子系统中 soft buffer key 和 RV transaction 的分层来自 T19.4。 | 本项目 T19.4。 | `docs/L3/T19.4_unified_decoder_subsystem_architecture.md` | 项目实现架构，不是 3GPP 条款。 |

## 图示
![图 1：soft buffer HARQ memory architecture](assets/T19.5_soft_buffer_HARQ_memory_architecture.png)
...
| 图形生成 | `python3 tools/figures/render_t14_5_soft_buffer_harq_memory.py` | `WROTE .../docs/L3/assets/T19.5_soft_buffer_HARQ_memory_architecture.png (2600, 2500)`。 |
...
| 术语审计 | `python3 tools/audit_lesson_terms.py docs/L3/T19.5_soft_buffer_HARQ_memory_architecture.md` | 首轮发现 BLER 未展开，已补“块错误率”；复跑输出 `LESSON_TERM_AUDIT_OK`。 |
| 标题审计 | `python3 tools/audit_markdown_headings.py docs/L3/T19.5_soft_buffer_HARQ_memory_architecture.md` | `MARKDOWN_HEADING_AUDIT_OK`。 |
| 深度审计 | `python3 tools/audit_lesson_depth.py --strict docs/L3/T19.5_soft_buffer_HARQ_memory_architecture.md` | `LESSON_DEPTH_AUDIT_OK`。 |
| LaTeX 渲染审计 | `python3 tools/audit_latex_render.py docs/L3/T19.5_soft_buffer_HARQ_memory_architecture.md` | `LATEX_RENDER_AUDIT_OK formulas=15`。 |
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L3/T19.5_soft_buffer_HARQ_memory_architecture.md` | 输出 `REFERENCE_REBUILD_AUDIT_CANDIDATES`；候选已分类：TS 36.213 精确锚点按要求保留 `待核验`，TS 38.212 `k0` 表格抽取不完整且本节只使用语义/上游 T9.3 边界，T19.5 的正文图表为本节教学内容，项目内 T7.3/T9.3/T19.4 为本地参考讲义，不是外部论文公式缺失。 |
