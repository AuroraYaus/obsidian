...
type: spec
aliases:
  - T15.3 coverage regression strategy
tags:
  - 3gpp
  - docs
  - l3
  - lesson
source_spec: "docs/L3/T15.3_coverage_regression_strategy.md"
---
# T15.3 覆盖率与回归方案：把 directed vectors 变成可签收证据
## 本节学习目标
...
- 把 T15.2 的协议向量、边界案例和 expected-fail policy 转成 coverage bins。
...
| T13.6 Bit-exact 回归框架 | 知道 `vector.json`、checkpoint manifest、failure bundle、first mismatch 和 CI 命令模板。 |
| T15.1 Testbench 架构 | 知道 driver、monitor、scoreboard、assertions、reset/timeout tests 如何产生可观测证据。 |
| T15.2 协议向量与边界案例套件 | 知道 LTE Turbo、NR LDPC、NR Polar 的 directed cases、expected-fail class 和最小 dump 包。 |
...
| 问题 | 若回答不出来，先回看 |
|:---|:---|
| 为什么 `CRC_PASS` 不能证明所有 rate recovery 地址都正确？ | T7.3、T9.1、T13.6。 |
| 为什么 `100% code coverage` 仍可能漏掉 NR LDPC CBG hold bug？ | T9.3、T15.2。 |
| 为什么 expected-fail vector 的“失败”也要纳入覆盖率？ | T15.1、T15.2。 |
| 为什么 nightly 的随机 seed 必须能 replay？ | T12.1、T13.6。 |
...
| 证据边界 | 无需直接 3GPP 引用 for coverage methodology. Coverage bins that depend on protocol parameters must reference upstream Rel-19 evidence from T7/T9/T10/T15.2. |
...
| T15.1/T15.2 | `docs/L3/T15.1_decoder_testbench_architecture.md`、`docs/L3/T15.2_protocol_vector_corner_case_suite.md` | 本节将其组织成 coverage/regression plan。 |

协议前因后果可以这样理解：协议定义了“接收端可能遇到哪些合法和非法状态”；T15.2 把这些状态变成向量；T15.3 再把向量覆盖、RTL 代码覆盖、断言覆盖、失败分类和夜间回归组织成签收证据。若 coverage bin 没有可观测字段或可复现向量，它只是愿望清单，不是验证计划。
...
图中英文标题为 `T15.3 Coverage and Regression Strategy`。图的视觉检查注记也迁移为正文要求：`Visual check: all arrows are straight two-point shafts with vector-derived heads; tables use 24px text and centered cells.` 这条注记用于约束后续重绘时的箭头、表格字号和单元格对齐，避免图形内容迁移后出现视觉结构偏差。
...
功能覆盖率需要先定义对象模型。建议从 T15.2 的向量对象中抽取 coverage event：
...
| Block length | `small`, `boundary`, `large`, `max_supported` | `B/A/K/E/N` 分类 | 具体边界来自 T7/T9/T10/T15.2。 |
...
功能覆盖率的一个常见错误是把 `block_length` 写成真实数字一一覆盖。真实 TB/CB 长度太多，不可能穷尽。工程上应先分类：小块、协议分支边界、表格最大支持、实现最大支持、随机长块。具体到 LTE Turbo，`K` 合法集合来自 TS 36.212 Table 5.1.3-3，上游 T3.3/T6.4 已复现；T15.3 不重新列全表，只把“合法集合中的最小、边界、最大和随机抽样”定义为 coverage bins。

## 三类译码器的覆盖矩阵

### LTE Turbo 覆盖点

LTE Turbo coverage 必须围绕 T7/T15.2 的接收端风险：
...
| Control | `timeout`, `mid_run_reset`, `harq_context_switch` | 抓半提交 soft buffer 和 HARQ 上下文污染。 | T7.3, T14.5, T15.1。 |
...
| Numeric health | `min_sum`, `nms`, `oms`, `llr_sat`, `cn_sat`, `vn_sat` | 抓定点缩放和饱和。 | T8.6, T13.3, T14.2。 |
...
| List behavior | `L1_SC`, `L2`, `L4_or_more`, `PM_tie`, `all_crc_fail`, `second_best_crc_pass` | 抓路径复制、剪枝、tie-break 和 CRC-aided 选择。 | T10.5, T10.6, T13.4。 |
...
断言覆盖率要和 T15.1 的 assertions 对齐。建议必选断言包括：
...
| `commit` | merge queue 或 CI | T15.2 directed positive/negative、reset/timeout、LLR saturation、family selector。 | required directed vectors 全 pass；expected-fail 分类一致；关键断言无 fail。 |
...
随机回归必须可重放。建议使用 T12.1 的分层 seed 派生方式：
...
| `vector_id` | T15.2 vector 或 random vector 名称。 |
...
本仓库当前没有 `decoder_regress` 可执行程序；上面是 T15.3 定义的工程命令模板。当前可直接运行的是本文末尾列出的审计脚本、图形脚本和 Python 片段。
...
print("T15.3_COVERAGE_TOY_OK", missing)
```

输出应为：

```text
T15.3_COVERAGE_TOY_OK []
```
...
| Gate | 标准 |
|:---|:---|
| Directed suite | T15.2 required positive/negative vectors 100% pass；expected-fail class 与 policy 一致。 |
...
| Coverage event | 从 T15.1 monitor/scoreboard summary 生成归一化 event，检查字段完整性。 |
...
| Directed vector suite | T15.2 | `docs/L3/T15.2_protocol_vector_corner_case_suite.md` |
| Testbench observability | T15.1 | `docs/L3/T15.1_decoder_testbench_architecture.md` |

## 图示
![图 1：coverage regression strategy](assets/T15.3_coverage_regression_strategy.png)

## 执行与证据记录

| 项目 | 证据 |
|:---|:---|
| 图形生成 | `python3 tools/figures/render_t15_3_coverage_regression_strategy.py` -> `WROTE ... (3000, 2860)`。 |
| 图形脚本编译 | `PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile tools/figures/render_t15_3_coverage_regression_strategy.py` -> 退出码 0。 |
| 本节 Python 片段 | `T15.3_COVERAGE_TOY_OK []`；见 `## 具体数值例子`。 |
...
| 协议证据 | Coverage methodology 为工程策略。协议参数 bins 回链 T7/T9/T10/T15.2 和本地 Rel-19 路径。 |
...
- 本项目 `docs/L3/T13.6_bit_exact_regression_harness.md`。
