...
译码器工作在 L1 与 L2 的 MAC 交付边界：物理层译码产出的 TB 校验结果（ACK/NACK）就是交付给 MAC 层的信号，认清分层才能定位协议分册。3GPP 无线接口协议栈自有三层命名（不是 OSI（开放式系统互联参考模型）七层模型）：L1 物理层（PHY）、L2 的 MAC（媒体接入控制层）、RLC（无线链路控制层）、PDCP（分组数据汇聚协议）三个子层（NR 另加 SDAP（服务数据适配协议））、L3 的 RRC（无线资源控制，控制面）及其上的 NAS（非接入层）。协议锚点：TS 38.300 §4.4、§6（本地 `3GPP_Rel19/processed/TS_38.300_38300-j20/content.md`）。
...
图注：3GPP 用户面与控制面协议栈合并示意，参照 TS 38.300 Figure 4.4.1-1/4.4.2-1。层2 = SDAP/PDCP/RLC/MAC 子层的集合（用户面；控制面无 SDAP）；**图示为 NR（含 SDAP），LTE 用户面无 SDAP**。
...
NRD["NR 数据主线<br/>LDPC 数据译码<br/>TS 38.212 §5.3.2<br/>BG/Zc/rate recovery"]
NRC["NR 控制主线<br/>Polar 控制译码<br/>TS 38.212 §5.3.1<br/>SC/SCL/CRC aided"]
...
译码三条主线之外，全链路已补齐五个分支（全链路规划阶段 0-6 收官）：**控制面**（M14：PDCCH 盲检/DCI/PUCCH/TBCC）、**上行链路**（M15：PRACH/SRS/功控/DL-UL 差异）、**发送端镜像**（TX：调制/加扰/层映射预编码/RE 映射）、**调度与系统**（M16：HARQ/波束/CA-BWP/MAC 映射）、**工程实现**（T22/T23：接收链子系统/后端）。控制面是译码主线的调度入口——DCI 经 PDCCH 盲检拿到后解析出 MCS/TBS/RV 等 descriptor 字段（T9.0），Polar 译码在控制信道上的 CRC/RNTI 边界见 T10.6/T10.8。各分支的完整讲义：[[T14.1_PDCCH_blind_decoding|M14 控制面]]、[[T15.2_PRACH_random_access|M15 上行]]、[[TX1_modulation_mapping|TX 发送端]]、[[M16.1_scheduler_HARQ_process|M16 调度]]、[[T22.1_sync_channel_estimation_engineering|T22 接收链工程]]、[[T23.1_synthesis_backend_physical|T23 后端]]。概念锚点：[[TBCC_咬尾卷积码]]、[[PSS_SSS_同步信号与小区搜索]]、[[PBCH_MIB_广播信道]]、[[PDCCH_物理下行控制信道]]、[[DCI_下行控制信息]]、[[PUCCH_上行控制信道与UCI]]、[[TX_Chain_发送端处理链总览]]。
...
| 项目 | 记录 |
|:---|:---|
| 新增文件 | `docs/L0_协议阅读引导/T0.1_LTE_NR_decoder_protocol_reading_map.md`。 |
| 新增证据资产 | `docs/L0_协议阅读引导/assets/T0.1_LTE_NR_decoder_protocol_reading_map.svg`（手绘 SVG，替代原 Mermaid 渲染 PNG 及 `tools/archive_python_drawing/figures/render_t0_1_lte_nr_decoder_protocol_map.py`）。 |
...
- 本项目 `docs/L1_基础`、`docs/L2_协议算法`、`docs/L3_工程实现` 现有 LTE/NR 译码讲义。
