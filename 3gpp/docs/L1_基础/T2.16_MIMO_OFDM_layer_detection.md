...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1_基础/T2.16_MIMO_OFDM_layer_detection.md"
---
# T2.16 MIMO-OFDM：层映射、每 RE 信道矩阵与接收检测

## 本节学习目标

T2.15 已经介绍均衡模型。本节将单层 OFDM 推广到多天线、多层传输。MIMO-OFDM 的核心思想是：OFDM 先将宽带频率选择性信道分解为多个 RE；MIMO 再在每个 RE 上处理多层符号经过多天线信道后的混合。
...
在单层 OFDM 中，一个 RE 可写成（标量模型，T2.15 已讲）：
...
![图 2：2×2 MIMO 每 RE 接收模型 y=Hx+n——发送层 x0/x1 经信道矩阵 H 混合加噪声 n 得到观测 y0/y1；青绿色为对角线目标项，橙色为交叉干扰项](assets/T2.16_2x2_mimo_channel_model.svg)
...
![图 1：码字→层→天线端口映射（NR 下行示例）——泳道 A：1 个码字分 2 层（串接交错）；泳道 B：2 个码字分 4 层；层→端口恒等映射，端口数=层数，天线数可大于端口数](assets/T2.16_layer_mapping_ports.svg)
...
![图 3：秩（层数）与吞吐、鲁棒性的权衡——rank2 在满秩强信道高 SNR 下吞吐接近翻倍，低秩/低 SNR 时弱层拖累整体；rank1 稳健但吞吐封顶；曲线为教学示意，非协议数值](assets/T2.16_rank_throughput_tradeoff.svg)
...
**实现要点**：检测器输出接口应携带"每层、每 RE 的 CSI（等效噪声方差或 SINR）"而不是单一标量；后续 soft demapper 与 LLR 缩放（T2.11）都依赖这个 per-layer CSI。若接口只传"平均 SINR"，低秩信道下弱层的 LLR 会被系统性高估——这正是"LLR 过度自信"的主要来源之一。
...
本节给出 T2.16 涉及的协议条款级证据。本地证据路径统一为 `3GPP_Rel19/processed/TS_38.211_38211-j30/full.md`（下称"38.211"）与 `3GPP_Rel19/processed/TS_38.214_38214-j30/full.md`（下称"38.214"）等抽取件；条款内容均为从抽取件实际文本核对后摘录或概括。**说明**：抽取件中部分公式与表格以 OLE 图片嵌入（`media_svg/`），正文文字完整；凡以图片形式嵌入的公式，本文以抽取件的"[公式≈]"文本近似恢复，并在此注明。
...
| 图解材料 | 新增手绘 SVG 3 张：`assets/T2.16_layer_mapping_ports.svg`（码字→层→端口，双泳道）、`assets/T2.16_2x2_mimo_channel_model.svg`（2×2 接收模型，对角/交叉项着色）、`assets/T2.16_rank_throughput_tradeoff.svg`（秩与吞吐权衡曲线+对比卡）；保留原 Mermaid 链路图。 |
| 图解验证 | 三张 SVG 均通过：① Y 坐标扫描（相邻元素间距 ≥8px）；② Python bbox 审计（文字不出 viewBox、text-text 无重叠、文字不溢出宿主矩形）；③ `cairosvg -s 1.5` 渲染无异常。 |
