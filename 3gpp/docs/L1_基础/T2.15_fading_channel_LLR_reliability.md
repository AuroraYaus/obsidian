...
type: definition
aliases:
  - T2.15 fading channel LLR reliability
tags:
  - 3gpp
  - docs
  - l1
  - lesson
source_spec: "docs/L1_基础/T2.15_fading_channel_LLR_reliability.md"
---
# T2.15 衰落信道与软信息可靠度：均衡输出幅度与可信度差异
## 本节学习目标

[[T2.9_AWGN_noise_scaling|T2.9]] 讲了加性白高斯噪声：发送值上直接加噪声。[[T2.13_BPSK_QPSK_soft_demapping|T2.13]] 和 [[T2.14_QAM_Max_Log_MAP_demapping|T2.14]] 讲了软解调：把接收样本变成 LLR（对数似然比，Log-Likelihood Ratio）。这些内容先假设信号没有被无线传播路径额外“放大、衰减或旋转”。
...
| 问题 | 需要达到的程度 |
|:---|:---|
| BPSK（二进制相移键控，Binary Phase Shift Keying）映射是什么？ | 本课程教学映射为 `0 -> +1`，`1 -> -1`。 |
...
| TS 38.215 | `3GPP_Rel19/processed/TS_38.215_38215-j20` | §5.1.1/§5.1.2 RSRP；§5.1.5/§5.1.6 SINR；§5.1.15-§5.1.17 E-UTRA 测量入口 | 说明协议定义了参考信号接收功率、质量和信干噪比等测量名词和入口。 |
...
本节没有使用 TS 38.214/38.215 中任何具体表格、测量公式或参数值。DM-RS（解调参考信号，Demodulation Reference Signal）、CSI-RS（信道状态信息参考信号，Channel State Information Reference Signal）、RSRP（参考信号接收功率，Reference Signal Received Power）和 SINR（信干噪比，Signal-to-Interference-plus-Noise Ratio）只作为“接收端为什么有机会估计信道与可靠度”的协议入口；本节的 LLR 推导完全基于教学单抽头模型。
...
- $x$ 是发送符号。
- $h$ 是信道系数。
- $n$ 是 AWGN（加性白高斯噪声，Additive White Gaussian Noise）噪声。
- $y$ 是接收符号。
...
[[T2.9_AWGN_noise_scaling|T2.9]] 已经推导过，BPSK 在 AWGN 下的 LLR 为：
...
真实 OFDM（正交频分复用，Orthogonal Frequency Division Multiplexing）接收中，$h$ 通常是复数。常见零迫均衡可以写成：
...
| 用例 | 输入 | 输出 | 失败案例 | 验证方式 |
|:---|:---|:---|:---|:---|
| PDSCH（物理下行共享信道，Physical Downlink Shared Channel）均衡后软解调 | 均衡后符号、信道估计、噪声/干扰估计、调制阶数 | 逐比特 LLR | 弱子载波被朴素公式给出过大 LLR | 单资源元素黄金参考测试（golden test）、频域衰落场景块错误率对比。 |
| 高速移动信道 | 时变 $h$、DM-RS 位置、插值后的信道估计 | 随时间变化的 LLR 可靠度 | 使用过期信道估计导致 LLR 方向或幅度错误 | Doppler 扫描、导频插值误差注入、循环冗余校验失败率统计。 |

## 定点和硬件映射预览

这里的寄存器传输级和专用集成电路只是硬件实现层级预览：RTL（寄存器传输级，Register Transfer Level）描述时钟周期内寄存器和组合逻辑如何传递数据，ASIC（专用集成电路，Application-Specific Integrated Circuit）则是把这些逻辑做成专用芯片。
...
| 把 RSRP/SINR 测量当成逐 RE（资源元素，Resource Element）LLR 公式 | 测量定义不是软解调实现公式。 | 协议测量和接收机算法分开写。 |
...
| 示例验证 | 使用 Python 3 执行本节单抽头 BPSK 衰落 LLR 片段，验收输出为 `T2.5_PYTHON_EXAMPLE_OK`。 |
