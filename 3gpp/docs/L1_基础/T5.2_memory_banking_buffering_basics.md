...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1_基础/T5.2_memory_banking_buffering_basics.md"
---
...
- 解释 SRAM（静态随机存取存储器，Static Random-Access Memory）、寄存器文件、缓存、bank 和端口的区别。
- 说明为什么并行译码器需要把数据分散到多个 bank。
- 用取模映射手算某组地址是否发生 bank 冲突。
- 解释乒乓缓存如何让“写入下一块”和“处理当前块”重叠。
- 说明 LTE Turbo（Turbo 码，Turbo Code）、NR 低密度奇偶校验码和 NR Polar（极化码，Polar Code）译码为什么都绕不开存储组织。
...
| 前置项 | 本节需要达到的程度 |
|:---|:---|
| LLR（对数似然比，Log-Likelihood Ratio） | 知道它是译码器输入软信息。 |
| 定点数 | 知道一个 LLR 在硬件中通常保存为固定位宽整数。 |
| 数组地址 | 能理解 `buffer[7]` 表示访问第 7 个元素。 |
| 时钟周期 | 能理解硬件在每个周期做有限次数读写。 |
| 混合自动重传请求 | 知道 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）会保留软信息等待重传合并。 |
...
| NR TS 38.212 Rel-19 `38212-j30` §5.4.1 | `3GPP_Rel19/processed/TS_38.212_38212-j30/content.md` |
...
| SRAM | 面积效率更高的片上存储器。 | HARQ soft buffer、LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）消息、Turbo 交织输入输出。 |
...
这些仿真不是为了得到块错误率，而是为了在 RTL（寄存器传输级，Register Transfer Level）之前发现地址计划问题。
...
| 把协议 `<NULL>` 或 filler 当普通缓存数据 | CRC（循环冗余校验，Cyclic Redundancy Check）或解交织错误。 | 区分协议占位、未发送位置和中性软信息。 |
...
| TS 38.212 §5.4.2 | 已定位 | NR LDPC rate matching/RV（冗余版本，Redundancy Version）背景会影响 soft buffer 位置。 |
| TS 38.212 §5.4.1 | 已定位 | NR Polar rate matching/rate recovery 背景会影响控制信道译码缓存。 |
| SRAM bank 数、端口数 | 不适用 | 属于 RTL/ASIC（专用集成电路，Application-Specific Integrated Circuit）实现选择，不是 3GPP 强制字段。 |
## 执行与证据记录
