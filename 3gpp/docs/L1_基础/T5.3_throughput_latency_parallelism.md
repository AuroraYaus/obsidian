...
- 分别说明 Turbo、低密度奇偶校验码和Polar的吞吐瓶颈直觉。
...
| HARQ 相关流程 | TS 36.213/38.214/36.321/38.321 后续专章核验 |
...
如果一个 CB 译码需要 $C_{\mathrm{blk}}$ 个周期，时钟频率为 $f_{\mathrm{clk}}$，则延迟为：
...
但实际链路中，CRC 早停可能让很多块提前结束。若第 $i$ 次迭代停止的概率为 $p_i$，平均迭代次数为：
...
| LLR 位宽 | 每 word 可放 LLR 数 | 对并行度的影响 |
|---:|---:|:---|
| 6 bit | 8 | 可自然支持 8 路读取。 |
| 8 bit | 6 | 同样 word 宽下 8 路读取需要更多 bank 或更宽 word。 |
| 10 bit | 4 | 存储带宽压力明显增加。 |

因此，提升定点精度可能改善 BLER，但也可能降低可实现吞吐。架构评估必须把性能曲线和面积/带宽放在同一张表里比较。
...
| 解速率匹配 | 每周期 LLR 放置数 | RV 地址生成不产生不可接受 stall。 |
...
| 压力测试 | 连续 TB/CB 输入 | FIFO 不溢出，输出不乱序。 |
...
| 提高位宽不重算带宽 | SRAM 和互连成为瓶颈。 | 位宽变更同步更新容量和端口需求。 |
