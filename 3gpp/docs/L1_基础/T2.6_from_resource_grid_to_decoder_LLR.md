...
T2.1–T2.5 分别构建了 OFDM 原理、NR 时频结构、频域资源网格、MCS 参数和 LTE 帧结构。本节把这些零件组装成一条完整的端到端数据流：从 RE 网格上的复数调制符号，到译码器核心消费的一维 LLR 向量。
...
| 阶段 | 操作 | 详述章节 |
|:---|:---|:---|
| 信道估计与均衡 | DM-RS/CSI-RS 辅助，单抽头均衡 | T2.14、T2.15 |
| 软解调 | 每 RE 复数符号 → $Q_m$ 个逐比特 LLR | T2.8（BPSK/QPSK）, T2.9（QAM） |
...
| 协议依据 | 本节采用程度 |
|:---|:---|
| TS 38.211 §5 / TS 36.211 §7 | 调制映射入口；软解调公式见 T2.7–T2.9 |
...
| 资料 | 边界 |
| :--- | :--- |
| TS 38.211 §5 / TS 36.211 §7 | 调制映射入口；软解调见 T2.7–T2.9 |
