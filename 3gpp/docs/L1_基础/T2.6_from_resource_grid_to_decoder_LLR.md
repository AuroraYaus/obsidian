...
T2.1–T2.5 分别构建了 OFDM 原理、NR 时频结构、频域资源网格、LTE 帧结构和 MCS 参数。本节把这些零件组装成一条完整的端到端数据流：从 RE 网格上的复数调制符号，到译码器核心消费的一维 LLR 向量。
...
| 问题 | 需要达到的程度 |
|:---|:---|
| RE 网格是什么？ | 时频二维格点，每 RE 承载一个调制符号（T2.3）。 |
| $Q_m$ 是什么？ | 每调制符号承载 $Q_m$ 个比特（T2.5）。 |
| LLR 是什么？ | 对数似然比，正值→更像 0，幅度→可靠度（T1.5）。 |
...
| 阶段 | 操作 | 详述章节 |
|:---|:---|:---|
| 信道估计与均衡 | DM-RS/CSI-RS 辅助，单抽头均衡 | T2.10、T2.11 |
| 软解调 | 每 RE 复数符号 → $Q_m$ 个逐比特 LLR | T2.13（BPSK/QPSK）, T2.14（QAM） |
| 速率恢复 | LLR 反填 circular buffer，puncturing 位填 0 | T7（LTE Turbo）, T9（NR LDPC） |
| 码块切分 | 码字级 LLR → 按 CB 边界切为 $C$ 个向量 | T3.2–T3.5 |
...
这些在上游已被剥离。Descriptor（T4.6/T9.0）保存调度参数（$K_r$、$E_r$、RV、HARQ process），不保存 RE→LLR 的映射历史。
...
| 协议依据 | 本节采用程度 |
|:---|:---|
| TS 38.211 §5 / TS 36.211 §7 | 调制映射入口；软解调公式见 T2.9、T2.13–T2.14 |
| TS 38.212 §5 / TS 36.212 §5 | 码块分段入口；完整算法见 T3.2–T3.5 |
| TS 38.214 §5.1.3.2 | $N_{RE}$ 完整定义（含 $N_{oh}^{PRB}$）见 T9.0 |
...
| 资料 | 边界 |
| :--- | :--- |
| TS 38.211 §5 / TS 36.211 §7 | 调制映射入口；软解调见 T2.9、T2.13–T2.14 |
| TS 38.214 §5.1.3.2 | $N_{RE}$ 完整定义见 T9.0 |
