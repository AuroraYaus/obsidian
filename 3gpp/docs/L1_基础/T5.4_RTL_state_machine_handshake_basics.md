...
- 说明协议字段如何进入 descriptor，而不是直接散落在 RTL 状态机里。
...
|:---|:---|
| LLR | 知道译码器输入是一串软信息。 |
| CRC | 知道 CRC pass/fail 可作为译码结果验收。 |
...
3. 不一定。如果允许继续迭代或等待 HARQ 重传，可能回到 DECODE 或保留 soft buffer；无 retry 或出现不可恢复错误时进入 ERROR。
...
| TB/CB/RV/HARQ descriptor | 下游协议章节提供来源 | 本节说明 descriptor 如何驱动状态机。 |
## 执行与证据记录
