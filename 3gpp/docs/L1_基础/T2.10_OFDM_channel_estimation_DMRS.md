...
回答的问题：插值什么时候会"跟不上"信道？用奈奎斯特思想给定量判据——导频采样间隔必须小于信道变化尺度的一半。相干带宽 $B_c$ 与相干时间 $T_c$ 的物理定义与测量口径见 [[Coherence_Bandwidth_Time_相干带宽与时间]]。
...
| 序列与初始化 | $c_{init}$ 各输入（$N_{ID}$、$n_{SCID}$、$\lambda$、$l$、$n_{s,f}^{\mu}$） | 某端口 residual 系统性偏大。 |
