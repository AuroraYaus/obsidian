...
| `max_iterations` | `uint8` | 是 | 大于 0；Polar SC（逐次消除，Successive Cancellation）可固定为 1 | 控制迭代、列表或树遍历预算。 |
...
| `ERR_SOFT_BUFFER_MISS` | 重传时找不到 HARQ soft buffer key | 触发 NACK 或上层恢复策略 | HARQ process ID、NDI（新数据指示，New Data Indicator）或 RV 状态错。 |
