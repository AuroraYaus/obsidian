...
发送链每一环都携带参数，接收链的对应逆环节必须使用同一组参数。下表按 11 环逐项核对"参数由谁定、接收端如何获知、不一致时会发生什么"——这是互操作测试与仿真验收的核对口径。

| 发送端参数 | 参数确定方式 | 接收端获知途径 | 不一致的后果 |
|:---|:---|:---|:---|
| CRC 多项式与长度 | 协议按信道固定 | 无需信令（双方查同一规定） | 校验恒失败，或多项式错配时错误数据被当作正确（漏检率失控） |
| 分段规则（码块大小/填充） | 由传输块大小按协议公式推得 | 由传输块大小反推同一分段（T3.2/T3.5 的分段规则） | LLR 分组边界错位，全部码块译码失败 |
| 编码类型与码率 | 类型随信道固定，码率随调度配置 | 类型按信道得知；码率由调度指示携带 | 译码器选错结构直接失败（T6-T10） |
| 速率匹配图样（打孔/重复位置） | 按协议规则随传输参数生成 | 由同一规则做速率恢复（T7/T9） | 打孔位置漏置零、重复位置漏累加——LLR 错位 |
| 加扰序列与种子 | 用户身份与小区 ID 决定种子（TX2 讲组成） | 归属判定获得用户身份、小区搜索获得小区 ID | 解扰输出全错，等效于满强度干扰 |
| 调制阶数与星座 | 随调度配置（TX1 的星座公式） | 调度指示携带阶数，星座按同一公式（T2.13/T2.14） | 距离度量用错星座，LLR 幅度与符号系统性偏差 |
| 层数/端口与预编码矩阵 W | 层数随调度配置，W 按码本选择（TX3） | 检测只需等效信道（参考信号观测，T12） | 层数错则流无法分离；W 错则等效信道失配 |
| RE 映射范围与虚拟-物理资源块映射类型 | 调度指示（TX4 的映射规则） | 按同一指示提取资源格（T2.x） | 提取位置错位，符号块整体置换 |
| 波形参数（子载波间隔、CP 长度、IFFT 尺寸） | 高层配置与系统参数 | 同步阶段检测符号节奏（T2.7/T2.8） | 定时/频域错位，所有子载波污染 |

核对表的使用方式：仿真与硬件联调验收时按行比对收发双方日志——发送端每环的参数快照与接收端每环的解码假设逐项相等，是"零误码"之外更细的验收证据。任何一行不一致，链路的失败都表现为块级全错而非 SNR（信噪比，Signal-to-Noise Ratio）边缘的少量错误——这正是发送链各环"协议强制"的原因：把参数错配的可能收敛为可逐项核对的条款。
...
本节的协议锚点只有两处：TS 38.212 §5（传输信道处理五环）与 TS 38.211 §5-§7（物理信道处理六环）。11 环的顺序是协议规定的流水线；正文的端到端仿真与重复码演示是教学规模的实现示例，不改变协议口径。

| 维度 | 内容 |
|:---|:---|
| 协议强制 | 发送链的环节顺序：CRC 附着 → 分段 → 编码 → 速率匹配 → 级联（38.212 §5）→ 加扰 → 调制 → 层映射 → 预编码 → RE 映射 → 波形生成（38.211 §5-§7）。 |
| 协议不规定（实现域） | 各环节的算法选择与实现结构（编码器/交织器/映射器的硬件或软件形态）；量化位宽与并行度；发射机的射频与功放实现。 |
| 接收端镜像约束 | 接收链是发送链的逆流水线，但"逆序不等价"：精确逆（解扰/逆层映射）、概率逆（软解调/译码）、恢复逆（速率恢复/检测）、校验逆（CRC）四类各有特殊语义（T0.1 翻译表）。 |

一句话边界：**协议规定"按什么顺序走完 11 环"，不规定"每一环怎么造"**；接收端逐环执行对应逆操作，逆的性质决定了接收端每环的复杂度与软信息要求。
