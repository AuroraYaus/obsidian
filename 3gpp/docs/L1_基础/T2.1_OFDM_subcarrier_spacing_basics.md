...
[[T2.0_OFDM_system_overview|T2.0]] 已经建立了 OFDM（正交频分复用，Orthogonal Frequency Division Multiplexing）收发机全景：bit 经调制、资源映射、IFFT（逆快速傅里叶变换，Inverse Fast Fourier Transform）、CP（循环前缀，Cyclic Prefix）和无线信道到达接收端，再经过同步、FFT（快速傅里叶变换，Fast Fourier Transform）、信道估计、均衡和软解调变成 LLR（对数似然比，Log-Likelihood Ratio）。本节从那张系统图里抽出三个最核心的机制，回答 OFDM 为什么能成立：子载波为什么能重叠而不互相干扰、CP 为什么能保护 FFT 窗口、IFFT/FFT 为什么能把频域资源网格变成时域波形再还原回来。
...
| 调制符号 | modulation symbol | 星座图上的一个复数点（BPSK（二进制相移键控，Binary Phase Shift Keying）$\pm 1$、QPSK（正交相移键控，Quadrature Phase Shift Keying）$\pm 1 \pm j$），承载 $Q_m$（调制阶数，Modulation order）个比特。 |
...
这正是 IDFT（逆离散傅里叶变换，Inverse Discrete Fourier Transform）的定义。通常加一个 $1/N$ 归一化因子：
...
接收端去除 CP 后的 N 个采样点做 DFT（离散傅里叶变换，Discrete Fourier Transform）：
...
| 调制符号 | 频域 | 星座图上的一个复数点（BPSK=$\pm 1$，QPSK=$\pm 1 \pm j$，16QAM（16 阶正交幅度调制，16 Quadrature Amplitude Modulation）=16 格点之一） | 承载 $Q_m$ 个比特 |

两者的关系是：在一个 OFDM 符号周期内，N 个子载波各承载一个调制符号——N 个调制符号经 IFFT 并行变换为一个 OFDM 符号。在后续 [[T2.6_from_resource_grid_to_decoder_LLR|T2.6]]（端到端数据流）和 [[T3.2_transport_code_block_filler_bits|T3.2]]（码块分段）中，"调制符号"指的是 constellation symbol 放到 RE（资源元素，Resource Element）上，不是 OFDM symbol 的传输。
...
OFDM 不是 3GPP 发明的——它的理论基础（子载波正交性、CP、IFFT/FFT 实现）远早于蜂窝通信。3GPP 做的事情是选择具体的参数：NR 的 N（IFFT 大小）、$\Delta f$（SCS（子载波间隔，Subcarrier Spacing）取值）、CP 长度和 CP 类型（普通/扩展），以及物理信道到资源网格的映射规则。
...
| 参考书 | 《5G移动通信系统设计与标准详解》§4.2 (P73–78) | `references/` | 参考 CP-OFDM（循环前缀正交频分复用，Cyclic Prefix OFDM）选为 NR 下行波形的技术理由；本节不逐项复现其参数表。 |

这里没有逐项核验原始 Word、表格 HTML 或公式 XML，因此本节不宣称已经完成协议 OFDM 参数的精读。

## 示范例题

### 例题 1：三种子载波间隔的符号时长与采样率换算

NR 可配置的子载波间隔（SCS，Subcarrier Spacing）取 15 kHz、30 kHz、60 kHz 三种（对应 μ=0、1、2，见 [[T2.2_NR_numerology_time_domain_hierarchy|T2.2]]）。本题用正交条件 $T_u = 1/\Delta f$ 与 IFFT 采样关系 $f_s = N \cdot \Delta f$ 做手算。

(1) 分别求三种 SCS 下的有用符号时长 $T_u$。
(2) 取 FFT 点数 $N = 2048$（LTE 20 MHz 配置的典型取值），求基带采样率 $f_s$ 与采样周期 $T_s$。
(3) 验证 $T_u = N \cdot T_s$ 是否成立，并解释其含义。

### 例题 2：循环前缀长度的选择与开销核算

某小区测得最大多径时延扩展 $\tau_{max} = 4.7$ μs，系统采用 $\Delta f = 15$ kHz 传输。

(1) 循环前缀（CP，Cyclic Prefix）长度 $T_{CP}$ 至少要取多少，才能消除符号间干扰？
(2) 取 $T_{CP} = 4.7$ μs（LTE 普通 CP 的量级），求完整符号时长 $T_{symbol}$ 与 CP 开销比例。
(3) 若该小区改用 $\Delta f = 30$ kHz 而 $T_{CP}$ 不变，开销比例变为多少？这说明什么折中？

### 参考答案

**例题 1**。(1) 由 $\Delta f = 1/T_u$ 反解：15 kHz 下 $T_u = 1/15000 \approx 66.67\ \mu s$；30 kHz 下 $T_u = 1/30000 \approx 33.33\ \mu s$；60 kHz 下 $T_u = 1/60000 \approx 16.67\ \mu s$。SCS 每翻一倍，$T_u$ 恰好减半——正交条件要求子载波间隔与符号时长严格互为倒数。

(2) $f_s = N \cdot \Delta f = 2048 \times 15000 = 30.72 \times 10^6$ Hz = 30.72 Msps（兆采样每秒，Mega samples per second）；$T_s = 1/f_s \approx 32.55$ ns。

(3) $N \cdot T_s = 2048 \times 32.55\ \mathrm{ns} \approx 66.66\ \mu s$，与 15 kHz 下的 $T_u$ 一致，成立。含义：一个有用符号在时域由 N 个采样点组成——IFFT 把频域的 N 个调制符号变成时域的 N 个采样，采样率取 $N \cdot \Delta f$ 正是为了让"一个符号 = N 个采样"成立。

**例题 2**。(1) CP 消除符号间干扰的条件是 $\tau_{max} \leq T_{CP}$（见"循环前缀"一节），因此 $T_{CP} \geq 4.7$ μs。

(2) $T_u = 66.67$ μs，$T_{symbol} = T_u + T_{CP} = 66.67 + 4.7 = 71.37\ \mu s$。开销比例为 $T_{CP}/(T_u + T_{CP}) = 4.7/71.37 \approx 6.6\%$——每传输 100 个符号的时长，约有 6.6 个符号的时长花在 CP 上。

(3) 30 kHz 下 $T_u = 33.33$ μs，$T_{symbol} = 33.33 + 4.7 = 38.03\ \mu s$，开销比例 $= 4.7/38.03 \approx 12.4\%$。同样长度的 CP 在大 SCS 下开销占比几乎翻倍：CP 要对抗的是多径时延的绝对时长（与 SCS 无关），而 $T_u$ 随 SCS 增大而缩短。折中结论：CP 长度必须按场景重新选择，这也是不同 μ 的 CP 采样点数不同的原因（[[T2.2_NR_numerology_time_domain_hierarchy|T2.2]] 展开）。

## 引导练习（带提示）

### 练习 1：由传输带宽反推子载波数与 FFT 点数

LTE 20 MHz 信道，$\Delta f = 15$ kHz，有效传输带宽 $B_{tx} = 18$ MHz（20 MHz 中两侧各留约 1 MHz 保护带，见"三层带宽"一节）。

提示：
- 传输带宽与子载波数的关系为 $B_{tx} = N_{sc} \cdot \Delta f$，先反解 $N_{sc}$。
- FFT 点数 N 需大于 $N_{sc}$ 且取 2 的整数次幂（IFFT/FFT 快速算法的工程要求）。
- 采样率仍为 $f_s = N \cdot \Delta f$。

请完成：(1) 求子载波数 $N_{sc}$。(2) 若取 $N = 2048$，未被数据占用的子载波有多少个？(3) 采样率 $f_s$ 是多少？

### 参考答案

(1) $N_{sc} = B_{tx}/\Delta f = 18 \times 10^6 / 15000 = 1200$ 个子载波。

(2) $2048 - 1200 = 848$ 个。这些"空子载波"不承载数据，用于直流（Direct Current, DC）保护——基带中心频率 $f = 0$ 处不放置调制符号——以及频谱整形与相邻信道隔离。

(3) $f_s = 2048 \times 15000 = 30.72$ MHz，即 30.72 Msps。
...
| [[T2.0_OFDM_system_overview|T2.0]] OFDM 系统全景 | OFDM 发射/接收完整链路和同步、CFO（载波频率偏移，Carrier Frequency Offset）/SFO（采样频率偏移，Sampling Frequency Offset）、信道估计、PAPR（峰均功率比，Peak-to-Average Power Ratio）、窗口函数、MIMO-OFDM（多输入多输出正交频分复用，Multiple-Input Multiple-Output OFDM）的工程问题地图见 [[T2.0_OFDM_system_overview|T2.0]]。 |
...
| [[T2.9_AWGN_noise_scaling|T2.9]] AWGN（加性白高斯噪声，Additive White Gaussian Noise）噪声缩放 | [[T2.9_AWGN_noise_scaling|T2.9]] 中噪声独立加在每个子载波上的前提建立于子载波正交性——若正交性不成立，子载波间干扰会产生与 AWGN 不同的噪声结构。 |
