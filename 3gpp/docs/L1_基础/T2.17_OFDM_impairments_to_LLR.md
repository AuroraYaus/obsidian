...
![图 3：同一 16QAM 星座在四种失真下的退化 — ①干净星座（青色，协议 16 点）；②相位噪声/残余 CFO 整体旋转（橙色）；③PA 非线性/削峰使外层点压缩内收（品红）；④噪声与干扰使点散布成团簇（蓝灰）。判决边界 0、±2a（虚线）与每个面板的失真点位置对照，可直观看到"边界裕量"如何被各类失真消耗](assets/T2.17_16qam_impairment_degradation.svg)
