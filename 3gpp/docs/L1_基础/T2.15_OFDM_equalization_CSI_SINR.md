...
tags:
  ...
  - sinr
  - equalization
source_spec: "docs/L1_基础/T2.15_OFDM_equalization_CSI_SINR.md"
---
...
![图 1：ZF 与 MMSE 均衡对比——上图为各子载波的信道幅度 |H| 与 ZF 均衡增益 1/|H|（深衰落处噪声功率放大 25 倍，MMSE 增益被 σ² 压到 2.5）；下图为均衡前后 QPSK 星座，展示噪声增强与 MMSE 有偏收缩](assets/T2.15_zf_mmse_equalizer_compare.svg)
...
![图 2：频选信道下均衡前后 QPSK 星座——子载波 A（平坦 |H|=1）均衡后干净恢复；子载波 B（深衰落 |H|=0.2）ZF 均衡后噪声方差 ×25、簇严重重叠；子载波 C（同起点）MMSE 均衡后有偏收缩但总 MSE 更低](assets/T2.15_freq_selective_constellation_equalize.svg)
...
**接收端必须做什么**：UE 拿到高层配置后，知道“哪些 RE 上是 CSI-RS、用什么序列、几个端口”，然后在这组 RE 上做三件事：① NZP CSI-RS 上做信道估计（方法就是 T2.14 的 LS/MMSE，属于实现自由度）；② ZP CSI-RS/CSI-IM 上测干扰与噪声；③ 按 TS 38.215 的定义算功率/SINR。**边界**：CSI-RS 的序列、位置、端口、功率偏移是协议强制；在这些 RE 上用何种估计器、如何算 SINR 是接收机实现自由度。
...
![图 3：CSI 测量→报告→调度闭环——CSI-RS 发送（TS 38.211 §7.4.1.5）→ UE 信道/干扰测量（CSI-IM）→ SINR 计算与 CQI 量化（TS 38.214 §5.2.2.1）→ PUCCH/PUSCH 上报 → gNB 调度器选 MCS → 下一次 PDSCH；底部标出协议强制与接收机实现自由度的边界](assets/T2.15_csi_measurement_report_loop.svg)
...
| 图解材料 | 本节新增三张手绘 SVG：`assets/T2.15_zf_mmse_equalizer_compare.svg`（图 1）、`assets/T2.15_freq_selective_constellation_equalize.svg`（图 2）、`assets/T2.15_csi_measurement_report_loop.svg`（图 3）；原 Mermaid 均衡链路图保留。 |
