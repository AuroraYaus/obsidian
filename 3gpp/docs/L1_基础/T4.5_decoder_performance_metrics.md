...
$E_b/N_0$ 表示每信息比特能量与噪声功率谱密度之比。T2.9 已经给出 AWGN 噪声缩放方法。本节只强调绘图口径：
...
| 项目 | 推荐设置 |
|:---|:---|
| 信道 | AWGN，噪声缩放沿用 T2.9 的 $E_b/N_0$ 到噪声方差公式。 |
...
| 引用重建审计 | `python3 tools/audit_reference_rebuilds.py docs/L1_基础/T4.5_decoder_performance_metrics.md` 返回 `REFERENCE_REBUILD_AUDIT_CANDIDATES`，退出码为 0；候选为本节自写工程统计公式、T2.9 噪声缩放引用说明和执行记录中的“公式”字样，未发现本节新增外部协议表/图/公式缺失复现项。 |
