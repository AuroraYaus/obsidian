...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1_基础/T3.5_NR_Polar_segmentation_crc.md"
---
# T3.5 NR Polar 分段与 CRC 附加：控制信息的短块保护与路径选择
## 本节学习目标

本节第一次把 **Polar**（极化码，Polar Code）作为主角。Polar 不是一个普通“加校验位”的小技巧，而是一类信道编码方法：它把若干个原本相同的二元输入信道组合起来，再通过递归变换产生一组“合成信道”。随着码长增加，有些合成信道会变得越来越可靠，有些会变得越来越不可靠。发送端把真正的信息放到可靠位置，把固定值放到不可靠位置；接收端再利用这个结构恢复信息。
...
- 协议线：TS 38.212 Rel-19 如何规定 NR 控制信息的 Polar 分段、循环冗余校验附加、下行控制信息的 24 bit CRC（循环冗余校验，Cyclic Redundancy Check）和无线网络临时标识（Radio Network Temporary Identifier, RNTI）加扰。
...
| 对数似然比 | 知道 LLR（对数似然比，Log-Likelihood Ratio）是接收端传给译码器的软信息。 |
| 传输块/码块 | 知道 TB（传输块，Transport Block）和 CB（码块，Code Block）是数据链路里的分层对象。 |
| HARQ | 知道 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）会根据 ACK/NACK 和 CRC 结果决定是否重传或合并。 |
...
本节会复现本节实际使用的协议表格子集和教学公式。由于 `content.md` 对 §5.2.1 的分段公式和若干符号抽取不完整，涉及完整 UCI（上行控制信息，Uplink Control Information）分段条件、所有 $E$/$K$/$C$ 判定细节时，会明确标注边界，不把未逐符号核验的内容写成 bit-exact 结论。
...
|:---|:---|
| DCI（下行控制信息，Downlink Control Information） | Polar code |
| UCI | Block code |
| UCI | Polar code |
...
|:---|:---|:---|:---|
| 上行共享信道/下行共享信道数据 | 用户数据、媒体接入控制层PDU | LDPC（低密度奇偶校验码，Low-Density Parity-Check Code） | 大块译码失败导致数据不可用，可通过 HARQ 重传或软合并补救。 |
| UCI | HARQ-ACK、调度请求、信道状态信息 | block code 或 Polar | ACK/NACK 错误会改变重传行为；CSI（信道状态信息，Channel State Information）错误会影响调度和链路自适应。 |
...
式 (14) 回答的是“payload 加 CRC 以后有多少个待保护比特”。这里的 $K$ 是 Polar 语境下的信息集合大小，不是 [[T3.4_NR_LDPC_segmentation_rules|T3.4]] NR LDPC 中的基图扩展后码块长度。工程实现里必须把每个 descriptor 的 $A$、$L_{\mathrm{UCI}}$、$K$、rate matching output length $E$ 和 code block count $C$ 一起记录，否则接收端无法正确做速率恢复和 CRC 检查。
...
|:---|:---|:---|:---|:---|:---|
| PDCCH DCI 盲检 | PDCCH candidate LLR、candidate RNTI、DCI payload size、list size | DCI payload、CRC 状态、RNTI 命中状态 | 多个候选 RNTI、多种 DCI size alignment、低 SNR（信噪比，Signal-to-Noise Ratio）下路径度量接近 | 错误 RNTI 也通过 CRC，导致 false alarm；正确 RNTI CRC fail，导致 missed detection | 枚举 candidate/RNTI，统计 false alarm、missed detection 和 CRC pass path metric 分布。 |
...
| 对象 | 定点关注点 | 工程建议 |
|:---|:---|:---|
| 输入 LLR | 符号约定、位宽、饱和、缩放 | 复用 [[T2.16_LLR_clipping_scaling_quantization|T2.16]] 的 LLR 量化规则，单独统计控制信道饱和率。 |
...
一个常见硬件错误是：SCL 排序器只保留路径度量和 payload，不保留 CRC 相关状态，最后只能对一条路径做 CRC。这会把 CA-SCL（CRC 辅助 SCL，CRC-aided SCL）退化成“best metric then CRC”，性能和错误行为都不对。
