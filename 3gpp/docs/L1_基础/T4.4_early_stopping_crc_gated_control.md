...
本节第一次出现的技术缩写先统一展开：循环冗余校验是错误检测规则；对数似然比是译码器输入的软信息；传输块是物理层一次处理的主要数据单元；码块是 TB（传输块，Transport Block）分段后的译码子块；混合自动重传请求决定译码失败后的重传和软合并；Turbo（Turbo 码，Turbo Code）、低密度奇偶校验码和Polar（极化码，Polar Code）是本课程关注的三类编码；寄存器传输级和专用集成电路是硬件落地层级。


- 解释奇偶校验、syndrome 和 CRC（循环冗余校验，Cyclic Redundancy Check）如何给译码器提供停止线索。
- 区分算法停止策略（algorithmic stopping）、CRC 门控停止、校验综合（syndrome）停止和 CRC 辅助路径选择。
- 对比 Turbo、LDPC（低密度奇偶校验码，Low-Density Parity-Check Code）和 Polar 三类译码器的停止对象和风险。
- 用一个可手算例子判断 syndrome 是否为零，并说明为什么仍要用 CRC 做最终验收。
- 说明 CRC 误通过风险为什么存在，以及工程上怎样降低它的影响。
- 把停止控制映射到浮点仿真、定点模型、RTL（寄存器传输级，Register Transfer Level）/ASIC（专用集成电路，Application-Specific Integrated Circuit）状态机和验证覆盖点。
...
| LLR（对数似然比，Log-Likelihood Ratio） | 知道正负号表示更像 `0` 或 `1`，绝对值表示置信度。 |
...
早停控制必须先分清协议锚点和实现策略。3GPP 的 TS 36.212/38.212 规定的是编码、分段、CRC、速率匹配和连接关系；TS 38.214 给出调度、调制编码和 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）相关的过程入口。它们不规定厂商的 Turbo 最大迭代次数、LDPC syndrome 检查周期、Polar 列表大小或 CRC 早停门限。
...
协议前因后果可以概括为三层。第一，发送端附加 TB/CB（码块，Code Block）/控制信息 CRC，并按 Turbo、LDPC 或 Polar 结构编码。第二，接收端按相同结构译码，得到候选比特。第三，接收端用 CRC、syndrome 或路径 CRC 结果决定“继续算、提前停、输出通过、输出失败、保留 HARQ 软缓存”。其中第三层的控制动作是实现设计，但它必须受第一层协议对象边界约束。
...
LTE 的协议锚点是 TS 36.212 §5.1.2 的 CB 分段和 CB CRC、§5.1.3.2 的 Turbo 编码结构，以及 §5.2.2/§5.3.2 中 UL-SCH（上行共享信道，Uplink Shared Channel）/DL-SCH（下行共享信道，Downlink Shared Channel）处理链路。接收端必须按这些边界恢复每个 CB，再检查相应 CRC。至于“第 2 轮以后才允许查 CRC”“每一整轮查一次还是每半轮查一次”“CB CRC 通过后是否停止该 CB”，都属于实现策略。
...
NR 的协议锚点包括 TS 38.212 §5.2.1 的 Polar code block segmentation、§5.3.1 的 Polar encoding、§7.3.2 的 DCI（下行控制信息，Downlink Control Information）CRC attachment，以及 §8.3.2/§8.4.2 的 UCI（上行控制信息，Uplink Control Information）CRC attachment 入口。控制信息常见特点是 payload 较短、时延预算紧、候选检测敏感；CRC 在这里同时承担错误检测和路径筛选帮助。列表大小、路径度量公式、何时做 CRC 检查和多路径并列处理，仍是接收端算法实现策略。
...
再考虑 Polar DCI 候选检测。接收端列表中有 8 条路径，其中错误路径 `P3` 的路径度量最好并且 CRC 通过，正确路径因为早期剪枝已经不在列表中。接收端选择 `P3`，得到一个看似有效的 DCI。后果不是“一个数据 bit 错了”这么简单，而是后续 PDSCH（物理下行共享信道，Physical Downlink Shared Channel）资源、MCS（调制与编码方案，Modulation and Coding Scheme）、RV（冗余版本，Redundancy Version）或 HARQ process 解释都可能错。工程上必须通过候选搜索空间、CRC 掩码、路径度量、列表大小和误检率仿真一起控制风险。
...
| LDPC 停止策略对比 | syndrome-only、CRC-only、syndrome+CRC 三组配置 | syndrome/CRC trace、BLER（块错误率，Block Error Rate）、平均层数 | syndrome+CRC 的不一致案例可被日志定位。 |
...
Pipeline flush 也必须定义清楚。早停 pass 发生时，Turbo/LDPC 后续已经发出的下一轮读请求、消息更新或 CRC 请求可能还在 pipeline 中；Polar SCL（连续消除列表译码，Successive Cancellation List）的路径扩展、排序和 CRC 检查也可能有未提交结果。正确策略不是简单拉低 `enable`，而是：
...
| [[T4.1_iterative_decoding_extrinsic_information|T4.1]] 迭代译码与外信息 | 本节不推导 Turbo MAP（最大后验概率，Maximum A Posteriori）或完整 LDPC BP。 |
...
| TS 38.214 Rel-19 `38214-j30` §5.1、§5.1.3、§6.1.4 | `3GPP_Rel19/processed/TS_38.214_38214-j30/content.md`，`sections.jsonl` | 已定位 | HARQ process、MCS/TBS（传输块大小，Transport Block Size）/RV 是停止结果进入系统控制的 descriptor 背景。 |
## 执行与证据记录
