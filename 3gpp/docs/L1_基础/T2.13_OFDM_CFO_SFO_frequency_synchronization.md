...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1_基础/T2.13_OFDM_CFO_SFO_frequency_synchronization.md"
---
...
把 OFDM 接收机想象成一台需要同时对准“时间格子”和“频率格子”的机器。T2.12 对准了时间格子（符号边界落在哪里）；本节对准频率格子——载波频率差多少、采样节拍差多少。两个通俗类比：
...
![图 3：CFO 破坏子载波正交性 — 理想 sinc 对齐时邻 bin 采样恰为零，频偏 ε 使采样点偏移、泄漏到邻 bin 形成 ICI](assets/T2.13_cfo_subcarrier_leakage.svg)
...
![图 1：CFO 使 QPSK 星座随 OFDM 符号持续旋转 — t0 到 t3 依次旋转 18°，同一符号内所有子载波近似同转（CPE）](assets/T2.13_cfo_constellation_rotation.svg)
...
![图 2：CFO 与 SFO 的相位特征对比 — 左：CFO 的相位与子载波无关、随符号整体抬升；右：SFO 的相位随子载波线性增长、斜率随符号累积](assets/T2.13_cfo_vs_sfo_phase_slope.svg)
...
| 图解材料 | 新增三张手绘 SVG：`assets/T2.13_cfo_constellation_rotation.svg`（星座旋转）、`assets/T2.13_cfo_vs_sfo_phase_slope.svg`（相位特征对比）、`assets/T2.13_cfo_subcarrier_leakage.svg`（频谱泄漏）；保留原 Mermaid 块。 |
...
- 配图：`docs/L1_基础/assets/T2.13_cfo_constellation_rotation.svg`、`docs/L1_基础/assets/T2.13_cfo_vs_sfo_phase_slope.svg`、`docs/L1_基础/assets/T2.13_cfo_subcarrier_leakage.svg`.
