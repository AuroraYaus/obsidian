...
[[T2.10_OFDM_channel_estimation_DMRS|T2.10]] 得到了每个 RE（资源元素，Resource Element）上的信道估计 $\hat{H}[k,l]$。本节讲怎么使用它。均衡（equalization）的目标不是直接输出硬比特，而是把接收符号尽量变回星座点附近，并给软解调器提供可靠度信息。
...
| CSI（信道状态信息，Channel State Information）/SINR（信干噪比，Signal-to-Interference-plus-Noise Ratio）/等效噪声 | 可靠度估计 | soft demapper 缩放 LLR（对数似然比，Log-Likelihood Ratio），译码器感知置信度。 |

- 推导 SISO（单入单出，Single-Input Single-Output）单抽头均衡。
- 对比 ZF（迫零，Zero Forcing）和 MMSE（最小均方误差，Minimum Mean Square Error）的工程差异。
- 说明 MIMO（多输入多输出，Multiple-Input Multiple-Output）线性均衡为什么需要矩阵运算。
- 解释 post-eq SINR/CSI 如何影响 LLR。
- 明确均衡算法通常不是 3GPP 协议强制项。
- 解释 CSI-RS（信道状态信息参考信号，Channel State Information Reference Signal，[[CSI_RS_信道状态信息参考信号]]）在协议中的角色（TS 38.211 §7.4.1.5）与 CSI 报告闭环（TS 38.214 §5.2）。
...
| CSI-RS 与 DM-RS（解调参考信号，Demodulation Reference Signal）有什么区别？ | 知道 CSI-RS 不随 PDSCH（物理下行共享信道，Physical Downlink Shared Channel）调度、位置由高层配置，专用于信道与干扰测量。 |
| CSI 报告是什么？ | 知道 UE 按周期或按需把 CQI（信道质量指示，Channel Quality Indicator）/PMI（预编码矩阵指示，Precoding Matrix Indicator）/RI（秩指示，Rank Indicator）等反馈给基站，驱动链路自适应。 |
...
- **CSI（信道状态信息，Channel State Information）与 SINR（信号与干扰加噪声比，Signal-to-Interference-plus-Noise Ratio）**：可靠度的一种量化。同样的星座距离，在强信道下很可信、在深衰落信道下不可信；CSI/SINR 就是给 soft demapper 的“批改笔”：信道好把 LLR 放大，信道差把 LLR 收缩。它同时是基站做链路自适应的依据——UE 把 CSI 浓缩成 CQI 上报，基站据此选 MCS（调制与编码方案，Modulation and Coding Scheme）。
- **协议边界**：CSI-RS 放哪、序列是什么、报告什么字段——3GPP 协议规定；用 ZF、MMSE、IRC（干扰抑制合并，Interference Rejection Combining）中哪个均衡器、SINR 怎么估、CQI 怎么映射——接收机实现自由度。本节全部公式若未注明“协议强制”，一律按接收机实现理解。

## SISO 单抽头均衡

OFDM（正交频分复用，Orthogonal Frequency Division Multiplexing）把频率选择性信道拆成许多子载波上的窄带问题。单个 RE 上可以写成：
...
假设 BPSK（二进制相移键控，Binary Phase Shift Keying）发送 $X=+1$。两个 RE 的均衡后符号都为：
...
| `noise_var` | slot、PRB（物理资源块，Physical Resource Block）或 RE | soft demapper 距离归一化。 |
| `eq_gain` | RE/layer | MMSE 输出归一化调试。 |
| `condition_metric` | RE/PRB | 判断矩阵求逆和噪声增强风险。 |

这些字段不会全部进入最终产品接口，但在仿真和 bring-up 阶段很有价值。缺少它们时，BLER（块错误率，Block Error Rate）曲线一旦异常，调试者只能猜是信道估计、均衡、soft demapper 还是译码器出了问题。

诊断时建议再记录两个元信息：`csi_source`（标记 `csi` 来自哪种估计：基于参考信号、基于均衡权重解析推导、还是基于 EVM（误差矢量幅度，Error Vector Magnitude）），以及 `cqi_reported`（本周期实际上报的 CQI）。前者让“可靠度不对”能追到估计方法，后者让“上报 CQI 与均衡器内部 SINR 不一致”能被发现——两者的差异超过阈值（例如 3 dB）说明口径或映射有问题，而不是译码器的问题。
...
MIMO-OFDM（多输入多输出正交频分复用，Multiple-Input Multiple-Output OFDM）中，每个 RE 的模型变成：
...
**逐符号解释**：$N_{symb}^{slot}$ 是每个时隙的 OFDM 符号数（常规 CP（循环前缀，Cyclic Prefix）为 14），$n_{s,f}^{\mu}$ 是无线帧内的时隙号，$l$ 是时隙内的符号号，$n_{ID}$ 是高层参数 `scramblingID` 或 `sequenceGenerationConfig`。三个时间量都进初始化公式，意味着**不同符号、不同时隙的 CSI-RS 序列不同**——这是为了平滑掉小区间序列的固定碰撞。
...
**为什么**：gNB 不知道 UE 侧的信道（FDD（频分双工，Frequency Division Duplexing）上下行频率不同，互易性不可用），必须靠 UE 报告。报告内容决定三件事：CQI → MCS（调制+码率）、PMI → 预编码、RI/LI → 层数。没有 CSI 报告，链路自适应就退化成固定 MCS，深衰落用户要么 BLER 高企、要么吞吐浪费。
...
- **上报载体**（§5.2.3 CSI reporting using PUSCH / §5.2.4 CSI reporting using PUCCH）：周期与半持续报告在 **PUCCH**（物理上行控制信道，Physical Uplink Control Channel）上发送；非周期报告在 **PUSCH**（物理上行共享信道，Physical Uplink Shared Channel）上发送，由 DCI（下行控制信息，Downlink Control Information）触发。
- **CQI 定义**（§5.2.2.1，原文摘录）：“the UE shall derive ... the highest CQI index which satisfies the following condition: A single PDSCH transport block with a combination of modulation scheme, target code rate and transport block size corresponding to the CQI index ... could be received with a transport block error probability not exceeding: 0.1, if the higher layer parameter cqi-Table ... configures 'table1'/'table2'/'table4-r17' ..., or 0.00001, if ... 'table3' ...”。**翻译**：CQI 索引不是裸 SINR 数值，而是“调制方式+目标码率+TBS（传输块大小，Transport Block Size）”的组合编号；UE 必须找到满足错误率目标（普通场景传输块错误概率 $\le 0.1$，低 BLER 场景 $\le 0.00001$）的**最高**索引——目标是在错误率约束下选最激进的 MCS。
...
- **SS-SINR**（§5.1.5，原文摘录）：“SS signal-to-noise and interference ratio (SS-SINR), is defined as the linear average over the power contribution (in [W]) of the resource elements carrying secondary synchronisation signals divided by the linear average of the noise and interference power contribution (in [W]).”——分子是携带辅同步信号（Secondary Synchronization Signal, SSS）的 RE 上接收功率的线性平均，分母是噪声与干扰功率的线性平均。
...
**逐符号解释**：分子分母都是“线性平均”——在功率域（W）上平均，不是 dB 平均后相减；分子只统计配置给 CSI-RS 测量用的 RE；分母统计同一带宽内的噪声与干扰。**附加约束**（原文）：CSI-SINR 测量使用天线端口 3000 的 CSI-RS（用于 L1-SINR 报告时可扩展到 3001）；FR1（频率范围 1，Frequency Range 1）的参考点是 UE 天线连接器；FR2（频率范围 2，Frequency Range 2）基于接收分支的组合信号；接收分集生效时，上报值不得低于任一接收分支的测量值。
...
对 BPSK/QPSK（正交相移键控，Quadrature Phase Shift Keying）/QAM（正交幅度调制，Quadrature Amplitude Modulation） soft demapper 来说，均衡器至少要提供：
...
TS 38.211 规定物理信道、调制、层映射、预编码和参考信号结构；TS 38.214 规定 PDSCH/PUSCH 过程、MCS、CSI 报告等上下文；TS 38.215 定义 RSRP（参考信号接收功率，Reference Signal Received Power）/RSRQ/SINR 等测量指标。具体使用 ZF、MMSE、IRC、ML（最大似然，Maximum Likelihood）、球面检测，如何估计 post-eq SINR，如何归一化和裁剪 LLR，通常是接收机实现自由度。
...
日志字段建议固定为：`re_index`、`layer_id`、`h_norm`、`eq_gain`、`post_sinr`、`noise_var`、`x_hat_iq`、`llr_scale`。这些字段能把一个异常 LLR 追回具体 RE 和均衡状态，避免只剩 TB（传输块，Transport Block）级失败结论。
