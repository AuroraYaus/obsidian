...
[[T3.2_transport_code_block_filler_bits|T3.2]] 已经建立了传输块、码块和填充位的基本关系。本节把范围收窄到 LTE 的 Turbo链路：一个已经附加循环冗余校验的传输块，如何按 TS 36.212 Rel-19 规则拆成若干个码块，每个 CB 如何附加码块 CRC，填充位如何进入第一个 CB，以及接收端为什么可以按 CB 并行译码。
