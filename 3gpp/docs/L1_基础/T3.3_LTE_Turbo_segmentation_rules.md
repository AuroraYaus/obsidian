...
[[T3.2_transport_code_block_filler_bits|T3.2]] 已经建立了传输块、码块和填充位的基本关系。本节把范围收窄到 LTE 的 Turbo（Turbo 码，Turbo Code）链路：一个已经附加循环冗余校验的传输块，如何按 TS 36.212 Rel-19 规则拆成若干个码块，每个 CB（码块，Code Block）如何附加码块 CRC（循环冗余校验，Cyclic Redundancy Check），填充位如何进入第一个 CB，以及接收端为什么可以按 CB 并行译码。
...
- 解释 TB（传输块，Transport Block）和 CB 在 LTE Turbo 链路中的前后关系。
...
| $f_1$ | Turbo 内部交织器二次置换多项式的第一参数。 | 本节只记录和复现；[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 再用它生成交织地址。 |
| $f_2$ | Turbo 内部交织器二次置换多项式的第二参数。 | 本节只记录和复现；[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 再用它生成交织地址。 |

Turbo 内部交织器会使用二次置换多项式形式。这里给出它的用途，完整地址生成由 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 承接：
...
其中 $k$ 是交织前的位置索引，$\Pi(k)$ 是交织后的位置索引。这个公式说明为什么表中不能只有 $K$：同一个块长必须配套对应的 $f_1$ 和 $f_2$，接收端译码器才能生成与发送端一致的交织和解交织地址。本节做分段尺寸计算时使用 $K$ 列；[[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 会把公式、$f_1/f_2$ 和地址生成逐步展开。
...
![TS 36.212 Table 5.1.3-3: Turbo code internal interleaver parameters](assets/T3.2_TS36.212_Table_5.1.3-3_original_crop.png)
...
第三，TS 36.212 §5.1.3.2 进一步涉及第 0 个码块存在 filler 时 Turbo 编码器输入和部分输出的处理。当前 `content.md` 里相关符号抽取不完整，因此正文记录“存在专门处理规则”这一事实；具体输出流下标由 [[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]] 在核验原始 Word XML、图 5.1.3-2 和公式 XML 后逐符号展开。
...
| TB CRC 最终验收 | UL-SCH（上行共享信道，Uplink Shared Channel）/DL-SCH（下行共享信道，Downlink Shared Channel）TB CRC 在分段前附加 | 所有 CB 局部通过后仍需整包检查。 |
...
| 用例 | 输入 | 输出 | 失败案例 | 验证方式 |
|:---|:---|:---|:---|:---|
| LTE DL-SCH 大 TB Turbo 译码 | 解速率匹配后的每 CB LLR（对数似然比，Log-Likelihood Ratio）、TS 36.212 §5.1.2 分段参数、Table 5.1.3-3 合法长度 | 每个 CB 的硬判决、CRC24B 结果、重组后的 TB CRC 结果 | CB 长度按平均分配而不是按合法 $K$ 表选择，导致交织/解交织地址错位 | 协议尺寸向量、CB CRC、TB CRC、逐位一致 golden test。 |
...
这个状态图是实现建议，不是 3GPP 协议图。协议规定的是比特处理和块边界，RTL（寄存器传输级，Register Transfer Level）需要把这些规则映射成地址、状态、并行调度和错误上报。
...
| TS 36.212 Table 5.1.3-3 | 使用协议原文截图（`source.pdf` 第 16 页）展示完整 Turbo 内部交织器参数表，包含 188 组 `i/K/f1/f2`。`table_0009.csv/html` 作为程序化读取证据保留。 | 本节只使用 $K$ 列完成分段尺寸计算；$f_1/f_2$ 的交织地址公式会在 [[T6.4_LTE_Turbo_internal_interleaver|T6.4]] 精读。 |
| TS 36.212 §5.1.3.2 | 使用 PCCC、两个 8 状态组成编码器、内部交织器、1/3 码率和每 CB 独立 Turbo 编码的定位。 | 完整传递函数、尾比特和 filler 输出流下标由 [[T6.3_LTE_Turbo_encoder_trellis_termination|T6.3]] 在核验公式 XML/图 5.1.3-2 后承接。 |
...
| 计划复核 | 复核遗留项台账（`docs/audits/lte_nr_decoding_remaining_work_register.md`）中 T3.3 任务要求。 |
...
| 表格核验 | 查看 `tables/table_0009.csv` 和 `tables/table_0009.html`，确认 Turbo 合法 $K$ 集合和 $f_1/f_2$ 参数可由本地表格抽取；从 `source.pdf` 第 16 页提取协议原文截图，替换原有生成图表；2026-08-07 按用户指示与 T3.2 共用资产 `docs/L1_基础/assets/T3.2_TS36.212_Table_5.1.3-3_original_crop.png`（T3.3 独立副本已删除，git 可恢复）。 |
