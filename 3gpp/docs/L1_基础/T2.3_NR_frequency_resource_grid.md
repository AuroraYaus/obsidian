...
[[T2.2_NR_numerology_time_domain_hierarchy|T2.2]] 建立了 NR 的时域层级——从 μ 出发推导无线帧、子帧、时隙和 OFDM 符号。现在补上频域维度，构成完整的二维资源网格。NR 频域资源不是简单的"第几个子载波"，而是一条五级概念链。
...
| OFDM 符号是什么？ | 时域上一个 CP + N 点 IFFT 输出的时间片段（[[T2.1_OFDM_subcarrier_spacing_basics|T2.1]]）。 |
...
UE 开机沿同步栅格（GSCN（全球同步信道号，Global Synchronization Channel Number）列表）盲检 SSB，读到 MIB 后用 ARFCN 对齐小区频点——栅格是「路网」，频点是「停车位」。本节之后 RE/PRB/CRB 的编号都从 Point A 这个绝对坐标出发。完整关系链见 [[Spectrum_and_Frequency_Point_频谱与频点]]。
