...
### 例题 1：给定 SCS 与载波带宽的 PRB 与 RE 核算

NR FR1 20 MHz 载波，μ=0（15 kHz SCS），按 TS 38.101 Table 5.3.2-1 配置约 106 个 PRB（见"载波带宽与保护带"一节）。普通 CP 下每时隙 14 个 OFDM 符号。

(1) 一个 PRB 的物理带宽是多少？106 个 PRB 的总传输带宽是多少？
(2) 一个时隙内，整个载波共有多少个 RE？
(3) 若全部 RE 承载 QPSK（每 RE 2 bit），一个时隙最多承载多少比特？换成 64QAM（每 RE 6 bit）呢？

### 例题 2：Point A 到 BWP 内 PRB 的编号换算

某载波下 UE 的激活 BWP 从 CRB 30 开始、宽 48 个 PRB（$N_{BWP}^{start} = 30$，$N_{BWP}^{size} = 48$）。

(1) 以 Point A 子载波 0 为原点的绝对子载波索引 $k = 372$ 落在哪个 CRB？是否在 BWP 内？若在，BWP 内 PRB 编号是多少？
(2) BWP 内最后一个 PRB 的局部编号是多少？对应的 CRB 编号与起始子载波 $k$ 是多少？
(3) 该 BWP 在 μ=0 下的物理带宽是多少 MHz？

### 参考答案

**例题 1**。(1) 一个 PRB 的物理带宽 $= 12 \times \Delta f = 12 \times 15 = 180$ kHz。总传输带宽 $= 106 \times 180 = 19080$ kHz = 19.08 MHz——略小于 20 MHz 信道栅格，差值落在两侧保护带（见"载波带宽与保护带"一节）。

(2) RE 是 1 个子载波 × 1 个 OFDM 符号的二维最小单元。一个时隙内 $106 \times 12 \times 14 = 17808$ 个 RE。

(3) QPSK 下 $17808 \times 2 = 35616$ bit；64QAM 下 $17808 \times 6 = 106848$ bit。注意这是未扣除 DM-RS、CORESET 与保护带的毛数——实际可承载数据比特数按 $N_{RE}^{data}$ 扣减（见"CORESET"与"载波带宽与保护带"两节）。

**例题 2**。(1) $n_{CRB} = \lfloor k/12 \rfloor = \lfloor 372/12 \rfloor = 31$。BWP 范围是 CRB 30 至 77（30 + 48 − 1），31 在其中，故 BWP 内 PRB 编号 $n_{PRB} = 31 - 30 = 1$。

(2) 局部编号 $48 - 1 = 47$；对应 $n_{CRB} = 30 + 47 = 77$；起始子载波 $k = 77 \times 12 = 924$。

(3) $48 \times 180 = 8640$ kHz = 8.64 MHz。

补充观察：若另一 UE 的 BWP 从 CRB 50 开始，两个 UE 在相同物理频率位置（例如 CRB 60）接收时，生成的 DM-RS 序列相同——序列取决于 CRB 绝对编号而非各 UE 的 BWP 局部编号。这正是 CRB 作为"绝对编号层"存在的深层动机（见"CRB 绝对编号对 DM-RS 序列生成的意义"）。

## 引导练习（带提示）

### 练习 1：μ=1 BWP 的带宽与 RE 总量核算

某小区 μ=1（30 kHz SCS），UE 的激活 BWP 从 CRB 10 开始、宽 25 个 PRB。

提示：
- 一个 PRB 的物理带宽是 $12 \times \Delta f$，μ=1 时的取值见"PRB"一节的对照表。
- RE 总量 = PRB 数 × 12 子载波 × 14 符号。
- 子载波到 CRB 的换算用 $n_{CRB} = \lfloor k/12 \rfloor$，再判断是否落在 BWP 范围 $[10, 10+25)$ 内。

请完成：(1) 该 BWP 的物理带宽。(2) 一个时隙内该 BWP 的 RE 总数。(3) 绝对子载波 $k = 300$ 是否落在 BWP 内？若在，局部 PRB 编号是多少？

### 参考答案

(1) 一个 PRB 的物理带宽 $= 12 \times 30 = 360$ kHz；BWP 带宽 $= 25 \times 360 = 9000$ kHz = 9 MHz。

(2) $25 \times 12 \times 14 = 4200$ 个 RE（未扣除参考信号与控制资源）。

(3) $n_{CRB} = \lfloor 300/12 \rfloor = 25$，落在 $[10, 35)$ 内，故 BWP 内 PRB 编号 $= 25 - 10 = 15$。
