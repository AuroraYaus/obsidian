...
![图 2：2×2 MIMO 每 RE 接收模型 y=Hx+n——发送层 x0/x1 经信道矩阵 H 混合加噪声 n 得到观测 y0/y1；青绿色为对角线目标项，橙色为交叉干扰项](assets/T2.12_2x2_mimo_channel_model.svg)
...
![图 1：码字→层→天线端口映射（NR 下行示例）——泳道 A：1 个码字分 2 层（串接交错）；泳道 B：2 个码字分 4 层；层→端口恒等映射，端口数=层数，天线数可大于端口数](assets/T2.12_layer_mapping_ports.svg)
