...
[[T2.11_OFDM_equalization_CSI_SINR|T2.11]] 已经介绍均衡模型。本节将单层 OFDM（正交频分复用，Orthogonal Frequency Division Multiplexing）推广到多天线、多层传输。MIMO-OFDM（多输入多输出正交频分复用，Multiple-Input Multiple-Output OFDM）的核心思想是：OFDM 先将宽带频率选择性信道分解为多个 RE（资源元素，Resource Element）；MIMO（多输入多输出，Multiple-Input Multiple-Output）再在每个 RE 上处理多层符号经过多天线信道后的混合。
...
- 说明 DM-RS（解调参考信号，Demodulation Reference Signal）-based 估计为什么给接收端有效信道。
- 对比 ZF（迫零，Zero Forcing）、MMSE（最小均方误差，Minimum Mean Square Error）、ML（最大似然，Maximum Likelihood）/球面检测的复杂度和性能边界。
- 讲清 MIMO 检测如何输出每层 LLR（对数似然比，Log-Likelihood Ratio）。
- 定位本节知识点在 TS 38.211（层映射、端口映射、预编码）与 TS 38.214（传输方案、DM-RS 接收、CSI（信道状态信息，Channel State Information）上报）中的条款，并划清“协议强制”与“接收机实现自由度”的边界。
...
本节不要求已经掌握信息论 MIMO 容量公式，但需要理解"多层同时发送会互相混合，接收端必须分离"。如果单层均衡还不熟，先读 [[T2.11_OFDM_equalization_CSI_SINR|T2.11]]。
...
**天线**：物理的发射/接收单元。直观：天线是“传送带两端的物理装置”。关键：天线数可以多于层数（多余天线用于波束成形、分集或提升接收 SNR（信噪比，Signal-to-Noise Ratio）），也可以少于层数（理论上可以，但信道矩阵会因此“矮”，层数受秩限制）。
...
在单层 OFDM 中，一个 RE 可写成（标量模型，[[T2.11_OFDM_equalization_CSI_SINR|T2.11]] 已讲）：
...
第一根接收天线上看到的 $0.7$ 不是第一层符号本身，而是两层混合后的结果。若接收机把每根天线当成一个独立 SISO（单入单出，Single-Input Single-Output）链路，就会把层间干扰当成噪声甚至当成数据。MIMO 检测器必须使用整个 $\mathbf{H}_{eff}$ 来同时估计两层。
...
这类错误经常不会表现为“程序崩溃”，而是 BLER（块错误率，Block Error Rate）曲线完全不对。因为译码器收到的是合法长度的 LLR，只是这些 LLR 对应错了码字位置。
...
1. UE 用 CSI-RS（信道状态信息参考信号，Channel State Information Reference Signal）估计信道，计算各空间方向强度，向 gNB 上报**秩指示（rank indicator, RI）**、**预编码矩阵指示（precoding matrix indicator, PMI）**与**信道质量指示（channel quality indicator, CQI）**（TS 38.214 §5.2）。
2. gNB 综合 RI/PMI/CQI 与调度需要，决定实际层数、预编码和 MCS（调制与编码方案，Modulation and Coding Scheme）。
3. gNB 通过 DCI（下行控制信息，Downlink Control Information）的 Antenna ports 等字段告知 UE 本次传输的 DM-RS 端口集合；端口数就是层数。
...
- ML 是穷举：候选数 = 星座点数^层数。对 4 层 256QAM（256 阶正交幅度调制，256 Quadrature Amplitude Modulation），需要比较 $256^4$ 个候选组合：
...
**这个公式回答**：为什么 ML 在工程上通常不可实现。工程上常以 MMSE/IRC（干扰抑制合并，Interference Rejection Combining）等线性或近似检测为主，再配合 CSI 缩放、LLR 裁剪和 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）增益兜底；需要接近 ML 性能时使用 K-best 等列表检测（保留每层若干最优分支，$\mathcal{O}(K\cdot M)$ 量级），是“球面检测思想的固定复杂度实现”。

**每 RE 处理率的工程后果**：以 100 RB（资源块，Resource Block）、30 kHz SCS（子载波间隔，Subcarrier Spacing）为例，一个时隙有 $100\times12\times14=16800$ 个数据 RE，时隙长 0.5 ms，即每秒约 $3.36\times10^7$ 次“RE 级矩阵处理”。4 层时每个 RE 都涉及 4×4 相关矩阵、噪声项与求逆（或分解）结果，以及每层 CSI 输出。即使权重可按 PRB（物理资源块，Physical Resource Block）或 DM-RS 邻域复用，缓存和控制仍然复杂。工程文档应明确权重复用粒度：per RE、per PRB、per symbol、还是 per DM-RS interpolation region。复用粒度越粗，复杂度越低，但快速变化信道下性能越差。**注意**：协议在频域给了"同一预编码在 PRG 内不变"的允许假设（TS 38.214 §5.1.2.3，见"协议锚点精读"），这是接收端做粗粒度权重复用的协议依据，但"是否复用、复用多粗"仍是接收机实现自由度。
...
- **DM-RS 与数据同端口同预编码**：TS 38.214 §5.1.6.2 规定 UE 按 DCI 指示的 DM-RS 端口接收，且同一 CDM 组内其余正交端口不关联其他 UE 的 PDSCH（物理下行共享信道，Physical Downlink Shared Channel）——端口专属本 UE，估计出的端口信道即本 UE 数据的信道。这就是"DM-RS 估计给出有效信道"的协议来源。
- **频域预编码粒度**：TS 38.214 §5.1.2.3 允许 UE 假设同一 PRG（precoding resource block group，预编码资源块组）内预编码不变——接收端可在 PRG 粒度复用信道估计与权重。

**接收端必须做**：按 DCI 指示的端口集合做 DM-RS 信道估计（每个端口一份 $\hat{\mathbf{H}}_{eff}$ 列），把端口顺序与层顺序一一对应，再送入检测器。**接收端不必做**：恢复 gNB 实际用的预编码矩阵 $\mathbf{P}$（下行）；这正是“预编码对 UE 透明”的工程意义。上行则相反：PUSCH（物理上行共享信道，Physical Uplink Shared Channel）预编码由 TPMI 显式指示（TS 38.211 §6.3.1.5），gNB 知道 UE 用的 $\mathbf{W}$（详见“协议锚点精读”）。
...
**实现要点**：检测器输出接口应携带"每层、每 RE 的 CSI（等效噪声方差或 SINR）"而不是单一标量；后续 soft demapper 与 LLR 缩放（[[T2.16_LLR_clipping_scaling_quantization|T2.16]]）都依赖这个 per-layer CSI。若接口只传"平均 SINR"，低秩信道下弱层的 LLR 会被系统性高估——这正是"LLR 过度自信"的主要来源之一。
...
MIMO-OFDM 的验证不能只用“单层 AWGN（加性白高斯噪声，Additive White Gaussian Noise）能过”作为证据。多层接收最容易错在维度、顺序和可靠度，而这些问题常常不会造成明显异常长度或程序错误。译码器收到的 LLR 长度可能完全正确，但每个 LLR 对应了错误的层、错误的 RE 或错误的 codeword 位置。
...
| 高阶 QAM（正交幅度调制，Quadrature Amplitude Modulation）多层 | 4 层 64/256QAM | LLR scaling 不出现系统性过强。 |
...
MIMO 专题还要特别关注“局部失败”。一个 TB（传输块，Transport Block）里可能只有某些 PRB、某些 OFDM 符号或某一层很差。平均 BLER 只能说明结果，不能说明原因。建议把 per-layer EVM、per-layer SINR 和 per-layer LLR histogram 作为常规图表输出。若 layer 2 的 LLR 明显比其他层更激进，而其 SINR 又最低，通常说明 CSI 缩放或层排序有错。
...
**逐符号解释**：$y^{(\lambda)}(i)$ 是层 $\lambda$ 的信号（上行先做层映射再做预编码）；$z^{(p)}(i)$ 是端口/天线上的信号；$W$ 是预编码矩阵（维度：端口数×层数），码本传输时从表 6.3.1.5-1 ~ 6.3.1.5-47 按 **TPMI（transmitted precoding matrix indicator，发送预编码矩阵指示）** 选取；非码本传输时 $W=\mathbf{I}$（接收端可用 SRS（探测参考信号，Sounding Reference Signal）探测信道、自己做波束，无需显式预编码）。
...
| 调制 | QPSK（正交相移键控，Quadrature Phase Shift Keying）、64QAM（64 阶正交幅度调制，64 Quadrature Amplitude Modulation）、256QAM | 覆盖候选集合和 LLR 近似压力。 |
...
T2.12 是 [[T2.11_OFDM_equalization_CSI_SINR|T2.11]] 的多层推广。[[T2.11_OFDM_equalization_CSI_SINR|T2.11]] 中的标量可靠度在这里变成 per-layer、per-RE 的可靠度；[[T2.10_OFDM_channel_estimation_DMRS|T2.10]] 中的标量信道估计在这里变成有效信道矩阵。学习时先理解单层，再理解矩阵，不要直接跳到高阶 MIMO 检测算法。

T2.12 还连接后续译码链路的码字顺序。层检测输出不是最终结果，它还要经过 soft demapper、码字重组、rate recovery 和 CB（码块，Code Block）切分。任何 layer/codeword 顺序错误都会在 T3+ 中表现为译码失败。[[T2.17_OFDM_impairments_to_LLR|T2.17]] 会把这类错误统一归入 LLR 语义错误。
