...
[[T2.11_OFDM_equalization_CSI_SINR|T2.11]] 已经介绍均衡模型。本节将单层 OFDM 推广到多天线、多层传输。MIMO-OFDM 的核心思想是：OFDM 先将宽带频率选择性信道分解为多个 RE；MIMO 再在每个 RE 上处理多层符号经过多天线信道后的混合。
...
T2.12 还连接后续译码链路的码字顺序。层检测输出不是最终结果，它还要经过 soft demapper、码字重组、rate recovery 和 CB 切分。任何 layer/codeword 顺序错误都会在 T3+ 中表现为译码失败。[[T2.17_OFDM_impairments_to_LLR|T2.17]] 会把这类错误统一归入 LLR 语义错误。
