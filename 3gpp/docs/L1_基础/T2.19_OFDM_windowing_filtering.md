...
理想 OFDM（正交频分复用，Orthogonal Frequency Division Multiplexing）符号在时域常被建模为“矩形截断”：一个有用符号区间加上 CP（循环前缀，Cyclic Prefix），符号边界突然开始、突然结束。矩形窗在频域会产生较高旁瓣，带外泄漏可能影响邻道或相邻 numerology。窗口函数和滤波用于改善频谱，但会引入额外的 EVM（误差矢量幅度，Error Vector Magnitude）、时延和 CP 余量代价。

- 解释矩形窗为什么带来频谱旁瓣，并用手算数值说明第一旁瓣电平。
- 说明窗口、WOLA 和滤波的基本思路。
- 讲清频谱改善与 CP 保护、EVM、时延之间的折中。
- 区分发射端窗口/滤波与接收端 FFT（快速傅里叶变换，Fast Fourier Transform）窗口。
...
| EVM 与 LLR（对数似然比，Log-Likelihood Ratio）有何关系？ | 知道星座误差越大，soft demapper 越难给准 LLR。 |
...
窗口/滤波特别影响频带边缘。边缘 PRB（物理资源块，Physical Resource Block）离相邻载波、保护带或另一种 numerology 更近，因此更容易受到谱泄漏和滤波滚降影响。
...
| 窄 BWP（带宽部分，Bandwidth Part） | 滤波过渡带占比变大 | BWP 内边缘 RE（资源元素，Resource Element）的 SINR（信干噪比，Signal-to-Interference-plus-Noise Ratio）。 |
| 高阶 QAM（正交幅度调制，Quadrature Amplitude Modulation） | 星座间距小，EVM 容忍度低 | BLER（块错误率，Block Error Rate） vs EVM 联合曲线。 |
...
| 优化目标 | OOB、ACLR、EVM、PA（功率放大器，Power Amplifier）友好性 | ISI/ICI 最小、信道估计稳定。 |
...
这也是 T2.19 位于 PAPR（峰均功率比，Peak-to-Average Power Ratio）之后、误差到 LLR 之前的原因。发射端波形质量不能只由频谱图判定，它最终要回到接收端 soft demapper 的输入模型。
...
第二类测试是多径时延 sweep。固定窗口，逐步增加 TDL（抽头延迟线信道模型，Tapped Delay Line）信道的最大时延扩展。若窗口占用了 CP 余量，多径越长，ISI/ICI 越容易出现。这个测试能把"窗口本身造成的星座失真"和"窗口缩短 CP 余量造成的多径污染"区分开。
...
窗口函数还影响噪声假设。接收窗会对采样加权，使 FFT 后噪声不再严格保持各子载波同方差、互不相关的理想 AWGN（加性白高斯噪声，Additive White Gaussian Noise）模型。多数接收机仍使用近似噪声方差计算 LLR，但在高干扰或强窗口下，需要通过 EVM、SINR 或仿真校准修正可靠度。
...
| 接收窗噪声失配 | LLR 分布异常 | 比较 window on/off 噪声估计 | 修正等效噪声/CSI（信道状态信息，Channel State Information）缩放。 |
