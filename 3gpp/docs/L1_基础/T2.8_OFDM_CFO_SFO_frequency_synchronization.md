...
[[T2.7_OFDM_timing_synchronization|T2.7]] 解决“FFT（快速傅里叶变换，Fast Fourier Transform）窗口取哪里”的问题。本节解决“接收机的频率和采样节拍是否跟发送机一致”的问题。OFDM（正交频分复用，Orthogonal Frequency Division Multiplexing）的优势来自子载波正交性；载波频偏（Carrier Frequency Offset, CFO）和采样频偏（Sampling Frequency Offset, SFO）会让这个正交性逐步失效。
...
| SFO | ADC（模数转换器，Analog-to-Digital Converter）/DAC（数模转换器，Digital-to-Analog Converter）采样时钟不一致 | 采样点慢慢漂移，频域相位随子载波索引和时间变化。 |

- 推导归一化 CFO 如何破坏 FFT bin 的正交投影。
- 区分整数 CFO、小数 CFO、公共相位误差和 ICI（子载波间干扰，Inter-Carrier Interference）。
- 说明 SFO 为什么同时造成定时漂移和频域相位斜率。
- 解释 PSS（主同步信号，Primary Synchronization Signal）/SSS（辅同步信号，Secondary Synchronization Signal）、DM-RS（解调参考信号，Demodulation Reference Signal）、PTRS（相位跟踪参考信号，Phase Tracking Reference Signal）在频率同步中的不同角色。
- 把残余 CFO/SFO 如何影响 LLR（对数似然比，Log-Likelihood Ratio）质量讲清楚。
...
| 问题 | 需要达到的程度 |
|:---|:---|
| 子载波正交条件是什么？ | 知道 [[T2.1_OFDM_subcarrier_spacing_basics|T2.1]] 中 $\Delta f=1/T_u$ 是正交性的核心。 |
...
把 OFDM 接收机想象成一台需要同时对准“时间格子”和“频率格子”的机器。[[T2.7_OFDM_timing_synchronization|T2.7]] 对准了时间格子（符号边界落在哪里）；本节对准频率格子——载波频率差多少、采样节拍差多少。两个通俗类比：
...
| ICI（子载波间干扰） | 邻子载波的能量泄漏进当前 bin | 星座扩散、EVM（误差矢量幅度，Error Vector Magnitude）恶化，无法用单相位补偿 | 正交性破坏（§5.3.1 指数结构） |
...
$\varepsilon$ 的意思是“频偏占子载波间隔的几分之几”，单位是 SCS 的倍数。它把不同 numerlogy 下的频偏统一到一个无量纲数上：同样 1 kHz 频偏，15 kHz SCS（子载波间隔，Subcarrier Spacing）下 $\varepsilon=0.067$，120 kHz SCS 下只有 $\varepsilon=0.0083$，危害程度完全不同。
...
图示从理想正交开始，分别引入 CFO 和 SFO，再落到软解调。CFO 更直接表现为公共相位旋转和 ICI；SFO 更像采样网格随时间漂移，在频域表现为随子载波索引变化的相位斜率。两者都可能让同一个 RE（资源元素，Resource Element）的 LLR 幅度变小或符号错误。
...
沿用 §5.3.1 的指数结构（见“协议锚点一”）：一个 OFDM 有用符号（去掉 CP（循环前缀，Cyclic Prefix）后，以符号起点为时间零点）可以写成
...
![图 1：CFO 破坏子载波正交性 — 理想 sinc 对齐时邻 bin 采样恰为零，频偏 ε 使采样点偏移、泄漏到邻 bin 形成 ICI](assets/T2.8_cfo_subcarrier_leakage.svg)
...
$C_0$ 对应主子载波上的衰减和相位，求和项就是 ICI。CFO 越大，ICI 项越强。对高阶 QAM（正交幅度调制，Quadrature Amplitude Modulation），CPE 会让星座整体旋转；ICI 会让星座点扩散，两者都会影响 soft demapper 的欧氏距离计算。工程上 CPE 可以用参考信号相位估计后“单相位旋转”补偿掉，而 ICI 是补偿后仍残留的“看不见的干扰”——它最终体现为 EVM 抬高和 post-eq SINR（信干噪比，Signal-to-Interference-plus-Noise Ratio）下降（见“对 LLR 的影响”一节）。
...
约等于 18 度。捷径校验：$\Delta\phi=2\pi\varepsilon=2\pi\times0.05=0.314\ \mathrm{rad}$——**每符号旋转角只由归一化频偏决定**。对 QPSK（正交相移键控，Quadrature Phase Shift Keying），这可能还可以由 DM-RS 相位补偿吸收；对 256QAM（256 阶正交幅度调制，256 Quadrature Amplitude Modulation），18 度足以造成明显星座旋转。如果 CFO 未被建模，soft demapper 会把旋转后的点投到错误的候选距离上。

![图 2：CFO 使 QPSK 星座随 OFDM 符号持续旋转 — t0 到 t3 依次旋转 18°，同一符号内所有子载波近似同转（CPE）](assets/T2.8_cfo_constellation_rotation.svg)

图 1 把示例一画成四帧 QPSK 星座：灰色点是理想位置，彩色点是实际接收位置。注意旋转是“整片星座一起转”的——这就是 CPE 的可补偿性；而弧线之间每帧转 18°，不跟踪的话十几帧就转了一整圈。

再看跨符号累积。若 residual CFO 没有被继续跟踪，第 10 个 OFDM 符号附近相位可以累积到约 180 度量级。实际系统会在粗频偏校正后继续使用 DM-RS/PTRS 做细相位跟踪（连接态的周期跟踪参考信号 [[TRS_跟踪参考信号]] 提供持续输入），否则星座会持续转动。
...
即 **1 kHz 残余 CFO 在 30 kHz SCS 下恰好一个 slot 转半圈**。这意味着：如果接收机只做每 slot 一次 DM-RS 相位补偿，slot 内后半个 slot 的星座已经转到接近 180°，64QAM（64 阶正交幅度调制，64 Quadrature Amplitude Modulation）/256QAM 无法解调；必须按符号跟踪（PTRS 时间密度 $L_{\mathrm{PT\text{-}RS}}\in\{1,2,4\}$ 就是在买这种每符号相位跟踪能力）。
...
折算成等效 SNR（信噪比，Signal-to-Noise Ratio）损失约 $10\lg(1+0.0037)\approx0.016\ \mathrm{dB}$，几乎可以忽略。这个对比很有教学价值：**小 CFO 的第一危害是 CPE（可补偿的旋转），而不是 ICI（不可补偿的泄漏）**；工程上先花力气把 CFO 压进 $\varepsilon<0.01$ 量级，然后靠 CPE 跟踪解决残余。
...
取 $k=600$（20 MHz 信道 @ 30 kHz SCS 共 51 PRB（物理资源块，Physical Resource Block），$k=600$ 接近频带边缘）：
...
| 成分 | 主要后果 | 诊断方式 |
|:---|:---|:---|
| 整数 CFO $q$ | 子载波索引整体搬移，资源网格频域对不齐 | PSS/SSS 频域搜索、SSB（同步信号块，Synchronization Signal Block）光栅偏差。 |
...
![图 3：CFO 与 SFO 的相位特征对比 — 左：CFO 的相位与子载波无关、随符号整体抬升；右：SFO 的相位随子载波线性增长、斜率随符号累积](assets/T2.8_cfo_vs_sfo_phase_slope.svg)
...
**(1) 时间单位（§4.1，full.md 第 877 行起）**。协议原文：时间域各字段以时间单位 $T_c$ 表示，其中 $\Delta f_{\max}=480\cdot10^3$ Hz、$N_f=4096$：
...
**(2) numerology（§4.2 Table 4.2-1，full.md 第 883 行起）**。协议表 4.2-1 规定支持的传输 numerology：
...
**(3) OFDM 基带信号生成（§5.3.1，full.md 第 1475–1560 行）**。协议原文给出天线端口 $p$、numerlogy $\mu$、符号 $l$ 的时域连续信号（除 PRACH（物理随机接入信道，Physical Random Access Channel）与 RIM-RS 外）：
...
接收端流程翻译（5 步）：(1) 从高层参数 `subcarrierSpacing` 得到 $\mu$，按 Table 4.2-1 确定 $\Delta f$；(2) 按 §5.3.1 的指数结构建立“期望频率网格”（bin $k$ 对应 $k\Delta f$）；(3) 本振残余差 $\Delta f_c$ 就是 CFO，把它按式 (2) 归一化为 $\varepsilon$；(4) 采样时钟相对 $T_c$ 网格的速率差就是 SFO $\eta$；(5) 用 PSS/SSS 做粗校准、DM-RS/PTRS 做细校准（下一节），把网格拉回协议定义的位置。
...
|:---|:---|:---|
| 0.1 ppm @ 3.5 GHz（FR1（频率范围 1，Frequency Range 1）载波） | 350 Hz | 30 kHz SCS 下 0.012 |
| 0.1 ppm @ 28 GHz（FR2（频率范围 2，Frequency Range 2）载波） | 2.8 kHz | 120 kHz SCS 下 0.023 |
...
| [[T2.1_OFDM_subcarrier_spacing_basics|T2.1]] | 子载波正交性基础 | 本节只讨论频率误差破坏正交性的工程模型。 |
| [[T2.7_OFDM_timing_synchronization|T2.7]] | FFT 窗口和 CP 容差 | SFO 与定时漂移的关系在本节衔接。 |
...
| PTRS/DM-RS 相位 | CPE 跟踪曲线 | 相位噪声峰值后 BLER（块错误率，Block Error Rate）跳变。 |
...
对定点实现，频偏补偿通常是复数旋转器。旋转因子精度、相位累加器位宽和查表/坐标旋转数字计算机实现都会影响残余相位误差。若相位累加器截断过粗，星座会出现周期性抖动；若旋转器饱和或缩放不一致，后续信道估计幅度也会偏。硬件验证应加入固定 CFO、扫频 CFO、SFO ppm 和相位噪声四类向量，而不是只跑静态 AWGN（加性白高斯噪声，Additive White Gaussian Noise）。
...
### 例题 1：晶振误差到绝对频偏与归一化频偏的换算

收发两端晶振精度均为 ±0.1 ppm，载波 $f_c = 3.5$ GHz。设最坏情况下两端晶振的相对偏差为 0.2 ppm。

(1) 绝对频偏 $\Delta f_c$ 是多少 Hz？
(2) 分别换算为 15 kHz、30 kHz、120 kHz SCS 下的归一化频偏 $\varepsilon$。
(3) 三种 SCS 下每个有用符号的星座旋转角与 ICI 功率占比各是多少？

### 例题 2：SFO 相位旋转与采样漂移的手算

NR μ=0（15 kHz SCS），SFO 为 $\eta = 10$ ppm。μ=0 的每符号时长（普通 CP、非符号 0）按"协议锚点一"的计数核算：$T_{symb} = (N_u + N_{CP}) T_c \approx (131072 + 9216) \times 0.509\ \mathrm{ns} \approx 71.4\ \mu s$，其中 $N_u = 2048\kappa$、$N_{CP} = 144\kappa$、$\kappa = 64$。

(1) 求频带边缘附近子载波 $k = 1000$ 的每符号相位增量（用式 (16)）。
(2) 一个时隙（μ=0 为 1 ms）内累积多少度？
(3) 频带边缘子载波"看起来"额外偏了多少 Hz（$k \cdot \Delta f \cdot \eta$）？
(4) 采样点漂移速率是多少 ns/时隙？漂移到普通 CP 边界（约 4.69 μs）需要多少 ms？

### 参考答案

**例题 1**。(1) $\Delta f_c = 3.5 \times 10^9 \times 0.2 \times 10^{-6} = 700$ Hz。

(2) 由 $\varepsilon = \Delta f_c/\Delta f$：15 kHz 下 $\varepsilon = 700/15000 \approx 0.0467$；30 kHz 下 $\varepsilon = 700/30000 \approx 0.0233$；120 kHz 下 $\varepsilon = 700/120000 \approx 0.00583$。

(3) 每符号旋转角 $\Delta\phi = 2\pi\varepsilon$（式 (10) 的结论），ICI 功率占比按式 (6) 的 $(\pi\varepsilon)^2/3$：

| SCS | 归一化频偏 $\varepsilon$ | 每符号旋转角 | ICI 功率占比 |
|:---|:---|:---|:---|
| 15 kHz | 0.0467 | $2\pi \times 0.0467 \approx 0.293$ rad ≈ 16.8° | $(\pi \times 0.0467)^2/3 \approx 0.72\%$ |
| 30 kHz | 0.0233 | ≈ 0.147 rad ≈ 8.4° | ≈ 0.18% |
| 120 kHz | 0.00583 | ≈ 0.0366 rad ≈ 2.1° | ≈ 0.011% |

规律：同一个 700 Hz 频偏，SCS 越大，$\varepsilon$ 越小，旋转角与 ICI 都越小——且 ICI 按 $\varepsilon^2$ 衰减（$\varepsilon$ 减半，ICI 降到 1/4）。这就是高频段载波（频偏 Hz 数大）必须配大 SCS 的量化理由，与"协议锚点三"表格的结论一致。

**例题 2**。(1) $\Delta\phi_{\mathrm{SFO}}(k) = 2\pi k \Delta f \eta T_{symb} = 2\pi \times 1000 \times 15000 \times 10^{-5} \times 71.4 \times 10^{-6} \approx 0.0673$ rad ≈ 3.9°/符号。

(2) $2\pi \times 1000 \times 15000 \times 10^{-5} \times 1 \times 10^{-3} \approx 0.942$ rad ≈ 54°。

(3) $k \cdot \Delta f \cdot \eta = 1000 \times 15000 \times 10^{-5} = 150$ Hz——SFO 在频带边缘等效为一个与子载波位置成正比的"假 CFO"，这是它与真实 CFO 容易混淆的原因。

(4) 漂移速率 $= \eta \times T_{slot} = 10^{-5} \times 1\ \mathrm{ms} = 10$ ns/时隙。漂移到普通 CP 边界：$4.69\ \mu s \div 10\ \mathrm{ns} = 469$ 个时隙 ≈ 469 ms。短包内几乎无害，长 burst 才会把 FFT 窗口推向 CP 边缘——SFO 靠子载波索引与符号时间两个方向的"双重累积"在长时间里发难。

## 引导练习（带提示）

### 练习 1：FR2 频偏链路的合成核算

FR2 场景：载波 $f_c = 28$ GHz，晶振误差 0.1 ppm，UE 以 300 km/h 移动，SCS 取 120 kHz。求总 CFO、归一化频偏、每符号旋转角与 ICI 功率占比。

提示：
- 晶振项与多普勒项分别算绝对 Hz 再相加：晶振项 $\Delta f_c = f_c \times \text{ppm}$；多普勒项 $f_d = (v/c) f_c$，速度先换算成 m/s。
- $\varepsilon = \Delta f_c/120000$；每符号旋转角 $= 2\pi\varepsilon$；ICI 功率占比 $\approx (\pi\varepsilon)^2/3$。
- 得到旋转角后对照 PTRS 时间密度 $L_{\mathrm{PT\text{-}RS}} \in \{1,2,4\}$ 思考：为什么高频段必须逐符号相位跟踪？

### 参考答案

晶振项：$28 \times 10^9 \times 0.1 \times 10^{-6} = 2800$ Hz = 2.8 kHz。多普勒项：$v = 300/3.6 \approx 83.3$ m/s，$f_d = 83.3 \times 28 \times 10^9 / (3 \times 10^8) \approx 7.78$ kHz。总 CFO $\approx 2.8 + 7.78 = 10.58$ kHz。

$\varepsilon = 10580/120000 \approx 0.0882$；每符号旋转角 $= 2\pi \times 0.0882 \approx 0.554$ rad ≈ 31.8°；ICI 功率占比 $\approx (\pi \times 0.0882)^2/3 \approx 2.6\%$（等效 SNR 损失约 $10\lg(1.026) \approx 0.11$ dB）。

每符号 31.8° 意味着一个时隙（14 个符号）累积超过 400°，若只按每时隙一次的 DM-RS 相位补偿，时隙后半段的星座已转过数圈，64QAM 及以上无法解调——必须靠 PTRS 逐符号跟踪（$L_{\mathrm{PT\text{-}RS}} = 1$ 即每个符号一个相位采样点）。这正是"协议锚点三"表中 FR2 高多普勒行对应的工作区。
...
T2.8 接在 [[T2.7_OFDM_timing_synchronization|T2.7]] 后面，因为频率误差估计必须建立在大致正确的时间窗口上。窗口错位会制造频域相位斜率，容易和 SFO 混淆；残余 CFO 又会污染 DM-RS 相位，影响 [[T2.10_OFDM_channel_estimation_DMRS|T2.10]] 的信道估计。学习时应将 [[T2.7_OFDM_timing_synchronization|T2.7]] 和 T2.8 视为时间参考与频率参考校准的两个连续环节。

它还直接影响 [[T2.11_OFDM_equalization_CSI_SINR|T2.11]] 和 [[T2.17_OFDM_impairments_to_LLR|T2.17]]。均衡器假设输入 RE 已经落在正确子载波上；若残余 CFO 仍产生 ICI，均衡器输出的 post-eq SINR 应包含这部分残留干扰。否则 [[T2.17_OFDM_impairments_to_LLR|T2.17]] 中的 LLR 会过度自信，译码器会把前端频偏误差当成可靠证据。
...
- 2026-08-14 审核整改：补示范例题与引导练习（原 0 例题，违反零基础保护规则）。
