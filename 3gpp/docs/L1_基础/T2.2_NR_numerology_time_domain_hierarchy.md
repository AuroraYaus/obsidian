...
tags:
  ...
  - l1
  - lesson
source_spec: "docs/L1_基础/T2.2_NR_numerology_time_domain_hierarchy.md"
---
# T2.2 NR Numerology 与时域层级：从 μ 到帧结构的完整推导链（含 LTE 帧结构对照）
## 本节学习目标

[[T2.0_OFDM_system_overview|T2.0]] 建立了 OFDM（正交频分复用，Orthogonal Frequency Division Multiplexing）收发机全景，[[T2.1_OFDM_subcarrier_spacing_basics|T2.1]] 建立了 OFDM 的核心机制——子载波正交性、CP（循环前缀，Cyclic Prefix）和 IFFT（逆快速傅里叶变换，Inverse Fast Fourier Transform）/FFT（快速傅里叶变换，Fast Fourier Transform）实现——但刻意没有给出任何具体 NR 数值。本节把框架填上实数：NR 定义了五套 Numerology（参数集），每套用单一参数 μ 确定从子载波间隔到无线帧的全部时域参数。

LTE 只用一套（15 kHz），NR 用五套（15–240 kHz）。这不是简单的数量增加——它反映了 5G 同时服务 eMBB（增强移动宽带，Enhanced Mobile Broadband；大带宽）、uRLLC（超可靠低时延通信，Ultra-Reliable Low-Latency Communications；低时延）和 mMTC（海量机器类通信，massive Machine Type Communications；广覆盖）三种差异化场景的系统设计选择。

先区分本节容易混在一起的三个时间尺度概念：

| 术语 | 英文 | 先这样理解 |
|:---|:---|:---|
| Numerology | — | 一套以 μ 为索引的参数集，确定 SCS（子载波间隔，Subcarrier Spacing）、符号时长、CP 长度和时隙结构。 |
...
| 问题 | 需要达到的程度 |
|:---|:---|
| 子载波间隔 Δf 是什么？ | 能说出 $\Delta f = 1/T_u$，是相邻子载波中心频率间距（[[T2.1_OFDM_subcarrier_spacing_basics|T2.1]]）。 |
| CP 的作用是什么？ | 能说出 CP 消除 ISI 和维持子载波正交性（[[T2.1_OFDM_subcarrier_spacing_basics|T2.1]]）。 |
| OFDM 符号的组成部分是什么？ | $T_{symbol} = T_u + T_{CP}$，CP + 有用符号（[[T2.1_OFDM_subcarrier_spacing_basics|T2.1]]）。 |
...
| 1 | 30 | 33.33 | 0.500 | eMBB 中低频段（FR1（频率范围 1，Frequency Range 1）, <6 GHz） | 中等符号时长 → 覆盖与时延的平衡点 |
| 2 | 60 | 16.67 | 0.250 | eMBB/uRLLC（FR1 中高频段 + 扩展 CP 选项） | 唯一同时支持普通 CP 和扩展 CP 的 Numerology |
| 3 | 120 | 8.33 | 0.125 | eMBB 高频段 + uRLLC（FR2（频率范围 2，Frequency Range 2）, >24 GHz） | 短符号 → 抵抗高频相位噪声和多普勒频偏 |
...
μ=2 或 3 时，$T_{slot}$ 分别降至 0.25 ms 和 0.125 ms。配合微时隙（mini-slot）机制，传输时间间隔可进一步缩短。大 SCS 的短时隙 + 微时隙灵活起始位置 + 快速 HARQ（混合自动重传请求，Hybrid Automatic Repeat Request）反馈，共同实现 1 ms 级端到端时延。

### mMTC 广覆盖

μ=0（15 kHz）的长 OFDM 符号使信号能量在时间上更集中——在相同功率下，每个符号的能量更高，有利于小区边缘的覆盖。窄带传输（如 NB-IoT（窄带物联网，Narrowband Internet of Things）仅占 1 个 PRB（物理资源块，Physical Resource Block）= 180 kHz）降低了 UE 的射频复杂度和功耗。
...
| (6) $T_{subframe}$ → $T_{frame}$ | $T_{frame} = 10 \cdot T_{subframe} = 10$ ms | 无线帧是系统帧号（SFN，System Frame Number）的周期 |
...
| 概念 | LTE 中角色 | NR 中角色 |
|:---|:---|:---|
| 子帧（subframe） | **基本调度单位**（TTI（传输时间间隔，Transmission Time Interval）= 1 ms = 2 slots） | **纯计时单位**（固定 1 ms，与 μ 无关） |
| 时隙（slot） | 0.5 ms 固定，7 符号（普通 CP） | **基本调度单位**（14 符号，时长随 μ 变） |

在 LTE 中，MAC（媒体接入控制层，Medium Access Control）层以 1 ms 子帧为粒度分配资源——物理资源块（PRB）绑定到子帧内的时隙。在 NR 中，1 ms 子帧只作为跨 Numerology 的统一计时锚点（系统帧号 SFN、HARQ 定时器等都以子帧/帧为参考），实际调度完全以时隙为单位。这意味着 μ=3 时，1 ms 内有 8 个独立的调度机会——调度灵活性是 LTE 的 8 倍。
...
![图 1：LTE FDD Type 1 与 TDD Type 2（配置 1）帧结构对比](assets/T2.2_lte_fdd_tdd_frames.svg)
...
| PRB pair | 12 SC（子载波，Sub-Carrier）× 1 子帧，**调度最小粒度** | 无 |
| 频域参考 | DC 子载波（$k=0$，载波中心） | Point A（下边界） |
| 资源编排 | 直接 PRB 索引 | CRB→BWP（带宽部分，Bandwidth Part）→PRB 三级 |

LTE"给 UE 分 N 个 PRB"= 频域 N×12 SC + 时域 1 子帧（时频绑定）。NR 中频域和时域独立指定——这就是"解耦"的含义。

## 下行控制区域

LTE 下行子帧前 1–3 符号为 PDCCH（物理下行控制信道，Physical Downlink Control Channel）控制区域（全带宽）。NR 用 CORESET（控制资源集，Control Resource Set）替代——频域灵活配置，与数据 FDM（频分复用，Frequency Division Multiplexing）。

![图 2：LTE vs NR 时域层级并排对比 — 帧→子帧→时隙→符号四级对照](assets/T2.2_lte_nr_compare.svg)

## LTE vs NR 完整对比

| 概念 | LTE | NR |
|:---|:---|:---|
| SCS（子载波间隔，Subcarrier Spacing） | 固定 15 kHz | $15 \cdot 2^\mu$ kHz, μ=0–4 |
...
| $Q_m$（调制阶数，Modulation order）上限 | 8（256QAM（256 阶正交幅度调制，256 Quadrature Amplitude Modulation）） | 10（1024QAM（1024 阶正交幅度调制，1024 Quadrature Amplitude Modulation）） |
| MCS（调制与编码方案，Modulation and Coding Scheme）→TBS（传输块大小，Transport Block Size） | $I_{MCS}\to I_{TBS}\to$ 二维表 | $I_{MCS}\to Q_m,R\to$ 公式 |
...
1. **uRLLC [[Preemption_Indication_抢占指示|抢占]]**：低时延分组到达时，如果等到下一个时隙边界才开始传输，额外等待可能达数十个符号周期。微时隙可立即在下一个符号起始传输，抢占正在进行的 eMBB 时隙的部分资源。

2. **毫米波波束时分复用**：FR2 频段每个波束方向下的 UE 集合不同。一个时隙内可通过微时隙将时间切给多个波束方向，实现模拟波束的 TDM 切换。

3. **非授权频谱信道抢占**：NR-U（非授权频谱 NR，NR in unlicensed spectrum）中，LBT（先听后说，Listen Before Talk）成功后获得的信道使用权可能不对齐时隙边界。微时隙可从任意符号起始，充分利用抢占的信道时间。

### NR 时隙格式（SFI）：符号级的上下行配置

LTE TDD 只有 7 种子帧级配置（见上文"LTE 帧结构对照"节的 TDD 配置表）。NR 将灵活性推到符号级——通过时隙格式指示（Slot Format Indication, SFI），一个时隙内的 14 个 OFDM 符号可各自标记为下行（D）、上行（U）或灵活（F）。

SFI 由 RRC（无线资源控制，Radio Resource Control）配置 + DCI（下行控制信息，Downlink Control Information）format 2_0 动态指示（TS 38.213 §11.1）。协议预定义了 256 种时隙格式（Table 11.1.1-1），从全下行（格式 0: 14×D）到全上行（格式 1: 14×U），覆盖各种混合比例。F 符号可被后续调度动态覆写为 D 或 U。

SFI 对译码链路的影响：数据信道只能在指定方向的符号上传输——$N_{symb}^{data}$ 的计算不仅扣除参考信号，还受 SFI 约束。一个标记为 U 的符号上不会有下行 DM-RS（解调参考信号，Demodulation Reference Signal），这改变了 RE（资源元素，Resource Element）网格上有效数据 RE 的计算。

![图 3：微时隙与普通时隙调度对比 — 普通 eMBB 整时隙、uRLLC 抢占微时隙、毫米波波束 TDM 微时隙](assets/T2.2_minislot.svg)

图 3 用三行对比了三种调度场景。第一行是普通 14 符号整时隙调度（eMBB）。第二行是 uRLLC [[Preemption_Indication_抢占指示|抢占]]：低时延分组到达后不从时隙边界起始，而是在符号 4 处立即启动一个 4 符号微时隙。第三行是毫米波波束 TDM：一个时隙被拆分为三个微时隙，各指向不同波束方向。
...
设 NR 小区配置 μ=2（$\Delta f = 60$ kHz，普通 CP）。以"完整参数速查"表为基准，逐级完成帧 → 子帧 → 时隙 → 符号的换算。

(1) 写出 $\Delta f$、$T_u$、每子帧时隙数 $2^\mu$ 与 $T_{slot}$。
(2) 求一个无线帧内的时隙数与 OFDM 符号总数。
(3) 用 $T_u$ 直接乘符号总数去验证帧时长，得到多少？为什么不等于 10 ms？用什么量验证才能闭合？
(4) 求 2 ms 内包含的时隙数与符号数。

### 例题 2：跨 Numerology 符号边界对齐验算

图 2 展示了不同 SCS 符号边界的对齐关系。以 $T_u$ 为口径验算：一个 μ=0 的符号时长等于几个 μ=1、μ=2、μ=3 的符号时长？

### 参考答案

**例题 1**。(1) $\Delta f = 15 \times 2^2 = 60$ kHz；$T_u = 1/60000 \approx 16.67\ \mu s$；每子帧时隙数 $2^2 = 4$；$T_{slot} = 1\ \mathrm{ms}/4 = 0.25$ ms（与速查表一致）。

(2) 每帧时隙数 $= 10 \times 2^\mu = 10 \times 4 = 40$；符号总数 $= 40 \times 14 = 560$ 个 OFDM 符号（普通 CP 每时隙 14 个符号）。

(3) $560 \times 16.67\ \mu s \approx 9.34$ ms，不到 10 ms。原因：$T_u$ 不含 CP——帧时长闭合必须用含 CP 的符号时长 $T_{symbol} = T_u + T_{CP}$。用速查表的 $T_{slot}$ 验证：$40 \times 0.25 = 10$ ms，闭合。这正是"常见错误"第一条的由来：正交条件只用 $T_u$，帧结构核算必须用 $T_{symbol}$。

(4) $2\ \mathrm{ms}/0.25\ \mathrm{ms} = 8$ 个时隙，$8 \times 14 = 112$ 个符号。

**例题 2**。$T_u$ 严格反比于 $\Delta f = 15 \cdot 2^\mu$ kHz，μ 每加 1 减半。以秒为单位精确写出：

$$
\frac{1}{15000} = 2 \times \frac{1}{30000} = 4 \times \frac{1}{60000} = 8 \times \frac{1}{120000}
$$

即 1 个 15 kHz 符号 = 2 个 30 kHz 符号 = 4 个 60 kHz 符号 = 8 个 120 kHz 符号。用 μs 表示是 66.67 ≈ 2×33.33 ≈ 4×16.67 ≈ 8×8.33（速查表四舍五入到两位小数，乘积可能有 0.01 量级的出入，精确等式以分数形式为准）。TDD 上下行切换与跨 Numerology 共存都依赖这类公共边界对齐。

## 引导练习（带提示）

### 练习 1：LTE 与 NR μ=0 帧内符号数的对照核算

LTE 无线帧 10 ms = 20 个时隙、每时隙 7 个 OFDM 符号（普通 CP）；NR μ=0 无线帧 = 10 个时隙、每时隙 14 个符号。分别核算两代系统一个无线帧内的符号总数，并说明结果。

提示：
- 符号总数 = 每帧时隙数 × 每时隙符号数，两边各自相乘再比较。
- 结合"子帧 vs 时隙"一节思考：LTE 把 1 ms 子帧切成 2 个 0.5 ms 时隙，NR μ=0 的时隙长 1 ms——两代系统各保住了什么公共锚点？

### 参考答案

LTE：$20 \times 7 = 140$ 个符号。NR μ=0：$10 \times 14 = 140$ 个符号。两者相等。说明：NR 把每时隙符号数从 7 翻倍到 14、把每子帧时隙数从 2 减半到 1，帧内符号总数保持不变——LTE 的 1 ms 子帧计时锚点被完整保留，只是调度粒度从"0.5 ms 时隙"改为"1 ms 时隙"（μ=0 时）。这正呼应"子帧 vs 时隙"一节的结论：子帧是两代系统共用的计时锚点，时隙是各自可重定义的调度粒度。
...
| [[T2.1_OFDM_subcarrier_spacing_basics|T2.1]] OFDM 基础 | $T_u = 1/\Delta f$ 和 CP 的 ISI/ICI 消除机制基于 [[T2.1_OFDM_subcarrier_spacing_basics|T2.1]] 推导。 |
| 本节 LTE 帧结构对照 | NR 与 LTE 的帧结构对比在本节"LTE 帧结构对照"节展开；MCS 对照见 [[T2.5_MCS_modulation_order_target_code_rate|T2.5]]。 |
