...
LTE TDD 只有 7 种子帧级配置（见 [[T2.4_LTE_frame_structure_time_frequency|T2.4]]）。NR 将灵活性推到符号级——通过时隙格式指示（Slot Format Indication, SFI），一个时隙内的 14 个 OFDM 符号可各自标记为下行（D）、上行（U）或灵活（F）。
...
| [[T2.4_LTE_frame_structure_time_frequency|T2.4]] LTE 帧结构 | NR 与 LTE 的帧结构对比在 [[T2.5_MCS_modulation_order_target_code_rate|T2.5]] 用完整对照表展开。 |
