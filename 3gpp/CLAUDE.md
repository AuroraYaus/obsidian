...
**每次会话开始即知**：本项目经验教训的权威副本在 `docs/audits/lessons/lesson-*.md`（25 条，随 git 双推保存，与 `~/.claude/projects/*/memory/` 的 auto-memory 同步维护，**以项目副本为准**）。
...
9. **协议族分布展示（2026-09-03 用户确立）**：凡概念含协议锚点（TS/TR 引用）的 3GPP 概念问答，**无论命中新旧知识点，终端回答必须列全该概念涉及的相关 3GPP 协议，逐协议说明它讲了概念的哪个方面，并以联系链讲清协议间关系**——"谁定义 → 谁调度 → 谁测量 → 谁配置 → 谁验收"（范式：`Pilot_导频.md`「导频在协议族中的分布」五层职责表）。知识库沉淀同步执行：新建概念笔记直接写入协议分布内容；命中旧笔记时随 `queries` +1 一并补充；存量笔记不回填（与第 4 条 queries 口径一致）。**顺序硬约束（2026-09-03 当日两度违反、经用户定稿）**：终端回答骨架固定为——标题 → 定位段（2-3 句：一句话本质 + 直观模型，**不展开原理**）→ 协议族分布（**第二小节必达**：列全协议 + 逐协议讲方面 + 联系链）→ 原理展开（2-3 段）→ 关键要点（≥5 条）→ 图谱关联。禁止原理长文先行、禁止协议表沉底或被埋没（用户纠偏：先列协议的重音在"列全 + 逐协议讲方面 + 讲联系链"的可发现性，而非物理置顶；第一小节仅允许简短定位引导，避免零基础读者先撞术语墙）。教训来源：用户此前提出"列全协议 + 逐协议讲方面 + 讲联系"的要求未固化，2026-09-03 导频问答复发，故立此条；同日再发二度违反，先补"置顶"硬约束，后经用户拷问"先列协议还是先讲原理"定稿为本骨架。

### 16. 编辑前同步远程仓库（2026-09-03 用户确立，同步自 ic 项目第 10 条）
...
### 17. 记忆文件项目内双份维护（2026-09-03 用户确立，同步自 ic 项目第 11 条）

auto-memory（经验教训）必须随项目文件夹走，防止项目拷贝到其他位置后固化经验失效：
