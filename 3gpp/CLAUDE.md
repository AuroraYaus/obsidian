...
9. **协议族分布展示（2026-09-03 用户确立）**：凡概念含协议锚点（TS/TR 引用）的 3GPP 概念问答，**无论命中新旧知识点，终端回答必须列全该概念涉及的相关 3GPP 协议，逐协议说明它讲了概念的哪个方面，并以联系链讲清协议间关系**——"谁定义 → 谁调度 → 谁测量 → 谁配置 → 谁验收"（范式：`Pilot_导频.md`「导频在协议族中的分布」五层职责表）。知识库沉淀同步执行：新建概念笔记直接写入协议分布内容；命中旧笔记时随 `queries` +1 一并补充；存量笔记不回填（与第 4 条 queries 口径一致）。教训来源：用户此前提出"列全协议 + 逐协议讲方面 + 讲联系"的要求未固化，2026-09-03 导频问答复发，故立此条。
