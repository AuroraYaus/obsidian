...
- `docs/` — 156 篇讲义（L0 阅读引导 1 → L1 基础 46 → L2 协议/算法 73 → L3 工程 36；口径 = T/TX 编号正文讲义；另含入口/术语总表等文件 6 个；frontmatter `type` 一致性由 `tools/audit_frontmatter_types.py` 强制，2026-08-14 审核统一口径；2026-08-26 新增 T12.6/T15.6/T16.5）
...
### 14. 写作计划与决策问题先拷问（2026-08-11 用户确立）

**任何写作计划（讲义/教材/笔记/文档内容）、设计方案或需要用户决策的问题，动手前必须先对用户进行一轮拷问（grill-me 风格），决策树收敛后才开始执行。**

1. **触发范围**：新写/大改讲义、教材章节、概念笔记、文档内容规划；方案设计；需要用户拍板的决策点。
...
5. **教训来源**：2026-08-11 协议栈与 OSI 知识点入库——用户连续批准后追问"为什么不拷问我"，明确要求固化该流程；拷问实际发现 5 处计划漏洞（MAC 中文名不一致、索引区格式错误、LTE/NR 双侧缺失、译码主线桥接缺失、术语工具扩展决策），证明拷问在动手前的价值。

### 15. 问答与知识沉淀机制（Q&A Pipeline）（2026-08-11 用户确立，复制自 ic 项目）

**收到知识库相关问题（概念/协议/算法问题）后，必须先走查询流水线，回答与知识沉淀一体完成——不是答完就算完。**

1. **问题润色**（回答前必做）：把用户口语化/碎片化问题提炼为一个正式知识主题标题（作为回答锚点/小节标题）；回答覆盖该议题嵌入的更大语境，不只答字面——用户问题定义回答的**入口**，不定义**边界**。禁止「润色后的问题」等元标签入文档。
2. **检索（Search First）**：先在知识库检索是否已有相同/相似/相关问题——关键词搜概念笔记与讲义、按 wikilink 网络找邻近节点、查 frontmatter `queries` 字段。
3. **命中策略**：

   | 匹配程度 | 处理方式 |
   |:---|:---|
   | 完全一致 | frontmatter `queries` 计数 +1；内容过时则更新 |
   | 高度相关 | 在相关文件扩充对应章节，新问题作为新小节插入 |
   | 部分相关 | 在相关文件图谱关联加链接；必要时新建概念文件 |
   | 无匹配 | 新建概念文件详细解答 + 拓展，挂载概念图谱入口 + 登记 L0 术语总表 |

4. **查询计数与易忘排名**：概念笔记 frontmatter 新增 `queries` 字段（整数，默认 1），每次命中累加；入口文件（L0 术语入口/概念图谱入口）维护高频查询排名表（排名/概念/查询次数/最后查询），按 `queries` 降序，识别薄弱知识点。**存量笔记不回填**，新内容/命中时引入。
5. **输出标准**：回答含原理阐述（2-3 段）+ 关键要点（≥5 条）+ 与其他概念的关系（≥2 wikilink）；首次出现术语「中文（English Full Name, ABBR）」；解答后更新排名表。
6. **终端回答展示**：内容写入文件后，终端必须向用户展示归纳总结版答案（提炼标题 + 核心原理浓缩 + 要点 + 关联），禁止只写"已更新 XX 文件"。
7. **与规则 14 的关系**：Q&A 命中「无匹配→新建概念笔记」时，仍须先按规则 14 拷问（落点/范围/深度），再写计划执行。
8. **教训来源**：2026-08-11 用户指出 ic 项目（`~/AGENT/ic/CLAUDE.md` 第 9 条）已有该机制而 3gpp 缺失——每次概念问答都临时拷问入库，未形成沉淀闭环。规则适配：MOC → 3gpp 入口文件。
