...
│   ├── docs/L0_协议阅读引导/    # 协议阅读地图、术语总表（245 术语 + 109 概念索引）
...
│   ├── docs/concepts/          # 概念图谱（109 概念笔记，六段式模板）
...
4. **讲义体系**：L1 基础（T1-T5 + TX1-TX5 发送端镜像）→ L2 协议算法（M6-M16：译码/控制面/上行/调度）→ L3 工程实现（T17-T23：译码器/接收链/后端），全部完成 ✅（153 讲义 + 6 入口/术语文件 + 109 概念笔记）
